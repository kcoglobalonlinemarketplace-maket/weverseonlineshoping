import{D as A,l as y,b as x}from"./promo-backgrounds-C4TNWBGI.js";import{c as q,W as H,D as B,l as O,a as j,g as P,p as D,e as u,m as b,b as w}from"./promo-pool-D-DxY8Py.js";import{D as g,l as v,g as S}from"./site-content-slMvSP6y.js";const M="support@weverseonlineshop.com",F="weverse_brand_v1";function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function G(){try{const e=JSON.parse(localStorage.getItem(F)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function U(){const e=G(),t=e.brand_name||e.site_name||q;return`
    ...
    </section>`}const I=[{id:"trust-shipping",icon:"package",tone:"blue",title:"Shipping & Delivery",body:`
    ...
    </div>`}function K(){const e=I.slice(0,6),t=I.slice(6),a=r=>r.map(N).join("");return`
    ...
    </section>`}const Z=[{name:"Megan R.",country:"United States",text:"My order arrived ahead of schedule and the quality was exactly as described. I shop here without any doubt.",verified:!0},{name:"Sarah & James",country:"United Kingdom",text:"Ordered for our whole family and the tracking updates made it feel safe and reliable from checkout to delivery.",verified:!0},{name:"Oliver K.",country:"Germany",text:"The customer support team answered my questions in minutes. Genuinely trustworthy shopping experience.",verified:!0}];function W(e){return[1,2,3,4,5].map(t=>`<i data-lucide="star" class="w-3.5 h-3.5 ${t<=e?"fill-amber-400 text-amber-400":"text-slate-500"}"></i>`).join("")}function k(e){const t=String(e.name||"Verified shopper").trim()||"Verified shopper",a=(t.charAt(0)||"V").toUpperCase(),r=[e.country||"",e.verified?"Verified buyer":"",e.date||""].filter(Boolean).join(" · ");return`
    <div class="bg-white/[.07] border border-white/10 rounded-2xl p-4 backdrop-blur flex flex-col">
      <div class="flex items-center gap-1 mb-2.5">${W(e.rating||5)}</div>
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 pt-14 sm:pt-20 pb-8 sm:pb-10">
        ...
        <div class="text-center max-w-3xl mx-auto">
          <div class="mx-auto w-16 h-16 rounded-2xl bg-white/[.07] border border-white/10 flex items-center justify-center overflow-hidden backdrop-blur">
            ...
            <span style="display:none">${H("w-9 h-9")}</span>
          </div>
          <p class="mt-4 text-[10px] font-black uppercase tracking-[0.28em] text-white">${o(n)}</p>
        ...
        <div class="mt-12 max-w-4xl mx-auto rounded-3xl border border-white/10 bg-white/[.05] backdrop-blur-md p-6 sm:p-8">
          <div class="flex flex-col lg:flex-row items-center gap-6 justify-between text-center lg:text-left">
            ...
            <div class="flex flex-col sm:flex-row gap-3 shrink-0">
              <a href="mailto:${o(M)}" class="btn-press inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
          ...
          <div class="mt-6 pt-5 border-t border-white/10 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center text-[11px] text-slate-400">
            <span class="inline-flex items-center justify-center gap-1.5"><i data-lucide="mail" class="w-3.5 h-3.5 text-cyan-300"></i> ${o(M)}</span>
    ...
    </section>`}function z(e){e.addEventListener("click",t=>{const a=t.target.closest("[data-acc]");if(!a)return;const r=a.dataset.acc,i=e.querySelector(`[data-acc-body="${r}"]`),n=e.querySelector(`[data-acc-icon="${r}"]`);if(!i)return;i.dataset.open==="1"?(i.style.maxHeight="0px",i.style.opacity="0",i.dataset.open="0",n.classList.remove("rotate-180"),a.setAttribute("aria-expanded","false")):(i.style.maxHeight=i.scrollHeight+"px",i.style.opacity="1",i.dataset.open="1",n.classList.add("rotate-180"),a.setAttribute("aria-expanded","true"))})}function _(e){const t=document.querySelectorAll("[data-bg-slot]");t.length&&t.forEach(a=>{const r=a.dataset.bgSlot;r==="trust_promo"?a.innerHTML=x(e.trust_promo_bg_image,e.trust_promo_bg_video):r==="reviews"&&(a.innerHTML=x(e.reviews_bg_image,e.reviews_bg_video))})}function Y(e){const t=e.querySelector("#fb-stars");t&&t.addEventListener("click",i=>{const n=i.target.closest(".fb-star");if(!n)return;const l=parseInt(n.dataset.star,10),s=e.querySelector("#fb-rating");s&&(s.value=String(l)),t.querySelectorAll(".fb-star").forEach((c,h)=>{const p=c.querySelector("i, svg");p&&(h<l?(p.classList.add("fill-amber-400","text-amber-400"),p.classList.remove("text-slate-500")):(p.classList.remove("fill-amber-400","text-amber-400"),p.classList.add("text-slate-500")))})});const a=e.querySelector("#fb-form");a&&a.addEventListener("submit",i=>{i.preventDefault(),Q(a)});const r=e.querySelector("[data-feedback-backtop]");r&&r.addEventListener("click",()=>{const i=e.querySelector('[data-acc="trust-reviews-more"]');i&&i.click();const n=document.getElementById("customer-feedback");n&&n.scrollIntoView({behavior:"smooth",block:"start"})})}function m(e,t,a){const r=e.closest("#customer-feedback")?.querySelector("#fb-msg");r&&(r.textContent=t,r.classList.remove("hidden","text-emerald-300","text-amber-300"),a&&r.classList.add(a))}async function Q(e){const a=(e.querySelector("#fb-text")?.value||"").trim();if(!a){m(e,"Please write your feedback first.","text-amber-300");return}const r=e.querySelector("[type=submit]"),i=r.innerHTML;if(r.disabled=!0,r.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline mr-2"></i>Sending…',window.lucide)try{lucide.createIcons()}catch{}try{const n=await S();let l=null;try{l=(await n.auth.getUser()).data?.user?.id||null}catch{}if(!l){if(m(e,"Only account holders can submit feedback. Please sign in first.","text-amber-300"),r.disabled=!1,r.innerHTML=i,window.lucide)try{lucide.createIcons()}catch{}return}const{error:s}=await n.from("site_feedback").insert({user_id:l,name:e.querySelector("#fb-name")?.value.trim()||"Anonymous shopper",email:e.querySelector("#fb-email")?.value.trim()||"",rating:parseInt(e.querySelector("#fb-rating")?.value||"5",10),feedback:a,is_approved:!1});if(s)throw new Error(s.message);m(e,"✓ Thank you! Your feedback has been sent.","text-emerald-300"),e.reset();const c=e.closest("#customer-feedback")?.querySelector("#fb-stars");c&&c.querySelectorAll(".fb-star").forEach(h=>{const p=h.querySelector("i, svg");p&&(p.classList.remove("fill-amber-400","text-amber-400"),p.classList.add("text-slate-500"))})}catch{m(e,"Could not send your feedback right now. Please try again later.","text-amber-300")}if(r.disabled=!1,r.innerHTML=i,window.lucide)try{lucide.createIcons()}catch{}}async function X(e){const t=e?.querySelector("#fb-more-list"),a=e?.querySelector("#fb-featured");if(!(!t&&!a))try{const r=await S(),{data:i,error:n}=await r.from("site_feedback").select("name,rating,feedback,created_at").eq("is_approved",!0).order("created_at",{ascending:!1}).limit(30);if(n||!i||!i.length)return;const l=i.map(s=>k({name:s.name||"Verified shopper",text:s.feedback||"",rating:s.rating||5,verified:!0,country:"Verified customer",date:s.created_at?new Date(s.created_at).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):""}));t&&(t.innerHTML=l.join("")+t.innerHTML),a&&l.length&&(a.innerHTML=l.slice(0,3).join(""))}catch{}}async function ee(e){const t=e?.querySelector("#fb-form-holder"),a=e?.querySelector("#fb-signin-zone");if(!t||!a)return;let r=null;try{r=(await(await S()).auth.getUser()).data?.user||null}catch{}if(r){a.classList.add("hidden"),t.classList.remove("hidden");const i=r.user_metadata||{},n=i.name||i.full_name||(r.email?r.email.split("@")[0]:""),l=e.querySelector("#fb-name"),s=e.querySelector("#fb-email");l&&!l.value&&n&&(l.value=n),s&&!s.value&&r.email&&(s.value=r.email)}else t.classList.add("hidden"),a.classList.remove("hidden")}function te(){if(document.getElementById("trust-info-style"))return;const e=document.createElement("style");e.id="trust-info-style",e.textContent=`
    .trust-acc-body{overflow:hidden;max-height:0;opacity:0;transition:max-height .38s cubic-bezier(.2,.8,.2,1),opacity .28s ease}
    .trust-acc-body[data-open="1"]{opacity:1}`,document.head.appendChild(e)}async function L(){if(document.body&&document.body.dataset.homepage==="true")return;const e=document.getElementById("trust-info-area");if(!e)return;te();let t={...g};try{t=await v()}catch{}if(e.innerHTML=[U(),R(),K(),J(),E(t)].join(""),window.lucide)try{lucide.createIcons()}catch{}z(e),Y(e),ee(e),X(e);let a={...A};try{a=await y()}catch{}_(a),window.addEventListener("promo-backgrounds-updated",()=>{y().then(_).catch(()=>{})});const r=()=>{const i=e.querySelector("#site-closing-section");if(!i)return;const n=document.getElementById("app-promo-banner");n&&n.nextSibling!==i&&n.after(i)};r(),window.addEventListener("app-promo-banner-ready",r),window.addEventListener("site-content-updated",()=>{v().then(i=>{const n=e.querySelector("#site-closing-section");if(n&&(n.outerHTML=E(i)),window.lucide)try{lucide.createIcons()}catch{}r()}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",L):L();const ae=()=>document.getElementById("app-promo-banner");function ie(){return`
  ...
  <svg viewBox="0 0 560 720" class="w-full h-full block" aria-hidden="true">
    ...
    <g>
  ...
  </svg>`}function ne(e){const t=e[0],a=e[1],r=(i,n)=>{const l=u((i.title||i.name||"").slice(0,34));return`
      <a href="/details.html?id=${encodeURIComponent(i.property_id||i.id)}"
        ...
        <div class="aspect-square bg-gray-100 overflow-hidden">
          ${b(i,"w-full h-full object-cover")}
        </div>
        <div class="p-2">
          <p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${l}</p>
          <div class="flex items-center justify-between mt-1">
            <span class="text-[11px] text-blue-600 font-black">${w(i)}</span>
    ...
    <div class="absolute inset-0 flex flex-col bg-[#f1f5f9] overflow-hidden" id="promo-phone-screen">
      ...
      <div class="flex gap-1.5 px-3 pt-2 overflow-hidden">
        ${["All Houses","Villas","Apartments","Beach Houses","Land","Mansions"].map((i,n)=>`
          <span class="shrink-0 px-2 py-1 rounded-full text-[8px] font-black ${n===0?"bg-blue-500 text-white":"bg-white border border-gray-200 text-gray-600"}">${i}</span>`).join("")}
      </div>
      <!-- live product cards -->
    ...
    </div>`}function le(e,t,a){const r=D(t,e,12),i=(e.app_play_store_url||"").trim(),n={...g,...a||{}},l=(n.app_banner_title||e.app_banner_headline||B.app_banner_headline).trim(),s=n.app_banner_description,c=n.app_banner_button_text,h=n.app_banner_secondary_text,p=i?`<a href="${u(i)}" target="_blank" rel="noopener" class="inline-flex items-center gap-2.5 bg-white text-blue-900 font-black text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
    ...
    <section class="relative overflow-hidden bg-[#060c1c] text-white">
        ...
        <div class="grid lg:grid-cols-[1.05fr_.95fr] gap-10 lg:gap-6 items-center">
          ...
          <div class="max-w-xl lg:pr-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mt-7 max-w-lg">
              ${[{icon:"shopping-bag",label:"Shop Products"},{icon:"package-plus",label:"New Arrivals"},{icon:"package-search",label:"Manage Orders"},{icon:"heart",label:"Save Favorites"}].map(C=>`
                <div class="bg-white/[.06] border border-white/10 rounded-2xl px-3 py-3.5 text-center backdrop-blur">
                  <i data-lucide="${C.icon}" class="w-4.5 h-4.5 w-5 h-5 text-cyan-300 mx-auto"></i>
                  <p class="text-[10px] font-bold text-slate-200 mt-2">${C.label}</p>
                </div>`).join("")}
            </div>
          ...
          <div class="relative mx-auto w-full max-w-[300px] sm:max-w-[360px] lg:max-w-[440px]">
            ...
            <div class="absolute inset-0 hands-front pointer-events-none" aria-hidden="true">${re()}</div>
            <div class="relative flex justify-center pt-[30%] sm:pt-[27%] lg:pt-[26%]">
              <div class="scale-100 sm:scale-105 lg:scale-110">${se(r)}</div>
            </div>
          </div>
        </div>
      </div>
    </section>`}let f=null;function oe(e){f&&(clearInterval(f),f=null);const t=document.getElementById("promo-phone-screen"),a=document.getElementById("promo-phone-grid");if(!t||!a||!e.length)return;const r=e.slice();let i=0;const n=()=>{if(!a||!r.length)return;const l=r[i%r.length],s=r[(i+1)%r.length],c=u((l.title||l.name||"").slice(0,34)),h=u((s.title||s.name||"").slice(0,34));if(a.innerHTML=`
      <a href="/details.html?id=${encodeURIComponent(l.property_id||l.id)}" class="block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition active:scale-[.98]">
        <div class="aspect-square bg-gray-100 overflow-hidden">${b(l,"w-full h-full object-cover")}</div>
        <div class="p-2"><p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${c}</p>
        <div class="flex items-center justify-between mt-1"><span class="text-[11px] text-blue-600 font-black">${w(l)}</span><span class="bg-blue-500 text-white text-[9px] font-black px-2 py-1 rounded-lg">Buy</span></div></div>
      </a>
      <a href="/details.html?id=${encodeURIComponent(s.property_id||s.id)}" class="block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition active:scale-[.98]">
        <div class="aspect-square bg-gray-100 overflow-hidden">${b(s,"w-full h-full object-cover")}</div>
        <div class="p-2"><p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${h}</p>
        <div class="flex items-center justify-between mt-1"><span class="text-[11px] text-blue-600 font-black">${w(s)}</span><span class="bg-blue-500 text-white text-[9px] font-black px-2 py-1 rounded-lg">Buy</span></div></div>
      </a>`,window.lucide)try{lucide.createIcons()}catch{}};n(),f=setInterval(()=>{i+=2,n()},4500)}async function $(){if(document.body&&document.body.dataset.homepage==="true")return;const e=ae();if(!e)return;let t={...B};try{t=await O()}catch{}if(t.app_banner_enabled===!1)return;let a=[];try{await j(),a=P()||[]}catch{}const r=a.length?a:[{property_id:"browse",title:"Browse the full Weverse Online Shop",price:0,currency:"USD",images:[]}];let i={...A};try{i=await y()}catch{}const n=s=>{const c=e.querySelector('[data-bg-slot="app_banner"]');c&&(c.innerHTML=x(s.app_banner_bg_image,s.app_banner_bg_video))};async function l(){let s={...g};try{s=await v()}catch{}if(e.innerHTML=le(t,r,s),window.lucide)try{lucide.createIcons()}catch{}oe(r),window.dispatchEvent(new CustomEvent("app-promo-banner-ready")),n(i)}await l(),n(i),window.addEventListener("site-content-updated",()=>{l().catch(()=>{})}),window.addEventListener("promo-backgrounds-updated",()=>{y().then(s=>{i=s,n(s)}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",$):$();
