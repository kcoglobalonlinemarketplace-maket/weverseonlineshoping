import{V as f,k as w,f as H,J as P,G as D,H as F,S as Z,g as G,d as L,h as R,N as c,j as k,K as C}from"./supabase-client-DvCmNkpI-BBtcpOs0-Btt9PamL.js";const h={trust_promo_bg_image:"",trust_promo_bg_video:"",app_banner_bg_image:"",app_banner_bg_video:"",reviews_bg_image:"",reviews_bg_video:""},W=Object.keys(h).join(","),B="kco_promo_backgrounds_v1",V=60*1e3;function U(){try{const e=JSON.parse(localStorage.getItem(B)||"{}");if(e.ts&&Date.now()-e.ts<V&&e.data&&typeof e.data=="object")return e.data}catch{}return null}function Y(e){try{localStorage.setItem(B,JSON.stringify({ts:Date.now(),data:e}))}catch{}}async function g(){const e=U();if(e)return{...h,...e};try{const t=await L(),{data:a,error:r}=await t.from("site_settings").select(W).limit(1).maybeSingle(),s={...h,...r||!a?{}:a};return Y(s),s}catch{return{...h}}}function S(e,t){const a=(e||"").trim(),r=(t||"").trim(),s=[];return r&&s.push(`<video class="absolute inset-0 w-full h-full object-cover" autoplay muted loop playsinline webkit-playsinline preload="metadata" poster="${M(a)}"><source src="${M(r)}" type="video/mp4"></video>`),a&&s.push(`<div class="absolute inset-0 bg-cover bg-center" style="background-image:url('${N(a)}')"></div>`),(r||a)&&s.push('<div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-900/70 to-slate-900/45"></div>'),s.join("")}function M(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function N(e){return String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'")}const y="support@weverseonlineshop.com",z="weverse_brand_v1";function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function O(){try{const e=JSON.parse(localStorage.getItem(z)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function J(){const e=O(),t=e.brand_name||e.site_name||Z;return`
    ...
    </div>`}function X(){const e=j.slice(0,6),t=j.slice(6),a=r=>r.map(Q).join("");return`
    ...
    .trust-acc-body[data-open="1"]{opacity:1}`,document.head.appendChild(e)}async function T(){if(document.body&&document.body.dataset.homepage==="true")return;const e=document.getElementById("trust-info-area");if(!e)return;oe();let t={...f};try{t=await w()}catch{}if(e.innerHTML=[J(),K(),X(),se(),q(t)].join(""),window.lucide)try{lucide.createIcons()}catch{}re(e),ie(e),ne(e);let a={...h};try{a=await g()}catch{}I(a),window.addEventListener("promo-backgrounds-updated",()=>{g().then(I).catch(()=>{})}),window.addEventListener("site-content-updated",()=>{w().then(r=>{const s=e.querySelector("#site-closing-section");if(s&&(s.outerHTML=q(r)),window.lucide)try{lucide.createIcons()}catch{}}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",T):T();const de=()=>document.getElementById("app-promo-banner"),$="/fallback.svg";function ce(){return`
