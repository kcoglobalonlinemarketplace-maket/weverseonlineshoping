...
import{_ as m}from"./preload-helper-CS1eXPs2.js";import{d as O,g as D,C as U}from"./localization-Dv_Dmb1N.js";import{supabase as y}from"./supabase-client-nvpjTmO6.js";import{getCurrentUser as q}from"./auth-C8sthrmE.js";import{l as k,f as $,g as C,b as I,c as F,d as G,P as R,a as H}from"./motorhome-data-_WKsMac0.js";import{g as S,a as A}from"./phone-data-_dXqvG67.js";import{c as z,a as V,b as X,d as T,r as J,e as Y}from"./payment-settings-IUSBTmBD.js";import"./native-bridge-DjUVSZMf.js";import"./localization-bootstrap-E5SvJk7W.js";import"./customer-ai-widget-kK49-yEd.js";import"./brand-Ch4-wKat.js";const Q=[...R,...H];function P(t){return Q.find(r=>r.property_id===t)||null}const f="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function K(){const t=Date.now().toString(36).toUpperCase().slice(-6),r=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${r}`}function g(t){const r=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,r.classList.remove("translate-y-20","opacity-0"),clearTimeout(r._t),r._t=setTimeout(()=>r.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function M(t){t.addEventListener("click",function(r){if(this.disabled)return;const s=this.getBoundingClientRect(),a=document.createElement("span");a.className="ripple";const n=Math.max(s.width,s.height);a.style.width=a.style.height=n+"px",a.style.left=r.clientX-s.left-n/2+"px",a.style.top=r.clientY-s.top-n/2+"px",this.appendChild(a),setTimeout(()=>a.remove(),600)})}function b(t,r){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${r}`}function W(t){const r=()=>{const s=document.createElement("textarea");s.value=t,document.body.appendChild(s),s.select();try{document.execCommand("copy")}catch{}document.body.removeChild(s)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>r()):r(),g("Copied to clipboard.")}function Z(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,r=J(e.manualPaymentAccounts,e.countryCode,t);return{...r,fallbackNotice:r.isFallback?Y(r.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function ee(){const t=document.getElementById("particles");if(t)for(let r=0;r<12;r++){const s=document.createElement("div"),a=Math.random()*3+1;s.className="particle",s.style.width=s.style.height=a+"px",s.style.left=Math.random()*100+"%",s.style.bottom="-10px",s.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",s.style.animationDuration=Math.random()*20+15+"s",s.style.animationDelay=Math.random()*20+"s",t.appendChild(s)}}ee();async function te(){const t=document.getElementById("checkout-root"),r=new URLSearchParams(window.location.search),s=r.get("status"),a=r.get("transaction_id"),n=r.get("tx_ref"),c=r.get("order_number");if(s==="verify"&&a){await ue(a,n,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=r.get("guest")==="1",!e.isGuest&&(e.user=await q(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const i=r.get("id");if(i){if(await k(),e.listing=$(i)||C(i)||I(i)||S(i)||A(i)||P(i),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:l}]=await Promise.all([m(()=>import("./catalog-BzyCL9ZE.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-BJoPnRIT.js"),__vite__mapDeps([1,2]))]);await l(),e.listing=o(i)}if(e.listing||(e.listing=$(i)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await k();const l=F(),[{generateListingById:d},{loadHiddenCatalogIds:v}]=await Promise.all([m(()=>import("./catalog-BzyCL9ZE.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-BJoPnRIT.js"),__vite__mapDeps([1,2]))]);if(await v(),e.cartItems=o.map(u=>{const _=l.find(L=>L.property_id===u)||C(u)||I(u)||S(u)||A(u)||P(u)||d(u);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=E(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await y.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=O(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await z(),e.manualPaymentAccounts=V(e.paymentSettings),e.manualPaymentInstructions=X(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),T(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await y.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const l=e.addresses.find(v=>v.is_default);l&&(e.selectedAddressId=l.id);const{data:d}=await y.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=K(),p()}function h(){return e.cartItems.reduce((t,r)=>t+r.listing.price*r.quantity,0)}function re(){return 0}function se(){return 0}function w(){return Math.round(h()*se()*100)/100}function x(){return h()+re()+w()}function p(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(M)}function ae(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
    <div class="flex items-center justify-center gap-2 sm:gap-4 mb-6">
      ${t.map((r,s)=>`
        <div class="flex items-center gap-2 sm:gap-4">
          ...
          ${s<t.length-1?`<div class="step-bar w-8 sm:w-16 h-0.5 ${e.step>r.num?"bg-blue-500":"bg-blue-50"}"></div>`:""}
        </div>
      `).join("")}
    </div>
  `}function ie(){return`
        ...
        ${e.cartItems.map((t,r)=>`
            <div class="flex items-center gap-3 p-3 bg-gray-50 border border-blue-100 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-600 mt-1">${G(t.listing)}</p>
              </div>
  ...
  `}function ne(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-gray-900 uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(s=>{const a=D(s.country_code);return`
            <div onclick="selectAddress('${s.id}')" class="cursor-pointer p-3 border rounded-xl transition ${e.selectedAddressId===s.id?"bg-blue-50 border-blue-300":"bg-gray-50 border-blue-100 hover:border-blue-200"}">
              ...
              <p class="text-xs text-gray-600">${s.full_name} · ${s.address_line1}, ${s.city}, ${s.state} ${s.postal_code} · ${a?a.flag+" "+a.name:s.country_code}</p>
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
              ${U.map(s=>`<option value="${s.code}" ${e.shippingCountry===s.code?"selected":""}>${s.flag} ${s.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function le(){const t=T(e.manualPaymentAccounts),r=Z(),s=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",a=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
      ...
      <div class="space-y-3">
        ...
        ${a?`<div onclick="selectPaymentMethod('manual_bank_transfer')" class="pay-method cursor-pointer p-4 border rounded-xl transition ${e.paymentMethod==="manual_bank_transfer"?"selected":"bg-gray-50 border-blue-100 hover:border-blue-200"}">
          <div class="flex items-center gap-3">
    ...
    ${e.paymentMethod==="manual_bank_transfer"?de(r.account,r.fallbackNotice,e.manualPaymentInstructions):""}
  ...
  `}function oe(){const t=h(),r=x();return`
        ...
        ${e.cartItems.map(s=>`
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-lg bg-gray-50 overflow-hidden shrink-0 ring-1 ring-blue-500/10">
                <img src="${s.listing.images?.[0]||f}" class="w-full h-full object-cover" onerror="this.src='${f}'">
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-gray-900 truncate">${s.listing.title}</p>
                <p class="text-xs text-gray-500">Qty: ${s.quantity}</p>
              </div>
              <p class="text-xs font-bold text-amber-600 shrink-0">${b(s.listing.price*s.quantity,s.listing.currency||"USD")}</p>
            </div>
          `).join("")}
      </div>
  ...
  `}window.changeQty=(t,r)=>{e.cartItems[t].quantity=Math.max(1,e.cartItems[t].quantity+r),p()};window.removeCartItem=t=>{if(e.cartItems.splice(t,1),e.cartItems.length===0){document.getElementById("checkout-root").innerHTML=E(),window.lucide&&lucide.createIcons();return}p()};window.goToStep=t=>{t===3&&!ce()||(t===2&&N(),e.step=t,p(),window.scrollTo({top:0,behavior:"smooth"}))};function N(){(e.isGuest||!e.selectedAddressId)&&(e.fullName=document.getElementById("ship-name")?.value||e.fullName,e.phone=document.getElementById("ship-phone")?.value||e.phone,e.email=document.getElementById("ship-email")?.value||e.email||e.user?.email||"",e.shippingAddr1=document.getElementById("ship-addr1")?.value||"",e.shippingAddr2=document.getElementById("ship-addr2")?.value||"",e.shippingCity=document.getElementById("ship-city")?.value||"",e.shippingState=document.getElementById("ship-state")?.value||"",e.shippingPostal=document.getElementById("ship-postal")?.value||"",e.shippingCountry=document.getElementById("ship-country")?.value||e.shippingCountry),e.billingSame=document.getElementById("billing-same")?.checked??!0,e.billingSame||(e.billingAddress=document.getElementById("billing-address")?.value||"")}function ce(){return N(),e.selectedAddressId&&!e.isGuest?!0:!e.fullName||!e.phone||!e.shippingAddr1||!e.shippingCity||!e.shippingState||!e.shippingPostal?(g("Please fill in all required shipping fields."),!1):e.isGuest&&!e.email?(g("Please enter your email address."),!1):!0}window.selectAddress=t=>{if(e.selectedAddressId=t||null,t){const r=e.addresses.find(s=>s.id===t);r&&(e.fullName=r.full_name,e.phone=r.phone,e.shippingAddr1=r.address_line1,e.shippingAddr2=r.address_line2||"",e.shippingCity=r.city,e.shippingState=r.state,e.shippingPostal=r.postal_code,e.shippingCountry=r.country_code)}p()};window.toggleBilling=()=>{e.billingSame=document.getElementById("billing-same").checked,document.getElementById("billing-fields").classList.toggle("hidden",e.billingSame)};window.selectPaymentMethod=t=>{e.paymentMethod=t,p()};window.selectCurrency=t=>{e.selectedCurrency=t,p()};window.copyToClipboard=W;async function j(t,r={}){const s=h(),a=x(),n=e.selectedAddressId&&e.addresses.length?(()=>{const l=e.addresses.find(d=>d.id===e.selectedAddressId);return l?`${l.address_line1}, ${l.city}, ${l.state} ${l.postal_code}, ${l.country_code}`:""})():`${e.shippingAddr1}${e.shippingAddr2?", "+e.shippingAddr2:""}, ${e.shippingCity}, ${e.shippingState} ${e.shippingPostal}, ${e.shippingCountry}`,c=e.billingSame?n:e.billingAddress||n,i={order_number:e.orderNumber,listing_id:e.listing.property_id,listing_title:e.listing.title,amount:a,currency:e.selectedCurrency,full_name:e.fullName,email:e.email||e.user?.email||"",phone:e.phone,status:"order_placed",payment_method:t,subtotal:s,quantity:e.cartItems.reduce((l,d)=>l+d.quantity,0),billing_address:c,...r};e.isGuest?(i.is_guest=!0,i.user_id=null,i.guest_shipping_address=n,i.guest_country=e.shippingCountry,i.guest_state=e.shippingState,i.guest_city=e.shippingCity,i.guest_postal_code=e.shippingPostal):(i.user_id=e.user.id,e.selectedAddressId&&(i.shipping_address_id=e.selectedAddressId));const{error:o}=await y.from("payment_receipts").insert(i);if(o)throw new Error("Failed to create order: "+o.message);return i}window.payWithFlutterwave=async()=>{const t=document.getElementById("flw-pay-btn");t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Initializing payment...',window.lucide&&lucide.createIcons();try{await j("flutterwave"),localStorage.setItem("kco_pending_order",e.orderNumber);const s=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=initialize",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({amount:x(),currency:e.selectedCurrency,customer_name:e.fullName,customer_email:e.email||e.user?.email,customer_phone:e.phone,order_number:e.orderNumber,redirect_url:`${window.location.origin}/checkout.html?status=verify&order_number=${e.orderNumber}`})}),a=await s.json();if(!s.ok||a.error)throw new Error(a.error||"Failed to initialize payment");window.location.href=a.payment_link}catch(r){t.disabled=!1,t.innerHTML='<i data-lucide="zap" class="w-5 h-5"></i> Pay with Flutterwave',window.lucide&&lucide.createIcons(),g(r.message||"Payment initialization failed.")}};async function ue(t,r,s){const a=document.getElementById("checkout-root");a.innerHTML=`
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:r,order_number:s})}),i=await c.json();if(!c.ok||i.error||i.status==="failed"){a.innerHTML=B(i.message||i.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}a.innerHTML=pe(s),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(M),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(n){a.innerHTML=B(n.message),window.lucide&&lucide.createIcons()}}function pe(t){return`
