...
import{supabase as f}from"./supabase-client-7_ZWSEp6.js";import{d as N,g as L,C as q}from"./localization-B1uQqmO-.js";import{_ as g}from"./preload-helper-CLcXU_4U.js";import{g as D}from"./auth-D3ZJ7kZA.js";import{f as k,l as $,g as O,a as U}from"./showroom-data-DoMXl563.js";import{g as C,a as S,b as I}from"./car-data-BznJ4eS3.js";import{c as F,a as G,b as R,d as P,r as H,e as z}from"./payment-settings-BTsVTq0X.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-BV4Diyt0.js";import"./customer-ai-widget-DHsAzpRV.js";import"./brand-DYhAh01b.js";const h="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function V(){const t=Date.now().toString(36).toUpperCase().slice(-6),s=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${s}`}function m(t){const s=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,s.classList.remove("translate-y-20","opacity-0"),clearTimeout(s._t),s._t=setTimeout(()=>s.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function B(t){t.addEventListener("click",function(s){if(this.disabled)return;const i=this.getBoundingClientRect(),r=document.createElement("span");r.className="ripple";const l=Math.max(i.width,i.height);r.style.width=r.style.height=l+"px",r.style.left=s.clientX-i.left-l/2+"px",r.style.top=s.clientY-i.top-l/2+"px",this.appendChild(r),setTimeout(()=>r.remove(),600)})}function b(t,s){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${s}`}function J(t){const s=()=>{const i=document.createElement("textarea");i.value=t,document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>s()):s(),m("Copied to clipboard.")}function X(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,s=H(e.manualPaymentAccounts,e.countryCode,t);return{...s,fallbackNotice:s.isFallback?z(s.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function Y(){const t=document.getElementById("particles");if(t)for(let s=0;s<12;s++){const i=document.createElement("div"),r=Math.random()*3+1;i.className="particle",i.style.width=i.style.height=r+"px",i.style.left=Math.random()*100+"%",i.style.bottom="-10px",i.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",i.style.animationDuration=Math.random()*20+15+"s",i.style.animationDelay=Math.random()*20+"s",t.appendChild(i)}}Y();async function Q(){const t=document.getElementById("checkout-root"),s=new URLSearchParams(window.location.search),i=s.get("status"),r=s.get("transaction_id"),l=s.get("tx_ref"),c=s.get("order_number");if(i==="verify"&&r){await le(r,l,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=s.get("guest")==="1",!e.isGuest&&(e.user=await D(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const a=s.get("id");if(a){if(e.listing=k(a)||C(a)||S(a)||I(a),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:n}]=await Promise.all([g(()=>import("./catalog-DBQWiGKW.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await n(),e.listing=o(a)}if(e.listing||(await $(),e.listing=k(a)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await $();const n=O(),[{generateListingById:d},{loadHiddenCatalogIds:v}]=await Promise.all([g(()=>import("./catalog-DBQWiGKW.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);if(await v(),e.cartItems=o.map(p=>{const _=n.find(j=>j.property_id===p)||C(p)||S(p)||I(p)||d(p);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=T(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await f.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=N(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await F(),e.manualPaymentAccounts=G(e.paymentSettings),e.manualPaymentInstructions=R(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),P(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await f.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const n=e.addresses.find(v=>v.is_default);n&&(e.selectedAddressId=n.id);const{data:d}=await f.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=V(),u()}function x(){return e.cartItems.reduce((t,s)=>t+s.listing.price*s.quantity,0)}function W(){return 0}function K(){return 0}function w(){return Math.round(x()*K()*100)/100}function y(){return x()+W()+w()}function u(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(B)}function Z(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
    <div class="flex items-center justify-center gap-2 sm:gap-4 mb-6">
      ${t.map((s,i)=>`
        <div class="flex items-center gap-2 sm:gap-4">
          <div class="flex items-center gap-2">
            <div class="step-bar w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${e.step>=s.num?"bg-blue-500/20 border border-blue-500/50 text-blue-400":"bg-blue-950/40 border border-blue-500/10 text-gray-600"} ${e.step===s.num?"pulse-glow":""}">
              ${e.step>s.num?'<i data-lucide="check" class="w-4 h-4 text-emerald-400"></i>':`<i data-lucide="${s.icon}" class="w-4 h-4"></i>`}
            </div>
            <span class="text-xs font-bold ${e.step>=s.num?"text-white":"text-gray-600"} hidden sm:inline">${s.label}</span>
          </div>
          ${i<t.length-1?`<div class="step-bar w-8 sm:w-16 h-0.5 ${e.step>s.num?"bg-blue-500":"bg-blue-500/10"}"></div>`:""}
        </div>
      `).join("")}
    </div>
  `}function ee(){return`
        ...
        ${e.cartItems.map((t,s)=>`
            <div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-400 mt-1">${U(t.listing)}</p>
              </div>
              <div class="flex items-center gap-2 shrink-0">
                <button onclick="changeQty(${s}, -1)" class="w-9 h-9 bg-blue-900/40 hover:bg-blue-500/20 border border-blue-500/20 rounded-lg text-gray-400 hover:text-white transition flex items-center justify-center"><i data-lucide="minus" class="w-4 h-4"></i></button>
                <span class="text-sm font-bold text-white w-8 text-center">${t.quantity}</span>
                <button onclick="changeQty(${s}, 1)" class="w-9 h-9 bg-blue-900/40 hover:bg-blue-500/20 border border-blue-500/20 rounded-lg text-gray-400 hover:text-white transition flex items-center justify-center"><i data-lucide="plus" class="w-4 h-4"></i></button>
              </div>
              <button onclick="removeCartItem(${s})" class="p-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 rounded-lg text-red-400 transition shrink-0"><i data-lucide="trash-2" class="w-4 h-4"></i></button>
            </div>
          `).join("")}
      </div>
    </div>
  ...
  `}function te(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-white uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(i=>{const r=L(i.country_code);return`
            <div onclick="selectAddress('${i.id}')" class="cursor-pointer p-3 border rounded-xl transition ${e.selectedAddressId===i.id?"bg-blue-500/15 border-blue-500/50":"bg-blue-950/30 border-blue-500/10 hover:border-blue-500/30"}">
              <div class="flex items-center gap-2 mb-1">
                <span class="text-sm font-bold text-white">${i.label}</span>
                ${i.is_default?'<span class="text-[10px] text-emerald-400 font-bold uppercase">Default</span>':""}
              </div>
              <p class="text-xs text-gray-400">${i.full_name} · ${i.address_line1}, ${i.city}, ${i.state} ${i.postal_code} · ${r?r.flag+" "+r.name:i.country_code}</p>
              <p class="text-xs text-gray-500 mt-0.5">${i.phone}</p>
            </div>
          `}).join("")}
  ...
  `:"",s=!e.selectedAddressId||e.isGuest;return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      <h3 class="text-sm font-bold text-white uppercase tracking-wide flex items-center gap-2 mb-4">
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-[#0a1124]/80 border border-blue-500/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500">
              ${q.map(i=>`<option value="${i.code}" ${e.shippingCountry===i.code?"selected":""}>${i.flag} ${i.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function se(){const t=P(e.manualPaymentAccounts),s=X(),i=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",r=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ...
      <div class="space-y-3">
        <!-- Flutterwave -->
        ${i?`<div onclick="selectPaymentMethod('flutterwave')" class="pay-method cursor-pointer p-4 border rounded-xl transition ${e.paymentMethod==="flutterwave"?"selected":"bg-blue-950/30 border-blue-500/10 hover:border-blue-500/30"}">
        ...
        ${r?`<div onclick="selectPaymentMethod('manual_bank_transfer')" class="pay-method cursor-pointer p-4 border rounded-xl transition ${e.paymentMethod==="manual_bank_transfer"?"selected":"bg-blue-950/30 border-blue-500/10 hover:border-blue-500/30"}">
          <div class="flex items-center gap-3">
    ...
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${t.map(l=>{const c=e.manualPaymentAccounts.find(a=>a.currency===l);return c?`<button onclick="selectCurrency('${l}')" class="btn-press flex flex-col items-center gap-1 p-2.5 rounded-xl border transition relative overflow-hidden ${l===e.selectedCurrency?"bg-blue-500/15 border-blue-500/50 text-blue-400":"bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30"}">
            <span class="text-xl">${c.flag}</span><span class="text-xs font-bold">${l}</span>
          </button>`:""}).join("")}
      </div>
    </div>

    <!-- Bank account details (if manual) -->
    ${e.paymentMethod==="manual_bank_transfer"?ie(s.account,s.fallbackNotice,e.manualPaymentInstructions):""}
    ...
    <div class="space-y-3">
      ${e.paymentMethod==="flutterwave"?`
        <button onclick="payWithFlutterwave()" id="flw-pay-btn" class="btn-press w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="zap" class="w-5 h-5"></i> Pay ${b(y(),e.selectedCurrency)} with Flutterwave
        </button>
      `:`
  ...
  `}function ie(t,s,i){const r=[{label:"Beneficiary Name",value:t.beneficiary},{label:"Bank Name",value:t.bankName},{label:"Account Number",value:t.accountNumber},{label:"IBAN",value:t.iban},{label:"SWIFT / BIC",value:t.swift},{label:"Routing (ABA)",value:t.routing},{label:"Sort Code",value:t.sortCode},{label:"Bank Code",value:t.bankCode},{label:"Branch Code",value:t.branchCode},{label:"Institution Number",value:t.institutionNumber},{label:"Transit Number",value:t.transitNumber},{label:"BSB Code",value:t.bsbCode},{label:"Bank Address",value:t.address}].filter(l=>l.value&&l.value.trim()!=="");return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ${s?`<div class="mb-4 p-3 bg-amber-500/8 border border-amber-500/20 rounded-xl text-sm text-amber-200">${s.message}</div>`:""}
      ...
      <div class="space-y-2">
        ${r.map(l=>`
          <div class="flex items-center justify-between gap-3 bg-blue-950/40 border border-blue-500/10 rounded-xl px-4 py-2.5">
            <div class="min-w-0 flex-1"><div class="text-gray-500 text-[11px] uppercase tracking-wide">${l.label}</div><div class="text-gray-100 text-sm font-medium font-mono break-all">${l.value}</div></div>
            <button onclick="copyToClipboard('${l.value.replace(/'/g,"\\'")}')" class="shrink-0 p-2 bg-blue-900/40 hover:bg-blue-500/20 border border-blue-500/20 rounded-lg transition"><i data-lucide="copy" class="w-4 h-4 text-gray-400"></i></button>
          </div>
        `).join("")}
      </div>
      <div class="mt-4 p-3 bg-blue-950/40 border border-blue-500/10 rounded-xl text-xs text-gray-300 leading-relaxed">${i||"After payment, upload your receipt for verification so your goods can be shipped immediately."}</div>
    </div>
  `}function re(){const t=x(),s=y();return`
        ...
        ${e.cartItems.map(i=>`
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-lg bg-gray-900 overflow-hidden shrink-0 ring-1 ring-blue-500/10">
                <img src="${i.listing.images?.[0]||h}" class="w-full h-full object-cover" onerror="this.src='${h}'">
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-white truncate">${i.listing.title}</p>
                <p class="text-xs text-gray-500">Qty: ${i.quantity}</p>
              </div>
              <p class="text-xs font-bold text-amber-400 shrink-0">${b(i.listing.price*i.quantity,i.listing.currency||"USD")}</p>
            </div>
          `).join("")}
      </div>
      <div class="space-y-2 pt-4 border-t border-blue-500/10">
        <div class="flex justify-between text-sm"><span class="text-gray-500">Subtotal</span><span class="text-white font-bold">${b(t,e.cartItems[0]?.listing?.currency||"USD")}</span></div>
        ...
        ${w()>0?`<div class="flex justify-between text-sm"><span class="text-gray-500">Tax</span><span class="text-white font-bold">${b(w(),e.cartItems[0]?.listing?.currency||"USD")}</span></div>`:""}
        <div class="flex justify-between text-lg pt-2 border-t border-blue-500/10"><span class="text-white font-bold">Total</span><span class="text-amber-400 font-black">${b(s,e.cartItems[0]?.listing?.currency||"USD")}</span></div>
      </div>
  ...
  `}window.changeQty=(t,s)=>{e.cartItems[t].quantity=Math.max(1,e.cartItems[t].quantity+s),u()};window.removeCartItem=t=>{if(e.cartItems.splice(t,1),e.cartItems.length===0){document.getElementById("checkout-root").innerHTML=T(),window.lucide&&lucide.createIcons();return}u()};window.goToStep=t=>{t===3&&!ae()||(t===2&&M(),e.step=t,u(),window.scrollTo({top:0,behavior:"smooth"}))};function M(){(e.isGuest||!e.selectedAddressId)&&(e.fullName=document.getElementById("ship-name")?.value||e.fullName,e.phone=document.getElementById("ship-phone")?.value||e.phone,e.email=document.getElementById("ship-email")?.value||e.email||e.user?.email||"",e.shippingAddr1=document.getElementById("ship-addr1")?.value||"",e.shippingAddr2=document.getElementById("ship-addr2")?.value||"",e.shippingCity=document.getElementById("ship-city")?.value||"",e.shippingState=document.getElementById("ship-state")?.value||"",e.shippingPostal=document.getElementById("ship-postal")?.value||"",e.shippingCountry=document.getElementById("ship-country")?.value||e.shippingCountry),e.billingSame=document.getElementById("billing-same")?.checked??!0,e.billingSame||(e.billingAddress=document.getElementById("billing-address")?.value||"")}function ae(){return M(),e.selectedAddressId&&!e.isGuest?!0:!e.fullName||!e.phone||!e.shippingAddr1||!e.shippingCity||!e.shippingState||!e.shippingPostal?(m("Please fill in all required shipping fields."),!1):e.isGuest&&!e.email?(m("Please enter your email address."),!1):!0}window.selectAddress=t=>{if(e.selectedAddressId=t||null,t){const s=e.addresses.find(i=>i.id===t);s&&(e.fullName=s.full_name,e.phone=s.phone,e.shippingAddr1=s.address_line1,e.shippingAddr2=s.address_line2||"",e.shippingCity=s.city,e.shippingState=s.state,e.shippingPostal=s.postal_code,e.shippingCountry=s.country_code)}u()};window.toggleBilling=()=>{e.billingSame=document.getElementById("billing-same").checked,document.getElementById("billing-fields").classList.toggle("hidden",e.billingSame)};window.selectPaymentMethod=t=>{e.paymentMethod=t,u()};window.selectCurrency=t=>{e.selectedCurrency=t,u()};window.copyToClipboard=J;async function E(t,s={}){const i=x(),r=y(),l=e.selectedAddressId&&e.addresses.length?(()=>{const n=e.addresses.find(d=>d.id===e.selectedAddressId);return n?`${n.address_line1}, ${n.city}, ${n.state} ${n.postal_code}, ${n.country_code}`:""})():`${e.shippingAddr1}${e.shippingAddr2?", "+e.shippingAddr2:""}, ${e.shippingCity}, ${e.shippingState} ${e.shippingPostal}, ${e.shippingCountry}`,c=e.billingSame?l:e.billingAddress||l,a={order_number:e.orderNumber,listing_id:e.listing.property_id,listing_title:e.listing.title,amount:r,currency:e.selectedCurrency,full_name:e.fullName,email:e.email||e.user?.email||"",phone:e.phone,status:"order_placed",payment_method:t,subtotal:i,quantity:e.cartItems.reduce((n,d)=>n+d.quantity,0),billing_address:c,...s};e.isGuest?(a.is_guest=!0,a.user_id=null,a.guest_shipping_address=l,a.guest_country=e.shippingCountry,a.guest_state=e.shippingState,a.guest_city=e.shippingCity,a.guest_postal_code=e.shippingPostal):(a.user_id=e.user.id,e.selectedAddressId&&(a.shipping_address_id=e.selectedAddressId));const{error:o}=await f.from("payment_receipts").insert(a);if(o)throw new Error("Failed to create order: "+o.message);return a}window.payWithFlutterwave=async()=>{const t=document.getElementById("flw-pay-btn");t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Initializing payment...',window.lucide&&lucide.createIcons();try{await E("flutterwave"),localStorage.setItem("kco_pending_order",e.orderNumber);const i=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=initialize",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({amount:y(),currency:e.selectedCurrency,customer_name:e.fullName,customer_email:e.email||e.user?.email,customer_phone:e.phone,order_number:e.orderNumber,redirect_url:`${window.location.origin}/checkout.html?status=verify&order_number=${e.orderNumber}`})}),r=await i.json();if(!i.ok||r.error)throw new Error(r.error||"Failed to initialize payment");window.location.href=r.payment_link}catch(s){t.disabled=!1,t.innerHTML='<i data-lucide="zap" class="w-5 h-5"></i> Pay with Flutterwave',window.lucide&&lucide.createIcons(),m(s.message||"Payment initialization failed.")}};async function le(t,s,i){const r=document.getElementById("checkout-root");r.innerHTML=`
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:s,order_number:i})}),a=await c.json();if(!c.ok||a.error||a.status==="failed"){r.innerHTML=A(a.message||a.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}r.innerHTML=ne(i),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(B),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(l){r.innerHTML=A(l.message),window.lucide&&lucide.createIcons()}}function ne(t){return`
  ...
  `}window.placeOrderManual=async()=>{const t=document.getElementById("manual-pay-btn");t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Creating order...',window.lucide&&lucide.createIcons();try{await E("manual_bank_transfer");try{await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/send-order-notification",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa"},body:JSON.stringify({order_number:e.orderNumber})})}catch{}const s=new URLSearchParams({id:e.listing.property_id,order:e.orderNumber});e.isGuest&&s.set("guest","1"),window.location.href=`/payment.html?${s.toString()}`}catch(s){t.disabled=!1,t.innerHTML='<i data-lucide="check-circle" class="w-5 h-5"></i> Place Order & Upload Receipt',window.lucide&&lucide.createIcons(),m(s.message||"Failed to create order.")}};Q();
