import{h as G,a as W,S as U,c as q,b as K,d as Y,e as O}from"./showroom-data-DqHxX4e9.js";import{P as V,a as B,T as X,M as J,f as Q}from"./motorhome-data-CupbOvk0.js";import{isCatalogListingHidden as E}from"./catalog-hidden-store-ikn3_XIs.js";const v=".showroom-card video, [data-showroom-grid] video, .kco-video-section video, .kco-video-el, .kco-hero-media video",Z="#video-tour-modal video";function m(e){return!!(e&&e.matches&&e.matches(v)&&!e.closest(Z))}const T=600;function R(e){if(m(e)){if(e.setAttribute("muted",""),e.setAttribute("loop",""),e.setAttribute("autoplay",""),e.setAttribute("playsinline",""),e.setAttribute("webkit-playsinline",""),e.muted=!0,e.loop=!0,e.playsInline=!0,e.autoplay=!0,e.ended)try{e.currentTime=0}catch{}e.paused&&e.play().catch(()=>{})}}function ee(e){if(!e.paused)try{e.pause()}catch{}}const g=typeof WeakSet<"u"?new WeakSet:null,l=typeof WeakMap<"u"?new WeakMap:null;let h=null;function oe(){h||typeof IntersectionObserver>"u"||(h=new IntersectionObserver(e=>{for(const o of e){const r=o.target;m(r)&&(o.isIntersecting?(l&&l.set(r,!0),R(r)):(l&&l.set(r,!1),ee(r)))}},{rootMargin:`${T}px 0px ${T}px 0px`}))}function y(e){if(m(e)){if(oe(),!h){R(e);return}g&&g.has(e)||(g&&g.add(e),h.observe(e))}}function L(e){!e||!e.querySelectorAll||e.querySelectorAll(v).forEach(y)}document.addEventListener("ended",e=>{const o=e.target;if(o&&o.tagName==="VIDEO"&&m(o)&&h&&(l?l.get(o):!0)!==!1){try{o.currentTime=0}catch{}o.play().catch(()=>{})}},!0);const z="MutationObserver"in window?new MutationObserver(e=>{for(const o of e)for(const r of o.addedNodes)!r||r.nodeType!==1||(r.matches&&m(r)&&y(r),r.querySelectorAll&&r.querySelectorAll(v).forEach(y))}):null;function I(){if(typeof IntersectionObserver>"u"){const e=()=>{L(document),setTimeout(e,2e3)};e();return}L(document),z&&document.body&&z.observe(document.body,{childList:!0,subtree:!0})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",I):I();const D="kco-hero-rows",re="/fallback.svg",te=new Set(["Cars","Cars & Vehicles","Trucks","Buses","Buses & Coaches","Motorhomes","Motorcycles","Marine & Boating","RV & Camper Accessories","Vehicles"]),k=new Set(["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"]),ie=["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"];function a(e,o){return e.specifications&&typeof e.specifications=="object"&&e.specifications[o]!=null?e.specifications[o]:e[o]}function A(e){return e.listing_type==="vehicle"||te.has(e.category)}function f(e){return!e||typeof e!="string"?!1:/^data:video\//i.test(e)?!0:e.startsWith("blob:")?!1:/\.(mp4|webm|mov|m4v|avi|mkv|ogv)(\?|#|$)/i.test(e)}function ae(e){if(!e)return null;for(const o of[e.video,e.video_url])if(f(o))return o;if(Array.isArray(e.images)){for(const o of e.images)if(f(o))return o}return null}function ce(e){if(Array.isArray(e.images)){const o=e.images.find(r=>!f(r));if(o)return o}return""}function se(e){const o=new Set,r=[];for(const t of e){if(!t)continue;const c=t.property_id||t.id;!c||o.has(c)||(o.add(c),r.push(t))}return r}function F(e){return se(e).filter(o=>E&&E(o.property_id)?!1:o.property_id&&String(o.property_id).startsWith("W")?!0:o.is_active!==!1).sort((o,r)=>{const t=Number.isFinite(parseFloat(o.price))?parseFloat(o.price):0;return(Number.isFinite(parseFloat(r.price))?parseFloat(r.price):0)-t})}function ne(e){const o=e.property_type||e.subcategory||e.category;if(o&&!p.has(o)){const r=String(o).toLowerCase();for(const t of ie)if(r===t.toLowerCase()||r.includes(t.toLowerCase())){p.set(o,t);break}p.has(o)||p.set(o,String(o))}return o?p.get(o):"Homes"}const p=new Map;function ue(){const o=(O()||[]).filter(t=>t.listing_type==="property"||t.category==="Real Estate"||t.category==="Houses & Real Estate"||k.has(t.property_type)||k.has(t.subcategory)),r=[...V,...B,...U].filter(t=>t&&(t.category==="Houses & Real Estate"||t.category==="Real Estate"||k.has(t.property_type||t.subcategory)));return F([...o,...r])}function ge(){const o=(O()||[]).filter(t=>A(t)),r=[...X,...J,...V,...B].filter(t=>t&&A(t));return F([...o,...r])}function i(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function de(){if(document.getElementById("kco-hero-styles"))return;const e=document.createElement("style");e.id="kco-hero-styles",e.textContent=`
...
`,document.head.appendChild(e)}function be(e,o,r){q(e);const t=r,c=ae(e)||e.images&&e.images[0]||re,x=ce(e),S=x?` poster="${i(x)}"`:"",$=w(e.property_id||e.id),_=o==="vehicle"&&e.category==="Trucks"?Q(e):K(e);let s="",u="";if(o==="house"){const P=Y(e.country_code),C=[e.city,e.state].filter(Boolean).join(", ")||e.country||"";C&&(u=`<span class="kco-hero-loc"><i data-lucide="map-pin" class="w-3 h-3 shrink-0"></i>${i(P+" "+C)}</span>`);const d=[];a(e,"bedrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bed-double" class="w-3.5 h-3.5"></i>${i(a(e,"bedrooms"))} Beds</span>`),a(e,"bathrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bath" class="w-3.5 h-3.5"></i>${i(a(e,"bathrooms"))} Baths</span>`),e.land_size&&d.push(`<span class="kco-hero-chip"><i data-lucide="ruler" class="w-3.5 h-3.5"></i>${i(e.land_size)}</span>`),a(e,"building_size")&&d.push(`<span class="kco-hero-chip"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>${i(a(e,"building_size"))}</span>`),a(e,"year_built")&&d.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${i(a(e,"year_built"))}</span>`);const j=ne(e);return s=d.join(""),u||(u=""),`
      <a href="/details.html?id=${w($)}" class="${t}">
        <div class="kco-hero-media">
          ${f(c)?`<video src="${i(c)}"${S} muted loop autoplay playsinline preload="metadata" class="kco-hero-media-video"></video><span class="kco-hero-video-badge"><i data-lucide="play" class="w-3 h-3"></i>Video Tour</span>`:`<img src="${i(c)}" alt="${i(e.title||"")}" loading="lazy">`}
          <span class="kco-hero-type"><i data-lucide="home" class="w-3 h-3"></i>${i(j)}</span>
          ${u}
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${_}</b><span>${e.listing_status==="rent"?"per month · for rent":"for sale"}</span></div>
      ...
      </a>`}const n=[];a(e,"model_year")&&n.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${i(a(e,"model_year"))}</span>`),a(e,"mileage")&&n.push(`<span class="kco-hero-chip"><i data-lucide="gauge" class="w-3.5 h-3.5"></i>${i(a(e,"mileage"))}</span>`),a(e,"fuel_type")&&n.push(`<span class="kco-hero-chip"><i data-lucide="fuel" class="w-3.5 h-3.5"></i>${i(a(e,"fuel_type"))}</span>`),a(e,"transmission")&&n.push(`<span class="kco-hero-chip"><i data-lucide="cog" class="w-3.5 h-3.5"></i>${i(a(e,"transmission"))}</span>`),a(e,"body_type")&&n.push(`<span class="kco-hero-chip"><i data-lucide="car-front" class="w-3.5 h-3.5"></i>${i(a(e,"body_type"))}</span>`),s=n.join("");const N=f(c)?`<video src="${i(c)}"${S} muted loop autoplay playsinline preload="metadata" class="kco-hero-media-video"></video><span class="kco-hero-video-badge"><i data-lucide="play" class="w-3 h-3"></i>Video Tour</span>`:`<img src="${i(c)}" alt="${i(e.title||"")}" loading="lazy">`;return`
      <a href="/details.html?id=${w($)}" class="${t}">
        <div class="kco-hero-media">
          ${N}
          <span class="kco-hero-type"><i data-lucide="car-front" class="w-3 h-3"></i>${i(le(e))}</span>
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${_}</b><span>· ${i(a(e,"condition")||"ready")}</span></div>
          <p class="kco-hero-title">${i(e.title||"")}</p>
          ${s?`<div class="kco-hero-chips">${s}</div>`:""}
          <span class="kco-hero-btn">View Vehicle <i data-lucide="arrow-right" class="w-4 h-4"></i></span>
        </div>
      </a>`}function w(e){return String(e??"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function le(e){const o=String(e.category||""),r={Cars:"Car","Cars & Vehicles":"Car",Trucks:"Truck",Buses:"Bus","Buses & Coaches":"Bus",Motorhomes:"Motorhome / RV",Motorcycles:"Motorcycle","Marine & Boating":"Boat / Marine"};return r[o]?r[o]:a(e,"body_type")||o||"Vehicle"}function b(){const e=document.getElementById(D);if(!e)return;de();const o=document.createDocumentFragment();e.replaceChildren(o),window.lucide&&lucide.createIcons()}let H=!1;function M(){H||(H=!0,document.getElementById(D)&&(G(),b(),W().then(()=>b()).catch(()=>{}),window.addEventListener("showroom-categories-ready",()=>b()),window.addEventListener("kco-db-refresh",()=>b())))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",M):M();export{ge as a,ne as b,be as c,ue as g,de as h,le as v};
