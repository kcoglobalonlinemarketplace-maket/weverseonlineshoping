...
import"./modulepreload-polyfill-B5Qt9EMX.js";import{l as H,f as I,_ as f,a as z,g as J}from"./showroom-data-Dx7pQsXv.js";import{supabase as h}from"./supabase-client-nvpjTmO6.js";import{getCurrentUser as V}from"./auth-DlKVuhCK.js";import{P as X,a as Y,g as A,b as P}from"./motorhome-data-CupbOvk0.js";import{g as B,a as T}from"./phone-data-Of7KtnOV.js";import{d as N,g as Q,a as W}from"./country-data-BCWopR2U.js";import{c as C,d as K,a as Z,b as ee,r as te,e as re}from"./payment-settings-pG5W3oFr.js";import{f as ae,c as g,p as se,a as _}from"./fx-B0aevBpC.js";/* empty css                                       */const ie=[...X,...Y];function M(t){return ie.find(r=>r.property_id===t)||null}const x="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:"",currencyManuallySelected:!1};function ne(){const t=Date.now().toString(36).toUpperCase().slice(-6),r=Math.random().toString(36).toUpperCase().slice(2,6);return`W-${t}${r}`}function m(t){const r=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,r.classList.remove("translate-y-20","opacity-0"),clearTimeout(r._t),r._t=setTimeout(()=>r.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function j(t){t.addEventListener("click",function(r){if(this.disabled)return;const a=this.getBoundingClientRect(),i=document.createElement("span");i.className="ripple";const s=Math.max(a.width,a.height);i.style.width=i.style.height=s+"px",i.style.left=r.clientX-a.left-s/2+"px",i.style.top=r.clientY-a.top-s/2+"px",this.appendChild(i),setTimeout(()=>i.remove(),600)})}function k(t,r){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${r}`}function le(t){const r=k(t.price,t.currency||"USD"),a=parseFloat(t.real_price);return Number.isFinite(a)&&a>0&&a>parseFloat(t.price)?`<span class="price-strike line-through text-gray-400 mr-1 text-xs">${k(a,t.currency||"USD")}</span><span class="text-amber-600 font-bold">${r}</span>`:r}function oe(t){const r=()=>{const a=document.createElement("textarea");a.value=t,document.body.appendChild(a),a.select();try{document.execCommand("copy")}catch{}document.body.removeChild(a)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>r()):r(),m("Copied to clipboard.")}function de(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,r=te(e.manualPaymentAccounts,e.countryCode,t);return{...r,fallbackNotice:r.isFallback?re(r.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function ce(){const t=document.getElementById("particles");if(t)for(let r=0;r<12;r++){const a=document.createElement("div"),i=Math.random()*3+1;a.className="particle",a.style.width=a.style.height=i+"px",a.style.left=Math.random()*100+"%",a.style.bottom="-10px",a.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",a.style.animationDuration=Math.random()*20+15+"s",a.style.animationDelay=Math.random()*20+"s",t.appendChild(a)}}ce();async function ue(){const t=document.getElementById("checkout-root"),r=new URLSearchParams(window.location.search),a=r.get("status"),i=r.get("transaction_id"),s=r.get("tx_ref"),l=r.get("order_number");if(a==="verify"&&i){await we(i,s,l||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=r.get("guest")==="1",!e.isGuest&&(e.user=await V(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const n=r.get("id");if(n){let d=await H(n);if(e.listing=d||I(n)||A(n)||P(n)||B(n)||T(n)||M(n),!e.listing){const[{generateListingById:b},{loadHiddenCatalogIds:o}]=await Promise.all([f(()=>import("./catalog-BaQ2rKyX.js"),__vite__mapDeps([0,1,2])),f(()=>import("./catalog-hidden-store-CPivWtWH.js"),__vite__mapDeps([1,2]))]);await o(),e.listing=b(n)}if(e.listing||(e.listing=I(n)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const d=JSON.parse(localStorage.getItem("kco_cart")||"[]");await z();const b=J(),[{generateListingById:o},{loadHiddenCatalogIds:w}]=await Promise.all([f(()=>import("./catalog-BaQ2rKyX.js"),__vite__mapDeps([0,1,2])),f(()=>import("./catalog-hidden-store-CPivWtWH.js"),__vite__mapDeps([1,2]))]);if(await w(),e.cartItems=d.map(c=>{const u=typeof c=="string"?c:c&&c.id;if(!u)return null;const R=c&&typeof c=="object"&&c.qty?Math.max(1,parseInt(c.qty,10)||1):1,S=b.find(G=>G.property_id===u)||A(u)||P(u)||B(u)||T(u)||M(u)||o(u);return S?{listing:S,quantity:R}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=q(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:d}=await h.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();d?.country_code&&(e.countryCode=d.country_code)}if(e.autoDetectedCurrency=N(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.shippingCountry=e.countryCode,e.paymentSettings=await K(),e.manualPaymentAccounts=Z(e.paymentSettings),e.manualPaymentInstructions=ee(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),C(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),await se(),e.user&&!e.isGuest){const{data:d}=await h.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=d||[];const b=e.addresses.find(w=>w.is_default);b&&(e.selectedAddressId=b.id);const{data:o}=await h.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();o&&(e.fullName=o.display_name||`${o.first_name||""} ${o.last_name||""}`.trim(),e.email=e.user.email,e.phone=o.phone_code&&o.phone_number?`+${o.phone_code} ${o.phone_number}`:"",e.shippingCountry=o.country_code||e.countryCode)}e.orderNumber=ne(),p()}function y(){return e.cartItems.reduce((t,r)=>t+r.listing.price*r.quantity,0)}function pe(){return 0}function be(){return 0}function $(){return Math.round(y()*be()*100)/100}function v(){return y()+pe()+$()}function L(){return g(v(),e.selectedCurrency)}function D(){return _(L(),e.selectedCurrency)}function O(){const t=(e.shippingCountry||"").toUpperCase();if(t&&(e.countryCode=t,localStorage.setItem("kco_country",t),!e.currencyManuallySelected)){e.autoDetectedCurrency=N(t)||"";const r=C(e.manualPaymentAccounts);e.selectedCurrency=e.autoDetectedCurrency&&r.includes(e.autoDetectedCurrency)?e.autoDetectedCurrency:"USD"}}function p(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(j)}function ge(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
        ...
        ${e.cartItems.map((t,r)=>`
            <div class="flex items-center gap-3 p-3 bg-gray-50 border border-blue-100 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-600 mt-1">${le(t.listing)}</p>
              </div>
  ...
  `}function ye(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-gray-900 uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(a=>{const i=Q(a.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
              ${W.map(a=>`<option value="${a.code}" ${e.shippingCountry===a.code?"selected":""}>${a.flag} ${a.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function fe(){const t=C(e.manualPaymentAccounts),r=de(),a=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",i=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    ${e.paymentMethod==="manual_bank_transfer"?he(r.account,r.fallbackNotice,e.manualPaymentInstructions):""}
    ...
    <div class="space-y-3">
      ${e.paymentMethod==="flutterwave"?`
        <button onclick="payWithFlutterwave()" id="flw-pay-btn" class="btn-press w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="zap" class="w-5 h-5"></i> Pay ${D()} with Flutterwave
        </button>
      `:`
  ...
  `}function xe(){const t=y();return v(),`
    <div class="glass border border-blue-200 rounded-2xl p-5 slide-up lg:sticky lg:top-20">
      ...
      <div class="space-y-2 pt-4 border-t border-blue-100">
        ...
        <div class="flex justify-between text-lg pt-2 border-t border-blue-100"><span class="text-gray-900 font-bold">Total</span><span class="text-amber-600 font-black">${D()}</span></div>
      </div>
