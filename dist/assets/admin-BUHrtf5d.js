...
This is permanent and cannot be undone. Your built-in showroom catalog will stay.`))return;const{error:t}=await p.from("showroom_listings").delete().neq("property_id","__none__");if(t)return G(t)?u("⚠️ Delete blocked: database admin role rejected the write. Re-run the admin permission migration.","error"):u("Clear failed: "+t.message,"error");try{localStorage.removeItem("kco_local_showroom_listings_v1")}catch{}u("All products deleted. The manager now shows your showroom catalog."),R()};window.openProductMoreActions=function(e){U(`
  ...
  <div class="scan-review-card rounded-xl border border-violet-500/30 bg-violet-500/10 p-3 space-y-2 fade-in" data-i="${t}">
    ...
    </div>`)};window.saveCoupon=async function(e){e.preventDefault();const t=new FormData(e.target),a=Object.fromEntries(t.entries()),i={code:a.code.toUpperCase(),discount_type:a.discount_type,discount_value:parseFloat(a.discount_value),min_amount:a.min_amount?parseFloat(a.min_amount):null,usage_limit:a.usage_limit?parseInt(a.usage_limit):null,expires_at:a.expires_at||null,is_active:!0},{error:o}=await p.from("coupons").insert(i);if(o){u(o.message,"error");return}u("Coupon created!"),ie(),Ye()};window.toggleCoupon=async function(e,t){await p.from("coupons").update({is_active:t}).eq("id",e),u(t?"Coupon activated":"Coupon deactivated"),Ye()};window.deleteCoupon=async function(e){confirm("Delete this coupon?")&&(await p.from("coupons").delete().eq("id",e),u("Coupon deleted"),Ye())};async function no(){const e=document.getElementById("content");try{const{data:t}=await p.from("notification_log").select("*").order("created_at",{ascending:!1}).limit(100),a=t||[];e.innerHTML=`
      <div class="space-y-4 fade-in">
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      ...
      </div>`,window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400">${s(t.message)}</div>`)}}window.renderAds=_e;const Je=[{id:"gemini",name:"Google Gemini",tag:"FREE",color:"blue",icon:"sparkles",kf:"gemini_key",ph:"AIzaSy…",signup:"https://aistudio.google.com/apikey",models:["gemini-3-flash-preview","gemini-3.1-flash-lite-preview"],mf:"gemini_model",dm:"gemini-3-flash-preview",desc:"Google's best free AI. Great for coding, writing apps & websites.",free_tier:"15 req/min · 1M tokens/day — Free forever"}],X={border:{blue:"border-blue-500/50"},bg:{blue:"bg-blue-500/8"},text:{blue:"text-blue-400"},badge:{blue:"bg-blue-500/15 text-blue-300"}};async function La(){const e=document.getElementById("content");try{let t=function(r){const n=o===r.id,l=i[r.kf],c=i[r.mf]||r.dm;return`
...
${a.text}`}catch(a){t.textContent=`❌ ${a.message}`}};function ot(e){if(!e)return null;let t=String(e).trim();const a=t.match(/```(?:json)?\s*([\s\S]*?)```/i);a&&(t=a[1].trim());const i=t.indexOf("{"),o=t.lastIndexOf("}");if(i===-1||o===-1||o<=i)return null;const r=t.slice(i,o+1);try{return JSON.parse(r)}catch{return null}}async function bo(){const e=document.getElementById("content");try{const[{data:t},a]=await Promise.all([p.from("site_settings").select("*").limit(1).maybeSingle(),go()]),i=t||{},o=new Set(Array.isArray(i.live_promo_product_ids)?i.live_promo_product_ids:[]),r=a.length?`
    ...
    </div>`}).join("")}window.heroVideoUpload=function(e,t){const a=document.createElement("input");a.type="file",a.accept=t==="video"?"video/mp4,video/webm,.mp4,.webm":"image/*",a.onchange=()=>{const i=a.files&&a.files[0];i&&vo(e,t,i)},a.click()};window.heroVideoField=function(e,t,a){const i=te();i[e]&&(i[e][t]=a,Qe())};window.heroVideoPreset=function(e,t){const a=te();a[e]&&(a[e].buttonText=t,me())};window.heroVideoToggle=function(e){const t=te();t[e]&&(t[e].enabled=t[e].enabled===!1,me())};window.heroVideoMove=function(e,t){const a=te(),i=e+t;i<0||i>=a.length||([a[e],a[i]]=[a[i],a[e]],me())};window.heroVideoDelete=function(e){const t=te();e<0||e>=t.length||confirm("Delete this hero video slide?")&&(t.splice(e,1),me())};window.heroVideoRemoveMedia=function(e,t){const a=te();a[e]&&(t==="video"?a[e].video="":t==="poster"&&(a[e].poster=""),me())};window.addHeroVideoSlide=function(){te().push({id:"hv"+Date.now()+Math.floor(Math.random()*999),enabled:!0,video:"",poster:"",title:"",subtitle:"",buttonText:"SHOP NOW",buttonLink:"/#showroom-directory"}),me(),u("New slide added — upload a video and press Save to show it.","info")};async function ho(e,t){try{const{data:{session:a}}=await p.auth.getSession(),i=(e.name.split(".").pop()||(t==="video"?"mp4":"jpg")).toLowerCase().replace(/[^a-z0-9]/g,"");if(!a)return URL.createObjectURL(e);const o=`hero/${t}/${Date.now()}-${Math.random().toString(36).slice(2)}.${i}`,{error:r}=await p.storage.from("product-images").upload(o,e,{contentType:e.type,cacheControl:"3600",upsert:!0});if(r)return URL.createObjectURL(e);const{data:n}=p.storage.from("product-images").getPublicUrl(o);return n&&n.publicUrl||URL.createObjectURL(e)}catch{return URL.createObjectURL(e)}}async function vo(e,t,a){const i=te();if(!a||!i[e])return;if(t==="video"){if(!/video\/(mp4|webm)|\.(mp4|webm)$/i.test(a.type+" "+a.name)){u("Please choose an MP4 or WebM video file.","error");return}}else if(!a.type.startsWith("image/")){u("Please choose an image for the poster.","error");return}const o=await ho(a,t);t==="video"?i[e].video=o:i[e].poster=o,me(),u("✓ "+(t==="video"?"Video":"Poster")+" uploaded — press Save to apply","success")}function xo(e){const t=Array.isArray(e)?e.map(i=>({...i})):[];return window._heroVideoDraft=t,Qe(),`
    <div class="space-y-3">
