    ...
    <div class="space-y-4">
      ...
      <div class="space-y-6 fade-in">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          ${k("Total Revenue",`$${n.toLocaleString("en-US",{maximumFractionDigits:0})}`,"dollar-sign","emerald",`$${K.toLocaleString("en-US",{maximumFractionDigits:0})} this month`)}
          ${k("Total Orders",r.length,"shopping-bag","blue",`${d} pending`)}
          ${k("Customers",g,"users","violet")}
          ${k("Products",c,"package","amber",`${b} active`)}
            ...
            ${G.length===0?'<p class="text-xs text-gray-500 text-center py-8">No orders yet</p>':G.map(v=>`
                <div class="flex items-center justify-between py-2 border-b border-blue-500/5 last:border-0">
                  <div class="min-w-0">
                    ...
                    <p class="text-[10px] text-gray-500">${R(v.created_at)}</p>
                  </div>
...
<div class="space-y-4">
    ...
    ${a.length?`<div class="flex flex-wrap gap-1">${a.slice(0,6).map(b=>`<span class="text-[10px] px-2 py-0.5 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-200">${l(b)}</span>`).join("")}</div>`:'<div class="text-[10px] text-gray-500">No tags</div>'}
    ...
    </div>`)};window.saveQuickEditProduct=async function(e,t){e.preventDefault();const a=new FormData(e.target),i={title:a.get("title")||"Untitled Product",price:Math.max(M,Math.min(V,parseFloat(a.get("price"))||0)),stock_quantity:a.get("stock_quantity")===""?null:parseInt(a.get("stock_quantity"),10),availability_status:a.get("availability_status")||"In Stock",is_featured:a.get("is_featured")==="on",is_active:a.get("is_active")==="on"},s=Q((window._productsData||[]).find(r=>r.property_id===t)),{error:o}=await p.from("showroom_listings").upsert({...s,...i,property_id:t},{onConflict:"property_id"});if(o){if(B(o)){u("⚠️ Save blocked: database admin role rejected the write. Re-run the admin permission migration.","error"),H(),C();return}ka(t,i),u("Quick edit saved locally","info")}else u("Quick edit saved");H(),C()};window.publishProduct=function(e){return toggleProductActive(e,!0)};window.unpublishProduct=function(e){return toggleProductActive(e,!1)};window.shareProduct=async function(e){const t=`${window.location.origin}/details.html?id=${encodeURIComponent(e)}`;try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(t),u("Product link copied to clipboard");return}}catch{}window.prompt("Copy product link:",t)};window.deleteProduct=async function(e){if(!confirm("Delete this product permanently? This action cannot be undone."))return;const{error:t}=await p.from("showroom_listings").delete().eq("property_id",e);if(t)return B(t)?u("⚠️ Delete blocked: database admin role rejected the write. Re-run the admin permission migration.","error"):u("Delete failed: "+t.message,"error");u("Product deleted"),C()};window.openProductMoreActions=function(e){L(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box">
        <div class="flex items-center justify-between mb-4">
        ...
        <form id="product-form" onsubmit="saveProduct(event,'${l(e)}','${a?t.property_id:""}')" class="space-y-4">
          ...
          <div class="form-grid form-grid-2">
            ...
            <div class="p-3 glass-soft border border-blue-500/15 rounded-xl">
              ...
              <p class="text-[11px] text-gray-500 mt-1">Allowed price range is ${M} to ${V} in the selected currency.</p>
            </div>
          </div>
  ...
  `}window.previewProductDraft=function(){const e=document.getElementById("product-form");if(!e)return;const t=document.querySelector("#image-preview img")?.src||"/fallback.svg",a=e.querySelector('[name="title"]')?.value||"Untitled Product",i=e.querySelector('[name="description"]')?.value||"No description yet.",s=e.querySelector('[name="brand"]')?.value||"N/A",o=parseFloat(e.querySelector('[name="price"]')?.value||"0")||0,r=e.dataset.category||"Product",n=e.querySelector('[name="stock_quantity"]')?.value||"Unlimited",d=e.querySelector('[name="is_active"]')?.checked;L(`
    ...
    </div>`)};window.updateOrderStatus=async function(e){const t=document.getElementById("order-status-select")?.value;if(!t)return;const{error:a}=await p.from("payment_receipts").update({status:t}).eq("id",e);if(a){u(a.message,"error");return}u("Order status updated"),H(),ia()};async function Ci(){const e=document.getElementById("content");try{const{data:t}=await p.from("profiles").select("*").order("created_at",{ascending:!1}).limit(200),a=t||[];e.innerHTML=`
      <div class="space-y-4 fade-in">
        ...
        <div class="glass-soft border ${d?F.border[n.color]+" "+F.bg[n.color]:"border-blue-500/10"} rounded-2xl p-4 space-y-3 ai-pcard" id="apc-${n.id}">
          ...
          <div>
            ...
            <select class="input-field text-xs" name="${n.mf}">
              ${n.models.map(b=>`<option value="${b}" ${m===b?"selected":""}>${b}</option>`).join("")}
            </select>
          </div>
...
${a.text}`}catch(a){t.textContent=`❌ ${a.message}`}};let h=null;const da=[{key:"rewrite",label:"Rewrite Title & Description",icon:"pen-line",prompt:"Rewrite the title and description to be more professional, compelling, and conversion-focused. Keep all product facts accurate. The title must be a real, professional product name that matches the item and its category — never a generic placeholder."},{key:"name",label:"Professional Name",icon:"badge-check",prompt:'Give this product a real, professional product name that accurately matches what it actually is and its category. Never use placeholder names like "AI Product", "AI Curated Product", or anything similar.'},{key:"price",label:"Optimize Price & Stock",icon:"dollar-sign",prompt:"Suggest a competitive, realistic price and an optimal stock quantity for this product based on the category and current data."},{key:"category",label:"Fix Category & Specs",icon:"tags",prompt:"Fix the category, subcategory, and specifications so they accurately describe this product. Use the category list if possible."},{key:"images",label:"Clean Up Duplicate/Bad Images",icon:"images",prompt:"Identify duplicate, broken, or low-quality images among the CURRENT images list only, and list their exact URLs in images_to_remove. Do not invent any image URLs."}];function ji(e){return[...window._productsData||[],...window._propertiesData||[]].find(a=>a.property_id===e)||null}function Oi(e){const t=["property_id","listing_type","category","subcategory","title","description","price","currency","brand","color","size","condition","availability_status","stock_quantity","features","tags","highlights","seo_keywords","specifications","bedrooms","bathrooms","city","state","country","images"],a={};for(const i of t)i in e&&(a[i]=e[i]);return a}function be(e){if(!e)return null;let t=String(e).trim();const a=t.match(/```(?:json)?\s*([\s\S]*?)```/i);a&&(t=a[1].trim());const i=t.indexOf("{"),s=t.lastIndexOf("}");if(i===-1||s===-1||s<=i)return null;const o=t.slice(i,s+1);try{return JSON.parse(o)}catch{return null}}function wt(e){return Array.isArray(e)?e.length?e.join(" · "):"(empty)":e&&typeof e=="object"?JSON.stringify(e):e==null||e===""?"(empty)":String(e)}window.openProductAiAssistant=async function(e,t="products"){let a=ji(e);if(!a)try{const{data:i}=await p.from("showroom_listings").select("*").eq("property_id",e).maybeSingle();i&&(a=i)}catch{}if(!a){u("Product not found.","error");return}h={pid:e,from:t==="properties"?"properties":"products",product:a,busy:!1,suggestions:null,approved:new Set,removedImages:new Set,addedImages:[],instruction:""},q()};function q(){const e=h;if(!e)return;const t=e.product,a=t.images&&t.images[0]?t.images[0]:"/fallback.svg";L(`
    ...
    </div>`),window.lucide&&lucide.createIcons()}function qi(){const e=h;if(!e)return"";const t=Array.isArray(e.product.images)?e.product.images:[],a=[...t,...e.addedImages],i={};t.forEach(r=>{i[r]=(i[r]||0)+1});const s=Object.keys(i).filter(r=>i[r]>1),o=a.map((r,n)=>{const d=n<t.length,c=d&&e.removedImages.has(r),m=d&&s.includes(r);return`
    ...
    </div>`}function Ni(){const e=h;return e?`
    <div class="glass-soft border border-fuchsia-500/20 rounded-2xl p-4">
    ...
    </div>`:""}function Hi(){const e=h;if(!e)return"";if(!e.suggestions)return`
      <div class="glass-soft border border-blue-500/15 rounded-2xl p-4">
    ...
    <div class="glass-soft border border-emerald-500/20 rounded-2xl p-4 space-y-2">
      ...
      </div>`}window.productAiSyncInput=function(e){h&&(h.instruction=e)};window.productAiQuickAction=function(e){const t=da.find(a=>a.key===e);!t||!h||(h.instruction=t.prompt,runProductAiInstruction())};window.productAiToggleApproved=function(e,t){h&&(t?h.approved.add(e):h.approved.delete(e),q())};window.productAiApproveAll=function(){if(!(!h||!h.suggestions)){for(const e of Object.keys(h.suggestions))h.approved.add(e);h.removedImages.size&&h.approved.add("images_to_remove"),q()}};window.productAiToggleRemoveImage=function(e){if(!h)return;const t=Array.isArray(h.product.images)?h.product.images:[],i=[...t,...h.addedImages][e];i&&(e<t.length?h.removedImages.has(i)?h.removedImages.delete(i):h.removedImages.add(i):h.addedImages=h.addedImages.filter(s=>s!==i),q())};window.productAiDetectDuplicates=function(){if(!h)return;const e=Array.isArray(h.product.images)?h.product.images:[],t={};e.forEach(s=>{t[s]=(t[s]||0)+1});let a=0;const i=new Set;for(const s of e)t[s]>1&&(i.has(s)?(h.removedImages.has(s)||(a+=1),h.removedImages.add(s)):i.add(s));a?(h.approved.add("images_to_remove"),u(`Marked ${a} duplicate image(s) for removal.`,"info")):u("No duplicate images found.","info"),q()};window.productAiUploadImages=async function(e){if(!h||!e||!e.files||!e.files.length)return;const t=[...e.files];let a=0;for(const i of t){if(!i.type.startsWith("image/"))continue;const s=await Se(i);s&&(h.addedImages.push(s),a+=1)}e.value="",a?u(`${a} image(s) uploaded and staged.`,"success"):u("Upload failed.","error"),q()};window.runProductAiInstruction=async function(){const e=h;if(!e||e.busy)return;const t=(e.instruction||"").trim()||"Improve this product listing professionally.";e.busy=!0,q();const a=Gi(e,t);try{const i=await W.prompt(a,{maxTokens:4e3,onProviderSwitch:o=>{}}),s=be(i.text);if(!s)throw new Error("The AI did not return valid JSON suggestions.");e.suggestions=s,e.approved=new Set(Object.keys(s)),e.approved.add("images_to_remove"),u("Suggestions ready — review and approve before saving.","info")}catch(i){u(`AI error: ${i.message}`,"error")}finally{e.busy=!1,q()}};function Gi(e,t){const a=e.product,i=JSON.stringify(Oi(a),null,2);return`You are the AI product manager assistant for ONE product in the Weverse Online Shop admin dashboard.
...
`),o=await W.prompt(s,{onProviderSwitch:r=>{i.value=`Generating with ${r}...`}});i.value=o.text||"",await dt({created_at:new Date().toISOString(),goal:t,brief:e,provider_name:o.provider,provider_id:a?.value||"",status:"script_generated",script:o.text||"",video_url:null}),u(`Script generated with ${o.provider}.`,"success")}catch(s){i.value="",u("Script generation failed: "+s.message,"error")}}};window.activateAiAdvertisement=async function(e){e.preventDefault();const t=new FormData(e.target),a=String(t.get("goal")||"Product launch"),i=String(t.get("provider_id")||""),s=nt.find(g=>g.id===i)?.name||i,o=String(t.get("brief")||"").trim(),r=String(t.get("script")||"").trim(),n=String(t.get("video_url")||"").trim(),d=Math.max(5,Math.min(900,parseInt(String(t.get("duration_seconds")||"30"),10)||30)),c=Date.now(),m=new Date(c).toISOString(),b=new Date(c+d*1e3).toISOString();if(!n){u("Video URL is required.","error");return}try{const g={ai_ad_enabled:!0,ai_ad_video_url:n,ai_ad_badge:String(t.get("badge")||"AI Advertisement").trim()||"AI Advertisement",ai_ad_title:String(t.get("title")||"").trim(),ai_ad_cta_label:String(t.get("cta_label")||"Shop Now").trim()||"Shop Now",ai_ad_duration_seconds:d,ai_ad_muted:t.get("muted")==="on",ai_ad_provider_id:i,ai_ad_starts_at:m,ai_ad_ends_at:b,ai_ad_updated_at:new Date().toISOString()};await pa(g),await dt({created_at:m,goal:a,brief:o,provider_name:s,provider_id:i,status:"active",script:r,video_url:n,ends_at:b}),u("AI advertisement activated. Homepage will switch to AI video now.","success"),Ie()}catch(g){u("Failed to activate AI advertisement: "+g.message,"error")}};window.deactivateAiAdvertisement=async function(){try{await pa({ai_ad_enabled:!1,ai_ad_updated_at:new Date().toISOString()}),await dt({created_at:new Date().toISOString(),goal:"Manual stop",provider_name:"Admin",provider_id:"manual",status:"inactive",script:"",video_url:null}),u("AI advertisement stopped.","success"),Ie()}catch(e){u("Failed to stop AI advertisement: "+e.message,"error")}};async function Ki(){const e=document.getElementById("content");try{const{data:t}=await p.from("site_settings").select("*").limit(1).maybeSingle(),a=t||{};e.innerHTML=`
      <div class="space-y-6 fade-in">
            ...
            <table class="w-full dt">
              ...
              <tbody>
                ${s.length===0?'<tr><td colspan="4" class="text-center text-gray-500 py-8">No security events yet</td></tr>':s.map(c=>{const m=["login_success","login_2fa_success"].includes(c.event_type),b=["login_failed","login_denied","login_backup_code_used"].includes(c.event_type),g=m?"text-emerald-400":b?"text-red-400":"text-gray-300",y={login_success:"Login ✓",login_failed:"Failed Login ✗",login_denied:"Access Denied ✗",login_2fa_success:"2FA Verified ✓",login_backup_code_used:"Backup Code Used",logout:"Logged Out",logout_all_devices:"Logout All Devices"}[c.event_type]||c.event_type;return`<tr>
                      <td><span class="text-xs font-bold ${g}">${l(y)}</span></td>
...
`+a],{type:"text/csv"}),s=document.createElement("a");s.href=URL.createObjectURL(i),s.download=`kco-orders-${new Date().toISOString().slice(0,10)}.csv`,s.click(),u("Orders exported!")};async function es(){const e=document.getElementById("content"),{data:t}=await p.from("site_settings").select("*").limit(1).maybeSingle(),a=t||{};e.innerHTML=`
    <div class="space-y-5 fade-in">
      ...
      <div class="space-y-5 fade-in">
      ...
      </div>`,window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400">${l(t.message)}</div>`)}}async function Ce(){const e=document.getElementById("content");e&&(e.innerHTML=Y());try{let t=function(n,d,c,m="",b="blue"){const g=!!(c&&c.trim());return`
        <div class="glass-soft border border-${b}-500/15 rounded-xl p-4 space-y-3">
          ...
          ${g?`<div class="relative group w-full h-24 rounded-xl overflow-hidden bg-gray-900 border border-blue-500/10 flex items-center justify-center">
               ...
               </div>`:`<div class="w-full h-24 rounded-xl border-2 border-dashed border-${b}-500/25 hover:border-${b}-500/50 flex flex-col items-center justify-center gap-2 cursor-pointer transition" onclick="triggerImgUpload('${d}')">
          ...
          <div class="flex gap-2">
            ...
            <button type="button" onclick="document.getElementById('url-${d}').classList.toggle('hidden')" class="text-[10px] text-${b}-400 hover:text-${b}-300 transition shrink-0">${g?"Edit URL":"Paste URL"}</button>
          </div>
    ...
    <div class="flex flex-wrap items-center gap-2">
      ...
      <button onclick="catalogSearch()" class="btn-press px-4 py-2 rounded-xl text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white transition">Search</button>
    </div>`;let c=[];if(i)if(o){const y=Math.min(s,8e3);for(let f=0;f<y&&c.length<le;f++){const x=yt(i.slug,f);if(!x)continue;`${x.property_id} ${x.title} ${x.subcategory||""} ${x.category||""}`.toLowerCase().includes(o)&&c.push(x)}}else{const y=S.page*le,f=Math.min(y+le,s);for(let x=y;x<f;x++){const $=yt(i.slug,x);$&&c.push($)}}const m=c.length?c.map(y=>{const f=t.has(y.property_id),x=y.images&&y.images[0]||"/fallback.svg";return`
          <div class="flex items-center gap-3 p-3 rounded-xl border ${f?"border-red-500/25 bg-red-500/5":"border-white/10 bg-white/[0.02]"}">
            <img src="${l(x)}" alt="" class="w-12 h-12 rounded-lg object-cover bg-gray-800 shrink-0" loading="lazy">
            <div class="flex-1 min-w-0">
          ...
          </div>`}).join(""):'<div class="text-center py-16 text-gray-500 text-sm">No catalog items match.</div>',b=o?1:Math.max(1,Math.ceil(s/le)),g=`
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs text-gray-500">Page ${S.page+1} / ${b}</span>
        <button onclick="catalogPage(1)" ${S.page>=b-1?"disabled":""} class="btn-press px-3 py-1.5 rounded-xl text-xs font-bold bg-white/5 text-gray-300 border border-white/10 hover:text-white disabled:opacity-40">Next</button>
      </div>
    </div>`;e.innerHTML=`
    <div class="space-y-4 fade-in">
      ${r}
