...
import{supabase as v}from"./supabase-client-7_ZWSEp6.js";import"./localization-B2rzni8q.js";import{_ as A}from"./preload-helper-CLcXU_4U.js";import{S as W,l as F,f as D,a as R,b as z}from"./showroom-data-BoOhjLzR.js";import{g as V,f as U,T as N}from"./truck-data-CXeZJZHb.js";import{g as k,f as E}from"./auth-D3ZJ7kZA.js";import"./live-stream-mode-CT1lAy4v.js";import"./customer-ai-widget-BvSrA37D.js";import"./brand-DYhAh01b.js";const y="/fallback.svg";function P(e){return typeof e=="number"&&!isNaN(e)?e.toFixed(1):"0.0"}function q(e){return Array.isArray(e)&&e.length>0?e:[y]}function O(){return new URLSearchParams(window.location.search).get("id")}function G(e){const t=document.getElementById("details-content"),a=U(e),n=q(e.images).map((d,m)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${m===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${l(d)}">
    ...
    <div class="fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          ...
          <p class="text-gray-500 text-sm mt-1">Stock #: <span class="text-blue-500 font-mono font-bold">${l(e.stock_number||"—")}</span> &middot; VIN: <span class="text-gray-400 font-mono">${l(e.vin||"—")}</span></p>
        </div>
  ...
  `;const g=document.getElementById("hero-image"),_=document.getElementById("gallery-label");t.querySelectorAll(".gallery-thumb").forEach((d,m)=>{d.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(f=>f.classList.remove("active","border-blue-500")),t.querySelectorAll(".gallery-thumb").forEach(f=>f.classList.add("border-gray-800")),d.classList.add("active","border-blue-500"),d.classList.remove("border-gray-800"),g.src=d.dataset.img,_.textContent=o[m]||`View ${m+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await k()?window.location.href=`/checkout.html?id=${e.property_id}`:(E(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const d=window.location.href;try{if(navigator.share)await navigator.share({title:l(e.title),url:d});else{await navigator.clipboard.writeText(d);const m=document.getElementById("share-btn"),f=m.innerHTML;m.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{m.innerHTML=f,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),Q(e),window.lucide&&lucide.createIcons()}function Y(e){const a=e.listing_type==="property"?"Share Property":"Share";return`
  ...
  `}function K(e){const t=e.listing_type==="property",a=`/contact.html?listing=${encodeURIComponent(e.property_id||"")}`;return`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5">
    ...
    </div>`}function J(e){const t=e.images&&e.images[0]||"/fallback.svg";return`<a href="/details.html?id=${encodeURIComponent(e.property_id)}" class="block bg-gray-900/60 border border-gray-800 rounded-xl overflow-hidden hover:border-blue-500/30 transition group">
      ...
      <div class="p-2"><p class="text-xs text-white font-bold truncate">${l(e.title)}</p><p class="text-xs text-blue-500 font-bold mt-1">${R(e)}</p></div>
    </a>`}function I(e,t){const a=document.getElementById(e);if(!a)return;const s=a.querySelector(".rel-grid");if(s){if(!t.length){a.classList.add("hidden");return}a.classList.remove("hidden"),s.innerHTML=t.slice(0,4).map(J).join("")}}function Q(e){const t=N.filter(o=>o.property_id!==e.property_id),a=t.filter(o=>o.category===e.category),s=t.filter(o=>o.brand===e.brand),n=[...t].sort((o,c)=>(c.rating||0)-(o.rating||0));I("similar-section",a.length?a:n.slice(0,4)),I("related-section",s.length?s:n.slice(0,4)),I("recommended-section",n)}function X(e){const t=document.getElementById("details-content"),a=e.listing_type==="property",s=R(e),n=z(e.country_code),o=e.listing_type==="product"?"Product ID":a?"Property ID":"Listing ID",u=q(e.images).map((i,r)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${r===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${l(i)}">
      <img src="${l(i)}" alt="View ${r+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${y}'">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ${[{icon:"globe",label:"Country",value:`${n} ${e.country}`},{icon:"map-pin",label:"State / Province",value:e.state},{icon:"building",label:"City",value:e.city},{icon:"navigation",label:"Town / Local Area",value:e.town}].filter(r=>r.value).map(r=>`
            <div class="flex items-center gap-2.5 text-sm">
              <div class="p-2 bg-gray-800 rounded-lg"><i data-lucide="${r.icon}" class="w-4 h-4 text-blue-500"></i></div>
    ...
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      ...
      <div class="flex flex-wrap gap-2">
        ${e.features.map(i=>`<span class="text-xs bg-gray-800 text-gray-300 px-3 py-1.5 rounded-full border border-gray-700">${l(i)}</span>`).join("")}
      </div>
    </div>`:"",d=e.highlights?.length?`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-4">Highlights</h3>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ${e.highlights.map(i=>`<div class="flex items-start gap-2 text-sm text-gray-300"><i data-lucide="sparkles" class="w-4 h-4 text-blue-500 mt-0.5"></i><span>${l(i)}</span></div>`).join("")}
      </div>
    </div>`:"",m=`
    <div class="flex items-center gap-4 mb-6">
      <div class="flex items-center gap-1.5">
    ...
    <div class="fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-black text-white leading-tight">${l(e.title)}</h1>
          <p class="text-gray-500 text-sm mt-1">${o}: <span class="text-blue-500 font-mono font-bold">${l(e.property_id)}</span></p>
        </div>
        <div class="text-right shrink-0">
          <div class="text-3xl font-black text-blue-500">${s}</div>
  ...
  `;const f=document.getElementById("hero-image");t.querySelectorAll(".gallery-thumb").forEach(i=>{i.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(r=>r.classList.remove("active","border-blue-500")),t.querySelectorAll(".gallery-thumb").forEach(r=>r.classList.add("border-gray-800")),i.classList.add("active","border-blue-500"),i.classList.remove("border-gray-800"),f.src=i.dataset.img})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await k()?window.location.href=`/checkout.html?id=${e.property_id}`:(E(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const i=window.location.href;try{if(navigator.share)await navigator.share({title:l(e.title),url:i});else{await navigator.clipboard.writeText(i);const r=document.getElementById("share-btn"),$=r.innerHTML;r.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{r.innerHTML=$,window.lucide&&lucide.createIcons()},2e3)}}catch{}});const B=document.getElementById("view-details-btn");B&&B.addEventListener("click",()=>{const i=document.getElementById("listing-details");i&&i.scrollIntoView({behavior:"smooth",block:"start"})});const w=document.getElementById("add-cart-btn");w&&w.addEventListener("click",()=>{let i=JSON.parse(localStorage.getItem("kco_cart")||"[]");i.includes(e.property_id)||(i.push(e.property_id),localStorage.setItem("kco_cart",JSON.stringify(i))),w.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Added to Cart',window.lucide&&lucide.createIcons(),setTimeout(()=>{w.innerHTML='<i data-lucide="shopping-cart" class="w-5 h-5"></i> Add to Cart',window.lucide&&lucide.createIcons()},2e3)}),ee(e),te(e),H(e),ae(e),window.lucide&&lucide.createIcons(),e.property_id,e.title,parseFloat(e.price),e.currency;const h=document.getElementById("listing-map");if(h&&window.L){const i=parseFloat(e.latitude)||null,r=parseFloat(e.longitude)||null,$=[e.town,e.city,e.state,e.country].filter(Boolean).join(", ");if(i&&r){const b=L.map(h).setView([i,r],13);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(b),L.marker([i,r]).addTo(b).bindPopup(e.title)}else $?fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q="+encodeURIComponent($)).then(b=>b.json()).then(b=>{if(b&&b[0]){const T=parseFloat(b[0].lat),j=parseFloat(b[0].lon),C=L.map(h).setView([T,j],12);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(C),L.marker([T,j]).addTo(C).bindPopup(e.title)}else h.style.display="none"}).catch(()=>{h.style.display="none"}):h.style.display="none"}}let x=0,M=!1;function Z(){if(M)return;M=!0;const e=document.createElement("style");e.textContent="@keyframes kcoWishPop{0%{transform:scale(1)}35%{transform:scale(1.45)}60%{transform:scale(.86)}100%{transform:scale(1)}}",document.head.appendChild(e)}function S(e,t){if(!e)return;Z(),e.innerHTML=`<i data-lucide="heart" class="w-5 h-5 ${t?"fill-red-500 text-red-500":""}"></i>`,e.classList.toggle("bg-red-500/10",t),e.classList.toggle("border",t),e.classList.toggle("border-red-500/20",t);const a=e.querySelector("span");a&&(a.textContent=t?"Saved to Wishlist":"Add to Wishlist"),window.lucide&&lucide.createIcons();const s=e.querySelector("i");s&&(s.style.animation="none",s.offsetWidth,s.style.animation="kcoWishPop .5s cubic-bezier(.34,1.56,.64,1)",setTimeout(()=>{s&&(s.style.animation="")},550))}async function ee(e){const t=document.getElementById("wishlist-btn");if(!t)return;const a=await k();if(!a){t.addEventListener("click",()=>{E(window.location.pathname+window.location.search),window.location.href=`/auth.html?redirect=${encodeURIComponent(window.location.pathname+window.location.search)}`});return}const{data:s,error:n}=await v.from("wishlist").select("id").eq("user_id",a.id).eq("listing_id",e.id).maybeSingle();if(n){console.error("Wishlist check failed:",n.message);return}s&&S(t,!0),t.addEventListener("click",async()=>{const{data:o,error:c}=await v.from("wishlist").select("id").eq("user_id",a.id).eq("listing_id",e.id).maybeSingle();if(c){console.error("Wishlist toggle failed:",c.message);return}if(o){const{error:u}=await v.from("wishlist").delete().eq("id",o.id);if(u){console.error("Wishlist delete failed:",u.message);return}S(t,!1)}else{const{error:u}=await v.from("wishlist").insert({user_id:a.id,listing_id:e.id});if(u){console.error("Wishlist insert failed:",u.message);return}S(t,!0)}})}async function te(e){const t=document.getElementById("review-form"),a=document.getElementById("review-login-msg"),s=await k();if(!s){t.classList.add("hidden"),a.classList.remove("hidden");return}document.querySelectorAll(".star-btn").forEach(n=>{n.addEventListener("click",()=>{x=parseInt(n.dataset.rating,10),document.querySelectorAll(".star-btn").forEach((o,c)=>{const u=o.querySelector("i");c<x?(u.classList.add("fill-blue-500","text-blue-500"),u.classList.remove("text-gray-600")):(u.classList.remove("fill-blue-500","text-blue-500"),u.classList.add("text-gray-600"))})})}),t.addEventListener("submit",async n=>{n.preventDefault();const o=document.getElementById("review-text").value.trim();if(!x){alert("Please select a rating.");return}if(!o){alert("Please write a review.");return}const{error:c}=await v.from("product_reviews").insert({listing_id:e.id,user_id:s.id,rating:x,review_text:o,is_approved:!1});if(c){alert("Error: "+c.message);return}document.getElementById("review-text").value="",x=0,document.querySelectorAll(".star-btn").forEach(u=>{const p=u.querySelector("i");p.classList.remove("fill-blue-500","text-blue-500"),p.classList.add("text-gray-600")}),alert("Review submitted! It will appear after admin approval."),H(e)})}async function H(e){const t=document.getElementById("reviews-list");if(!t||!e.id)return;const{data:a,error:s}=await v.from("product_reviews").select("*, profiles(full_name)").eq("listing_id",e.id).eq("is_approved",!0).order("created_at",{ascending:!1});if(s){t.innerHTML='<p class="text-gray-500 text-sm">Unable to load reviews.</p>';return}if(!a||a.length===0){t.innerHTML='<p class="text-gray-500 text-sm">No reviews yet. Be the first to review this product!</p>';return}t.innerHTML=a.map(n=>`
    <div class="border-b border-gray-800 pb-3 mb-3 last:border-0">
      <div class="flex items-center gap-2 mb-1">
        <div class="flex gap-0.5">${[1,2,3,4,5].map(o=>`<i data-lucide="star" class="w-3.5 h-3.5 ${o<=n.rating?"fill-amber-400 text-amber-400":"text-gray-600"}"></i>`).join("")}</div>
      ...
      <div class="p-2"><p class="text-xs text-white font-bold truncate">${l(c.title)}</p><p class="text-xs text-blue-500 font-bold mt-1">${g} ${p.toLocaleString()}</p></div>
    </a>`}).join("")}function l(e){return String(e||"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function re(){const e=O();if(!e){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}const t=V(e);if(t){document.title=`${t.title} | Weverse Online Shop`,G(t);return}let a=W.find(s=>s.property_id===e);if(!a){const[{generateListingById:s},{loadHiddenCatalogIds:n}]=await Promise.all([A(()=>import("./catalog-CKKE61jd.js"),__vite__mapDeps([0,1,2])),A(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await n(),a=s(e)}if(a||(await F(),a=D(e)),!a){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}document.title=`${a.title} | Weverse Online Shop`,X(a)}re();
