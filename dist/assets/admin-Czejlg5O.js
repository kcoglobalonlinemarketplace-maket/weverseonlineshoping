...
<div class="glass-soft border border-blue-500/15 rounded-2xl overflow-hidden">
    ...
    </div>`,window.lucide&&lucide.createIcons())}window.openSidebar=()=>{document.getElementById("sidebar").classList.add("open"),document.getElementById("sidebar-overlay").classList.remove("hidden")};window.closeSidebar=()=>{document.getElementById("sidebar").classList.remove("open"),document.getElementById("sidebar-overlay").classList.add("hidden")};document.getElementById("close-sidebar")?.addEventListener("click",closeSidebar);const _e="kco_admin_remember",Mt="kco_login_attempts",bt=5,Bi=15*60*1e3;function q(e){const t=document.getElementById("login-error"),a=document.getElementById("login-error-text");!t||!a||(a.textContent=e,t.classList.remove("hidden"),document.getElementById("login-success")?.classList.add("hidden"),window.lucide&&lucide.createIcons())}function Mi(e){const t=document.getElementById("login-success"),a=document.getElementById("login-success-text");!t||!a||(a.textContent=e,t.classList.remove("hidden"),document.getElementById("login-error")?.classList.add("hidden"))}function tt(){document.getElementById("login-error")?.classList.add("hidden"),document.getElementById("login-success")?.classList.add("hidden")}function Re(e){return String(e||"").trim().toLowerCase()}function Ni(){try{const e=JSON.parse(localStorage.getItem(_e)||"{}");e?.email&&!Re(e.email)&&localStorage.removeItem(_e)}catch{localStorage.removeItem(_e)}}function Ri(){try{const e=JSON.parse(localStorage.getItem(_e)||"{}");return Re(e?.email)}catch{return""}}function Nt(){Ni();const e=Ri(),t=document.getElementById("login-email");t&&(t.value=e||t.value||fa,t.removeAttribute("readonly"));const a=document.getElementById("reset-email");a&&(a.value=e||a.value||"",a.removeAttribute("readonly"))}function Fi(){return`${window.location.origin}/admin.html`}function he(e){const t=document.getElementById("login-header-title"),a=document.getElementById("login-header-icon");document.getElementById("login-form")?.classList.toggle("hidden",e!=="login"),document.getElementById("twofa-form")?.classList.toggle("hidden",e!=="2fa"),document.getElementById("forgot-form")?.classList.toggle("hidden",e!=="forgot"),tt(),e==="login"&&(t&&(t.textContent="Admin Access"),a&&a.setAttribute("data-lucide","shield-check")),e==="2fa"&&(t&&(t.textContent="Two-Factor Auth"),a&&a.setAttribute("data-lucide","smartphone")),e==="forgot"&&(t&&(t.textContent="Reset Password"),a&&a.setAttribute("data-lucide","mail")),window.lucide&&lucide.createIcons()}function U(e,t,a=""){const i=document.getElementById(e);i&&(i.disabled=t,t?i.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline-block mr-1"></i> Please waitâ€¦':a&&(i.innerHTML=a),window.lucide&&lucide.createIcons())}function ka(){try{return JSON.parse(localStorage.getItem(Mt)||'{"count":0}')}catch{return{count:0}}}function Sa(){const e=ka();return e.count=(e.count||0)+1,e.count>=bt&&(e.lockedUntil=Date.now()+Bi),localStorage.setItem(Mt,JSON.stringify(e)),e}function $a(){localStorage.removeItem(Mt)}function Ea(){const e=ka();if(!e.lockedUntil)return null;const t=e.lockedUntil-Date.now();return t<=0?($a(),null):Math.ceil(t/6e4)}async function le(e,t,a={}){try{await b.from("admin_security_logs").insert({user_id:e,event_type:t,ip_address:await Di(),user_agent:navigator.userAgent.slice(0,200),...a})}catch{}}async function Di(){try{return(await(await fetch("https://api64.ipify.org?format=json",{signal:AbortSignal.timeout(3e3)})).json()).ip||"unknown"}catch{return"unknown"}}async function Pa(e){if(!e)return!1;let t=!1,a=!1;try{const{data:i}=await b.rpc("is_current_user_admin");t=!0,a=!!i}catch{t=!1}return t?a:Re(e.email)===fa}async function Ui(){const e=window.location.hash;if(e.includes("type=recovery")||e.includes("access_token")){Ge(),Vi();return}const{data:{session:t}}=await b.auth.getSession();if(t?.user&&await Pa(t.user)){const{data:{currentUser:i}}=await b.auth.getUser(),n=await b.auth.mfa.getAuthenticatorAssuranceLevel(),o=n.data?.currentLevel;if(n.data?.nextLevel==="aal2"&&o!=="aal2"){I.user=t.user,Ge(),he("2fa"),Rt();return}I.user=t.user,at();return}Oi()}function Ge(){const e=document.getElementById("login-screen");e&&(e.style.display="flex")}function Oi(){Ge(),he("login"),Nt(),Aa(),Ia(),Rt(),ji();const e=Ea();e&&(q(`Too many failed attempts. Try again in ${e} minute${e>1?"s":""}.`),document.getElementById("login-btn").disabled=!0)}function ji(){document.getElementById("toggle-pw")?.addEventListener("click",()=>{const e=document.getElementById("login-password"),t=document.querySelector("#toggle-pw i");e&&(e.type=e.type==="password"?"text":"password",t&&t.setAttribute("data-lucide",e.type==="password"?"eye":"eye-off"),window.lucide&&lucide.createIcons())})}function Aa(){const e=document.getElementById("login-form");!e||e._bound||(e._bound=!0,e.addEventListener("submit",qi),document.getElementById("forgot-pw-btn")?.addEventListener("click",()=>he("forgot")))}async function qi(e){e.preventDefault();const t=Ea();if(t){q(`Account locked. Try again in ${t} minute${t>1?"s":""}.`);return}const a=document.getElementById("login-email"),i=Re(a?.value);if(!i){q("Enter your admin email address."),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}const n=document.getElementById("login-password").value,o=document.getElementById("remember-me")?.checked;U("login-btn",!0),tt();const{data:s,error:r}=await b.auth.signInWithPassword({email:i,password:n});if(r||!s.user){const f=String(r?.message||"").toLowerCase();if(f.includes("missing supabase credentials")||f.includes("authentication service is unavailable")){q("Authentication is temporarily unavailable due to configuration. Please contact support."),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}if(f.includes("failed to fetch")||f.includes("network request failed")){q("Network error while signing in. Check your connection and try again."),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}if(f.includes("email not confirmed")){q("Your admin email is not confirmed yet. Open your verification email and confirm first."),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}const y=Sa(),g=bt-y.count,h=y.lockedUntil?`Account locked for 15 minutes after ${bt} failed attempts.`:`Invalid email or password. ${g>0?g+" attempt"+(g!==1?"s":"")+" remaining.":""}`;q(h),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),s?.user&&await le(s.user.id,"login_failed",{metadata:{reason:"wrong_password"}});return}if(!await Pa(s.user)){await b.auth.signOut(),q(`Access denied for ${s.user.email}. This account is signed in but does not have administrator privileges.`),U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),await le(s.user.id,"login_denied",{metadata:{reason:"not_admin"}});return}if(o?localStorage.setItem(_e,JSON.stringify({email:i,ts:Date.now()})):localStorage.removeItem(_e),$a(),I.user=s.user,(await b.auth.mfa.getAuthenticatorAssuranceLevel()).data?.nextLevel==="aal2"){U("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),he("2fa"),Rt(),setTimeout(()=>document.getElementById("totp-code")?.focus(),100);return}await le(s.user.id,"login_success"),U("login-btn",!1),at()}function Rt(){const e=document.getElementById("verify-2fa-btn");e&&!e._bound&&(e._bound=!0,e.addEventListener("click",Qt));const t=document.getElementById("totp-code");t&&!t._bound&&(t._bound=!0,t.addEventListener("input",i=>{i.target.value=i.target.value.replace(/\D/g,"").slice(0,6),i.target.value.length===6&&Qt()})),document.getElementById("cancel-2fa-btn")?.addEventListener("click",async()=>{await b.auth.signOut(),I.user=null,he("login")}),document.getElementById("use-backup-btn")?.addEventListener("click",()=>{document.getElementById("backup-code-wrap")?.classList.toggle("hidden");const n=document.getElementById("backup-code");n&&n.focus()});const a=document.getElementById("verify-backup-btn");a&&!a._bound&&(a._bound=!0,a.addEventListener("click",Hi))}async function Qt(){const e=document.getElementById("totp-code")?.value?.trim();if(!e||e.length!==6){q("Enter the 6-digit code from your authenticator app.");return}U("verify-2fa-btn",!0),tt();try{const{data:t}=await b.auth.mfa.listFactors(),a=(t?.totp||[])[0];if(!a){q("No 2FA factor found. Please re-login."),U("verify-2fa-btn",!1,'<i data-lucide="shield-check" class="w-4 h-4 inline mr-1"></i> Verify & Sign In');return}const{data:i,error:n}=await b.auth.mfa.challenge({factorId:a.id});if(n)throw n;const{error:o}=await b.auth.mfa.verify({factorId:a.id,challengeId:i.id,code:e});if(o)throw o;await le(I.user.id,"login_2fa_success"),U("verify-2fa-btn",!1),at()}catch(t){Sa(),q(t.message?.includes("Invalid")?"Incorrect code. Check your authenticator and try again.":t.message),U("verify-2fa-btn",!1,'<i data-lucide="shield-check" class="w-4 h-4 inline mr-1"></i> Verify & Sign In'),document.getElementById("totp-code").value="",document.getElementById("totp-code").focus()}}async function Hi(){const e=document.getElementById("backup-code")?.value?.trim().toUpperCase().replace(/\s/g,"");if(!e){q("Enter a backup recovery code.");return}U("verify-backup-btn",!0);try{const{data:t}=await b.from("admin_2fa").select("backup_codes").eq("user_id",I.user.id).maybeSingle();if(!t?.backup_codes?.length){q("No backup codes found."),U("verify-backup-btn",!1,"Use Backup Code");return}if(!t.backup_codes.find(n=>(n.code||n).toUpperCase().replace(/-/g,"")===e.replace(/-/g,"")&&!n.used)){q("Backup code not found or already used."),U("verify-backup-btn",!1,"Use Backup Code");return}const i=t.backup_codes.map(n=>(n.code||n).toUpperCase().replace(/-/g,"")===e.replace(/-/g,"")?{...typeof n=="object"?n:{code:n},used:!0}:n);await b.from("admin_2fa").update({backup_codes:i}).eq("user_id",I.user.id),await le(I.user.id,"login_backup_code_used"),at()}catch(t){q(t.message),U("verify-backup-btn",!1,"Use Backup Code")}}function Ia(){document.getElementById("back-to-login")?.addEventListener("click",()=>he("login")),document.getElementById("send-reset-btn")?.addEventListener("click",Gi)}async function Gi(){const e=document.getElementById("reset-email"),t=Re(e?.value);if(!t){q("Enter your admin email address to receive a reset link.");return}U("send-reset-btn",!0),tt();const{error:a}=await b.auth.resetPasswordForEmail(t,{redirectTo:Fi()});if(U("send-reset-btn",!1,'<i data-lucide="mail" class="w-4 h-4 inline mr-1"></i> Send Reset Link'),a){q(a.message);return}Mi("Reset link sent! Check your inbox and open it from this device to continue.")}function Vi(){const e=document.getElementById("login-screen");if(!e)return;const t=e.querySelector(".login-card");t&&(t.innerHTML=`
    ...
    <div class="space-y-4">
      ...
      <div class="space-y-6 fade-in">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
          ${z("Total Revenue",`$${r.toLocaleString("en-US",{maximumFractionDigits:0})}`,"dollar-sign","emerald",`$${k.toLocaleString("en-US",{maximumFractionDigits:0})} this month`)}
          ${z("Total Orders",s.length,"shopping-bag","blue",`${u} pending`)}
          ${z("Customers",f,"users","violet")}
          ${z("Products",d,"package","amber",`${m} active`)}
          ${z("Properties",c,"home","blue")}
          ${z("Reviews",y,"star","blue",`${g} pending`)}
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="glass-soft border border-blue-500/15 rounded-2xl p-5">
            ...
            ${_.length===0?'<p class="text-xs text-gray-500 text-center py-8">No orders yet</p>':_.map(v=>`
      ...
      <div class="space-y-5 fade-in">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-6 gap-3">
          ...
          ${z("Inventory Units",n.reduce((d,c)=>d+(parseInt(c.stock_quantity,10)||0),0),"boxes","blue")}
          ${z("Avg Price",`$${Math.round(n.reduce((d,c)=>d+(parseFloat(c.price)||0),0)/Math.max(n.length,1)).toLocaleString()}`,"dollar-sign","blue")}
        </div>
...
<div class="space-y-4">
    ...
    </div>`)};window.quickEditProduct=async function(e){const t=await b.from("showroom_listings").select("*").eq("property_id",e).maybeSingle(),a=(window._productsData||[]).find(n=>n.property_id===e)||t.data;if(!a)return p("Product not found","error");const i=Array.isArray(a.images)?a.images:[];W(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box">
        <div class="flex items-center justify-between mb-4">
    ...
    </div>`),Ut(),Ot(),Ae(),xe(),window.lucide&&lucide.createIcons()};window.saveQuickEditProduct=async function(e,t){e.preventDefault();const a=new FormData(e.target),i=[...document.querySelectorAll("#image-preview .img-thumb")].map(c=>c.dataset.url||(c.querySelector("img")?c.querySelector("img").getAttribute("src"):"")).filter(c=>c&&!String(c).startsWith("blob:")),n={title:a.get("title")||"Untitled Product",price:Math.max(D,Math.min(Y,parseFloat(a.get("price"))||0)),stock_quantity:a.get("stock_quantity")===""?null:parseInt(a.get("stock_quantity"),10),availability_status:a.get("availability_status")||"In Stock",is_featured:a.get("is_featured")==="on",is_active:a.get("is_active")==="on"||i.length>=24,images:i},o=String(a.get("real_price")||"").trim(),s=o===""?null:parseFloat(o);if(s!=null&&!Number.isFinite(s)){p("Real Price must be a number.","error");return}const r=ke((window._productsData||[]).find(c=>c.property_id===t)),u=r.specifications&&typeof r.specifications=="object"?r.specifications:{};n.specifications={...u,real_price:s!=null&&s>0?Math.round(s):null};const{error:d}=await b.from("showroom_listings").upsert({...r,...n,property_id:t},{onConflict:"property_id"});if(d){if(Z(d)){p("âš ï¸ Save blocked: database admin role rejected the write. Re-run the admin permission migration.","error"),ne(),F();return}ri(t,n),p("Quick edit saved locally","info")}else p(n.is_active?"Saved & published â€” your showroom shows it now":"Quick edit saved (draft)");ne(),F()};window.publishProduct=function(e){return toggleProductActive(e,!0)};window.unpublishProduct=function(e){return toggleProductActive(e,!1)};window.shareProduct=async function(e){const t=`${window.location.origin}/details.html?id=${encodeURIComponent(e)}`;try{if(navigator.clipboard?.writeText){await navigator.clipboard.writeText(t),p("Product link copied to clipboard");return}}catch{}window.prompt("Copy product link:",t)};window.deleteProduct=async function(e){if(!confirm("Delete this product permanently? This action cannot be undone."))return;const t=(window._productsData||[]).find(i=>i.property_id===e)||(window._propertiesData||[]).find(i=>i.property_id===e)||Tt(e),{error:a}=await b.from("showroom_listings").delete().eq("property_id",e);if(a&&!Z(a))return p("Delete failed: "+a.message,"error");li(e);try{const i=await ma(e,!0);i&&i.error&&Z(i.error)?p("âš ï¸ Deleted, but the site-wide hidden list could not be saved: database admin role rejected the write. Re-run the admin permission migration.","error"):p("Product deleted")}catch{p("Product deleted")}t&&t.listing_type==="property"?ot():F()};window.clearAllProducts=async function(){const e=(window._productsData||[]).length;if(!confirm(`Delete ALL ${e} product(s) from the Product Manager and the database now?
...
This is permanent and cannot be undone. Your built-in showroom catalog will stay.`))return;const{error:t}=await b.from("showroom_listings").delete().neq("property_id","__none__");if(t)return Z(t)?p("âš ï¸ Delete blocked: database admin role rejected the write. Re-run the admin permission migration.","error"):p("Clear failed: "+t.message,"error");try{localStorage.removeItem("kco_local_showroom_listings_v1")}catch{}p("All products deleted. The manager now shows your showroom catalog."),F()};window.openProductMoreActions=function(e){W(`
        ...
        <form id="product-form" onsubmit="saveProduct(event,'${l(e)}','${a?t.property_id:""}')" class="space-y-6">
          ...
          <div class="form-grid form-grid-2">
            ...
            <div class="p-4 glass-soft border border-blue-500/15 rounded-2xl">
              ...
              <p class="text-sm text-gray-500 mt-1">Allowed price range is ${D} to ${Y} in the selected currency.</p>
            </div>
          </div>
  ...
  `}window.previewProductDraft=function(){const e=document.getElementById("product-form");if(!e)return;const t=document.querySelector("#image-preview img")?.src||"/fallback.svg",a=e.querySelector('[name="title"]')?.value||"Untitled Product",i=e.querySelector('[name="description"]')?.value||"No description yet.",n=e.querySelector('[name="brand"]')?.value||"N/A",o=parseFloat(e.querySelector('[name="price"]')?.value||"0")||0,s=parseFloat(e.querySelector('[name="real_price"]')?.value||"0")||0,r=e.dataset.category||"Product",u=e.querySelector('[name="stock_quantity"]')?.value||"Unlimited",d=e.querySelector('[name="is_active"]')?.checked;W(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box wide">
        <div class="flex items-center justify-between mb-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="space-y-2">
            ...
            <div class="flex items-center gap-2">${J(d?"active":"inactive")}<span class="badge bg-blue-500/10 text-blue-300 border-blue-500/20">${l(r)}</span></div>
...
`}const Fa=/^(n\/?a|none|unknown|not (available|specified|found|visible|applicable)|null|undefined|-{1,}|no data)$/i;function ea(e,t){const a={...t||{}},i=new Set(Array.isArray(a.estimated)?a.estimated.map(c=>String(c)):[]),n=new Set(Array.isArray(a.missing_fields)?a.missing_fields.map(c=>String(c)):[]),o=[],s=[],r=c=>{if(c==null)return"";Array.isArray(c)&&(c=c.filter(f=>f!=null&&String(f).trim()!=="").join(", "));let m=String(c).replace(/\s+/g," ").trim();return m=m.replace(/^(answer|value|result|extracted)\s*[:\-]\s*/i,""),m},u=c=>{const m=r(c).replace(/[^0-9.,\-]/g,"").replace(/,(?=\d{3}\b)/g,"").replace(",","."),f=parseFloat(m);return Number.isFinite(f)?f:NaN};for(const c of e||[]){if(ze.has(c.key))continue;const m={key:c.key,label:c.label,status:"empty-ok",value:null,note:""};if(c.type==="checkbox-group"){const g=Array.isArray(a[c.key])?a[c.key].map(r).filter(Boolean):[],h=c.options&&c.options.length?g.filter(S=>c.options.includes(S)):g;h.length?(a[c.key]=h,m.status="filled",m.value=h.join(", ")):(delete a[c.key],g.length&&(m.status="flagged",m.note="values not in the allowed badge list were dropped",s.push(`${c.label}: invalid selection ignored`))),o.push(m);continue}if(!(a[c.key]!=null&&r(a[c.key])!=="")){m.status=n.has(c.key)?"missing":"empty-ok",o.push(m);continue}if(Fa.test(r(a[c.key]))){delete a[c.key],n.add(c.key),m.status="missing",m.note="document/AI said the value is unavailable",o.push(m);continue}if(c.type==="number"){const g=r(a[c.key]),h=u(a[c.key]);if(!Number.isFinite(h)){delete a[c.key],n.add(c.key),m.status="flagged",m.note=`"${g}" is not a valid number`,s.push(`${c.label}: not a valid number`),o.push(m);continue}if(/year/.test(c.key)&&(h<1800||h>new Date().getFullYear()+2)){delete a[c.key],n.add(c.key),m.status="flagged",m.note=`${h} is outside the plausible range`,s.push(`${c.label}: implausible value ${h}`),o.push(m);continue}a[c.key]=h,m.status="filled",m.value=String(h),i.has(c.key)&&(m.status="estimated",m.note="AI estimate â€” confirm"),o.push(m);continue}if(c.type==="select"&&c.options&&c.options.length){const g=Da({options:c.options.map(h=>({value:h}))},r(a[c.key]));if(g==null){m.status="flagged",m.note=`"${r(a[c.key])}" does not match any option â€” left empty`,s.push(`${c.label}: no matching option`),delete a[c.key],n.add(c.key),o.push(m);continue}a[c.key]=g,m.status="filled",m.value=g,g!==r(t?.[c.key])&&(m.note="matched to the closest option"),o.push(m);continue}let y=r(a[c.key]);c.type!=="textarea"&&c.type!=="text-long"&&y.length>120&&!["title"].includes(c.key)&&(m.status="flagged",m.note="unusually long â€” check it landed in the right field",s.push(`${c.label}: suspiciously long value`)),a[c.key]=y,m.status="filled",m.value=y.length>48?y.slice(0,48)+"â€¦":y,i.has(c.key)&&(m.status="estimated",m.note="AI estimate â€” confirm"),o.push(m)}if(e&&e.length){const c=new Set([...e.map(m=>m.key),"estimated","missing_fields","features","highlights","seo_keywords"]);Object.keys(a).forEach(m=>{c.has(m)||delete a[m]})}a.missing_fields=o.filter(c=>c.status==="missing").map(c=>c.key),a.estimated=o.filter(c=>c.status==="estimated").map(c=>c.key);const d={total:o.length,filled:o.filter(c=>c.status==="filled").length,estimated:o.filter(c=>c.status==="estimated").length,flagged:o.filter(c=>c.status==="flagged").length,missing:o.filter(c=>c.status==="missing").length};return{specs:a,checklist:o,flags:s,summary:d}}function qt(e,t){if(!e||!e.length)return"";const a={filled:'<span class="text-emerald-400 font-bold">âœ“</span>',estimated:'<span class="text-blue-300 font-bold">â‰ˆ</span>',flagged:'<span class="text-red-400 font-bold">!</span>',missing:'<span class="text-gray-500">â€”</span>',"empty-ok":'<span class="text-gray-700">Â·</span>'},i=e.filter(o=>o.status!=="empty-ok").map(o=>`<li class="flex items-start gap-2"><span class="shrink-0 w-4">${a[o.status]||""}</span><span><b>${l(o.label)}</b> <span class="text-gray-600">(${l(o.key)})</span>${o.value?` â€” <span class="text-gray-300">${l(String(o.value))}</span>`:""}${o.note?` <span class="text-gray-500">${l(o.note)}</span>`:""}</span></li>`).join(""),n=t.total-t.filled-t.estimated-t.flagged-t.missing;return`<details class="mt-2 rounded-xl border border-violet-500/20 bg-violet-500/5 p-3">
  ...
  <div class="scan-review-card rounded-xl border border-violet-500/30 bg-violet-500/10 p-3 space-y-2 fade-in" data-i="${t}">
  ...
  </div>`}window.scanReviewRender=function(){const e=document.getElementById(ee);if(e){if(e.classList.remove("hidden","text-red-400","text-emerald-300","text-amber-300","text-blue-300","text-gray-400"),!H.length){e.classList.add("text-gray-400"),e.textContent="All detected products were removed â€” nothing was changed.";return}e.classList.add("text-gray-100"),e.innerHTML=`
    <div class="space-y-3">
      <div>
        <p class="text-xs font-bold text-white flex items-center gap-2"><i data-lucide="list-checks" class="w-4 h-4 text-violet-400"></i> ${H.length} distinct product${H.length>1?"s":""} detected</p>
      ...
      ${H.map((t,a)=>bn(t,a)).join("")}
    </div>`,window.lucide&&lucide.createIcons()}};window.scanReviewContinue=async function(e){const t=H[e];if(!t)return;const a=Ht(t,fe),i=nt(t.category);if(t.listing_type==="property"||i&&i.listing_type==="property"){(ee==="s1-scan-status"||ee==="scanner-scan-status")&&(ne(),me=[]),yn(t,a);return}const o=i.category||t.category||"Other";if(ee==="s1-scan-status"||ee==="scanner-scan-status"){try{localStorage.removeItem(Me(o,""))}catch{}me=[];let s=t.property_id?Se[t.property_id]:null;s&&s.specifications&&typeof s.specifications=="object"&&(s={...s,...s.specifications}),showAddProductStep2(o,s?{...s,images:a}:{images:a}),await ta(t,a,o)}else{const s=document.getElementById("product-form"),r=s&&s.dataset.category||"";if(o!==r){try{localStorage.removeItem(Me(o,""))}catch{}switchProductFormCategory(o);const u=document.getElementById(ee);u&&(u.classList.remove("hidden"),u.classList.add("text-blue-300"),u.textContent=`Category changed to ${o} â€” finishing the scanâ€¦`),window.lucide&&lucide.createIcons()}await ta(t,a,o)}};window.scanReviewEdit=function(e){const t=H[e];if(!t)return;const a=document.querySelector(`.scan-review-card[data-i="${e}"]`);if(!a)return;const i=nt(t.category),o=t.listing_type==="property"||i&&i.listing_type==="property"?"Real Estate":i.category||t.category||"Other",s=pe.map(r=>`<option value="${l(r)}" ${r===o?"selected":""}>${l(r)}</option>`).join("");a.innerHTML=`
    ...
    </div>`};window.scanReviewApplyEdit=function(e){const t=H[e];if(!t)return;const a=document.getElementById(`sr-name-${e}`)?.value,i=document.getElementById(`sr-brand-${e}`)?.value,n=document.getElementById(`sr-model-${e}`)?.value,o=document.getElementById(`sr-cat-${e}`)?.value;a&&(t.detected_name=a),i&&(t.brand=i),n&&(t.model=n),o&&(t.category=o),scanReviewRender()};window.scanReviewRemove=function(e){H.splice(e,1),scanReviewRender()};window.scanReviewCancel=function(){const e=document.getElementById(ee);e&&(e.classList.remove("hidden","text-red-400","text-emerald-300","text-amber-300","text-blue-300"),e.classList.add("text-gray-400"),e.textContent="Scan cancelled â€” nothing was changed.")};window.scanReviewContinueAll=async function(){const e=document.getElementById(ee),t=(d,c)=>{e&&(e.classList.remove("hidden","text-red-400","text-emerald-300","text-amber-300","text-blue-300","text-gray-400","text-gray-100"),c&&e.classList.add(c),e.innerHTML=d,window.lucide&&lucide.createIcons())};if(!H.length)return;const a=document.getElementById("btn-scan-continue-all");a&&(a.disabled=!0,a.textContent="Saving everythingâ€¦");const i=H.length;let n=0,o=0,s=0;const r=[];for(let d=0;d<i;d++){const c=H[d];if(!c)continue;const m=nt(c.category);if(c.listing_type==="property"||m&&m.listing_type==="property"){r.push(d);continue}const y=m.category||c.category||"Other",g=Ht(c,fe);t(`<p class="flex items-center gap-2"><i data-lucide="loader" class="w-4 h-4 animate-spin text-blue-300"></i> Completing ${d+1} of ${i}: <b>${l(c.detected_name||"product")}</b>â€¦</p>`,"text-blue-300");try{const h=await R.completeSpecsAndPrice(g,c,{category:y,maxImages:ge.maxImages}).catch(()=>null);let S=h&&h.specs||{},k=h?h.price:null;try{const B=await R.verifyExtraction(g,c,S,[],{maxImages:ge.maxImages});if(B&&B.corrections&&typeof B.corrections=="object"){for(const[O,$]of Object.entries(B.corrections))$==null||String(Array.isArray($)?$.join(", "):$).trim()===""||mt.includes(O)&&(S[O]=$);for(const[O,$]of Array.isArray(B.wrong_mapping)?B.wrong_mapping:[])mt.includes($)&&S[O]!=null&&(S[$]==null||String(S[$]).trim()==="")&&(S[$]=S[O],delete S[O])}}catch{}const _={};for(const[B,O]of Object.entries(S)){if(!mt.includes(B)||ze.has(B))continue;const $=Array.isArray(O)?O.map(se=>String(se??"").replace(/\s+/g," ").trim()).filter(Boolean):String(O??"").replace(/\s+/g," ").trim();if(!(!$.length||(Array.isArray($)?$.join(" "):$).match(Fa))){if(/^(model_)?year/.test(B)){const se=parseInt($,10);if(!Number.isFinite(se)||se<1800||se>new Date().getFullYear()+2)continue}_[B]=$}}S=_;const v=S||{},w=c.property_id&&Se[c.property_id]||null,x=w?w.specifications&&typeof w.specifications=="object"?{...w,...w.specifications}:w:null,G=[c.year||v.year,c.brand||v.brand,c.model||v.model].filter(Boolean).join(" ")||c.detected_name||"Product",X=k?Number(k.estimated_price??k.price??k.estimate):NaN;let Q=Number.isFinite(X)&&X>0?X:x?Number(x.price):NaN;(!Number.isFinite(Q)||Q<=0)&&(Q=D),Q=Math.max(D,Math.min(Y,Q));const j=new Set(["title","description","brand","color","size","condition","warranty","availability_status"]),C={};for(const[B,O]of Object.entries({...Object.fromEntries($t.filter($=>$!=="year_estimated"&&$!=="listing_status").map($=>[$,c[$]??null])),...v})){if(j.has(B)||ze.has(B))continue;const $=Array.isArray(O)?O.filter(se=>se!=null&&String(se).trim()!==""):O;$==null||typeof $=="string"&&!$.trim()||(C[B]=$)}const T={...x&&x.specifications&&typeof x.specifications=="object"?x.specifications:{},...C},E={listing_type:"product",category:y,subcategory:x?.subcategory||v.subcategory||null,title:v.title||x&&x.title||G,description:v.description||x&&x.description||"",price:Q,currency:x&&x.currency||"USD",country:x&&x.country||"",country_code:x&&x.country_code||"",listing_status:"sale",state:"",city:"",product_location:"",latitude:null,longitude:null,is_active:!0,is_featured:!!(x&&x.is_featured),brand:c.brand||v.brand||x&&x.brand||null,color:v.color||x&&x.color||null,size:v.size||x&&x.size||null,condition:x&&x.condition||null,warranty:x&&x.warranty||null,availability_status:x&&x.availability_status||"In Stock",stock_quantity:x&&x.stock_quantity?parseInt(x.stock_quantity):null,images:g.length?g:x&&x.images||[],features:Array.isArray(v.features)&&v.features.length?v.features:x&&x.features||[],tags:Array.isArray(v.tags)&&v.tags.length?v.tags:x&&x.tags||[],highlights:Array.isArray(v.highlights)&&v.highlights.length?v.highlights:x&&x.highlights||[],seo_keywords:Array.isArray(v.seo_keywords)&&v.seo_keywords.length?v.seo_keywords:x&&x.seo_keywords||[],is_ai_generated:!0,ai_generated_fields:["title","description","specifications","price"],specifications:T,updated_at:new Date().toISOString()};if(x&&x.property_id){E.property_id=x.property_id;const{error:B}=await b.from("showroom_listings").upsert(E,{onConflict:"property_id"});if(B)throw B;o++}else{E.property_id=et();const{error:B}=await b.from("showroom_listings").insert(E);if(B)throw B;n++}}catch(h){s++,t(`<p class="text-amber-300">Could not save "${l(c.detected_name||"product")}": ${l(String(h?.message||h))} â€” continuing with the restâ€¦</p>`,"text-amber-300")}}H=H.filter((d,c)=>r.includes(c));const u=`${o} updated, ${n} new${s?`, ${s} failed`:""}`;if(!H.length)Se={},fe=[],t(`<div class="space-y-1">
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
    ...
    </div>`),window.lucide&&lucide.createIcons()};function aa(e,t){const a=Symbol("ai-scan-timeout");return Promise.race([e,new Promise(i=>setTimeout(()=>i(a),t))]).then(i=>{if(i===a)throw new Error("A scan step took too long and timed out.");return i})}window.scanGeneralWithAI=async function(){let e=[];try{e=await aa(ja(),15e3)}catch{e=[]}if(!e.length){p("No products with photos are in the Product Manager yet â€” add a product first.","error");return}const t=document.getElementById("btn-scanner-scan"),a=document.getElementById("scanner-scan-status"),i=t?t.innerHTML:"",n=(w,x)=>{a&&(a.classList.remove("hidden","text-red-400","text-emerald-300","text-amber-300","text-blue-300","text-gray-400"),x&&a.classList.add(x),a.innerHTML=w)};try{const w=await R.getConfig();String(w.gemini_key||w.gemini_api_key||"").trim()||n("No Gemini key found â€” scanning anyway with the FREE built-in AI (no key needed). Products whose photos cannot be read will still be filled from their saved details. For the best photo recognition, add a FREE Gemini key in AI Settings (aistudio.google.com/apikey).","text-blue-300")}catch{}t&&(t.disabled=!0,t.innerHTML="Scanningâ€¦"),n(`Detecting and completing ${e.length} product${e.length===1?"":"s"}â€¦`,"text-blue-300");const o=[],s=[],r={};let u=0;const d=e.length,c=new Set;let m=0,f=0;window.__scanDupContinue=function(){const w=window.__scanDupResolve;window.__scanDupResolve=null;const x=document.getElementById("btn-scan-dup-continue");x&&(x.disabled=!0),w&&w()};const y=2,g=12e4;let h=0,S=0;const k=async w=>{const x=(w.images||[]).filter(Boolean);if(!x.length)return;const G=x.map(T=>String(T||"").trim()).filter(Boolean),X=G.length>0&&G.every(T=>c.has(T));for(const T of G)c.add(T);if(X){m++;return}const Q=o.length;o.push(...x);const j=x.map((T,E)=>Q+E);let C=[];try{const T=await aa(R.detectProducts(x,{category:w.category||"",maxImages:ge.maxImages}),g);C=T&&T.identified!==!1&&Array.isArray(T.products)?T.products:[]}catch{}C.length||(C=[{detected_name:w.title||w.property_id||"Product",category:w.category||"Other",listing_type:w.listing_type||"product",brand:w.brand||null,model:w.specifications&&w.specifications.model||w.model||null,confidence:"medium"}],f++),u++,r[w.property_id]=w;for(const T of C)s.push({...T,property_id:w.property_id,image_indices:Array.isArray(T.image_indices)&&T.image_indices.length?T.image_indices.map(E=>j[Number(E)]).filter(E=>E!==void 0):j,detected_name:T.detected_name||w.title||w.property_id,category:T.category||w.category||"Other",listing_type:T.listing_type||w.listing_type||"product"})},_=async()=>{for(;h<e.length;){const w=e[h++];n(`Scanning ${S+1} of ${d} product${d===1?"":"s"} (fast parallel mode)â€¦`,"text-blue-300"),await k(w),S++,S<d&&n(`Scanned ${S} of ${d} â€” continuingâ€¦`,"text-blue-300")}};if(await Promise.all(Array.from({length:Math.min(y,Math.max(1,e.length))},_)),t&&(t.disabled=!1,t.innerHTML=i),!s.length){n(`No product could be identified from the photos on your existing products${m?` (${m} duplicate product${m>1?"s":""} skipped)`:""}. Make sure each product has clear photos, then try again.`,"text-amber-300"),p("No products could be scanned.","error");return}H=s,fe=o,Se=r,ee="scanner-scan-status",scanReviewRender();const v=Date.now()<(R._geminiQuotaUntil||0);p(`Scan complete â€” ${u} product${u>1?"s":""} scanned${m?`, ${m} duplicate product${m>1?"s":""} skipped`:""}${f?`, ${f} completed from saved details (AI could not read the photo â€” review ${f>1?"those":"it"} and edit as needed)`:""}${v?". Your Gemini key hit its FREE rate limit during the scan â€” wait about 1 minute, then continue for full AI reading.":""}. Press "Continue with ALL" to save & publish everything at once.`,"success")};window.saveProduct=async function(e,t,a){e.preventDefault();const i=e.target,n=i.querySelector("[type=submit][name=action][value=publish]"),o=a?"One-Click Publish Changes":"One-Click Publish Product";n&&(n.disabled=!0,n.textContent="Savingâ€¦");try{const s=new FormData(i),r={};let u=0;for(const[y,g]of s.entries())if(y==="images"){r.images=r.images||[];const h=String(g);g&&!h.startsWith("blob:")?r.images.push(h):h.startsWith("blob:")&&u++}else y==="tags"?(r.tags=r.tags||[],r.tags.push(g)):r[y]=g;if(u&&!(r.images||[]).length){n&&(n.disabled=!1,n.textContent=o),p("Your images were still uploading â€” please wait a moment and press Publish again (the photos were not saved with the product).","error");return}r.is_featured=i.querySelector('[name="is_featured"]')?.checked?"on":"",r.is_active=i.querySelector('[name="is_active"]')?.checked?"on":"";const d=s.get("action")==="draft",c=y=>xt(y),m=y=>{const g=["model","storage","ram","processor","display","material","gender","platform","voltage","engine","transmission","fuel_type","horsepower","mileage","drive_type","body_type","model_year","seating_capacity","doors","real_price","type","size","age_range","skin_type","ingredients","dimensions","author","publisher","language","format","isbn","pages","edition","quantity","pet_type","lens","sensor","megapixels","video","license","version","duration","followers","engagement","niche","usage","shelf_life","assembly","weatherproof","movement","case_material","water_resistance","gemstone","movement_type","warranty_period"],h={};for(const S of g){const k=y[S];if(S==="real_price"){const _=k!=null&&String(k).trim()!==""?parseFloat(k):null;h[S]=_!=null&&Number.isFinite(_)&&_>0?Math.round(_):null;continue}h[S]=k!=null&&String(k).trim()!==""?k:null}if(y.safety_features){const S=c(y.safety_features);h.safety_features=S.length?S:null}return h};let f;if(a){let y=ke((window._productsData||[]).find(E=>E.property_id===a));if(!y){const{data:E}=await b.from("showroom_listings").select("*").eq("property_id",a).maybeSingle();y=E?ke(E):null}if(!y)throw new Error("Could not load the current product. Refresh the page and try again.");const g=(E,B)=>{const O=E===""||E==null?"":E,$=B===""||B==null?"":B;return String(O).trim()===String($).trim()},h={};["title","description","currency","subcategory","brand","color","size","condition","warranty","availability_status"].forEach(E=>{g(r[E],y[E])||(h[E]=r[E]==null||r[E]===""?null:r[E])});const S=r.price===""||r.price==null?null:parseFloat(r.price);g(S,y.price)||(h.price=S==null?y.price:Math.max(D,Math.min(Y,S)));const k=r.stock_quantity===""||r.stock_quantity==null?null:parseInt(r.stock_quantity,10);g(k,y.stock_quantity)||(h.stock_quantity=Number.isFinite(k)?k:null);const _=c(r.features_text);g(_.join("||"),(Array.isArray(y.features)?y.features:[]).join("||"))||(h.features=_);const v=r.tags||[];g(v.join("||"),(Array.isArray(y.tags)?y.tags:[]).join("||"))||(h.tags=v);const w=c(r.highlights_text);g(w.join("||"),(Array.isArray(y.highlights)?y.highlights:[]).join("||"))||(h.highlights=w);const x=c(r.seo_keywords_text);g(x.join("||"),(Array.isArray(y.seo_keywords)?y.seo_keywords:[]).join("||"))||(h.seo_keywords=x);const G=r.images||[];g(G.join("||"),(Array.isArray(y.images)?y.images:[]).join("||"))||(h.images=G);const X=r.is_featured==="on";!!y.is_featured!==X&&(h.is_featured=X);const Q=d?!1:r.is_active==="on";!!y.is_active!==Q&&(h.is_active=Q);const j=m(r),C={...y.specifications&&typeof y.specifications=="object"?y.specifications:{},...j};if(JSON.stringify(C)!==JSON.stringify(y.specifications||{})&&(h.specifications=C),Object.keys(h).length===0){p("No changes detected â€” nothing was saved.","info");try{localStorage.removeItem(Me(t,a))}catch{}n&&(n.disabled=!1,n.textContent=o);return}const T={...y,...h,property_id:a,updated_at:new Date().toISOString()};if({error:f}=await b.from("showroom_listings").upsert(T,{onConflict:"property_id"}),f&&vt(f,()=>gt(T),"Product update")){n&&(n.disabled=!1,n.textContent=o),ne(),F();return}p(d?"Draft saved!":`Published Successfully â€” your product is updated and live in your showroom (${Object.keys(h).length} change${Object.keys(h).length>1?"s":""}).`)}else{if(!r.title||!r.title.trim())throw new Error("A product title is required.");if(r.price===""||r.price==null||!isFinite(parseFloat(r.price)))throw new Error("A price is required.");if(!!i.querySelector('[name="condition"]')&&!r.condition)throw new Error("Please choose the product condition.");const g={listing_type:"product",category:t,subcategory:r.subcategory||null,title:r.title.trim(),description:r.description||"",price:Math.max(D,Math.min(Y,parseFloat(r.price)||0)),currency:r.currency||"USD",country:"",country_code:"",listing_status:"sale",state:"",city:"",product_location:"",latitude:null,longitude:null,is_active:d?!1:r.is_active==="on",is_featured:r.is_featured==="on",brand:r.brand||null,color:r.color||null,size:r.size||null,condition:r.condition||null,warranty:r.warranty||null,availability_status:r.availability_status||"In Stock",stock_quantity:r.stock_quantity?parseInt(r.stock_quantity):null,images:r.images||[],features:c(r.features_text).length?c(r.features_text):r.tags||[],tags:r.tags||[],highlights:c(r.highlights_text),seo_keywords:c(r.seo_keywords_text),is_ai_generated:!!r.catalog_template_id,ai_generated_fields:r.catalog_template_id?["title","description","features","highlights","seo_keywords"]:[],specifications:m(r)},h=et();if(g.property_id=h,{error:f}=await b.from("showroom_listings").insert(g),f&&vt(f,()=>gt({...g,property_id:g.property_id}),"Product publish")){n&&(n.disabled=!1,n.textContent=o),ne(),F();return}p(d?"Draft saved!":"Published Successfully! Your product is now live in your showroom.")}try{localStorage.removeItem(Me(t,a))}catch{}closeProductFormModal(),F()}catch(s){p("Error: "+s.message,"error"),n&&(n.disabled=!1,n.textContent=o)}};window.editProduct=async function(e){const{data:t,error:a}=await b.from("showroom_listings").select("*").eq("property_id",e).maybeSingle();let i=a?null:t;if(i||(i=Tt(e)),i||(i=(window._productsData||[]).find(n=>n.property_id===e)||null),!i)return p("Product not found","error");i.specifications&&typeof i.specifications=="object"&&(i={...i,...i.specifications}),showAddProductStep2(i.category||"Other",i)};window.toggleProductActive=async function(e,t){const a=ke((window._productsData||[]).find(n=>n.property_id===e)),{error:i}=await b.from("showroom_listings").upsert({...a,property_id:e,is_active:t,availability_status:t?"In Stock":"Out of Stock"},{onConflict:"property_id"});if(i)return Z(i)?p(`âš ï¸ ${t?"Publish":"Unpublish"} blocked: database admin role rejected the write. Re-run the admin permission migration.`,"error"):p(`${t?"Publish":"Unpublish"} failed: ${i.message}`,"error");p(t?"Product published":"Product unpublished"),F()};window.duplicateProduct=async function(e,t=!1){const{data:a}=await b.from("showroom_listings").select("*").eq("property_id",e).maybeSingle();if(!a)return;const{id:i,property_id:n,created_at:o,updated_at:s,...r}=a,u=et();await b.from("showroom_listings").insert({...r,property_id:u,title:a.title+" (Copy)",is_active:!1}),t||(p("Product duplicated"),F())};window.archiveProduct=async function(e){confirm("Archive this product? It will be hidden from the website but can be restored.")&&(await b.from("showroom_listings").update({is_active:!1,availability_status:"Archived"}).eq("property_id",e),p("Product archived"),F())};const Et=["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","Commercial Building","Hotel","Land","Other"];async function ot(){const e=document.getElementById("content");try{const{data:t,error:a}=await b.from("showroom_listings").select("*").eq("listing_type","property").order("created_at",{ascending:!1});let i=a?Qe().filter(o=>o.listing_type==="property"):t||[];if(Array.isArray(ae)){const o=new Set(i.map(r=>r.property_id)),s=ae.filter(r=>r.listing_type==="property"&&r.property_id&&!o.has(r.property_id));s.length&&(i=i.concat(s))}i.sort((o,s)=>new Date(s.created_at||0)-new Date(o.created_at||0));try{await Lt()}catch{}const n=new Set(Xe());i=i.filter(o=>!(o&&o.property_id&&n.has(o.property_id))),window._propertiesData=i,e.innerHTML=`
      <div class="space-y-4 fade-in">
      ...
      </div>`,window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400">${l(t.message)}</div>`)}}window.showAddPropertyModal=function(e={}){const t=!!e.property_id,a=ba("property","Real Estate"),i=e.country_code||"US",n=e.currency||Bt(i);W(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box wide">
        <div class="flex items-center justify-between mb-5">
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      ...
      </div>`,window._customersData=a,window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400">${l(t.message)}</div>`)}}window.searchCustomers=function(e){const t=e.toLowerCase();document.querySelectorAll(".cust-row").forEach(a=>{a.style.display=!t||a.dataset.search.toLowerCase().includes(t)?"":"none"})};window.viewCustomer=async function(e){const t=(window._customersData||[]).find(i=>i.user_id===e);if(!t)return;const{data:a}=await b.from("payment_receipts").select("order_number,amount,currency,status,created_at").eq("user_id",e).order("created_at",{ascending:!1}).limit(20);W(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box">
        <div class="flex items-center justify-between mb-5">
    ...
    <div class="flex items-start justify-between gap-3">
      ...
      </div>`,window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400">${l(t.message)}</div>`)}}window.showAddCouponModal=function(){W(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box">
        <div class="flex items-center justify-between mb-5">
...
`+a],{type:"text/csv"}),n=document.createElement("a");n.href=URL.createObjectURL(i),n.download=`kco-orders-${new Date().toISOString().slice(0,10)}.csv`,n.click(),p("Orders exported!")};async function Gn(){const e=document.getElementById("content"),{data:t}=await b.from("site_settings").select("*").limit(1).maybeSingle(),a=t||{};e.innerHTML=`
    ...
    <div class="flex flex-wrap gap-2">
      ${a.map(f=>`<button onclick="catalogSetCategory('${f.slug}')" class="btn-press px-3 py-1.5 rounded-xl text-xs font-bold border transition ${V.category===f.slug?"bg-blue-500/20 text-blue-200 border-blue-500/40":"bg-white/5 text-gray-400 border-white/10 hover:text-white"}">${l(f.name)}</button>`).join("")}
    </div>`,r=`
    <div class="flex flex-wrap items-center gap-2">
      <input id="catalog-search-input" class="input-field flex-1 min-w-[220px]" placeholder="Search title, id or subcategoryâ€¦" value="${l(V.query)}" onkeyup="if (event.key === 'Enter') catalogSearch()">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs text-gray-500">Page ${V.page+1} / ${c}</span>
        <button onclick="catalogPage(1)" ${V.page>=c-1?"disabled":""} class="btn-press px-3 py-1.5 rounded-xl text-xs font-bold bg-white/5 text-gray-300 border border-white/10 hover:text-white disabled:opacity-40">Next</button>
      </div>
    </div>`;e.innerHTML=`
    <div class="space-y-4 fade-in">
      ${o}
