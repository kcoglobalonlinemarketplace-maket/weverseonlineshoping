import{supabase as C}from"./supabase-client-7_ZWSEp6.js";import{b as L,c as W,e as q}from"./localization-nQ48c_Cg.js";const U="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),j=`${U}/functions/v1/ai-customer-assistant`,H="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",N=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let n={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null,attachments:[],recording:!1,mediaRecorder:null,mediaChunks:[],recordTimer:null,recordSeconds:0};function y(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function V(t){let e=y(t);return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^\- (.+)$/gm,"<li>$1</li>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,a=>`<ul>${a}</ul>`),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=`<p>${e}</p>`,e=e.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),e}function g(){return L()||localStorage.getItem("kco_language")||"en"}function T(){const t=window.location.pathname;return t.includes("checkout")?"checkout":t.includes("payment")?"payment":t.includes("auth")?"sign-in":t.includes("account")?"account":t.includes("details")?"product-details":t.includes("contact")?"contact":t.includes("help")?"help":t.includes("about")?"about":"home"}function P(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(e=>({title:e.title||e.name,price:e.price,quantity:e.quantity||1}))}catch{return[]}}function k(t){if(n.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t.replace(/[*#`]/g,"")),a=g(),o=W(a);a==="en"?e.lang=n.voiceAccent==="UK"?"en-GB":"en-US":e.lang=o.lang,e.rate=o.rate,e.pitch=o.pitch;const i=q(a==="en"?(n.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):a);i&&(e.voice=i),window.speechSynthesis.speak(e)}catch{}}function $(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function D(){if(document.getElementById("kco-ai-styles"))return;const t=document.createElement("style");t.id="kco-ai-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)}const w='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" fill="#60a5fa"/><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" stroke="#3b82f6" stroke-width="0.5"/></svg>',F='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" fill="#60a5fa"/><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z" stroke="#3b82f6" stroke-width="0.5"/></svg>',J='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.4 20.4l17.45-7.48a1 1 0 0 0 0-1.84L3.4 3.6a1 1 0 0 0-1.39 1.21L4 12l-1.99 7.19a1 1 0 0 0 1.39 1.21z" fill="white"/></svg>',B='<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 inline-block align-middle" aria-label="Verified" role="img"><circle cx="12" cy="12" r="11" fill="#3b82f6"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>';function G(){D();const t=document.createElement("button");t.id="kco-ai-fab",t.setAttribute("aria-label","Contact us"),t.className="fixed bottom-5 right-5 z-[60] flex items-center gap-2.5 pl-2.5 pr-4 py-2 rounded-2xl shadow-2xl transition-transform hover:scale-105 active:scale-95",t.style.background="linear-gradient(135deg,#1e1e2e 0%,#1a1a2e 50%,#16213e 100%)",t.style.border="1px solid rgba(59,130,246,0.35)",t.style.boxShadow="0 8px 24px rgba(59,130,246,0.25)",t.innerHTML=`
    ...
    <span class="text-left">
      <span class="text-sm font-bold text-white leading-tight tracking-wide flex items-center gap-1 whitespace-nowrap">Contact us ${B}</span>
  ...
  `,document.body.appendChild(e),document.getElementById("kco-ai-close").onclick=()=>d(!1),document.getElementById("kco-ai-minimize").onclick=()=>d(!1),document.getElementById("kco-ai-clear").onclick=ne,document.getElementById("kco-ai-voice-toggle").onclick=Y,document.getElementById("kco-ai-voice-accent").onclick=K,document.getElementById("kco-ai-send").onclick=p,document.getElementById("kco-ai-attach").onclick=()=>document.getElementById("kco-ai-file").click(),document.getElementById("kco-ai-file").onchange=Q,document.getElementById("kco-ai-mic").onclick=Z,document.getElementById("kco-ai-rec-cancel").onclick=te;const a=document.getElementById("kco-ai-input");a.addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),p())}),a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,96)+"px"}),document.querySelectorAll(".kco-ai-quick-btn").forEach(o=>{o.onclick=()=>{a.value=o.dataset.q,p()}}),z(),window.lucide&&lucide.createIcons()}function d(t){const e=document.getElementById("kco-ai-panel"),a=document.getElementById("kco-ai-fab");n.open=t!==void 0?t:e.classList.contains("hidden-panel"),n.open?(e.classList.remove("hidden-panel"),a.style.display="none",window.innerWidth<640&&(document.body.style.overflow="hidden"),n.welcomed||(O(),n.welcomed=!0),setTimeout(()=>document.getElementById("kco-ai-input")?.focus(),300)):(e.classList.add("hidden-panel"),a.style.display="flex",$(),b(),document.body.style.overflow="")}function Y(){n.voiceEnabled=!n.voiceEnabled,localStorage.setItem("kco_voice",n.voiceEnabled?"1":"0"),n.voiceEnabled||$(),z(),n.voiceEnabled&&k("Voice assistance is now on.")}function K(){n.voiceAccent=n.voiceAccent==="US"?"UK":"US",localStorage.setItem("kco_voice_accent",n.voiceAccent);const t=document.getElementById("kco-ai-voice-accent");t&&(t.textContent=n.voiceAccent);const e=n.voiceAccent==="UK"?"Voice accent switched to British English.":"Voice accent switched to American English.";k(e)}function z(){const t=document.getElementById("kco-ai-voice-toggle");if(!t)return;const e=n.voiceEnabled?"volume-2":"volume-x";t.title=n.voiceEnabled?"Voice is ON — click to mute":"Voice is OFF — click to enable",t.innerHTML=`<i data-lucide="${e}" class="w-4 h-4"></i>`,window.lucide&&lucide.createIcons()}function x(t){const e=Math.floor(t/60),a=String(Math.floor(t%60)).padStart(2,"0");return`${e}:${a}`}function f(){const t=document.getElementById("kco-ai-attachments");t&&(t.innerHTML=n.attachments.map((e,a)=>e.type==="image"?`<div class="kco-attach-chip relative shrink-0 w-16 h-16 rounded-lg overflow-hidden border border-blue-500/30 bg-slate-800">
    ...
    </div>`).join(""),t.classList.toggle("hidden",n.attachments.length===0),window.lucide&&lucide.createIcons(),t.querySelectorAll("[data-rm]").forEach(e=>{e.onclick=()=>{n.attachments.splice(Number(e.dataset.rm),1),f()}}))}function M(t){if(n.attachments.length>=4){h("You can attach up to 4 items per message.");return}n.attachments.push(t),f()}function X(t,e=900,a=.72){return new Promise(o=>{const i=new FileReader;i.onload=()=>{const s=new Image;s.onload=()=>{const m=Math.min(1,e/Math.max(s.width,s.height)),c=document.createElement("canvas");c.width=Math.max(1,Math.round(s.width*m)),c.height=Math.max(1,Math.round(s.height*m)),c.getContext("2d").drawImage(s,0,0,c.width,c.height),o(c.toDataURL("image/jpeg",a))},s.onerror=()=>o(i.result),s.src=i.result},i.onerror=()=>o(null),i.readAsDataURL(t)})}function Q(t){const e=t.target.files&&t.target.files[0];if(t.target.value="",!!e){if(!/^image\//.test(e.type)){h("Please choose an image file.");return}X(e).then(a=>{a&&M({type:"image",dataUrl:a,name:e.name})})}}function S(t){const e=document.getElementById("kco-ai-mic"),a=document.getElementById("kco-ai-recording");e&&(e.innerHTML=`<i data-lucide="${t?"square":"mic"}" class="w-5 h-5 ${t?"text-rose-400":""}"></i>`,e.title=t?"Stop recording":"Record voice note",e.style.color=t?"#fb7185":"",e.style.background=t?"rgba(244,63,94,0.15)":"",a&&a.classList.toggle("hidden",!t),window.lucide&&lucide.createIcons())}function Z(){if(n.recording){b();return}ee()}async function ee(){if(!navigator.mediaDevices||!window.MediaRecorder){h("Voice notes are not supported on this browser.");return}try{const t=await navigator.mediaDevices.getUserMedia({audio:!0});n.mediaChunks=[],n.recordSeconds=0;const e=MediaRecorder.isTypeSupported("audio/webm;codecs=opus")?"audio/webm;codecs=opus":"audio/webm";n.mediaRecorder=new MediaRecorder(t,e?{mimeType:e}:void 0),n.mediaRecorder.ondataavailable=o=>{o.data.size>0&&n.mediaChunks.push(o.data)},n.mediaRecorder.onstop=()=>{const o=new Blob(n.mediaChunks,{type:n.mediaRecorder.mimeType||"audio/webm"});t.getTracks().forEach(s=>s.stop());const i=new FileReader;i.onloadend=()=>M({type:"audio",dataUrl:i.result,duration:n.recordSeconds}),i.readAsDataURL(o)},n.recording=!0,S(!0);const a=document.getElementById("kco-ai-rec-timer");n.recordTimer=setInterval(()=>{if(n.recordSeconds+=1,n.recordSeconds>=60){b();return}a&&(a.textContent=x(n.recordSeconds))},1e3)}catch{h("Microphone access was denied.")}}function b(){if(!(!n.recording||!n.mediaRecorder)){try{n.mediaRecorder.stop()}catch{}clearInterval(n.recordTimer),n.recordTimer=null,n.recording=!1,S(!1)}}function te(){if(clearInterval(n.recordTimer),n.recordTimer=null,n.mediaChunks=[],n.mediaRecorder&&n.mediaRecorder.state!=="inactive"){try{n.mediaRecorder.onstop=null,n.mediaRecorder.stop()}catch{}n.mediaRecorder.stream&&n.mediaRecorder.stream.getTracks().forEach(t=>t.stop())}n.recording=!1,n.mediaRecorder=null,S(!1)}function ne(){const t=document.getElementById("kco-ai-messages");t&&(t.innerHTML=""),n.history=[],n.attachments=[],f(),b(),O(),ae("Conversation cleared")}function ae(t){const e=document.createElement("div");e.className="fixed top-16 left-1/2 -translate-x-1/2 z-[70] bg-gray-900 border border-blue-500/30 text-white text-xs font-medium px-4 py-2 rounded-full shadow-xl kco-ai-fade",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),2e3)}function h(t){const e=document.getElementById("kco-ai-input");e&&(e.placeholder=t,setTimeout(()=>{e.placeholder===t&&(e.placeholder="Type a message...")},2600))}function oe(t){return(t||[]).map(e=>e.type==="image"?`<img src="${e.dataUrl}" class="mt-2 w-full max-h-56 rounded-xl object-cover border border-white/10" alt="Shared image">`:e.type==="audio"?`<div class="mt-2 flex items-center gap-2">
  ...
  `,a.appendChild(i),window.lucide&&lucide.createIcons(),R(),o||k(t.content)}function O(){const t=g(),e={en:`Welcome to Weverse Online Shop!
...
Naweza kukusaidia vipi leo?`};u({role:"assistant",content:e[t]||e.en})}function ie(){const t=document.getElementById("kco-ai-messages");if(!t)return;const e=document.createElement("div");e.id="kco-ai-typing",e.className="flex justify-start kco-ai-fade",e.innerHTML=`
    ...
    </div>`,t.appendChild(e),window.lucide&&lucide.createIcons(),R()}function R(){const t=document.getElementById("kco-ai-messages");t&&(t.scrollTop=t.scrollHeight)}async function se(){const{data:t}=await C.auth.getSession();return{Authorization:`Bearer ${t.session?.access_token||H}`,"Content-Type":"application/json"}}async function p(){const t=document.getElementById("kco-ai-input");if(!t)return;const e=t.value.trim(),a=n.attachments.slice();if(!e&&a.length===0||n.sending)return;t.value="",t.style.height="auto",n.attachments=[],f(),n.sending=!0,document.getElementById("kco-ai-send").disabled=!0;const o=a.some(r=>r.type==="image"),i=a.some(r=>r.type==="audio"),s=[o&&"[Image attached]",i&&"[Voice note attached]"].filter(Boolean).join(" "),m=s?`${e?e+" ":""}${s}`.trim():e,c={role:"user",content:m,attachments:a};n.history.push(c),u(c),ie();try{const r=await se(),I=await(await fetch(j,{method:"POST",headers:r,body:JSON.stringify({action:"chat",message:m,history:n.history.slice(-20,-1).map(l=>({role:l.role,content:l.content})),language:g(),page_context:T(),cart_items:P(),attachments:a.map(l=>({type:l.type,dataUrl:l.dataUrl,duration:l.duration||0}))})})).json();document.getElementById("kco-ai-typing")?.remove();const E={role:"assistant",content:I.response||I.error||"I'm sorry, I didn't catch that. Could you rephrase?"};n.history.push(E),u(E)}catch{document.getElementById("kco-ai-typing")?.remove();const v={role:"assistant",content:"I'm having trouble connecting right now. Please try again in a moment."};n.history.push(v),u(v)}finally{n.sending=!1,document.getElementById("kco-ai-send").disabled=!1,document.getElementById("kco-ai-input")?.focus()}}function _(t,e){if(n.proactiveShown[t]||(n.proactiveShown[t]=!0,n.open)||!document.getElementById("kco-ai-fab"))return;const o=document.createElement("div");o.id=`kco-ai-nudge-${t}`,o.className="fixed bottom-[92px] right-5 z-[59] max-w-[280px] bg-slate-800 border border-blue-500/30 rounded-xl shadow-2xl px-3.5 py-2.5 kco-ai-fade",o.innerHTML=`
