const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/catalog-CKKE61jd.js","assets/catalog-hidden-store-DbF0xVeS.js","assets/preload-helper-CLcXU_4U.js"])))=>i.map(i=>d[i]);
import{supabase as g}from"./supabase-client-7_ZWSEp6.js";import{d as M,g as E,C as j}from"./localization-B2rzni8q.js";import{_ as m}from"./preload-helper-CLcXU_4U.js";import{g as N}from"./auth-D3ZJ7kZA.js";import{f as $,l as k,g as L,a as q}from"./showroom-data-BoOhjLzR.js";import{c as D,a as O,b as U,d as S,r as F,e as G}from"./payment-settings-DIUDlzx4.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-CT1lAy4v.js";import"./customer-ai-widget-BvSrA37D.js";import"./brand-DYhAh01b.js";const f="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function R(){const t=Date.now().toString(36).toUpperCase().slice(-6),s=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${s}`}function b(t){const s=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,s.classList.remove("translate-y-20","opacity-0"),clearTimeout(s._t),s._t=setTimeout(()=>s.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function I(t){t.addEventListener("click",function(s){if(this.disabled)return;const i=this.getBoundingClientRect(),r=document.createElement("span");r.className="ripple";const a=Math.max(i.width,i.height);r.style.width=r.style.height=a+"px",r.style.left=s.clientX-i.left-a/2+"px",r.style.top=s.clientY-i.top-a/2+"px",this.appendChild(r),setTimeout(()=>r.remove(),600)})}function p(t,s){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${s}`}function H(t){const s=()=>{const i=document.createElement("textarea");i.value=t,document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>s()):s(),b("Copied to clipboard.")}function z(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,s=F(e.manualPaymentAccounts,e.countryCode,t);return{...s,fallbackNotice:s.isFallback?G(s.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function V(){const t=document.getElementById("particles");if(t)for(let s=0;s<12;s++){const i=document.createElement("div"),r=Math.random()*3+1;i.className="particle",i.style.width=i.style.height=r+"px",i.style.left=Math.random()*100+"%",i.style.bottom="-10px",i.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",i.style.animationDuration=Math.random()*20+15+"s",i.style.animationDelay=Math.random()*20+"s",t.appendChild(i)}}V();async function J(){const t=document.getElementById("checkout-root"),s=new URLSearchParams(window.location.search),i=s.get("status"),r=s.get("transaction_id"),a=s.get("tx_ref"),c=s.get("order_number");if(i==="verify"&&r){await ie(r,a,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=s.get("guest")==="1",!e.isGuest&&(e.user=await N(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const l=s.get("id");if(l){if(e.listing=$(l),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:n}]=await Promise.all([m(()=>import("./catalog-CKKE61jd.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await n(),e.listing=o(l)}if(e.listing||(await k(),e.listing=$(l)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await k();const n=L(),[{generateListingById:d},{loadHiddenCatalogIds:y}]=await Promise.all([m(()=>import("./catalog-CKKE61jd.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);if(await y(),e.cartItems=o.map(w=>{const _=n.find(T=>T.property_id===w)||d(w);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=A(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await g.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=M(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await D(),e.manualPaymentAccounts=O(e.paymentSettings),e.manualPaymentInstructions=U(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),S(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await g.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const n=e.addresses.find(y=>y.is_default);n&&(e.selectedAddressId=n.id);const{data:d}=await g.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=R(),u()}function h(){return e.cartItems.reduce((t,s)=>t+s.listing.price*s.quantity,0)}function X(){return 0}function Y(){return 0}function v(){return Math.round(h()*Y()*100)/100}function x(){return h()+X()+v()}function u(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(I)}function Q(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
        ...
        ${e.cartItems.map((t,s)=>`
            <div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-400 mt-1">${q(t.listing)}</p>
              </div>
  ...
  `}function K(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-white uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(i=>{const r=E(i.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-[#0a1124]/80 border border-blue-500/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500">
              ${j.map(i=>`<option value="${i.code}" ${e.shippingCountry===i.code?"selected":""}>${i.flag} ${i.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function Z(){const t=S(e.manualPaymentAccounts),s=z(),i=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",r=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${t.map(a=>{const c=e.manualPaymentAccounts.find(l=>l.currency===a);return c?`<button onclick="selectCurrency('${a}')" class="btn-press flex flex-col items-center gap-1 p-2.5 rounded-xl border transition relative overflow-hidden ${a===e.selectedCurrency?"bg-blue-500/15 border-blue-500/50 text-blue-400":"bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30"}">
            <span class="text-xl">${c.flag}</span><span class="text-xs font-bold">${a}</span>
          </button>`:""}).join("")}
      </div>
    </div>

    <!-- Bank account details (if manual) -->
    ${e.paymentMethod==="manual_bank_transfer"?ee(s.account,s.fallbackNotice,e.manualPaymentInstructions):""}
    ...
    <div class="space-y-3">
      ${e.paymentMethod==="flutterwave"?`
        <button onclick="payWithFlutterwave()" id="flw-pay-btn" class="btn-press w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="zap" class="w-5 h-5"></i> Pay ${p(x(),e.selectedCurrency)} with Flutterwave
        </button>
      `:`
  ...
  `}function ee(t,s,i){const r=[{label:"Beneficiary Name",value:t.beneficiary},{label:"Bank Name",value:t.bankName},{label:"Account Number",value:t.accountNumber},{label:"IBAN",value:t.iban},{label:"SWIFT / BIC",value:t.swift},{label:"Routing (ABA)",value:t.routing},{label:"Sort Code",value:t.sortCode},{label:"Bank Code",value:t.bankCode},{label:"Branch Code",value:t.branchCode},{label:"Institution Number",value:t.institutionNumber},{label:"Transit Number",value:t.transitNumber},{label:"BSB Code",value:t.bsbCode},{label:"Bank Address",value:t.address}].filter(a=>a.value&&a.value.trim()!=="");return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ...
      <div class="space-y-2">
        ${r.map(a=>`
          <div class="flex items-center justify-between gap-3 bg-blue-950/40 border border-blue-500/10 rounded-xl px-4 py-2.5">
            <div class="min-w-0 flex-1"><div class="text-gray-500 text-[11px] uppercase tracking-wide">${a.label}</div><div class="text-gray-100 text-sm font-medium font-mono break-all">${a.value}</div></div>
            <button onclick="copyToClipboard('${a.value.replace(/'/g,"\\'")}')" class="shrink-0 p-2 bg-blue-900/40 hover:bg-blue-500/20 border border-blue-500/20 rounded-lg transition"><i data-lucide="copy" class="w-4 h-4 text-gray-400"></i></button>
          </div>
        `).join("")}
      </div>
  ...
  `}function te(){const t=h(),s=x();return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up lg:sticky lg:top-20">
        ...
        ${e.cartItems.map(i=>`
            <div class="flex items-center gap-3">
              <div class="w-12 h-12 rounded-lg bg-gray-900 overflow-hidden shrink-0 ring-1 ring-blue-500/10">
                <img src="${i.listing.images?.[0]||f}" class="w-full h-full object-cover" onerror="this.src='${f}'">
              </div>
              <div class="flex-1 min-w-0">
                <p class="text-xs font-bold text-white truncate">${i.listing.title}</p>
                <p class="text-xs text-gray-500">Qty: ${i.quantity}</p>
              </div>
              <p class="text-xs font-bold text-amber-400 shrink-0">${p(i.listing.price*i.quantity,i.listing.currency||"USD")}</p>
            </div>
          `).join("")}
      </div>
      <div class="space-y-2 pt-4 border-t border-blue-500/10">
        ...
        ${v()>0?`<div class="flex justify-between text-sm"><span class="text-gray-500">Tax</span><span class="text-white font-bold">${p(v(),e.cartItems[0]?.listing?.currency||"USD")}</span></div>`:""}
        <div class="flex justify-between text-lg pt-2 border-t border-blue-500/10"><span class="text-white font-bold">Total</span><span class="text-amber-400 font-black">${p(s,e.cartItems[0]?.listing?.currency||"USD")}</span></div>
      </div>
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:s,order_number:i})}),l=await c.json();if(!c.ok||l.error||l.status==="failed"){r.innerHTML=C(l.message||l.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}r.innerHTML=re(i),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(I),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(a){r.innerHTML=C(a.message),window.lucide&&lucide.createIcons()}}function re(t){return`
