import{k as f,b as w,Z,c as P,d as D,T as F,D as B,e as G,l as L,g as R,h as c,Y as k,X as C}from"./supabase-client-DvCmNkpI-BBtcpOs0-B154SqIU.js";const h={trust_promo_bg_image:"",trust_promo_bg_video:"",app_banner_bg_image:"",app_banner_bg_video:"",reviews_bg_image:"",reviews_bg_video:""},W=Object.keys(h).join(","),M="kco_promo_backgrounds_v1",Y=60*1e3;function U(){try{const e=JSON.parse(localStorage.getItem(M)||"{}");if(e.ts&&Date.now()-e.ts<Y&&e.data&&typeof e.data=="object")return e.data}catch{}return null}function V(e){try{localStorage.setItem(M,JSON.stringify({ts:Date.now(),data:e}))}catch{}}async function g(){const e=U();if(e)return{...h,...e};try{const t=await L(),{data:a,error:r}=await t.from("site_settings").select(W).limit(1).maybeSingle(),s={...h,...r||!a?{}:a};return V(s),s}catch{return{...h}}}function ge(){try{localStorage.removeItem(M)}catch{}window.dispatchEvent(new CustomEvent("promo-backgrounds-updated"))}function S(e,t){const a=(e||"").trim(),r=(t||"").trim(),s=[];return r&&s.push(`<video class="absolute inset-0 w-full h-full object-cover" autoplay muted loop playsinline webkit-playsinline preload="metadata" poster="${j(a)}"><source src="${j(r)}" type="video/mp4"></video>`),a&&s.push(`<div class="absolute inset-0 bg-cover bg-center" style="background-image:url('${N(a)}')"></div>`),(r||a)&&s.push('<div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-900/70 to-slate-900/45"></div>'),s.join("")}function j(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function N(e){return String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'")}const y="support@weverseonlineshop.com",z="weverse_brand_v1";function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function O(){try{const e=JSON.parse(localStorage.getItem(z)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function J(){const e=O(),t=e.brand_name||e.site_name||B;return`
