...
import{_ as m}from"./preload-helper-CS1eXPs2.js";import{d as O,g as D,C as U}from"./localization-DxUGQRcV.js";import{supabase as y}from"./supabase-client-nvpjTmO6.js";import{getCurrentUser as q}from"./auth-C8sthrmE.js";import{f as k,l as $,g as F,a as G}from"./showroom-data-BT2ougk1.js";import{g as C,a as I,P as R,b as H}from"./products-extra-DYXPKBE8.js";import{g as S,a as A}from"./phone-data-_dXqvG67.js";import{c as z,a as V,b as X,d as T,r as J,e as Y}from"./payment-settings-R0jbexV2.js";import"./native-bridge-DjUVSZMf.js";import"./localization-bootstrap-DFK7_aHn.js";import"./customer-ai-widget-MydSXXdn.js";import"./brand-Ch4-wKat.js";const Q=[...R,...H];function P(t){return Q.find(r=>r.property_id===t)||null}const f="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function K(){const t=Date.now().toString(36).toUpperCase().slice(-6),r=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${r}`}function g(t){const r=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,r.classList.remove("translate-y-20","opacity-0"),clearTimeout(r._t),r._t=setTimeout(()=>r.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function M(t){t.addEventListener("click",function(r){if(this.disabled)return;const a=this.getBoundingClientRect(),s=document.createElement("span");s.className="ripple";const n=Math.max(a.width,a.height);s.style.width=s.style.height=n+"px",s.style.left=r.clientX-a.left-n/2+"px",s.style.top=r.clientY-a.top-n/2+"px",this.appendChild(s),setTimeout(()=>s.remove(),600)})}function b(t,r){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${r}`}function W(t){const r=()=>{const a=document.createElement("textarea");a.value=t,document.body.appendChild(a),a.select();try{document.execCommand("copy")}catch{}document.body.removeChild(a)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>r()):r(),g("Copied to clipboard.")}function Z(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,r=J(e.manualPaymentAccounts,e.countryCode,t);return{...r,fallbackNotice:r.isFallback?Y(r.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function ee(){const t=document.getElementById("particles");if(t)for(let r=0;r<12;r++){const a=document.createElement("div"),s=Math.random()*3+1;a.className="particle",a.style.width=a.style.height=s+"px",a.style.left=Math.random()*100+"%",a.style.bottom="-10px",a.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",a.style.animationDuration=Math.random()*20+15+"s",a.style.animationDelay=Math.random()*20+"s",t.appendChild(a)}}ee();async function te(){const t=document.getElementById("checkout-root"),r=new URLSearchParams(window.location.search),a=r.get("status"),s=r.get("transaction_id"),n=r.get("tx_ref"),c=r.get("order_number");if(a==="verify"&&s){await ue(s,n,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=r.get("guest")==="1",!e.isGuest&&(e.user=await q(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const i=r.get("id");if(i){if(e.listing=k(i)||C(i)||I(i)||S(i)||A(i)||P(i),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:l}]=await Promise.all([m(()=>import("./catalog-BzyCL9ZE.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-BJoPnRIT.js"),__vite__mapDeps([1,2]))]);await l(),e.listing=o(i)}if(e.listing||(await $(),e.listing=k(i)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await $();const l=F(),[{generateListingById:d},{loadHiddenCatalogIds:v}]=await Promise.all([m(()=>import("./catalog-BzyCL9ZE.js"),__vite__mapDeps([0,1,2])),m(()=>import("./catalog-hidden-store-BJoPnRIT.js"),__vite__mapDeps([1,2]))]);if(await v(),e.cartItems=o.map(u=>{const _=l.find(L=>L.property_id===u)||C(u)||I(u)||S(u)||A(u)||P(u)||d(u);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=E(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await y.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=O(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await z(),e.manualPaymentAccounts=V(e.paymentSettings),e.manualPaymentInstructions=X(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),T(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await y.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const l=e.addresses.find(v=>v.is_default);l&&(e.selectedAddressId=l.id);const{data:d}=await y.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=K(),p()}function h(){return e.cartItems.reduce((t,r)=>t+r.listing.price*r.quantity,0)}function re(){return 0}function ae(){return 0}function w(){return Math.round(h()*ae()*100)/100}function x(){return h()+re()+w()}function p(){const t=document.getElementById("checkout-root");t.innerHTML=`
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(M)}function se(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
  ...
  `}function ne(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      ...
      <div class="space-y-2">
        ${e.addresses.map(a=>{const s=D(a.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
              ${U.map(a=>`<option value="${a.code}" ${e.shippingCountry===a.code?"selected":""}>${a.flag} ${a.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function le(){const t=T(e.manualPaymentAccounts),r=Z(),a=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",s=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:r,order_number:a})}),i=await c.json();if(!c.ok||i.error||i.status==="failed"){s.innerHTML=B(i.message||i.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}s.innerHTML=pe(a),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(M),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(n){s.innerHTML=B(n.message),window.lucide&&lucide.createIcons()}}function pe(t){return`
