...
import{supabase as f}from"./supabase-client-7_ZWSEp6.js";import{d as j,g as N,C as L}from"./localization-DIzE0wyu.js";import{_ as g}from"./preload-helper-CLcXU_4U.js";import{g as q}from"./auth-D3ZJ7kZA.js";import{f as k,l as $,g as D,a as O}from"./showroom-data-DPO_EY_W.js";import{g as C,a as S}from"./motorhome-data-DIOhWotq.js";import{c as U,a as F,b as G,d as A,r as R,e as H}from"./payment-settings-LqBp65aW.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-CBaRe6sn.js";import"./customer-ai-widget-DLHvZ8UI.js";import"./brand-DYhAh01b.js";const h="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function z(){const t=Date.now().toString(36).toUpperCase().slice(-6),s=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${s}`}function b(t){const s=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,s.classList.remove("translate-y-20","opacity-0"),clearTimeout(s._t),s._t=setTimeout(()=>s.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function P(t){t.addEventListener("click",function(s){if(this.disabled)return;const i=this.getBoundingClientRect(),r=document.createElement("span");r.className="ripple";const l=Math.max(i.width,i.height);r.style.width=r.style.height=l+"px",r.style.left=s.clientX-i.left-l/2+"px",r.style.top=s.clientY-i.top-l/2+"px",this.appendChild(r),setTimeout(()=>r.remove(),600)})}function p(t,s){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${s}`}function V(t){const s=()=>{const i=document.createElement("textarea");i.value=t,document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>s()):s(),b("Copied to clipboard.")}function J(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,s=R(e.manualPaymentAccounts,e.countryCode,t);return{...s,fallbackNotice:s.isFallback?H(s.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function X(){const t=document.getElementById("particles");if(t)for(let s=0;s<12;s++){const i=document.createElement("div"),r=Math.random()*3+1;i.className="particle",i.style.width=i.style.height=r+"px",i.style.left=Math.random()*100+"%",i.style.bottom="-10px",i.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",i.style.animationDuration=Math.random()*20+15+"s",i.style.animationDelay=Math.random()*20+"s",t.appendChild(i)}}X();async function Y(){const t=document.getElementById("checkout-root"),s=new URLSearchParams(window.location.search),i=s.get("status"),r=s.get("transaction_id"),l=s.get("tx_ref"),c=s.get("order_number");if(i==="verify"&&r){await ae(r,l,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=s.get("guest")==="1",!e.isGuest&&(e.user=await q(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const a=s.get("id");if(a){if(e.listing=k(a)||C(a)||S(a),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:n}]=await Promise.all([g(()=>import("./catalog-BPD_6M9e.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await n(),e.listing=o(a)}if(e.listing||(await $(),e.listing=k(a)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await $();const n=D(),[{generateListingById:d},{loadHiddenCatalogIds:v}]=await Promise.all([g(()=>import("./catalog-BPD_6M9e.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);if(await v(),e.cartItems=o.map(m=>{const _=n.find(E=>E.property_id===m)||C(m)||S(m)||d(m);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=B(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await f.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=j(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await U(),e.manualPaymentAccounts=F(e.paymentSettings),e.manualPaymentInstructions=G(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),A(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await f.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const n=e.addresses.find(v=>v.is_default);n&&(e.selectedAddressId=n.id);const{data:d}=await f.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=z(),u()}function x(){return e.cartItems.reduce((t,s)=>t+s.listing.price*s.quantity,0)}function Q(){return 0}function W(){return 0}function w(){return Math.round(x()*W()*100)/100}function y(){return x()+Q()+w()}function u(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(P)}function K(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
        ...
        ${e.cartItems.map((t,s)=>`
            <div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-400 mt-1">${O(t.listing)}</p>
              </div>
  ...
  `}function ee(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-white uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(i=>{const r=N(i.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-[#0a1124]/80 border border-blue-500/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500">
              ${L.map(i=>`<option value="${i.code}" ${e.shippingCountry===i.code?"selected":""}>${i.flag} ${i.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function te(){const t=A(e.manualPaymentAccounts),s=J(),i=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",r=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    ${e.paymentMethod==="manual_bank_transfer"?se(s.account,s.fallbackNotice,e.manualPaymentInstructions):""}
    ...
    <div class="space-y-3">
      ${e.paymentMethod==="flutterwave"?`
        <button onclick="payWithFlutterwave()" id="flw-pay-btn" class="btn-press w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="zap" class="w-5 h-5"></i> Pay ${p(y(),e.selectedCurrency)} with Flutterwave
        </button>
      `:`
  ...
  `}function ie(){const t=x(),s=y();return`
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:s,order_number:i})}),a=await c.json();if(!c.ok||a.error||a.status==="failed"){r.innerHTML=I(a.message||a.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}r.innerHTML=le(i),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(P),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(l){r.innerHTML=I(l.message),window.lucide&&lucide.createIcons()}}function le(t){return`
