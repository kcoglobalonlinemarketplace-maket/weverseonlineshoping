import{supabase as A}from"./supabase-client-7_ZWSEp6.js";import{b as L,c as W,e as q}from"./localization-CYjC8Dnv.js";const U="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),C=`${U}/functions/v1/ai-customer-assistant`,H="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",N=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let n={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null,attachments:[],recording:!1,mediaRecorder:null,mediaChunks:[],recordTimer:null,recordSeconds:0};function y(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function V(t){let e=y(t);return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^\- (.+)$/gm,"<li>$1</li>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=`<p>${e}</p>`,e=e.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),e}function g(){return L()||localStorage.getItem("kco_language")||"en"}function T(){const t=window.location.pathname;return t.includes("checkout")?"checkout":t.includes("payment")?"payment":t.includes("auth")?"sign-in":t.includes("account")?"account":t.includes("details")?"product-details":t.includes("contact")?"contact":t.includes("help")?"help":t.includes("about")?"about":"home"}function P(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(e=>({title:e.title||e.name,price:e.price,quantity:e.quantity||1}))}catch{return[]}}function k(t){if(n.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t.replace(/[*#`]/g,"")),o=g(),a=W(o);o==="en"?e.lang=n.voiceAccent==="UK"?"en-GB":"en-US":e.lang=a.lang,e.rate=a.rate,e.pitch=a.pitch;const i=q(o==="en"?(n.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):o);i&&(e.voice=i),window.speechSynthesis.speak(e)}catch{}}function z(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function D(){if(document.getElementById("kco-ai-styles"))return;const t=document.createElement("style");t.id="kco-ai-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)}const w='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" fill="#60a5fa"/><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" stroke="#3b82f6" stroke-width="0.5"/></svg>',F='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.4 20.4l17.45-7.48a1 1 0 0 0 0-1.84L3.4 3.6a1 1 0 0 0-1.39 1.21L4 12l-1.99 7.19a1 1 0 0 0 1.39 1.21z" fill="white"/></svg>',B='<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 inline-block align-middle" aria-label="Verified" role="img"><circle cx="12" cy="12" r="11" fill="#3b82f6"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>';function J(){D();const t=document.createElement("button");t.id="kco-ai-fab",t.setAttribute("aria-label","Contact us"),t.className="fixed bottom-5 right-5 z-[60] flex items-center gap-2 pl-2.5 pr-4 py-2 rounded-full shadow-xl transition-transform hover:scale-105 active:scale-95",t.style.background="linear-gradient(135deg,#3b82f6 0%,#2563eb 100%)",t.style.border="1px solid rgba(255,255,255,0.18)",t.style.boxShadow="0 8px 24px rgba(59,130,246,0.4)",t.innerHTML=`
  ...
  `,t.onclick=()=>c(!0),document.body.appendChild(t);const e=document.createElement("div");e.id="kco-ai-panel",e.className="hidden-panel fixed z-[60] flex flex-col overflow-hidden w-full sm:w-[400px] h-[92dvh] sm:h-[640px] sm:max-h-[calc(100dvh-120px)] bottom-0 sm:bottom-[90px] right-0 sm:right-5 rounded-t-2xl sm:rounded-2xl bg-slate-900/97 border-t sm:border border-blue-500/25 shadow-2xl",e.style.backdropFilter="blur(20px)",e.innerHTML=`
    ...
    <div class="flex items-center justify-between px-3.5 py-3 border-b border-white/10 shrink-0" style="background:linear-gradient(135deg,#1e1e2e 0%,#1a1a2e 50%,#16213e 100%)">
      <div class="flex items-center gap-2.5 min-w-0">
        ...
        <div class="min-w-0">
          <p class="text-sm font-bold text-white leading-tight tracking-wide flex items-center gap-1 truncate">Contact us ${B}</p>
  ...
  `,document.body.appendChild(e),document.getElementById("kco-ai-close").onclick=()=>c(!1),document.getElementById("kco-ai-minimize").onclick=()=>c(!1),document.getElementById("kco-ai-clear").onclick=te,document.getElementById("kco-ai-voice-toggle").onclick=Y,document.getElementById("kco-ai-voice-accent").onclick=G,document.getElementById("kco-ai-send").onclick=p,document.getElementById("kco-ai-attach").onclick=()=>document.getElementById("kco-ai-file").click(),document.getElementById("kco-ai-file").onchange=X,document.getElementById("kco-ai-mic").onclick=Q,document.getElementById("kco-ai-rec-cancel").onclick=ee;const o=document.getElementById("kco-ai-input");o.addEventListener("keydown",a=>{a.key==="Enter"&&!a.shiftKey&&(a.preventDefault(),p())}),o.addEventListener("input",()=>{o.style.height="auto",o.style.height=Math.min(o.scrollHeight,96)+"px"}),document.querySelectorAll(".kco-ai-quick-btn").forEach(a=>{a.onclick=()=>{o.value=a.dataset.q,p()}}),$(),window.lucide&&lucide.createIcons()}function c(t){const e=document.getElementById("kco-ai-panel"),o=document.getElementById("kco-ai-fab");n.open=t!==void 0?t:e.classList.contains("hidden-panel"),n.open?(e.classList.remove("hidden-panel"),o.style.display="none",window.innerWidth<640&&(document.body.style.overflow="hidden"),n.welcomed||(M(),n.welcomed=!0),setTimeout(()=>document.getElementById("kco-ai-input")?.focus(),300)):(e.classList.add("hidden-panel"),o.style.display="flex",z(),f(),document.body.style.overflow="")}function Y(){n.voiceEnabled=!n.voiceEnabled,localStorage.setItem("kco_voice",n.voiceEnabled?"1":"0"),n.voiceEnabled||z(),$(),n.voiceEnabled&&k("Voice assistance is now on.")}function G(){n.voiceAccent=n.voiceAccent==="US"?"UK":"US",localStorage.setItem("kco_voice_accent",n.voiceAccent);const t=document.getElementById("kco-ai-voice-accent");t&&(t.textContent=n.voiceAccent);const e=n.voiceAccent==="UK"?"Voice accent switched to British English.":"Voice accent switched to American English.";k(e)}function $(){const t=document.getElementById("kco-ai-voice-toggle");if(!t)return;const e=n.voiceEnabled?"volume-2":"volume-x";t.title=n.voiceEnabled?"Voice is ON — click to mute":"Voice is OFF — click to enable",t.innerHTML=`<i data-lucide="${e}" class="w-4 h-4"></i>`,window.lucide&&lucide.createIcons()}function x(t){const e=Math.floor(t/60),o=String(Math.floor(t%60)).padStart(2,"0");return`${e}:${o}`}function b(){const t=document.getElementById("kco-ai-attachments");t&&(t.innerHTML=n.attachments.map((e,o)=>e.type==="image"?`<div class="kco-attach-chip relative shrink-0 w-16 h-16 rounded-lg overflow-hidden border border-blue-500/30 bg-slate-800">
        <img src="${e.dataUrl}" class="w-full h-full object-cover" alt="Attachment">
        <button data-rm="${o}" class="absolute top-0.5 right-0.5 w-5 h-5 rounded-full bg-black/70 text-white text-[10px] flex items-center justify-center hover:bg-red-600 transition" aria-label="Remove image">✕</button>
      ...
      <span class="text-xs font-semibold text-rose-200 whitespace-nowrap">${x(e.duration)}</span>
      <button data-rm="${o}" class="w-5 h-5 rounded-full bg-black/50 text-white text-[10px] flex items-center justify-center hover:bg-red-600 transition" aria-label="Remove voice note">✕</button>
    </div>`).join(""),t.classList.toggle("hidden",n.attachments.length===0),window.lucide&&lucide.createIcons(),t.querySelectorAll("[data-rm]").forEach(e=>{e.onclick=()=>{n.attachments.splice(Number(e.dataset.rm),1),b()}}))}function R(t){if(n.attachments.length>=4){h("You can attach up to 4 items per message.");return}n.attachments.push(t),b()}function K(t,e=900,o=.72){return new Promise(a=>{const i=new FileReader;i.onload=()=>{const s=new Image;s.onload=()=>{const m=Math.min(1,e/Math.max(s.width,s.height)),r=document.createElement("canvas");r.width=Math.max(1,Math.round(s.width*m)),r.height=Math.max(1,Math.round(s.height*m)),r.getContext("2d").drawImage(s,0,0,r.width,r.height),a(r.toDataURL("image/jpeg",o))},s.onerror=()=>a(i.result),s.src=i.result},i.onerror=()=>a(null),i.readAsDataURL(t)})}function X(t){const e=t.target.files&&t.target.files[0];if(t.target.value="",!!e){if(!/^image\//.test(e.type)){h("Please choose an image file.");return}K(e).then(o=>{o&&R({type:"image",dataUrl:o,name:e.name})})}}function S(t){const e=document.getElementById("kco-ai-mic"),o=document.getElementById("kco-ai-recording");e&&(e.innerHTML=`<i data-lucide="${t?"square":"mic"}" class="w-5 h-5 ${t?"text-rose-400":""}"></i>`,e.title=t?"Stop recording":"Record voice note",e.style.color=t?"#fb7185":"",e.style.background=t?"rgba(244,63,94,0.15)":"",o&&o.classList.toggle("hidden",!t),window.lucide&&lucide.createIcons())}function Q(){if(n.recording){f();return}Z()}async function Z(){if(!navigator.mediaDevices||!window.MediaRecorder){h("Voice notes are not supported on this browser.");return}try{const t=await navigator.mediaDevices.getUserMedia({audio:!0});n.mediaChunks=[],n.recordSeconds=0;const e=MediaRecorder.isTypeSupported("audio/webm;codecs=opus")?"audio/webm;codecs=opus":"audio/webm";n.mediaRecorder=new MediaRecorder(t,e?{mimeType:e}:void 0),n.mediaRecorder.ondataavailable=a=>{a.data.size>0&&n.mediaChunks.push(a.data)},n.mediaRecorder.onstop=()=>{const a=new Blob(n.mediaChunks,{type:n.mediaRecorder.mimeType||"audio/webm"});t.getTracks().forEach(s=>s.stop());const i=new FileReader;i.onloadend=()=>R({type:"audio",dataUrl:i.result,duration:n.recordSeconds}),i.readAsDataURL(a)},n.recording=!0,S(!0);const o=document.getElementById("kco-ai-rec-timer");n.recordTimer=setInterval(()=>{if(n.recordSeconds+=1,n.recordSeconds>=60){f();return}o&&(o.textContent=x(n.recordSeconds))},1e3)}catch{h("Microphone access was denied.")}}function f(){if(!(!n.recording||!n.mediaRecorder)){try{n.mediaRecorder.stop()}catch{}clearInterval(n.recordTimer),n.recordTimer=null,n.recording=!1,S(!1)}}function ee(){if(clearInterval(n.recordTimer),n.recordTimer=null,n.mediaChunks=[],n.mediaRecorder&&n.mediaRecorder.state!=="inactive"){try{n.mediaRecorder.onstop=null,n.mediaRecorder.stop()}catch{}n.mediaRecorder.stream&&n.mediaRecorder.stream.getTracks().forEach(t=>t.stop())}n.recording=!1,n.mediaRecorder=null,S(!1)}function te(){const t=document.getElementById("kco-ai-messages");t&&(t.innerHTML=""),n.history=[],n.attachments=[],b(),f(),M(),ne("Conversation cleared")}function ne(t){const e=document.createElement("div");e.className="fixed top-16 left-1/2 -translate-x-1/2 z-[70] bg-gray-900 border border-blue-500/30 text-white text-xs font-medium px-4 py-2 rounded-full shadow-xl kco-ai-fade",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),2e3)}function h(t){const e=document.getElementById("kco-ai-input");e&&(e.placeholder=t,setTimeout(()=>{e.placeholder===t&&(e.placeholder="Type a message...")},2600))}function oe(t){return(t||[]).map(e=>e.type==="image"?`<img src="${e.dataUrl}" class="mt-2 w-full max-h-56 rounded-xl object-cover border border-white/10" alt="Shared image">`:e.type==="audio"?`<div class="mt-2 flex items-center gap-2">
        ...
        <span class="text-[10px] text-rose-200/80 font-mono">${x(e.duration)}</span>
      </div>`:"").join("")}function u(t,e=!0){const o=document.getElementById("kco-ai-messages");if(!o)return;const a=t.role==="user",i=document.createElement("div");i.className=`flex ${a?"justify-end":"justify-start"} ${e?"kco-ai-msg-in":""}`,i.innerHTML=a?`
    <div class="max-w-[80%] text-white rounded-2xl rounded-tr-sm px-3.5 py-2.5 shadow-lg" style="background:linear-gradient(135deg,#3b82f6 0%,#2563eb 100%);box-shadow:0 2px 8px rgba(59,130,246,0.25)">
      <p class="text-sm leading-relaxed whitespace-pre-wrap break-words">${y(t.content)}</p>
      ${oe(t.attachments)}
    </div>
  `:`
  ...
  `,o.appendChild(i),window.lucide&&lucide.createIcons(),O(),a||k(t.content)}function M(){const t=g(),e={en:`Welcome to Weverse Online Shop!
...
Naweza kukusaidia vipi leo?`};u({role:"assistant",content:e[t]||e.en})}function ae(){const t=document.getElementById("kco-ai-messages");if(!t)return;const e=document.createElement("div");e.id="kco-ai-typing",e.className="flex justify-start kco-ai-fade",e.innerHTML=`
    ...
    </div>`,t.appendChild(e),window.lucide&&lucide.createIcons(),O()}function O(){const t=document.getElementById("kco-ai-messages");t&&(t.scrollTop=t.scrollHeight)}async function ie(){const{data:t}=await A.auth.getSession();return{Authorization:`Bearer ${t.session?.access_token||H}`,"Content-Type":"application/json"}}async function p(){const t=document.getElementById("kco-ai-input");if(!t)return;const e=t.value.trim(),o=n.attachments.slice();if(!e&&o.length===0||n.sending)return;t.value="",t.style.height="auto",n.attachments=[],b(),n.sending=!0,document.getElementById("kco-ai-send").disabled=!0;const a=o.some(l=>l.type==="image"),i=o.some(l=>l.type==="audio"),s=[a&&"[Image attached]",i&&"[Voice note attached]"].filter(Boolean).join(" "),m=s?`${e?e+" ":""}${s}`.trim():e,r={role:"user",content:m,attachments:o};n.history.push(r),u(r),ae();try{const l=await ie(),I=await(await fetch(C,{method:"POST",headers:l,body:JSON.stringify({action:"chat",message:m,history:n.history.slice(-20,-1).map(d=>({role:d.role,content:d.content})),language:g(),page_context:T(),cart_items:P(),attachments:o.map(d=>({type:d.type,dataUrl:d.dataUrl,duration:d.duration||0}))})})).json();document.getElementById("kco-ai-typing")?.remove();const E={role:"assistant",content:I.response||I.error||"I'm sorry, I didn't catch that. Could you rephrase?"};n.history.push(E),u(E)}catch{document.getElementById("kco-ai-typing")?.remove();const v={role:"assistant",content:"I'm having trouble connecting right now. Please try again in a moment."};n.history.push(v),u(v)}finally{n.sending=!1,document.getElementById("kco-ai-send").disabled=!1,document.getElementById("kco-ai-input")?.focus()}}function _(t,e){if(n.proactiveShown[t]||(n.proactiveShown[t]=!0,n.open)||!document.getElementById("kco-ai-fab"))return;const a=document.createElement("div");a.id=`kco-ai-nudge-${t}`,a.className="fixed bottom-[92px] right-5 z-[59] max-w-[280px] bg-slate-800 border border-blue-500/30 rounded-xl shadow-2xl px-3.5 py-2.5 kco-ai-fade",a.innerHTML=`
    <div class="flex items-start gap-2">
      <i data-lucide="message-circle" class="w-4 h-4 text-blue-400 shrink-0 mt-0.5"></i>
      <p class="text-xs text-gray-300 leading-snug flex-1">${y(e)}</p>
    </div>
    <div class="flex gap-2 mt-2">
      <button class="text-[10px] font-semibold text-blue-400 hover:text-blue-300" onclick="document.getElementById('kco-ai-nudge-${t}').remove();window.__kcoAiOpen()">Ask Support</button>
      <button class="text-[10px] text-gray-500 hover:text-gray-400" onclick="document.getElementById('kco-ai-nudge-${t}').remove()">Dismiss</button>
    </div>`,document.body.appendChild(a),window.lucide&&lucide.createIcons(),setTimeout(()=>a?.remove(),1e4)}window.__kcoAiOpen=()=>c(!0);window.__kcoAiSend=t=>{if(typeof t!="string"||!t.trim())return;c(!0);const e=document.getElementById("kco-ai-input");e&&(e.value=t.trim(),e.style.height="auto",e.style.height=e.scrollHeight+"px"),setTimeout(()=>p(),150)};function se(){const t=T(),e={checkout:"I can help you complete your purchase. Need guidance with checkout?",payment:"Before you pay, I can explain your payment options. Just ask!","sign-in":"Welcome back! Need help signing in or recovering your password?",account:"I can help you set up your account. Just ask me anything!","product-details":"Have a question about this product? I'm here to help!"};e[t]&&setTimeout(()=>_(t,e[t]),4e3)}async function j(){if(!N){J();try{const{data:t}=await A.auth.getSession();if(t?.session?.access_token){const o=await(await fetch(C,{method:"POST",headers:{Authorization:`Bearer ${t.session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({action:"get_history"})})).json();if(o.history&&o.history.length>0){n.history=o.history.map(i=>({role:i.role,content:i.content}));const a=n.history.slice(-6);for(const i of a)u(i,!1)}}}catch{}window.addEventListener("kco-language-changed",()=>{if(n.open){const t=g(),e={en:"Language changed. I'll respond in your selected language from now on.",es:"Idioma cambiado. Responderé en tu idioma seleccionado.",fr:"Langue changée. Je répondrai dans votre langue sélectionnée.",de:"Sprache geändert. Ich werde in Ihrer gewählten Sprache antworten.",ar:"تم تغيير اللغة. سأرد بلغتك المختارة من الآن.",zh:"语言已更改。我将使用您选择的语言回复。",ja:"言語が変更されました。選択した言語で応答します。",hi:"भाषा बदल दी गई है। मैं अब से आपकी चुनी हुई भाषा में जवाब दूंगा।",ru:"Язык изменен. Я буду отвечать на выбранном вами языке."},o={role:"assistant",content:e[t]||e.en};u(o)}}),se()}}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",j):j();window.kcoCustomerAI={nudge:_,open:()=>c(!0),close:()=>c(!1),send:t=>{const e=document.getElementById("kco-ai-input");e&&(e.value=t,p())}};
