import{supabase as j}from"./supabase-client-7_ZWSEp6.js";import{b as L,c as _,e as q}from"./localization-Bv8KGKLQ.js";const U="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),C=`${U}/functions/v1/ai-customer-assistant`,H="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",N=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let n={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null,attachments:[],recording:!1,mediaRecorder:null,mediaChunks:[],recordTimer:null,recordSeconds:0};function y(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function V(t){let e=y(t);return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^\- (.+)$/gm,"<li>$1</li>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,o=>`<ul>${o}</ul>`),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=`<p>${e}</p>`,e=e.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),e}function g(){return L()||localStorage.getItem("kco_language")||"en"}function T(){const t=window.location.pathname;return t.includes("checkout")?"checkout":t.includes("payment")?"payment":t.includes("auth")?"sign-in":t.includes("account")?"account":t.includes("details")?"product-details":t.includes("contact")?"contact":t.includes("help")?"help":t.includes("about")?"about":"home"}function P(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(e=>({title:e.title||e.name,price:e.price,quantity:e.quantity||1}))}catch{return[]}}function k(t){if(n.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t.replace(/[*#`]/g,"")),o=g(),a=_(o);o==="en"?e.lang=n.voiceAccent==="UK"?"en-GB":"en-US":e.lang=a.lang,e.rate=a.rate,e.pitch=a.pitch;const i=q(o==="en"?(n.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):o);i&&(e.voice=i),window.speechSynthesis.speak(e)}catch{}}function z(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function F(){if(document.getElementById("kco-ai-styles"))return;const t=document.createElement("style");t.id="kco-ai-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)}const w='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" fill="#60a5fa"/><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" stroke="#3b82f6" stroke-width="0.5"/></svg>',D='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.4 20.4l17.45-7.48a1 1 0 0 0 0-1.84L3.4 3.6a1 1 0 0 0-1.39 1.21L4 12l-1.99 7.19a1 1 0 0 0 1.39 1.21z" fill="white"/></svg>',B='<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 inline-block align-middle" aria-label="Verified" role="img"><circle cx="12" cy="12" r="11" fill="#1877F2"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>';function J(){F();const t=document.createElement("button");t.id="kco-ai-fab",t.setAttribute("aria-label","Chat with Weverse support"),t.className="fixed bottom-5 right-5 z-[60] w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-transform hover:scale-105 active:scale-95",t.style.background="linear-gradient(135deg,#3b82f6 0%,#2563eb 100%)",t.style.border="1px solid rgba(255,255,255,0.18)",t.style.boxShadow="0 8px 24px rgba(59,130,246,0.4)",t.innerHTML=`
  ...
  `,t.onclick=()=>d(!0),document.body.appendChild(t);const e=document.createElement("div");e.id="kco-ai-panel",e.className="hidden-panel fixed z-[60] flex flex-col overflow-hidden w-full sm:w-[400px] h-[92dvh] sm:h-[640px] sm:max-h-[calc(100dvh-120px)] bottom-0 sm:bottom-[90px] right-0 sm:right-5 rounded-t-2xl sm:rounded-2xl bg-slate-900/97 border-t sm:border border-blue-500/25 shadow-2xl",e.style.backdropFilter="blur(20px)",e.innerHTML=`
    ...
    <div class="flex items-center justify-between px-3.5 py-3 border-b border-white/10 shrink-0" style="background:linear-gradient(135deg,#1e1e2e 0%,#1a1a2e 50%,#16213e 100%)">
      <div class="flex items-center gap-2.5 min-w-0">
        ...
        <div class="min-w-0">
          <p class="text-sm font-bold text-white leading-tight tracking-wide flex items-center gap-1 truncate">Weverse Support ${B}</p>
...
Naweza kukusaidia vipi leo?`};u({role:"assistant",content:e[t]||e.en})}function ae(){const t=document.getElementById("kco-ai-messages");if(!t)return;const e=document.createElement("div");e.id="kco-ai-typing",e.className="flex justify-start kco-ai-fade",e.innerHTML=`
    ...
    </div>`,t.appendChild(e),window.lucide&&lucide.createIcons(),O()}function O(){const t=document.getElementById("kco-ai-messages");t&&(t.scrollTop=t.scrollHeight)}async function ie(){const{data:t}=await j.auth.getSession();return{Authorization:`Bearer ${t.session?.access_token||H}`,"Content-Type":"application/json"}}async function p(){const t=document.getElementById("kco-ai-input");if(!t)return;const e=t.value.trim(),o=n.attachments.slice();if(!e&&o.length===0||n.sending)return;t.value="",t.style.height="auto",n.attachments=[],b(),n.sending=!0,document.getElementById("kco-ai-send").disabled=!0;const a=o.some(c=>c.type==="image"),i=o.some(c=>c.type==="audio"),s=[a&&"[Image attached]",i&&"[Voice note attached]"].filter(Boolean).join(" "),m=s?`${e?e+" ":""}${s}`.trim():e,r={role:"user",content:m,attachments:o};n.history.push(r),u(r),ae();try{const c=await ie(),S=await(await fetch(C,{method:"POST",headers:c,body:JSON.stringify({action:"chat",message:m,history:n.history.slice(-20,-1).map(l=>({role:l.role,content:l.content})),language:g(),page_context:T(),cart_items:P(),attachments:o.map(l=>({type:l.type,dataUrl:l.dataUrl,duration:l.duration||0}))})})).json();document.getElementById("kco-ai-typing")?.remove();const E={role:"assistant",content:S.response||S.error||"I'm sorry, I didn't catch that. Could you rephrase?"};n.history.push(E),u(E)}catch{document.getElementById("kco-ai-typing")?.remove();const v={role:"assistant",content:"I'm having trouble connecting right now. Please try again in a moment."};n.history.push(v),u(v)}finally{n.sending=!1,document.getElementById("kco-ai-send").disabled=!1,document.getElementById("kco-ai-input")?.focus()}}function W(t,e){if(n.proactiveShown[t]||(n.proactiveShown[t]=!0,n.open)||!document.getElementById("kco-ai-fab"))return;const a=document.createElement("div");a.id=`kco-ai-nudge-${t}`,a.className="fixed bottom-[92px] right-5 z-[59] max-w-[280px] bg-slate-800 border border-blue-500/30 rounded-xl shadow-2xl px-3.5 py-2.5 kco-ai-fade",a.innerHTML=`
