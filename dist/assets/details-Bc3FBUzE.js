import{supabase as v}from"./supabase-client-7_ZWSEp6.js";import"./localization-YU_l848j.js";import{S as R,l as q,f as A,a as T,b as H}from"./showroom-data-DQuiOO-D.js";import{g as F,f as P,T as z}from"./truck-data-CXeZJZHb.js";import{g as $,f as I}from"./auth-D3ZJ7kZA.js";import"./live-stream-mode-D6m6hLi6.js";import"./customer-ai-widget-C255UST7.js";import"./brand-BE-xoT4U.js";import"./preload-helper-CLcXU_4U.js";const y="/fallback.svg";function M(e){return typeof e=="number"&&!isNaN(e)?e.toFixed(1):"0.0"}function j(e){return Array.isArray(e)&&e.length>0?e:[y]}function D(){return new URLSearchParams(window.location.search).get("id")}function U(e){const t=document.getElementById("details-content"),r=P(e),s=j(e.images).map((d,g)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${g===0?"active border-orange-500":"border-gray-800"} shrink-0" data-img="${i(d)}">
      <img src="${i(d)}" alt="View ${g+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${y}'">
    ...
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      ...
      <div class="flex flex-wrap gap-2">
        ${e.features.map(d=>`<span class="text-xs bg-gray-800 text-gray-300 px-3 py-1.5 rounded-full border border-gray-700">${i(d)}</span>`).join("")}
      </div>
    </div>`:"",p=`
    <div class="flex items-center gap-4 mb-6">
      <div class="flex items-center gap-1.5">
        <i data-lucide="star" class="w-5 h-5 fill-orange-500 text-orange-500"></i>
        <span class="text-lg font-bold text-white">${M(e.rating)}</span>
    ...
    <div class="fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-black text-white leading-tight">${i(e.title)}</h1>
          <p class="text-gray-500 text-sm mt-1">Stock #: <span class="text-orange-500 font-mono font-bold">${i(e.stock_number||"—")}</span> &middot; VIN: <span class="text-gray-400 font-mono">${i(e.vin||"—")}</span></p>
        </div>
        <div class="text-right shrink-0">
          <div class="text-3xl font-black text-orange-500">${r}</div>
          <span class="inline-block bg-gray-800 text-gray-300 text-xs px-2 py-1 rounded-full mt-1">${i(e.condition||"Used")} &middot; For Sale</span>
        </div>
      </div>
      ...
      <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
        ...
        <p class="text-gray-400 text-sm leading-relaxed">${i(e.description||"")}</p>
      </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ${l.map(d=>`
            <div class="flex flex-col gap-1">
              ...
              <div class="text-gray-200 font-medium text-sm">${i(d.value)}</div>
            </div>
          `).join("")}
        </div>
      </div>
  ...
  `;const b=document.getElementById("hero-image"),k=document.getElementById("gallery-label");t.querySelectorAll(".gallery-thumb").forEach((d,g)=>{d.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(f=>f.classList.remove("active","border-orange-500")),t.querySelectorAll(".gallery-thumb").forEach(f=>f.classList.add("border-gray-800")),d.classList.add("active","border-orange-500"),d.classList.remove("border-gray-800"),b.src=d.dataset.img,k.textContent=o[g]||`View ${g+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await $()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const d=window.location.href;try{if(navigator.share)await navigator.share({title:i(e.title),url:d});else{await navigator.clipboard.writeText(d);const g=document.getElementById("share-btn"),f=g.innerHTML;g.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{g.innerHTML=f,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),O(e),window.lucide&&lucide.createIcons()}function V(e){const t=e.listing_type==="property",r=`/contact.html?listing=${encodeURIComponent(e.property_id||"")}`;return`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5">
      ...
      <div class="aspect-square overflow-hidden bg-gray-800"><img src="${i(t)}" alt="" class="w-full h-full object-cover group-hover:scale-105 transition" loading="lazy" onerror="this.src='/fallback.svg'"></div>
      <div class="p-2"><p class="text-xs text-white font-bold truncate">${i(e.title)}</p><p class="text-xs text-orange-500 font-bold mt-1">${T(e)}</p></div>
    </a>`}function _(e,t){const r=document.getElementById(e);if(!r)return;const c=r.querySelector(".rel-grid");if(c){if(!t.length){r.classList.add("hidden");return}r.classList.remove("hidden"),c.innerHTML=t.slice(0,4).map(N).join("")}}function O(e){const t=z.filter(o=>o.property_id!==e.property_id),r=t.filter(o=>o.category===e.category),c=t.filter(o=>o.brand===e.brand),s=[...t].sort((o,l)=>(l.rating||0)-(o.rating||0));_("similar-section",r.length?r:s.slice(0,4)),_("related-section",c.length?c:s.slice(0,4)),_("recommended-section",s)}function W(e){const t=document.getElementById("details-content"),r=e.listing_type==="property",c=T(e),s=H(e.country_code),o=e.listing_type==="product"?"Product ID":r?"Property ID":"Listing ID",u=j(e.images).map((n,a)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${a===0?"active border-orange-500":"border-gray-800"} shrink-0" data-img="${i(n)}">
      <img src="${i(n)}" alt="View ${a+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${y}'">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ${[{icon:"bed-double",label:"Bedrooms",value:e.bedrooms},{icon:"bath",label:"Bathrooms",value:e.bathrooms},{icon:"building",label:"Building Size",value:e.building_size},{icon:"ruler",label:"Land Size",value:e.land_size},{icon:"car",label:"Parking Spaces",value:e.parking_spaces},{icon:"home",label:"Property Type",value:e.property_type},{icon:"sofa",label:"Furnished",value:e.furnished},{icon:"calendar",label:"Year Built",value:e.year_built},{icon:"tag",label:"Status",value:e.listing_status==="rent"?"For Rent":"For Sale"}].filter(a=>a.value!=null&&a.value!=="").map(a=>`
            <div class="flex flex-col gap-1">
              ...
              <div class="text-gray-200 font-medium text-sm">${i(a.value)}</div>
            </div>
          `).join("")}
        </div>
    ...
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      ...
      <div class="flex flex-wrap gap-2">
        ${e.features.map(n=>`<span class="text-xs bg-gray-800 text-gray-300 px-3 py-1.5 rounded-full border border-gray-700">${i(n)}</span>`).join("")}
      </div>
    </div>`:"",d=e.highlights?.length?`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-4">Highlights</h3>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ${e.highlights.map(n=>`<div class="flex items-start gap-2 text-sm text-gray-300"><i data-lucide="sparkles" class="w-4 h-4 text-orange-500 mt-0.5"></i><span>${i(n)}</span></div>`).join("")}
      </div>
    </div>`:"",g=`
    <div class="flex items-center gap-4 mb-6">
      <div class="flex items-center gap-1.5">
    ...
    <div class="fade-in">
      <div class="flex items-center gap-2 text-xs text-gray-500 mb-4">
        ...
        <i data-lucide="chevron-right" class="w-3 h-3"></i>
        <span class="text-gray-300 truncate">${i(e.title)}</span>
      </div>

      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-black text-white leading-tight">${i(e.title)}</h1>
          <p class="text-gray-500 text-sm mt-1">${o}: <span class="text-orange-500 font-mono font-bold">${i(e.property_id)}</span></p>
        </div>
        <div class="text-right shrink-0">
  ...
  `;const f=document.getElementById("hero-image");t.querySelectorAll(".gallery-thumb").forEach(n=>{n.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(a=>a.classList.remove("active","border-orange-500")),t.querySelectorAll(".gallery-thumb").forEach(a=>a.classList.add("border-gray-800")),n.classList.add("active","border-orange-500"),n.classList.remove("border-gray-800"),f.src=n.dataset.img})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await $()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const n=window.location.href;try{if(navigator.share)await navigator.share({title:i(e.title),url:n});else{await navigator.clipboard.writeText(n);const a=document.getElementById("share-btn"),w=a.innerHTML;a.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{a.innerHTML=w,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),G(e),K(e),C(e),Y(e),window.lucide&&lucide.createIcons(),e.property_id,e.title,parseFloat(e.price),e.currency;const h=document.getElementById("listing-map");if(h&&window.L){const n=parseFloat(e.latitude)||null,a=parseFloat(e.longitude)||null,w=[e.town,e.city,e.state,e.country].filter(Boolean).join(", ");if(n&&a){const m=L.map(h).setView([n,a],13);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(m),L.marker([n,a]).addTo(m).bindPopup(e.title)}else w?fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q="+encodeURIComponent(w)).then(m=>m.json()).then(m=>{if(m&&m[0]){const S=parseFloat(m[0].lat),E=parseFloat(m[0].lon),B=L.map(h).setView([S,E],12);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(B),L.marker([S,E]).addTo(B).bindPopup(e.title)}else h.style.display="none"}).catch(()=>{h.style.display="none"}):h.style.display="none"}}let x=0;async function G(e){const t=document.getElementById("wishlist-btn");if(!t)return;const r=await $();if(!r){t.addEventListener("click",()=>{I(window.location.pathname+window.location.search),window.location.href=`/auth.html?redirect=${encodeURIComponent(window.location.pathname+window.location.search)}`});return}const{data:c,error:s}=await v.from("wishlist").select("id").eq("user_id",r.id).eq("listing_id",e.id).maybeSingle();if(s){console.error("Wishlist check failed:",s.message);return}c&&(t.innerHTML='<i data-lucide="heart" class="w-5 h-5 fill-red-500 text-red-500"></i>',t.classList.add("bg-red-500/10","border","border-red-500/20"),window.lucide&&lucide.createIcons()),t.addEventListener("click",async()=>{const{data:o,error:l}=await v.from("wishlist").select("id").eq("user_id",r.id).eq("listing_id",e.id).maybeSingle();if(l){console.error("Wishlist toggle failed:",l.message);return}if(o){const{error:u}=await v.from("wishlist").delete().eq("id",o.id);if(u){console.error("Wishlist delete failed:",u.message);return}t.innerHTML='<i data-lucide="heart" class="w-5 h-5"></i>',t.classList.remove("bg-red-500/10","border","border-red-500/20")}else{const{error:u}=await v.from("wishlist").insert({user_id:r.id,listing_id:e.id});if(u){console.error("Wishlist insert failed:",u.message);return}t.innerHTML='<i data-lucide="heart" class="w-5 h-5 fill-red-500 text-red-500"></i>',t.classList.add("bg-red-500/10","border","border-red-500/20")}window.lucide&&lucide.createIcons()})}async function K(e){const t=document.getElementById("review-form"),r=document.getElementById("review-login-msg"),c=await $();if(!c){t.classList.add("hidden"),r.classList.remove("hidden");return}document.querySelectorAll(".star-btn").forEach(s=>{s.addEventListener("click",()=>{x=parseInt(s.dataset.rating,10),document.querySelectorAll(".star-btn").forEach((o,l)=>{const u=o.querySelector("i");l<x?(u.classList.add("fill-orange-500","text-orange-500"),u.classList.remove("text-gray-600")):(u.classList.remove("fill-orange-500","text-orange-500"),u.classList.add("text-gray-600"))})})}),t.addEventListener("submit",async s=>{s.preventDefault();const o=document.getElementById("review-text").value.trim();if(!x){alert("Please select a rating.");return}if(!o){alert("Please write a review.");return}const{error:l}=await v.from("product_reviews").insert({listing_id:e.id,user_id:c.id,rating:x,review_text:o,is_approved:!1});if(l){alert("Error: "+l.message);return}document.getElementById("review-text").value="",x=0,document.querySelectorAll(".star-btn").forEach(u=>{const p=u.querySelector("i");p.classList.remove("fill-orange-500","text-orange-500"),p.classList.add("text-gray-600")}),alert("Review submitted! It will appear after admin approval."),C(e)})}async function C(e){const t=document.getElementById("reviews-list");if(!t||!e.id)return;const{data:r,error:c}=await v.from("product_reviews").select("*, profiles(full_name)").eq("listing_id",e.id).eq("is_approved",!0).order("created_at",{ascending:!1});if(c){t.innerHTML='<p class="text-gray-500 text-sm">Unable to load reviews.</p>';return}if(!r||r.length===0){t.innerHTML='<p class="text-gray-500 text-sm">No reviews yet. Be the first to review this product!</p>';return}t.innerHTML=r.map(s=>`
    <div class="border-b border-gray-800 pb-3 mb-3 last:border-0">
      <div class="flex items-center gap-2 mb-1">
        <div class="flex gap-0.5">${[1,2,3,4,5].map(o=>`<i data-lucide="star" class="w-3.5 h-3.5 ${o<=s.rating?"fill-orange-500 text-orange-500":"text-gray-600"}"></i>`).join("")}</div>
        <span class="text-xs text-gray-400 font-bold">${i(s.profiles?.full_name||"Anonymous")}</span>
      ...
      <p class="text-sm text-gray-300">${i(s.review_text||"")}</p>
      ${s.vendor_response?`<div class="mt-2 bg-gray-800/50 rounded-lg p-2 text-xs text-gray-400"><strong class="text-gray-300">Seller response:</strong> ${i(s.vendor_response)}</div>`:""}
    </div>`).join(""),window.lucide&&lucide.createIcons()}async function Y(e){const t=document.getElementById("recommendations-section"),r=document.getElementById("recommendations-grid");if(!t||!r||!e.id)return;const{data:c,error:s}=await v.from("product_recommendations").select("recommended_listing_id, showroom_listings!product_recommendations_recommended_listing_id_fkey(property_id, title, price, currency, images, listing_type)").eq("listing_id",e.id).order("score",{ascending:!1}).limit(4);if(s){console.error("Recommendations load failed:",s.message),t.classList.add("hidden");return}let o=(c||[]).map(l=>l.showroom_listings).filter(Boolean);if(o.length<4){const{data:l}=await v.from("showroom_listings").select("property_id, title, price, currency, images, listing_type").eq("category",e.category).neq("id",e.id).eq("is_active",!0).order("rating",{ascending:!1}).limit(4-o.length);o=[...o,...l||[]]}if(o.length===0){t.classList.add("hidden");return}t.classList.remove("hidden"),r.innerHTML=o.map(l=>{const u=l.images&&l.images[0]||"/fallback.svg",p=typeof l.price=="number"?l.price:parseFloat(l.price||0),b=l.currency||"USD";return`<a href="/details.html?id=${l.property_id}" class="block bg-gray-900/60 border border-gray-800 rounded-xl overflow-hidden hover:border-orange-500/30 transition group">
      <div class="aspect-square overflow-hidden bg-gray-800"><img src="${i(u)}" alt="" class="w-full h-full object-cover group-hover:scale-105 transition" loading="lazy" onerror="this.src='/fallback.svg'"></div>
      <div class="p-2"><p class="text-xs text-white font-bold truncate">${i(l.title)}</p><p class="text-xs text-orange-500 font-bold mt-1">${b} ${p.toLocaleString()}</p></div>
    </a>`}).join("")}function i(e){return String(e||"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function J(){const e=D();if(!e){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}const t=F(e);if(t){document.title=`${t.title} | KCO Global Online Marketplace`,U(t);return}let r=R.find(c=>c.property_id===e);if(r||(await q(),r=A(e)),!r){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}document.title=`${r.title} | KCO Global Online Marketplace`,W(r)}J();
