const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["assets/catalog-pfF1HJ-x.js","assets/catalog-hidden-store-Ci-G2dxV.js","assets/showroom-data-C8s3txE5.js"])))=>i.map(i=>d[i]);
import"./modulepreload-polyfill-B5Qt9EMX.js";import{l as z,f as A,_ as x,a as V,g as Y,v as J}from"./showroom-data-C8s3txE5.js";import{supabase as v}from"./supabase-client-DSkAnHxH.js";import{getCurrentUser as Q}from"./auth-CtTDfG3h.js";import{P as X,a as W,g as P,b as T}from"./motorhome-data-CupbOvk0.js";import{g as B,a as E}from"./phone-data-Of7KtnOV.js";import{d as j,g as K,C as Z}from"./country-data-BXM8T1RS.js";import{i as y,b as S,c as ee,d as te,a as re,r as ae,e as se}from"./payment-settings-BZM-gsG8.js";import{f as ie,c as f,p as ne,a as _}from"./fx-B0aevBpC.js";/* empty css                                       */const le=[...X,...W];function M(t){return le.find(r=>r.property_id===t)||null}function q(t){const r=J(t);return r?`<video src="${String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}" class="w-full h-full object-cover" muted playsinline preload="metadata"></video>`:'<div class="w-full h-full flex items-center justify-center bg-gray-50"><i data-lucide="video-off" class="w-5 h-5 text-gray-300"></i></div>'}let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",supportEmail:"",autoDetectedCurrency:"",currencyManuallySelected:!1};function oe(){const t=Date.now().toString(36).toUpperCase().slice(-6),r=Math.random().toString(36).toUpperCase().slice(2,6);return`W-${t}${r}`}function g(t){const r=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,r.classList.remove("translate-y-20","opacity-0"),clearTimeout(r._t),r._t=setTimeout(()=>r.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function L(t){t.addEventListener("click",function(r){if(this.disabled)return;const a=this.getBoundingClientRect(),i=document.createElement("span");i.className="ripple";const s=Math.max(a.width,a.height);i.style.width=i.style.height=s+"px",i.style.left=r.clientX-a.left-s/2+"px",i.style.top=r.clientY-a.top-s/2+"px",this.appendChild(i),setTimeout(()=>i.remove(),600)})}function $(t,r){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${r}`}function de(t){const r=$(t.price,t.currency||"USD"),a=parseFloat(t.real_price);return Number.isFinite(a)&&a>0&&a>parseFloat(t.price)?`<span class="price-strike line-through text-gray-400 mr-1 text-xs">${$(a,t.currency||"USD")}</span><span class="text-amber-600 font-bold">${r}</span>`:r}function ce(t){const r=()=>{const a=document.createElement("textarea");a.value=t,document.body.appendChild(a),a.select();try{document.execCommand("copy")}catch{}document.body.removeChild(a)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>r()):r(),g("Copied to clipboard.")}function ue(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,r=ae(e.manualPaymentAccounts,e.countryCode,t);return{...r,fallbackNotice:r.isFallback?se(r.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function pe(){const t=document.getElementById("particles");if(t)for(let r=0;r<12;r++){const a=document.createElement("div"),i=Math.random()*3+1;a.className="particle",a.style.width=a.style.height=i+"px",a.style.left=Math.random()*100+"%",a.style.bottom="-10px",a.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",a.style.animationDuration=Math.random()*20+15+"s",a.style.animationDelay=Math.random()*20+"s",t.appendChild(a)}}pe();async function be(){const t=document.getElementById("checkout-root"),r=new URLSearchParams(window.location.search),a=r.get("status"),i=r.get("transaction_id"),s=r.get("tx_ref"),l=r.get("order_number");if(a==="verify"&&i){await $e(i,s,l||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=r.get("guest")==="1",!e.isGuest&&(e.user=await Q(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const n=r.get("id");if(n){let o=await z(n);if(e.listing=o||A(n)||P(n)||T(n)||B(n)||E(n)||M(n),!e.listing){const[{generateListingById:d},{loadHiddenCatalogIds:c}]=await Promise.all([x(()=>import("./catalog-pfF1HJ-x.js"),__vite__mapDeps([0,1,2])),x(()=>import("./catalog-hidden-store-Ci-G2dxV.js"),__vite__mapDeps([1,2]))]);await c(),e.listing=d(n)}if(e.listing||(e.listing=A(n)),!e.listing){t.innerHTML=`
        ...
        </div>`,window.lucide&&lucide.createIcons();return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await V();const d=Y(),[{generateListingById:c},{loadHiddenCatalogIds:k}]=await Promise.all([x(()=>import("./catalog-pfF1HJ-x.js"),__vite__mapDeps([0,1,2])),x(()=>import("./catalog-hidden-store-Ci-G2dxV.js"),__vite__mapDeps([1,2]))]);if(await k(),e.cartItems=o.map(p=>{const b=typeof p=="string"?p:p&&p.id;if(!b)return null;const G=p&&typeof p=="object"&&p.qty?Math.max(1,parseInt(p.qty,10)||1):1,I=d.find(H=>H.property_id===b)||P(b)||T(b)||B(b)||E(b)||M(b)||c(b);return I?{listing:I,quantity:G}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=F(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await v.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=j(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.shippingCountry=e.countryCode,e.paymentSettings=await ee(),e.manualPaymentAccounts=await te(),e.manualPaymentInstructions=re(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.supportEmail=e.paymentSettings.contact_email||e.paymentSettings.brand_email||"",(e.paymentGateway==="manual"||y(u()))&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&!y(u())&&(e.paymentMethod="flutterwave"),S(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),await ne(),e.user&&!e.isGuest){const{data:o}=await v.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const d=e.addresses.find(k=>k.is_default);d&&(e.selectedAddressId=d.id);const{data:c}=await v.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();c&&(e.fullName=c.display_name||`${c.first_name||""} ${c.last_name||""}`.trim(),e.email=e.user.email,e.phone=c.phone_code&&c.phone_number?`+${c.phone_code} ${c.phone_number}`:"",e.shippingCountry=c.country_code||e.countryCode)}e.orderNumber=oe(),m()}function h(){return e.cartItems.reduce((t,r)=>t+r.listing.price*r.quantity,0)}function me(){return 0}function ge(t){if(!t)return!1;const r=String(t.listing_type||t.type||"").toLowerCase();if(["property","land","house","vehicle","car","truck","motorhome","fitting"].includes(r))return!0;const a=String(t.category||t.subcategory||t.title||"").toLowerCase();return/real estate|house|land|villa|apartment|penthouse|duplex|compound|farm|warehouse|plaza|shop|cinema|truck|car\b|vehicle|motorhome|inverter|generator/.test(a)}function ye(){return 0}function C(){return Math.round(h()*ye()*100)/100}function u(){return h()+me()+C()}function O(){return f(u(),e.selectedCurrency)}function w(){return _(O(),e.selectedCurrency)}function D(){const t=(e.shippingCountry||"").toUpperCase();if(t&&(e.countryCode=t,localStorage.setItem("kco_country",t),!e.currencyManuallySelected)){e.autoDetectedCurrency=j(t)||"";const r=S(e.manualPaymentAccounts);e.selectedCurrency=e.autoDetectedCurrency&&r.includes(e.autoDetectedCurrency)?e.autoDetectedCurrency:"USD"}}function m(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(L)}function fe(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
        ...
        ${e.cartItems.map((t,r)=>`
            <div class="flex items-center gap-3 p-3 bg-gray-50 border border-blue-100 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-600 mt-1">${de(t.listing)}</p>
              </div>
  ...
  `}function xe(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      ...
      <div class="space-y-2">
        ${e.addresses.map(a=>{const i=K(a.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
              ${Z.map(a=>`<option value="${a.code}" ${e.shippingCountry===a.code?"selected":""}>${a.flag} ${a.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function ve(){const t=S(e.manualPaymentAccounts),r=ue(),a=y(u()),i=e.manualPaymentAccounts.length===0,s=(e.paymentGateway==="both"||e.paymentGateway==="flutterwave")&&!a,l=(e.paymentGateway==="both"||e.paymentGateway==="manual")&&!i,n=e.paymentMethod==="manual_bank_transfer"?!!(r.account&&e.paymentSettings):s;return`
    ...
    ${e.paymentMethod==="manual_bank_transfer"&&r.account?we(r.account,r.fallbackNotice,e.manualPaymentInstructions,{}):""}
    ...
    <div class="space-y-3">
      ${e.paymentMethod==="flutterwave"?`
        <button onclick="payWithFlutterwave()" id="flw-pay-btn" class="btn-press w-full bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition shadow-lg shadow-blue-600/30 flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="credit-card" class="w-5 h-5"></i> Pay ${w()} with Card / ATM
        </button>
      `:`
        <button onclick="placeOrderManual()" id="manual-pay-btn" ${n?"":"disabled"} class="btn-press w-full ${n?"bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 shadow-lg shadow-blue-600/30":"bg-gray-300 text-gray-500 cursor-not-allowed"} text-white font-bold py-4 rounded-xl text-sm uppercase tracking-wide transition flex items-center justify-center gap-2 relative overflow-hidden">
          <i data-lucide="landmark" class="w-5 h-5"></i> ${a?`Pay ${w()} by Bank Transfer`:"Place Order & Upload Receipt"}
        </button>
      `}
...
`);return`
    <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
      ...
      <div class="bg-blue-600 text-white rounded-2xl p-4 mb-4 overflow-hidden relative">
        ...
        <div class="relative">
          ...
          <div class="text-2xl font-bold mt-0.5">${w()}</div>
  ...
  `}function ke(){const t=h();return u(),`
      ...
      <div class="space-y-3 mb-4">
        ${e.cartItems.map(r=>`
            <div class="flex items-center gap-3">
              ...
              <p class="text-xs font-bold text-amber-600 shrink-0">${$(r.listing.price*r.quantity,r.listing.currency||"USD")}</p>
            </div>
          `).join("")}
      </div>
      <div class="space-y-2 pt-4 border-t border-blue-100">
        <div class="flex justify-between text-sm"><span class="text-gray-500">Subtotal</span><span class="text-gray-900 font-bold">${_(f(t,e.selectedCurrency),e.selectedCurrency)}</span></div>
        <div class="flex justify-between text-sm"><span class="text-gray-500">Shipping</span><span class="text-emerald-600 font-bold">${ge(e.listing)?"To be quoted":"Free"}</span></div>
        ${C()>0?`<div class="flex justify-between text-sm"><span class="text-gray-500">Tax</span><span class="text-gray-900 font-bold">${_(f(C(),e.selectedCurrency),e.selectedCurrency)}</span></div>`:""}
        <div class="flex justify-between text-lg pt-2 border-t border-blue-100"><span class="text-gray-900 font-bold">Total</span><span class="text-amber-600 font-black">${w()}</span></div>
      </div>
