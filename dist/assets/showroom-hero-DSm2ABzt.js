import{h as R,a as D,S as F,c as j,b as N,d as P,e as z}from"./showroom-data-DqHxX4e9.js";import{P as H,a as B,T as G,M as q,f as U}from"./motorhome-data-CupbOvk0.js";import{isCatalogListingHidden as $}from"./catalog-hidden-store-ikn3_XIs.js";const k=".showroom-card video, [data-showroom-grid] video, .kco-video-section video, .kco-video-el, .kco-hero-media video",W="#video-tour-modal video";function w(e){return!!(e&&e.matches&&e.matches(k)&&!e.closest(W))}function b(e){if(w(e)){if(e.setAttribute("muted",""),e.setAttribute("loop",""),e.setAttribute("autoplay",""),e.setAttribute("playsinline",""),e.setAttribute("webkit-playsinline",""),e.muted=!0,e.loop=!0,e.playsInline=!0,e.autoplay=!0,e.ended)try{e.currentTime=0}catch{}e.paused&&e.play().catch(()=>{})}}function S(e){!e||!e.querySelectorAll||e.querySelectorAll(k).forEach(b)}document.addEventListener("ended",e=>{const o=e.target;if(o&&o.tagName==="VIDEO"&&w(o)){try{o.currentTime=0}catch{}o.play().catch(()=>{})}},!0);const _="MutationObserver"in window?new MutationObserver(e=>{for(const o of e)for(const t of o.addedNodes)!t||t.nodeType!==1||(t.matches&&w(t)&&b(t),t.querySelectorAll&&t.querySelectorAll(k).forEach(b))}):null;function C(){S(document),_&&document.body&&_.observe(document.body,{childList:!0,subtree:!0}),setInterval(()=>S(document),2e3)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",C):C();const I="kco-hero-rows",u="/fallback.svg",K=new Set(["Cars","Cars & Vehicles","Trucks","Buses","Buses & Coaches","Motorhomes","Motorcycles","Marine & Boating","RV & Camper Accessories","Vehicles"]),g=new Set(["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"]),Y=["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"];function i(e,o){return e.specifications&&typeof e.specifications=="object"&&e.specifications[o]!=null?e.specifications[o]:e[o]}function E(e){return e.listing_type==="vehicle"||K.has(e.category)}function h(e){return!e||typeof e!="string"?!1:/^data:video\//i.test(e)?!0:e.startsWith("blob:")?!1:/\.(mp4|webm|mov|m4v|avi|mkv|ogv)(\?|#|$)/i.test(e)}function X(e){const o=new Set,t=[];for(const r of e){if(!r)continue;const c=r.property_id||r.id;!c||o.has(c)||(o.add(c),t.push(r))}return t}function A(e){return X(e).filter(o=>$&&$(o.property_id)?!1:o.property_id&&String(o.property_id).startsWith("W")?!0:o.is_active!==!1).sort((o,t)=>{const r=Number.isFinite(parseFloat(o.price))?parseFloat(o.price):0;return(Number.isFinite(parseFloat(t.price))?parseFloat(t.price):0)-r})}function J(e){const o=e.property_type||e.subcategory||e.category;if(o&&!l.has(o)){const t=String(o).toLowerCase();for(const r of Y)if(t===r.toLowerCase()||t.includes(r.toLowerCase())){l.set(o,r);break}l.has(o)||l.set(o,String(o))}return o?l.get(o):"Homes"}const l=new Map;function ie(){const o=(z()||[]).filter(r=>r.listing_type==="property"||r.category==="Real Estate"||r.category==="Houses & Real Estate"||g.has(r.property_type)||g.has(r.subcategory)),t=[...H,...B,...F].filter(r=>r&&(r.category==="Houses & Real Estate"||r.category==="Real Estate"||g.has(r.property_type||r.subcategory)));return A([...o,...t])}function ce(){const o=(z()||[]).filter(r=>E(r)),t=[...G,...q,...H,...B].filter(r=>r&&E(r));return A([...o,...t])}function a(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Q(){if(document.getElementById("kco-hero-styles"))return;const e=document.createElement("style");e.id="kco-hero-styles",e.textContent=`
...
`,document.head.appendChild(e)}function se(e,o,t){j(e);const r=t,c=e.images&&e.images[0]||u,y=Z(e.property_id||e.id),v=o==="vehicle"&&e.category==="Trucks"?U(e):N(e);let s="",p="";if(o==="house"){const f=P(e.country_code),x=[e.city,e.state].filter(Boolean).join(", ")||e.country||"";x&&(p=`<span class="kco-hero-loc"><i data-lucide="map-pin" class="w-3 h-3 shrink-0"></i>${a(f+" "+x)}</span>`);const d=[];i(e,"bedrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bed-double" class="w-3.5 h-3.5"></i>${a(i(e,"bedrooms"))} Beds</span>`),i(e,"bathrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bath" class="w-3.5 h-3.5"></i>${a(i(e,"bathrooms"))} Baths</span>`),e.land_size&&d.push(`<span class="kco-hero-chip"><i data-lucide="ruler" class="w-3.5 h-3.5"></i>${a(e.land_size)}</span>`),i(e,"building_size")&&d.push(`<span class="kco-hero-chip"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>${a(i(e,"building_size"))}</span>`),i(e,"year_built")&&d.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${a(i(e,"year_built"))}</span>`);const O=J(e);return s=d.join(""),p||(p=""),`
      <a href="/product/${y}" class="${r}">
        <div class="kco-hero-media">
          ${h(c)?`<video src="${a(c)}" poster="${a((e.images||[]).find(V=>!h(V))||u)}" muted loop autoplay playsinline preload="metadata" class="kco-hero-media-video"></video><span class="kco-hero-video-badge"><i data-lucide="play" class="w-3 h-3"></i>Video Tour</span>`:`<img src="${a(c)}" alt="${a(e.title||"")}" loading="lazy">`}
          <span class="kco-hero-type"><i data-lucide="home" class="w-3 h-3"></i>${a(O)}</span>
          ${p}
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${v}</b><span>${e.listing_status==="rent"?"per month · for rent":"for sale"}</span></div>
      ...
      </a>`}const n=[];i(e,"model_year")&&n.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${a(i(e,"model_year"))}</span>`),i(e,"mileage")&&n.push(`<span class="kco-hero-chip"><i data-lucide="gauge" class="w-3.5 h-3.5"></i>${a(i(e,"mileage"))}</span>`),i(e,"fuel_type")&&n.push(`<span class="kco-hero-chip"><i data-lucide="fuel" class="w-3.5 h-3.5"></i>${a(i(e,"fuel_type"))}</span>`),i(e,"transmission")&&n.push(`<span class="kco-hero-chip"><i data-lucide="cog" class="w-3.5 h-3.5"></i>${a(i(e,"transmission"))}</span>`),i(e,"body_type")&&n.push(`<span class="kco-hero-chip"><i data-lucide="car-front" class="w-3.5 h-3.5"></i>${a(i(e,"body_type"))}</span>`),s=n.join("");const M=h(c)?`<video src="${a(c)}" poster="${a((e.images||[]).find(f=>!h(f))||u)}" muted loop autoplay playsinline preload="metadata" class="kco-hero-media-video"></video><span class="kco-hero-video-badge"><i data-lucide="play" class="w-3 h-3"></i>Video Tour</span>`:`<img src="${a(c)}" alt="${a(e.title||"")}" loading="lazy">`;return`
      <a href="/product/${y}" class="${r}">
        <div class="kco-hero-media">
          ${M}
          <span class="kco-hero-type"><i data-lucide="car-front" class="w-3 h-3"></i>${a(ee(e))}</span>
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${v}</b><span>· ${a(i(e,"condition")||"ready")}</span></div>
          <p class="kco-hero-title">${a(e.title||"")}</p>
          ${s?`<div class="kco-hero-chips">${s}</div>`:""}
          <span class="kco-hero-btn">View Vehicle <i data-lucide="arrow-right" class="w-4 h-4"></i></span>
        </div>
      </a>`}function Z(e){return String(e??"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function ee(e){const o=String(e.category||""),t={Cars:"Car","Cars & Vehicles":"Car",Trucks:"Truck",Buses:"Bus","Buses & Coaches":"Bus",Motorhomes:"Motorhome / RV",Motorcycles:"Motorcycle","Marine & Boating":"Boat / Marine"};return t[o]?t[o]:i(e,"body_type")||o||"Vehicle"}function m(){const e=document.getElementById(I);if(!e)return;Q();const o=document.createDocumentFragment();e.replaceChildren(o),window.lucide&&lucide.createIcons()}let L=!1;function T(){L||(L=!0,document.getElementById(I)&&(R(),m(),D().then(()=>m()).catch(()=>{}),window.addEventListener("showroom-categories-ready",()=>m()),window.addEventListener("kco-db-refresh",()=>m())))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",T):T();export{ce as a,J as b,se as c,ie as g,Q as h,ee as v};
