(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))r(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const i of o.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&r(i)}).observe(document,{childList:!0,subtree:!0});function a(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function r(n){if(n.ep)return;n.ep=!0;const o=a(n);fetch(n.href,o)}})();const Te="modulepreload",Ae=function(e){return"/"+e},te={},y=function(t,a,r){let n=Promise.resolve();if(a&&a.length>0){document.getElementsByTagName("link");const i=document.querySelector("meta[property=csp-nonce]"),s=i?.nonce||i?.getAttribute("nonce");n=Promise.allSettled(a.map(c=>{if(c=Ae(c),c in te)return;te[c]=!0;const d=c.endsWith(".css"),p=d?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${c}"]${p}`))return;const l=document.createElement("link");if(l.rel=d?"stylesheet":Te,d||(l.as="script"),l.crossOrigin="",l.href=c,s&&l.setAttribute("nonce",s),document.head.appendChild(l),d)return new Promise((f,b)=>{l.addEventListener("load",f),l.addEventListener("error",()=>b(new Error(`Unable to preload CSS for ${c}`)))})}))}function o(i){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=i,window.dispatchEvent(s),!s.defaultPrevented)throw i}return n.then(i=>{for(const s of i||[])s.status==="rejected"&&o(s.reason);return t().catch(o)})};let I=null;function C(){return I?Promise.resolve(I):y(()=>import("./supabase-client-nvpjTmO6.js"),[]).then(e=>(I=e.supabase,I))}const Ee="/verified-badge.svg",Z="Weverse Online Shop",Le="GLOBAL SHOPPING • WORLDWIDE DELIVERY",Ie="/w-logo.svg",Be=(e="weverse-badge w-4 h-4 sm:w-[18px] sm:h-[18px] lg:w-5 lg:h-5 shrink-0")=>`<svg viewBox="0 0 24 24" class="${e}" aria-label="Verified" role="img" data-weverse-badge="true"><circle cx="12" cy="12" r="11" fill="#3b82f6"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>`,he=(e="w-6 h-6 sm:w-7 sm:h-7 lg:w-8 lg:h-8")=>`  <svg viewBox="0 0 24 24" class="${e}" fill="none" aria-hidden="true"><path d="M3 5l4.5 14L12 8l4.5 11L21 5" stroke="#ffffff" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/></svg>`,G="weverse_brand_v1",Me="weverse_brand_override_v1",De=5*60*1e3;async function J(){try{const e=JSON.parse(localStorage.getItem(Me)||"null");if(e&&typeof e=="object")return e}catch{}try{const e=JSON.parse(localStorage.getItem(G)||"{}");if(e.ts&&Date.now()-e.ts<De&&e.data)return e.data}catch{}try{const e=await C(),{data:t}=await e.from("site_settings").select("brand_name,brand_slogan,brand_logo,brand_badge,brand_favicon,brand_mobile_logo,brand_header_logo,brand_footer_logo,brand_primary_color,brand_secondary_color,brand_tagline_color1,brand_tagline_color2,brand_font,brand_custom_font,brand_website_url,brand_email,site_name,site_tagline,homepage_banner_image,homepage_banner_alt").limit(1).maybeSingle(),a=t||{};return localStorage.setItem(G,JSON.stringify({ts:Date.now(),data:a})),a}catch{return{}}}function Q(e){if(!e)return;const t=e.brand_name||e.site_name||Z,a=e.brand_slogan||e.site_tagline||Le,r=e.brand_logo||e.brand_header_logo||Ie,n=e.brand_badge||Ee,o=e.brand_favicon||"",i=e.brand_custom_font||e.brand_font||"",s=e.brand_primary_color||"",c=e.brand_secondary_color||"",d=e.brand_tagline_color1||"",p=e.brand_tagline_color2||"";if($e(e.homepage_banner_image||"",e.homepage_banner_alt||"Homepage header banner"),(d||p)&&(document.querySelectorAll(".brand-tagline-1").forEach(l=>{d&&(l.style.color=d)}),document.querySelectorAll(".brand-tagline-2").forEach(l=>{p&&(l.style.color=p)})),document.title&&!document.title.startsWith(t)&&(document.title=document.title.replace(/^[^|]+\|/,t+" |").replace(/^[^–]+–/,t+" – ")),o){let l=document.querySelector("link[rel~='icon']");l||(l=document.createElement("link"),l.rel="icon",document.head.appendChild(l)),l.href=o}if(s||c||i){const l=i?`'${i}'`:null,f=document.getElementById("brand-css-vars")||(()=>{const b=document.createElement("style");return b.id="brand-css-vars",document.head.appendChild(b),b})();if(f.textContent=`:root {
      ${s?`--brand-primary: ${s};`:""}
      ${c?`--brand-secondary: ${c};`:""}
      ${l?`--brand-font: ${l}, system-ui, sans-serif;`:""}
    }`,i){const b="brand-gf-link";if(!document.getElementById(b)){const L=document.createElement("link");L.id=b,L.rel="stylesheet",L.href=`https://fonts.googleapis.com/css2?family=${encodeURIComponent(i)}:wght@400;600;700;900&display=swap`,document.head.appendChild(L)}}}document.querySelectorAll("[data-brand]").forEach(l=>{const f=l.dataset.brand;f==="name"&&(l.textContent=t,!l.querySelector("[data-weverse-badge]")&&!l.parentElement?.querySelector("[data-weverse-badge]")&&l.appendChild(Object.assign(document.createElement("span"),{className:"inline-flex items-center ml-1 align-middle",innerHTML:Be("weverse-badge w-4 h-4 sm:w-[18px] sm:h-[18px] lg:w-5 lg:h-5 shrink-0")}))),f==="slogan"&&(l.textContent=a),f==="logo"&&(l.src=r,l.alt=t,l.style.display=""),f==="badge"&&(l.src=n,l.alt="Verified",l.style.display=""),f==="tagline"&&(l.textContent=a),f==="footer-logo"&&(e.brand_footer_logo?(l.src=e.brand_footer_logo,l.alt=t):(l.src=r,l.alt=t)),f==="mobile-logo"&&(e.brand_mobile_logo?(l.src=e.brand_mobile_logo,l.alt=t):(l.src=r,l.alt=t))}),document.querySelectorAll('img[data-brand="logo"]').forEach(l=>{const b=l.closest(".text-center")?.querySelector('h1[data-brand="name"]');b&&(b.style.display="none")}),Oe(t,a,r,n),Pe(t,a,r),D()}function $e(e,t){const a=document.getElementById("homepage-banner-shell"),r=document.getElementById("homepage-banner-image");if(!(!a||!r)){if(!e){r.removeAttribute("src"),r.alt=t,a.classList.add("hidden");return}r.alt=t,r.onload=()=>D(),r.onerror=()=>{a.classList.add("hidden"),D()},r.src!==e&&(r.src=e),a.classList.remove("hidden")}}function D(){const e=document.getElementById("site-header"),t=document.getElementById("site-categories-nav"),a=document.querySelector("main");if(!e||!t||!a)return;const r=Math.ceil(e.getBoundingClientRect().height||e.offsetHeight||0),n=Math.ceil(t.getBoundingClientRect().height||t.offsetHeight||0);t.style.top=`${r}px`;const o=window.innerWidth<640?20:12;a.style.paddingTop=`${r+n+o}px`}function Oe(e,t,a,r,n){document.querySelectorAll('header a[href="/"], header a[href="./"], header a[href="index.html"], .brand-link, #brand-link').forEach(i=>{ge(i,e,t),i.querySelectorAll('img.brand-logo, img[data-brand="logo"]').forEach(s=>{s.src=a,s.alt=e})}),document.querySelectorAll("header span").forEach(i=>{if(i.classList.contains("brand-tagline-1")||i.classList.contains("brand-tagline-2"))return;const s=i.textContent.trim();(s==="Weverse Online Shop"||s==="KCO Global Online Marketplace"||i.classList.contains("brand-name"))&&(i.textContent=e),(s==="Your Trusted Global Shop"||s.includes("Globally")||s.includes("Worldwide")||i.classList.contains("brand-slogan"))&&(i.textContent=t)}),document.querySelectorAll('[data-brand="badge"], .brand-badge, #brand-badge').forEach(i=>{i.tagName==="IMG"&&(i.src=r,i.alt="Verified",i.style.display="")}),document.querySelectorAll("span").forEach(i=>{if(i.textContent.includes("Verified")&&!i.querySelector("img.brand-badge-img")){const s=document.createElement("img");s.src=r,s.alt="Verified",s.className="brand-badge-img w-4 h-4 inline-block ml-1",s.onerror=()=>s.remove(),i.appendChild(s)}}),document.querySelectorAll('header a[href="/"] .relative.shrink-0, header a .relative.w-7').forEach(i=>{if(!i.querySelector("img.injected-logo")){const s=document.createElement("img");s.src=a,s.alt=e,s.className="injected-logo w-8 h-8 sm:w-9 sm:h-9 object-contain rounded-lg",s.onerror=()=>s.style.display="none",i.style.display="flex",i.style.alignItems="center",i.innerHTML="",i.appendChild(s)}})}function Pe(e,t,a){document.querySelectorAll("footer").forEach(r=>{ge(r,e,t),r.querySelectorAll("img.brand-logo, img[data-brand], .footer-logo img").forEach(n=>{n.src=a,n.alt=e})})}function ge(e,t,a){const r=document.createTreeWalker(e,NodeFilter.SHOW_TEXT),n=[];let o;for(;o=r.nextNode();){const i=o.textContent.trim();(i==="Weverse Online Shop"||i==="KCO Global Online Marketplace")&&n.push({node:o,value:t}),(i==="Your Trusted Global Shop"||i==="Global Shopping • Worldwide Delivery")&&n.push({node:o,value:a})}n.forEach(({node:i,value:s})=>{i.textContent=i.textContent.replace(i.textContent.trim(),s)})}J().then(Q);window.addEventListener("load",()=>J().then(Q));window.addEventListener("resize",()=>D());window.addEventListener("storage",e=>{e.key===G&&J().then(Q)});const S={trust_promo_bg_image:"",trust_promo_bg_video:"",app_banner_bg_image:"",app_banner_bg_video:"",reviews_bg_image:"",reviews_bg_video:""},He=Object.keys(S).join(","),X="kco_promo_backgrounds_v1",Ue=60*1e3;function Fe(){try{const e=JSON.parse(localStorage.getItem(X)||"{}");if(e.ts&&Date.now()-e.ts<Ue&&e.data&&typeof e.data=="object")return e.data}catch{}return null}function We(e){try{localStorage.setItem(X,JSON.stringify({ts:Date.now(),data:e}))}catch{}}async function $(){const e=Fe();if(e)return{...S,...e};try{const t=await C(),{data:a,error:r}=await t.from("site_settings").select(He).limit(1).maybeSingle(),n={...S,...r||!a?{}:a};return We(n),n}catch{return{...S}}}function Tt(){try{localStorage.removeItem(X)}catch{}window.dispatchEvent(new CustomEvent("promo-backgrounds-updated"))}function q(e,t){const a=(e||"").trim(),r=(t||"").trim(),n=[];return r&&n.push(`<video class="absolute inset-0 w-full h-full object-cover" autoplay muted loop playsinline webkit-playsinline preload="metadata" poster="${ae(a)}"><source src="${ae(r)}" type="video/mp4"></video>`),a&&n.push(`<div class="absolute inset-0 bg-cover bg-center" style="background-image:url('${Ge(a)}')"></div>`),(r||a)&&n.push('<div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-900/70 to-slate-900/45"></div>'),n.join("")}function ae(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Ge(e){return String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'")}const x={app_banner_title:"Discover More with the Weverse Online Shop App",app_banner_description:"Shop products, discover new arrivals, manage your orders, save favorites, and enjoy a smooth shopping experience wherever you go.",app_banner_button_text:"Get it on Google Play",app_banner_secondary_text:"Browse the Shop",bottom_heading:"Thank You for Shopping With Us",bottom_main_message:"Every visit, every order, and every moment you spend with us means more than simply shopping. You are part of our journey toward creating a better global shopping experience.",bottom_closing_message:"We look forward to serving you again. ❤️",bottom_support_heading:"Customer Support",bottom_support_description:"Our support team is here for you 24/7 — before and after every order.",bottom_support_button_text:"Contact Support",bottom_footer_text:"GLOBAL SHOPPING · WORLDWIDE DELIVERY",bottom_footer_closing:"Made with ❤️ for shoppers everywhere",bottom_copyright:""},qe=Object.keys(x).join(","),R="kco_site_content_v1",Re=60*1e3;async function N(){try{const t=JSON.parse(localStorage.getItem(R)||"{}");if(t.ts&&Date.now()-t.ts<Re&&t.data&&typeof t.data=="object")return{...x,...t.data}}catch{}const e={...x};try{const t=await C(),{data:a,error:r}=await t.from("site_settings").select(qe).limit(1).maybeSingle();if(!r&&a)for(const n of Object.keys(x)){const o=a[n];typeof o=="string"&&o&&(e[n]=o)}}catch{}try{localStorage.setItem(R,JSON.stringify({ts:Date.now(),data:e}))}catch{}return e}function At(){try{localStorage.removeItem(R)}catch{}window.dispatchEvent(new CustomEvent("site-content-updated"))}const U="support@weverseonlineshop.com",Ne="weverse_brand_v1";function u(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function me(){try{const e=JSON.parse(localStorage.getItem(Ne)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function ze(){const e=me(),t=e.brand_name||e.site_name||Z;return`
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-12 sm:py-16 lg:py-20">
        <div class="max-w-2xl relative rounded-3xl bg-slate-950/55 border border-white/10 p-6 sm:p-8 md:p-10 backdrop-blur-md shadow-2xl shadow-slate-950/50">
          ...
          <p class="text-[15px] sm:text-base text-slate-200 mt-4 leading-relaxed max-w-xl">
            ...
            200+ countries worldwide — so shopping with ${u(t)} is always fast, safe and worry-free.
          </p>
    ...
    </section>`}const re=[{id:"trust-shipping",icon:"package",tone:"blue",title:"Shipping & Delivery",body:`
    ...
    </div>`}function Ye(){const e=re.slice(0,6),t=re.slice(6),a=r=>r.map(Ve).join("");return`
    ...
    </section>`}function m(e,t){return`<li><a href="${e}" class="text-xs text-slate-400 hover:text-white transition">${t}</a></li>`}function ie(e){const t={...x,...e||{}},a=me(),r=a.brand_name||a.site_name||Z,n=a.brand_logo||a.brand_header_logo||a.brand_footer_logo||"/w-logo.svg",o=t.bottom_footer_text||a.brand_slogan||a.site_tagline||"GLOBAL SHOPPING · WORLDWIDE DELIVERY",i=t.bottom_copyright?t.bottom_copyright:`© ${new Date().getFullYear()} ${r}. All rights reserved.`;return`
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 pt-14 sm:pt-20 pb-8 sm:pb-10">
        ...
        <div class="text-center max-w-3xl mx-auto">
          <div class="mx-auto w-16 h-16 rounded-2xl bg-white/[.07] border border-white/10 flex items-center justify-center overflow-hidden backdrop-blur">
            <img src="${u(n)}" alt="${u(r)}" class="w-10 h-10 object-contain" onerror="this.onerror=null;this.style.display='none';this.nextElementSibling.style.display='flex'">
            <span style="display:none">${he("w-9 h-9")}</span>
          </div>
          <p class="mt-4 text-[10px] font-black uppercase tracking-[0.28em] text-cyan-300">${u(o)}</p>
          ...
          <p class="mt-4 text-[15px] sm:text-base text-slate-300 leading-relaxed max-w-2xl mx-auto">
            ${u(t.bottom_main_message)}
          </p>
          <p class="mt-5 text-lg sm:text-xl font-semibold text-cyan-200">${u(t.bottom_closing_message)}</p>
        </div>
        ...
        <div class="mt-12 max-w-4xl mx-auto rounded-3xl border border-white/10 bg-white/[.05] backdrop-blur-md p-6 sm:p-8">
          <div class="flex flex-col lg:flex-row items-center gap-6 justify-between text-center lg:text-left">
            <div class="flex items-center gap-4">
              ...
              <div>
                <h3 class="text-lg sm:text-xl font-black text-white tracking-tight">${u(t.bottom_support_heading)}</h3>
                <p class="text-sm text-slate-300 mt-1 max-w-md">${u(t.bottom_support_description)}</p>
              </div>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 shrink-0">
              <a href="mailto:${u(U)}" class="btn-press inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
                ${u(t.bottom_support_button_text)} <i data-lucide="message-circle" class="w-4 h-4"></i>
              </a>
          ...
          <div class="mt-6 pt-5 border-t border-white/10 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center text-[11px] text-slate-400">
            <span class="inline-flex items-center justify-center gap-1.5"><i data-lucide="mail" class="w-3.5 h-3.5 text-cyan-300"></i> ${u(U)}</span>
        ...
        <div class="mt-12 grid grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-9">
          <div>
            ...
            <ul class="space-y-2.5">
              ${m("/about.html","About Us")}
              ${m("/team.html","Our Team")}
              ${m("/contact.html","Contact Us")}
              ${m("/help.html","Help Center")}
            </ul>
          </div>
          <div>
            <h4 class="text-xs font-black text-white uppercase tracking-widest mb-3.5">Legal</h4>
            <ul class="space-y-2.5">
              ${m("/privacy.html","Privacy Policy")}
              ${m("/terms.html","Terms & Conditions")}
              ${m("/refund-policy.html","Refund Policy")}
              ${m("/shipping-policy.html","Shipping Policy")}
            </ul>
          </div>
          <div>
            <h4 class="text-xs font-black text-white uppercase tracking-widest mb-3.5">Account</h4>
            <ul class="space-y-2.5">
              ${m("/account.html","My Account")}
              ${m("/auth.html","Sign In")}
              ${m("/auth.html","Register / Create Account")}
            </ul>
          </div>
          <div>
            <h4 class="text-xs font-black text-white uppercase tracking-widest mb-3.5">Support</h4>
            <ul class="space-y-2.5">
              ${m("mailto:"+U,"Email Support")}
              ${m("/help.html","FAQ")}
              ${m("/contact.html","Contact Us")}
            </ul>
          </div>
  ...
  <svg viewBox="0 0 560 720" class="w-full h-full block" aria-hidden="true">
    ...
    <g>
  ...
  </svg>`}function yt(e){const t=e[0],a=e[1],r=(n,o)=>{const i=g(O(n)),s=g((n.title||n.name||"").slice(0,34));return`
      <a href="/details.html?id=${encodeURIComponent(n.property_id||n.id)}"
        ...
        <div class="aspect-square bg-gray-100 overflow-hidden">
          <img src="${i}" alt="${s}" loading="lazy" decoding="async"
               class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${Y}'">
        </div>
        <div class="p-2">
    ...
    <div class="absolute inset-0 flex flex-col bg-[#f1f5f9] overflow-hidden" id="promo-phone-screen">
      ...
      <div class="flex gap-1.5 px-3 pt-2 overflow-hidden">
        ${["All","Cars","Phones","Fashion","Homes","Electronics"].map((n,o)=>`
          <span class="shrink-0 px-2 py-1 rounded-full text-[8px] font-black ${o===0?"bg-blue-500 text-white":"bg-white border border-gray-200 text-gray-600"}">${n}</span>`).join("")}
      </div>
      <!-- live product cards -->
    ...
    </div>`}function wt(e,t,a){const r=ke(t,e,12),n=(e.app_play_store_url||"").trim(),o={...x,...a||{}},i=(o.app_banner_title||e.app_banner_headline||E.app_banner_headline).trim(),s=o.app_banner_description,c=o.app_banner_button_text,d=o.app_banner_secondary_text,p=n?`<a href="${g(n)}" target="_blank" rel="noopener" class="inline-flex items-center gap-2.5 bg-white text-blue-900 font-black text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
    ...
    <section class="relative overflow-hidden bg-[#060c1c] text-white">
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-14 lg:py-16">
        <div class="grid lg:grid-cols-[1.05fr_.95fr] gap-10 lg:gap-6 items-center">
          ...
          <div class="max-w-xl lg:pr-6">
            ...
            <div class="flex flex-wrap items-center gap-3.5 mt-8">
              ...
              <a href="/#showroom-directory" class="inline-flex items-center gap-2 text-sm font-bold text-cyan-200 hover:text-white transition">
                ${g(d)} <i data-lucide="arrow-right" class="w-4 h-4"></i>
              </a>
            </div>
            ${n?"":'<p class="text-[11px] text-slate-500 mt-3">The Android app is in final review. We’ll publish the download link here the moment it is live.</p>'}
          </div>
          ...
          <div class="relative mx-auto w-full max-w-[300px] sm:max-w-[360px] lg:max-w-[440px]">
            <div class="absolute inset-0 woman-back" aria-hidden="true">${mt()}</div>
            <div class="absolute inset-0 hands-front pointer-events-none" aria-hidden="true">${ft()}</div>
            <div class="relative flex justify-center pt-[30%] sm:pt-[27%] lg:pt-[26%]">
              <div class="scale-100 sm:scale-105 lg:scale-110">${bt(r)}</div>
            </div>
          </div>
        </div>
      </div>
    </section>`}let B=null;function xt(e){B&&(clearInterval(B),B=null);const t=document.getElementById("promo-phone-screen"),a=document.getElementById("promo-phone-grid");if(!t||!a||!e.length)return;const r=e.slice();let n=0;const o=()=>{if(!a||!r.length)return;const i=r[n%r.length],s=r[(n+1)%r.length],c=g(O(i)),d=g(O(s)),p=g((i.title||i.name||"").slice(0,34)),l=g((s.title||s.name||"").slice(0,34));if(a.innerHTML=`
      <a href="/details.html?id=${encodeURIComponent(i.property_id||i.id)}" class="block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition active:scale-[.98]">
        <div class="aspect-square bg-gray-100 overflow-hidden"><img src="${c}" alt="${p}" class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${Y}'"></div>
        <div class="p-2"><p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${p}</p>
      ...
      <a href="/details.html?id=${encodeURIComponent(s.property_id||s.id)}" class="block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition active:scale-[.98]">
        <div class="aspect-square bg-gray-100 overflow-hidden"><img src="${d}" alt="${l}" class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${Y}'"></div>
        <div class="p-2"><p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${l}</p>
        <div class="flex items-center justify-between mt-1"><span class="text-[11px] text-blue-600 font-black">${P(s)}</span><span class="bg-blue-500 text-white text-[9px] font-black px-2 py-1 rounded-lg">Buy</span></div></div>
      </a>`,window.lucide)try{lucide.createIcons()}catch{}};o(),B=setInterval(()=>{n+=2,o()},4500)}async function pe(){const e=gt();if(!e)return;let t={...E};try{t=await _e()}catch{}if(t.app_banner_enabled===!1)return;let a=[];try{await ve(),a=V()||[]}catch{}const r=a.length?a:[{property_id:"browse",title:"Browse the full Weverse Online Shop",price:0,currency:"USD",images:["/fallback.svg"]}];let n={...S};try{n=await $()}catch{}const o=s=>{const c=e.querySelector('[data-bg-slot="app_banner"]');c&&(c.innerHTML=q(s.app_banner_bg_image,s.app_banner_bg_video))};async function i(){let s={...x};try{s=await N()}catch{}if(e.innerHTML=wt(t,r,s),window.lucide)try{lucide.createIcons()}catch{}xt(r),window.dispatchEvent(new CustomEvent("app-promo-banner-ready")),o(n)}await i(),o(n),window.addEventListener("site-content-updated",()=>{i().catch(()=>{})}),window.addEventListener("promo-backgrounds-updated",()=>{$().then(s=>{n=s,o(s)}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",pe):pe();const vt="/fallback.svg",K=()=>document.getElementById("live-promo-alerts"),Se="kco_live_promo_dismissed_v1";function Ce(){try{return JSON.parse(localStorage.getItem(Se)||"{}")}catch{return{}}}function _t(e){const a=Ce()[e];return a?Date.now()-a<2*60*60*1e3:!1}function kt(e){try{const t=Ce();t[e]=Date.now(),localStorage.setItem(Se,JSON.stringify(t))}catch{}}function St(e){const t=g(e.title||e.name||"Featured product"),a=e.listing_type==="vehicle"?e.category||"Featured Vehicle":e.listing_type==="property"?"Featured Property":e.subcategory||e.category||"Featured Product";return`
    <div class="live-promo-toast pointer-events-auto flex items-center gap-3 bg-white border border-gray-200 rounded-2xl shadow-2xl shadow-black/15 p-3 pr-4 cursor-pointer hover:shadow-black/25 transition" data-promo-id="${g(e.property_id||e.id||"")}">
      <div class="shrink-0 w-16 h-16 rounded-xl overflow-hidden bg-gray-100 border border-gray-100">
        <img src="${g(O(e))}" alt="${t}" loading="lazy" class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${vt}'">
      </div>
      <div class="min-w-0 flex-1">
        <p class="text-[9px] font-black uppercase tracking-wider text-blue-600 flex items-center gap-1">
          <i data-lucide="sparkles" class="w-3 h-3"></i> ${g(a)}
        </p>
    ...
    </div>`}function Ct(e){const t=K();if(!t)return;const a=e.property_id||e.id;if(_t(a))return;t.querySelectorAll(".live-promo-toast").forEach(s=>s.remove()),t.insertAdjacentHTML("beforeend",St(e));const r=t.querySelector(".live-promo-toast");if(!r)return;r.style.opacity="0",r.style.transform="translateY(14px)";const n=`/details.html?id=${encodeURIComponent(a)}`,o=r.querySelector(".live-promo-close"),i=s=>{s.preventDefault(),s.stopPropagation(),r.style.transform="translateY(12px)",r.style.opacity="0",setTimeout(()=>{r.remove()},220),window.location.href=n};if(o.addEventListener("click",s=>{s.preventDefault(),s.stopPropagation(),a&&kt(a),r.style.transform="translateY(12px)",r.style.opacity="0",setTimeout(()=>r.remove(),220)}),r.addEventListener("click",i),window.lucide)try{lucide.createIcons()}catch{}requestAnimationFrame(()=>{r.style.transition="transform .35s ease, opacity .35s ease",r.style.transform="translateY(0)",r.style.opacity="1"}),setTimeout(()=>{r.isConnected&&(r.style.transform="translateY(12px)",r.style.opacity="0",setTimeout(()=>r.remove(),260))},7e3)}async function ue(){if(!K())return;let t={...E};try{t=await _e()}catch{}if(t.live_promo_enabled===!1)return;let a=[];try{await ve(),a=V()||[]}catch{}if(!a.length)return;const r=ke(a,t,10);if(!r.length)return;let n=0;const o=Math.max(3,Number(t.live_promo_first_delay_seconds)||12)*1e3,i=Math.max(20,Number(t.live_promo_interval_seconds)||60)*1e3,s=()=>{if(K()&&V().length){const c=r[n%r.length];Ct(c),n+=1}};setTimeout(()=>{s(),setInterval(s,i)},o)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ue):ue();(()=>{if(typeof window>"u"||document.getElementById("scroll-progress-rail"))return;const e=document.createElement("div");e.id="scroll-progress-rail",e.setAttribute("role","scrollbar"),e.setAttribute("aria-label","Scroll progress — tap to jump to top or down the page"),e.innerHTML=`
    ...
    }`,o=document.createElement("style");o.textContent=n,document.head.appendChild(o);function i(){return Math.max(0,(document.documentElement.scrollHeight||document.body.scrollHeight||0)-window.innerHeight)}function s(){const d=i(),p=d>0?Math.min(1,window.scrollY/d):0;t&&(t.style.height=`${(p*100).toFixed(2)}%`),a.style.opacity=window.scrollY>300?"1":"0",a.style.pointerEvents=window.scrollY>300?"auto":"none"}function c(d){const p=e.getBoundingClientRect(),l=Math.min(1,Math.max(0,(d-p.top)/Math.max(1,p.height)));window.scrollTo({top:l*i(),behavior:"smooth"})}a.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})}),e.addEventListener("pointerdown",d=>{d.target!==a&&(r=!0,c(d.clientY),e.setPointerCapture(d.pointerId),d.preventDefault())}),e.addEventListener("pointermove",d=>{r&&c(d.clientY)}),["pointerup","pointercancel"].forEach(d=>{e.addEventListener(d,()=>{r=!1})}),s(),window.addEventListener("scroll",s,{passive:!0}),window.addEventListener("resize",s,{passive:!0})})();export{x as D,H as S,y as _,Tt as a,$ as b,fe as c,q as d,j as e,dt as f,we as g,it as h,At as i,C as j,ye as k,be as l,ot as m,Et as s};
