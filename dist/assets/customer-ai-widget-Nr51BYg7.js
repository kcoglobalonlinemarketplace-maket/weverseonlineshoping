import{supabase as C}from"./supabase-client-7_ZWSEp6.js";import{b as _,c as q,e as L}from"./localization-ozBqAmIG.js";const U="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),j=`${U}/functions/v1/ai-customer-assistant`,H="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",N=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let n={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null,attachments:[],recording:!1,mediaRecorder:null,mediaChunks:[],recordTimer:null,recordSeconds:0};function k(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function V(t){let e=k(t);return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^\- (.+)$/gm,"<li>$1</li>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,a=>`<ul>${a}</ul>`),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=`<p>${e}</p>`,e=e.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),e}function b(){return _()||localStorage.getItem("kco_language")||"en"}function T(){const t=window.location.pathname;return t.includes("checkout")?"checkout":t.includes("payment")?"payment":t.includes("auth")?"sign-in":t.includes("account")?"account":t.includes("details")?"product-details":t.includes("contact")?"contact":t.includes("help")?"help":t.includes("about")?"about":"home"}function P(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(e=>({title:e.title||e.name,price:e.price,quantity:e.quantity||1}))}catch{return[]}}function x(t){if(n.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t.replace(/[*#`]/g,"")),a=b(),o=q(a);a==="en"?e.lang=n.voiceAccent==="UK"?"en-GB":"en-US":e.lang=o.lang,e.rate=o.rate,e.pitch=o.pitch;const i=L(a==="en"?(n.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):a);i&&(e.voice=i),window.speechSynthesis.speak(e)}catch{}}function $(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function D(){if(document.getElementById("kco-ai-styles"))return;const t=document.createElement("style");t.id="kco-ai-styles",t.textContent=`
  ...
  `,document.body.appendChild(e),document.getElementById("kco-ai-close").onclick=()=>d(!1),document.getElementById("kco-ai-minimize").onclick=()=>d(!1),document.getElementById("kco-ai-clear").onclick=te,document.getElementById("kco-ai-voice-toggle").onclick=Y,document.getElementById("kco-ai-voice-accent").onclick=G,document.getElementById("kco-ai-send").onclick=p,document.getElementById("kco-ai-attach").onclick=()=>document.getElementById("kco-ai-file").click(),document.getElementById("kco-ai-file").onchange=X,document.getElementById("kco-ai-mic").onclick=Q,document.getElementById("kco-ai-rec-cancel").onclick=ee;const a=document.getElementById("kco-ai-input");a.addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),p())}),a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,96)+"px"}),document.querySelectorAll(".kco-ai-quick-btn").forEach(o=>{o.onclick=()=>{a.value=o.dataset.q,p()}}),O(),window.lucide&&lucide.createIcons()}function d(t){const e=document.getElementById("kco-ai-panel"),a=document.getElementById("kco-ai-fab");n.open=t!==void 0?t:e.classList.contains("hidden-panel"),n.open?(e.classList.remove("hidden-panel"),a.style.display="none",window.innerWidth<640&&(document.body.style.overflow="hidden"),n.welcomed||(M(),n.welcomed=!0),setTimeout(()=>document.getElementById("kco-ai-input")?.focus(),300)):(e.classList.add("hidden-panel"),a.style.display="flex",$(),v(),document.body.style.overflow="")}function Y(){n.voiceEnabled=!n.voiceEnabled,localStorage.setItem("kco_voice",n.voiceEnabled?"1":"0"),n.voiceEnabled||$(),O(),n.voiceEnabled&&x("Voice assistance is now on.")}function G(){n.voiceAccent=n.voiceAccent==="US"?"UK":"US",localStorage.setItem("kco_voice_accent",n.voiceAccent);const t=document.getElementById("kco-ai-voice-accent");t&&(t.textContent=n.voiceAccent);const e=n.voiceAccent==="UK"?"Voice accent switched to British English.":"Voice accent switched to American English.";x(e)}function O(){const t=document.getElementById("kco-ai-voice-toggle");if(!t)return;const e=n.voiceEnabled?"volume-2":"volume-x";t.title=n.voiceEnabled?"Voice is ON — click to mute":"Voice is OFF — click to enable",t.innerHTML=`<i data-lucide="${e}" class="w-4 h-4"></i>`,window.lucide&&lucide.createIcons()}function w(t){const e=Math.floor(t/60),a=String(Math.floor(t%60)).padStart(2,"0");return`${e}:${a}`}function f(){const t=document.getElementById("kco-ai-attachments");t&&(t.innerHTML=n.attachments.map((e,a)=>e.type==="image"?`<div class="kco-attach-chip relative shrink-0 w-16 h-16 rounded-lg overflow-hidden border border-blue-500/30 bg-slate-800">
    ...
    </div>`).join(""),t.classList.toggle("hidden",n.attachments.length===0),window.lucide&&lucide.createIcons(),t.querySelectorAll("[data-rm]").forEach(e=>{e.onclick=()=>{n.attachments.splice(Number(e.dataset.rm),1),f()}}))}function z(t){if(n.attachments.length>=4){g("You can attach up to 4 items per message.");return}n.attachments.push(t),f()}function K(t,e=900,a=.72){return new Promise(o=>{const i=new FileReader;i.onload=()=>{const s=new Image;s.onload=()=>{const m=Math.min(1,e/Math.max(s.width,s.height)),c=document.createElement("canvas");c.width=Math.max(1,Math.round(s.width*m)),c.height=Math.max(1,Math.round(s.height*m)),c.getContext("2d").drawImage(s,0,0,c.width,c.height),o(c.toDataURL("image/jpeg",a))},s.onerror=()=>o(i.result),s.src=i.result},i.onerror=()=>o(null),i.readAsDataURL(t)})}function X(t){const e=t.target.files&&t.target.files[0];if(t.target.value="",!!e){if(!/^image\//.test(e.type)){g("Please choose an image file.");return}K(e).then(a=>{a&&z({type:"image",dataUrl:a,name:e.name})})}}function S(t){const e=document.getElementById("kco-ai-mic"),a=document.getElementById("kco-ai-recording");e&&(e.innerHTML=`<i data-lucide="${t?"square":"mic"}" class="w-5 h-5 ${t?"text-rose-400":""}"></i>`,e.title=t?"Stop recording":"Record voice note",e.style.color=t?"#fb7185":"",e.style.background=t?"rgba(244,63,94,0.15)":"",a&&a.classList.toggle("hidden",!t),window.lucide&&lucide.createIcons())}function Q(){if(n.recording){v();return}Z()}async function Z(){if(!navigator.mediaDevices||!window.MediaRecorder){g("Voice notes are not supported on this browser.");return}try{const t=await navigator.mediaDevices.getUserMedia({audio:!0});n.mediaChunks=[],n.recordSeconds=0;const e=MediaRecorder.isTypeSupported("audio/webm;codecs=opus")?"audio/webm;codecs=opus":"audio/webm";n.mediaRecorder=new MediaRecorder(t,e?{mimeType:e}:void 0),n.mediaRecorder.ondataavailable=o=>{o.data.size>0&&n.mediaChunks.push(o.data)},n.mediaRecorder.onstop=()=>{const o=new Blob(n.mediaChunks,{type:n.mediaRecorder.mimeType||"audio/webm"});t.getTracks().forEach(s=>s.stop());const i=new FileReader;i.onloadend=()=>z({type:"audio",dataUrl:i.result,duration:n.recordSeconds}),i.readAsDataURL(o)},n.recording=!0,S(!0);const a=document.getElementById("kco-ai-rec-timer");n.recordTimer=setInterval(()=>{if(n.recordSeconds+=1,n.recordSeconds>=60){v();return}a&&(a.textContent=w(n.recordSeconds))},1e3)}catch{g("Microphone access was denied.")}}function v(){if(!(!n.recording||!n.mediaRecorder)){try{n.mediaRecorder.stop()}catch{}clearInterval(n.recordTimer),n.recordTimer=null,n.recording=!1,S(!1)}}function ee(){if(clearInterval(n.recordTimer),n.recordTimer=null,n.mediaChunks=[],n.mediaRecorder&&n.mediaRecorder.state!=="inactive"){try{n.mediaRecorder.onstop=null,n.mediaRecorder.stop()}catch{}n.mediaRecorder.stream&&n.mediaRecorder.stream.getTracks().forEach(t=>t.stop())}n.recording=!1,n.mediaRecorder=null,S(!1)}function te(){const t=document.getElementById("kco-ai-messages");t&&(t.innerHTML=""),n.history=[],n.attachments=[],f(),v(),M(),ne("Conversation cleared")}function ne(t){const e=document.createElement("div");e.className="fixed top-16 left-1/2 -translate-x-1/2 z-[70] bg-gray-900 border border-blue-500/30 text-white text-xs font-medium px-4 py-2 rounded-full shadow-xl kco-ai-fade",e.textContent=t,document.body.appendChild(e),setTimeout(()=>e.remove(),2e3)}function g(t){const e=document.getElementById("kco-ai-input");e&&(e.placeholder=t,setTimeout(()=>{e.placeholder===t&&(e.placeholder="Type a message...")},2600))}function ae(t){return(t||[]).map(e=>e.type==="image"?`<img src="${e.dataUrl}" class="mt-2 w-full max-h-56 rounded-xl object-cover border border-white/10" alt="Shared image">`:e.type==="audio"?`<div class="mt-2 flex items-center gap-2">
    ...
    <div class="max-w-[80%] text-white rounded-2xl rounded-tr-sm px-3.5 py-2.5 shadow-lg" style="background:linear-gradient(135deg,#3b82f6 0%,#2563eb 100%);box-shadow:0 2px 8px rgba(59,130,246,0.25)">
      <p class="text-sm leading-relaxed whitespace-pre-wrap break-words">${k(t.content)}</p>
      ${ae(t.attachments)}
    </div>
  `:`
  ...
  `,a.appendChild(i),window.lucide&&lucide.createIcons(),R(),o||x(t.content)}function M(){const t=b(),e={en:`Welcome to Weverse Online Shop!
...
Naweza kukusaidia vipi leo?`};u({role:"assistant",content:e[t]||e.en})}function oe(){const t=document.getElementById("kco-ai-messages");if(!t)return;const e=document.createElement("div");e.id="kco-ai-typing",e.className="flex justify-start kco-ai-fade",e.innerHTML=`
    ...
    </div>`,t.appendChild(e),window.lucide&&lucide.createIcons(),R()}function R(){const t=document.getElementById("kco-ai-messages");t&&(t.scrollTop=t.scrollHeight)}async function ie(){const{data:t}=await C.auth.getSession();return{Authorization:`Bearer ${t.session?.access_token||H}`,"Content-Type":"application/json"}}async function p(){const t=document.getElementById("kco-ai-input");if(!t)return;const e=t.value.trim(),a=n.attachments.slice();if(!e&&a.length===0||n.sending)return;t.value="",t.style.height="auto",n.attachments=[],f(),n.sending=!0,document.getElementById("kco-ai-send").disabled=!0;const o=a.some(r=>r.type==="image"),i=a.some(r=>r.type==="audio"),s=[o&&"[Image attached]",i&&"[Voice note attached]"].filter(Boolean).join(" "),m=s?`${e?e+" ":""}${s}`.trim():e,c={role:"user",content:m,attachments:a};n.history.push(c),u(c),oe();try{const r=await ie(),I=await(await fetch(j,{method:"POST",headers:r,body:JSON.stringify({action:"chat",message:m,history:n.history.slice(-20,-1).map(l=>({role:l.role,content:l.content})),language:b(),page_context:T(),cart_items:P(),attachments:a.map(l=>({type:l.type,dataUrl:l.dataUrl,duration:l.duration||0}))})})).json();document.getElementById("kco-ai-typing")?.remove();const E={role:"assistant",content:I.response||I.error||"I'm sorry, I didn't catch that. Could you rephrase?"};n.history.push(E),u(E)}catch{document.getElementById("kco-ai-typing")?.remove();const y={role:"assistant",content:"I'm having trouble connecting right now. Please try again in a moment."};n.history.push(y),u(y)}finally{n.sending=!1,document.getElementById("kco-ai-send").disabled=!1,document.getElementById("kco-ai-input")?.focus()}}function W(t,e){if(n.proactiveShown[t]||(n.proactiveShown[t]=!0,n.open)||!document.getElementById("kco-ai-fab"))return;const o=document.createElement("div");o.id=`kco-ai-nudge-${t}`,o.className="fixed bottom-[92px] right-5 z-[59] max-w-[280px] bg-slate-800 border border-blue-500/30 rounded-xl shadow-2xl px-3.5 py-2.5 kco-ai-fade",o.innerHTML=`
    <div class="flex items-start gap-2">
      <i data-lucide="message-circle" class="w-4 h-4 text-blue-400 shrink-0 mt-0.5"></i>
      <p class="text-xs text-gray-300 leading-snug flex-1">${k(e)}</p>
    </div>
