import{H as u,q as m,V as x}from"./supabase-client-DvCmNkpI-BBtcpOs0-B154SqIU.js";import"./localization-B_HxL1KQ-B_HxL1KQ-C9a7KFzV.js";import{g as y,b as h,P as b,a as f}from"./motorhome-data-CupbOvk0--c7gbSNU--c7gbSNU.js";import{e as w,i as v}from"./phone-data-Of7KtnOV-OoM3wJJD-dTQdNoKz.js";import{getCurrentUser as k,setRedirectAfterAuth as $}from"./auth-iMacRl0l-DlcadYa5-OJ73ZqUh.js";import{c as d,d as C,u as l,y as _,h as P}from"./cart-DNy8CJA3-Oz5JpZDi-B2DS8jkg.js";import"./native-bridge-z1PkSgfH-CV6U1YtG-D-j24ntm.js";import"./localization-bootstrap-BhglmvwT-CYE5uVRZ-C9Hs_pKH.js";import"./app-promo-banner-VTpzwpvR-CiDTzVNv-Dv-62U_4.js";import"./ai-chat-U-IZQEoO-DrOtSYGJ-BkDHyhCO.js";const o="/fallback.svg";function S(t){return b.find(e=>e.id===t||e.property_id===t)||f.find(e=>e.id===t||e.property_id===t)}function j(t){return x(t)||y(t)||h(t)||w(t)||v(t)||S(t)}function i(t,e){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${e}`}function q(t){const e=i(t.price,t.currency||"USD"),r=parseFloat(t.real_price);return Number.isFinite(r)&&r>0&&r>parseFloat(t.price)?`<span class="price-strike line-through text-gray-400 mr-1 text-xs">${i(r,t.currency||"USD")}</span><span class="text-amber-600 font-bold">${e}</span>`:`<span class="text-amber-600 font-bold">${e}</span>`}function p(t){const e=document.getElementById("toast-msg"),r=document.getElementById("toast");!e||!r||(e.textContent=t,r.classList.remove("translate-y-20","opacity-0"),r.classList.add("translate-y-0","opacity-100"),clearTimeout(window.__cartToastT),window.__cartToastT=setTimeout(()=>{r.classList.add("translate-y-20","opacity-0"),r.classList.remove("translate-y-0","opacity-100")},2e3))}function c(){return`
  ...
  `}function T(t){const e=t.reduce((s,a)=>s+(parseFloat(a.listing.price)||0)*a.qty,0),r=t.map(s=>{const a=s.listing,g=a.images?.[0]||o;return`
      <div class="flex flex-wrap items-center gap-x-4 gap-y-3 p-3 sm:p-4 bg-gray-50 border border-gray-200 rounded-xl" data-cart-row="${a.property_id}">
        <a href="/details.html?id=${a.property_id}" class="w-16 h-16 sm:w-20 sm:h-20 rounded-lg overflow-hidden shrink-0 bg-gray-100 ring-1 ring-gray-200">
          <img src="${g}" alt="${a.title}" class="w-full h-full object-cover" loading="lazy" onerror="this.src='${o}'">
        </a>
        <div class="flex-1 min-w-[160px] flex flex-col gap-1.5">
          ...
          <div class="flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-sm leading-snug">${q(a)}</div>
        </div>
  ...
  `}async function n(){const t=document.getElementById("cart-content");await u();const e=d();if(e.length===0){t.innerHTML=c(),window.lucide&&lucide.createIcons();return}const r=e.map(s=>{const a=j(s.id);return a?(m(a),{listing:a,qty:s.qty}):null}).filter(Boolean);if(r.length===0){t.innerHTML=c(),window.lucide&&lucide.createIcons();return}t.innerHTML=T(r),window.lucide&&lucide.createIcons()}window.cartPageChangeQty=(t,e)=>{const r=d().find(s=>s.id===t);r&&(C(t,r.qty+e),l(),n())};window.cartPageRemove=t=>{_(t),l(),n(),p("Removed from cart.")};window.cartPageClear=()=>{P(),l(),n(),p("Cart cleared.")};window.cartPageCheckout=async()=>{await k()?window.location.href="/checkout.html":($("/checkout.html"),window.location.href="/auth.html?redirect=/checkout.html")};n();
