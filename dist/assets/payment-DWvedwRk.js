...
import{supabase as L}from"./supabase-client-7_ZWSEp6.js";import{g as q,d as X}from"./localization-Cbv4Lckk.js";import{_ as O}from"./preload-helper-CLcXU_4U.js";import{f as R,l as Z,a as T,b as Q}from"./showroom-data-DQuiOO-D.js";import{g as ee}from"./auth-D3ZJ7kZA.js";import{c as te,a as le,b as ae,d as H,r as ie,e as re}from"./payment-settings-DelaQfNw.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-D95sBX0y.js";import"./customer-ai-widget-B-tKRbYv.js";import"./brand-C2DcJ48U.js";const z="/fallback.svg";let P=null,k=[],C="",S="";const oe=[{name:"PayPal",icon:"wallet",color:"text-blue-400"},{name:"Stripe",icon:"credit-card",color:"text-violet-400"},{name:"Flutterwave",icon:"zap",color:"text-orange-400"},{name:"Paystack",icon:"layers",color:"text-cyan-400"},{name:"Apple Pay",icon:"smartphone",color:"text-gray-300"},{name:"Google Pay",icon:"smartphone",color:"text-green-400"},{name:"Visa",icon:"credit-card",color:"text-blue-500"},{name:"Mastercard",icon:"credit-card",color:"text-red-500"},{name:"American Express",icon:"credit-card",color:"text-blue-300"},{name:"Discover",icon:"credit-card",color:"text-orange-500"},{name:"Verve",icon:"credit-card",color:"text-green-500"},{name:"Bitcoin (BTC)",icon:"bitcoin",color:"text-yellow-500"},{name:"Ethereum (ETH)",icon:"bitcoin",color:"text-indigo-400"},{name:"USDT",icon:"bitcoin",color:"text-green-400"}],_=[{id:"placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"submitted",label:"Payment Submitted",icon:"upload",color:"text-cyan-400",bg:"bg-cyan-500/15"},{id:"verification",label:"Pending Verification",icon:"loader",color:"text-amber-400",bg:"bg-amber-500/15"},{id:"approved",label:"Approved",icon:"check-circle",color:"text-emerald-400",bg:"bg-emerald-500/15"},{id:"processing",label:"Processing",icon:"package",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"shipped",label:"Shipped",icon:"truck",color:"text-indigo-400",bg:"bg-indigo-500/15"},{id:"delivered",label:"Delivered",icon:"package-check",color:"text-emerald-400",bg:"bg-emerald-500/15"}];function se(){return new URLSearchParams(window.location.search).get("id")}function ne(){return localStorage.getItem("kco_country")||"US"}function de(){const e=Date.now().toString(36).toUpperCase().slice(-6),l=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${e}${l}`}function U(e,l){const t=()=>{const r=document.createElement("textarea");r.value=e,r.style.position="fixed",r.style.opacity="0",document.body.appendChild(r),r.select();try{document.execCommand("copy")}catch{}document.body.removeChild(r)};if(navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),l){const r=l.innerHTML;l.innerHTML='<i data-lucide="check" class="w-3.5 h-3.5 text-emerald-400"></i>',window.lucide&&lucide.createIcons(),setTimeout(()=>{l.innerHTML=r,window.lucide&&lucide.createIcons()},1500)}G("Copied Successfully.")}function G(e){let l=document.getElementById("payment-toast");l||(l=document.createElement("div"),l.id="payment-toast",l.className="fixed bottom-5 right-5 z-[100] transform translate-y-20 opacity-0 bg-gray-900 border border-blue-500/30 text-white px-5 py-3 rounded-xl shadow-xl text-xs flex items-center gap-2 font-medium transition-all duration-300",l.innerHTML='<i data-lucide="info" class="w-4 h-4 text-blue-400"></i><span id="payment-toast-msg">Action</span>',document.body.appendChild(l)),l.querySelector("#payment-toast-msg").textContent=e,l.classList.remove("translate-y-20","opacity-0"),clearTimeout(l._t),l._t=setTimeout(()=>l.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function J(e,l){const t=ie(k,e,l);return{...t,fallbackNotice:t.isFallback?re(t.account,e,l,C):null}}function ce(){const e=document.getElementById("particles");if(e)for(let l=0;l<14;l++){const t=document.createElement("div"),r=Math.random()*3+1;t.className="particle",t.style.width=r+"px",t.style.height=r+"px",t.style.left=Math.random()*100+"%",t.style.bottom="-10px",t.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",t.style.animationDuration=Math.random()*20+15+"s",t.style.animationDelay=Math.random()*20+"s",e.appendChild(t)}}ce();function ue(e,l,t){const r=T(e);return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="flex gap-4">
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-2xl font-black text-blue-400 mt-2">${r}</p>
        </div>
      </div>
    </div>
  `}function A(e,l,t){const r=[{label:"Country",value:e.country},{label:"Bank Name",value:e.bankName},{label:"Transfer Type",value:e.transferType},{label:"Beneficiary Name",value:e.beneficiary},{label:"Account Number",value:e.accountNumber},{label:"Account Type",value:e.accountType},{label:"IBAN",value:e.iban},{label:"SWIFT / BIC Code",value:e.swift},{label:"Routing (ABA)",value:e.routing},{label:"Sort Code",value:e.sortCode},{label:"Bank Code",value:e.bankCode},{label:"Branch Code",value:e.branchCode},{label:"Institution Number",value:e.institutionNumber},{label:"Transit Number",value:e.transitNumber},{label:"BSB Code",value:e.bsbCode},{label:"Bank Address",value:e.address}].filter(a=>a.value&&a.value.trim()!==""),o=r.map(a=>({label:a.label,value:a.value}));return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
  ...
  `}function pe(e,l,t){const r=H(k),o=t?q(t):null;return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${r.map(a=>{const n=k.find(u=>u.currency===a);return`
            <button onclick="selectCurrency('${a}')" class="btn-press flex flex-col items-center gap-1 p-3 rounded-xl border transition relative overflow-hidden ${a===e?"bg-blue-500/15 border-blue-500/50 text-blue-400 pulse-glow":"bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30 hover:text-white"}">
  ...
  `}function Y(e){const l=_.findIndex(t=>t.id===e);return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="relative">
        ...
        <div class="space-y-4">
          ${_.map((t,r)=>{const o=r<=l,a=r===l;return`
              <div class="flex items-center gap-3 relative">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-300 ${o?t.bg+" border border-blue-500/30":"bg-blue-950/40 border border-blue-500/10"} ${a?"pulse-glow":""}">
                  <i data-lucide="${t.icon}" class="w-4 h-4 ${o?t.color:"text-gray-600"} ${a?"animate-pulse":""}"></i>
                </div>
                <div class="flex-1">
                  <div class="text-sm font-medium ${o?"text-white":"text-gray-600"}">${t.label}</div>
                </div>
                ${o&&!a?'<i data-lucide="check" class="w-4 h-4 text-emerald-400 shrink-0"></i>':""}
                ${a?'<span class="text-[10px] text-blue-400 font-bold uppercase shrink-0">Current</span>':""}
              </div>
            `}).join("")}
        </div>
      </div>
    </div>
  `}function xe(e,l,t,r,o){const a=o?`
      ...
      <form id="receipt-form" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" id="form-currency-display" value="${r}" disabled class="w-full bg-[#0a1124]/80 border border-blue-500/10 rounded-xl px-4 py-2.5 text-sm text-white font-mono">
          </div>
          <div>
  ...
  `}function fe(e,l,t,r){const o=T(l);return`
    <div class="fade-in text-center py-8">
      ...
      <div class="glass border border-blue-500/20 rounded-2xl p-5 max-w-md mx-auto mb-5 text-left">
        ...
        <div class="flex justify-between text-sm mb-4"><span class="text-gray-500">Currency</span><span class="text-white font-bold">${r}</span></div>
  ...
  `}async function ge(){const e=document.getElementById("payment-content"),t=new URLSearchParams(window.location.search).get("guest")==="1",r=t?null:await ee();if(!r&&!t){window.location.href="/";return}const o=se();let a=R(o);if(!a){const[{generateListingById:w},{loadHiddenCatalogIds:i}]=await Promise.all([O(()=>import("./catalog-DZE9rOFe.js"),__vite__mapDeps([0,1,2])),O(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await i(),a=w(o)}if(a||(await Z(),a=R(o)),!a){e.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}T(a);const n=a.listing_type==="property",y=a.images?.[0]||z;let u=ne();if(r&&!t){const{data:w}=await L.from("profiles").select("country_code").eq("user_id",r.id).maybeSingle();w?.country_code&&(u=w.country_code,localStorage.setItem("kco_country",u))}const $=q(u),E=$?$.name:u,p=X(u),g=new URLSearchParams(window.location.search).get("order")||de(),h=a.price;P=await te(),k=le(P),C=ae(P),S=p||"";let f=S||"USD";H(k).includes(f)||(f="USD");const v=J(u,S||"");e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="currency-selector-container">${pe(f,E,u)}</div>

      <div id="bank-account-container">${v.isFallback?V(v.fallbackNotice):A(v.account,null,C)}</div>

      <div id="upload-form-container">${xe(g,a,h,f,t)}</div>
  ...
  `,window.lucide&&lucide.createIcons(),ve(a,h,g,r,t)}function ve(e,l,t,r,o){document.querySelectorAll(".btn-press").forEach(i=>{i.addEventListener("click",function(d){if(this.disabled)return;const s=this.getBoundingClientRect(),c=document.createElement("span");c.className="ripple";const b=Math.max(s.width,s.height);c.style.width=c.style.height=b+"px",c.style.left=d.clientX-s.left-b/2+"px",c.style.top=d.clientY-s.top-b/2+"px",this.appendChild(c),setTimeout(()=>c.remove(),600)})}),window.selectCurrency=i=>{const d=document.getElementById("bank-account-container"),s=J(countryCode,i);d.innerHTML=s.isFallback?V(s.fallbackNotice):A(s.account,null,C),document.querySelectorAll("#currency-selector-container button").forEach(m=>{m.getAttribute("onclick")?.includes(`'${i}'`)?m.className=m.className.replace("bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30 hover:text-white","bg-blue-500/15 border-blue-500/50 text-blue-400 pulse-glow"):m.className=m.className.replace("bg-blue-500/15 border-blue-500/50 text-blue-400 pulse-glow","bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30 hover:text-white")});const c=document.getElementById("form-currency-display");c&&(c.value=i);const b=document.getElementById("form-currency");b&&(b.value=i),window.lucide&&lucide.createIcons()},window.copyAllDetails=i=>{const d=i.map(s=>`${s.label}: ${s.value}`).join(`
`);U(d)},window.copyToClipboard=U;const a=document.getElementById("file-drop-zone"),n=document.getElementById("form-receipt-file"),y=document.getElementById("file-prompt"),u=document.getElementById("file-info"),$=document.getElementById("file-name-display"),E=document.getElementById("file-size-display"),p=document.getElementById("file-error");window.removeReceiptFile=()=>{n.value="",y.classList.remove("hidden"),u.classList.add("hidden"),p.classList.add("hidden"),g.classList.add("hidden")};const N=document.getElementById("file-preview-img"),g=document.getElementById("file-preview-container"),h=i=>{if(p.classList.add("hidden"),!i)return;const d=["image/jpeg","image/jpg","image/png","image/webp","application/pdf"],s=i.name.split(".").pop()?.toLowerCase(),c=["jpg","jpeg","png","webp","pdf"].includes(s);if(!d.includes(i.type)||!c){p.textContent="Please upload a JPG, JPEG, PNG, WEBP, or PDF file.",p.classList.remove("hidden");return}if(i.size>20*1024*1024){p.textContent="File size must be 20 MB or less.",p.classList.remove("hidden");return}if($.textContent=i.name,E.textContent=(i.size/1024/1024).toFixed(2)+" MB",y.classList.add("hidden"),u.classList.remove("hidden"),i.type.startsWith("image/")){const b=new FileReader;b.onload=m=>{N.src=m.target.result,g.classList.remove("hidden")},b.readAsDataURL(i)}else g.classList.add("hidden");window.lucide&&lucide.createIcons()},f=document.getElementById("btn-take-photo"),v=document.getElementById("btn-choose-file");a.addEventListener("click",i=>{i.target.closest("#btn-take-photo")||i.target.closest("#btn-choose-file")||i.target.closest("#file-info")||n.click()}),f&&f.addEventListener("click",i=>{i.stopPropagation(),n.setAttribute("capture","environment"),n.click()}),v&&v.addEventListener("click",i=>{i.stopPropagation(),n.removeAttribute("capture"),n.click()}),n.addEventListener("change",i=>h(i.target.files[0])),a.addEventListener("dragover",i=>{i.preventDefault(),a.classList.add("border-blue-500/50","bg-blue-500/5")}),a.addEventListener("dragleave",()=>{a.classList.remove("border-blue-500/50","bg-blue-500/5")}),a.addEventListener("drop",i=>{if(i.preventDefault(),a.classList.remove("border-blue-500/50","bg-blue-500/5"),i.dataTransfer.files.length){const d=new DataTransfer;d.items.add(i.dataTransfer.files[0]),n.files=d.files,h(i.dataTransfer.files[0])}}),document.getElementById("receipt-form").addEventListener("submit",async i=>{i.preventDefault();const d=document.getElementById("submit-receipt-btn"),s=n.files[0];if(!s){p.textContent="Please upload your payment receipt.",p.classList.remove("hidden");return}d.disabled=!0,d.innerHTML='<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Submitting...',window.lucide&&lucide.createIcons();const c=document.getElementById("upload-progress"),b=document.getElementById("upload-progress-bar"),m=document.getElementById("upload-progress-text");c.classList.remove("hidden");try{const I=s.name.split(".").pop(),M=`${o?"guest":r.id}/${t}-${Date.now()}.${I}`,{error:D}=await L.storage.from("payment-receipts").upload(M,s,{onUploadProgress:B=>{const F=Math.round(B.loaded/B.total*100);b.style.width=F+"%",m.textContent=`Uploading receipt... ${F}%`}});if(D)throw new Error("Failed to upload receipt: "+D.message);b.style.width="100%",m.textContent="Saving payment record...";const x={order_number:t,listing_id:document.getElementById("form-listing-id").value,listing_title:document.getElementById("form-listing-title").value,amount:parseFloat(document.getElementById("form-amount-paid").value),currency:document.getElementById("form-currency").value,full_name:document.getElementById("form-full-name").value,email:document.getElementById("form-email").value,phone:document.getElementById("form-phone").value,payment_date:document.getElementById("form-payment-date").value,transaction_reference:document.getElementById("form-tx-ref").value,receipt_file_path:M,receipt_file_name:s.name,additional_notes:document.getElementById("form-notes").value||null,status:"pending_verification"};o?(x.is_guest=!0,x.user_id=null,x.guest_shipping_address=document.getElementById("form-shipping-address")?.value||null,x.guest_country=document.getElementById("form-guest-country")?.value||null,x.guest_state=document.getElementById("form-guest-state")?.value||null,x.guest_city=document.getElementById("form-guest-city")?.value||null,x.guest_postal_code=document.getElementById("form-guest-postal")?.value||null):x.user_id=r.id;const{error:j}=await L.from("payment_receipts").insert(x);if(j)throw new Error("Failed to save payment: "+j.message);try{fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/send-order-notification",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({order_number:t})}).catch(()=>{})}catch{}const K=document.getElementById("payment-content"),W=e;K.innerHTML=fe(t,W,parseFloat(document.getElementById("form-amount-paid").value),document.getElementById("form-currency").value),window.lucide&&lucide.createIcons(),G("Payment receipt submitted successfully.")}catch(I){c.classList.add("hidden"),d.disabled=!1,d.innerHTML='<i data-lucide="send" class="w-5 h-5"></i> Submit Payment',window.lucide&&lucide.createIcons(),p.textContent=I.message||"Something went wrong. Please try again.",p.classList.remove("hidden")}})}ge();
