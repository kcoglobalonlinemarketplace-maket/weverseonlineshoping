import{h as H,a as L,S as E,c as T,b as B,d as I,e as y}from"./showroom-data-A4bpNOne.js";import{P as v,a as _,T as M,M as R,f as V}from"./motorhome-data-CupbOvk0.js";import{isCatalogListingHidden as b}from"./catalog-hidden-store-Br-Df_3h.js";const C="kco-hero-rows",O="/fallback.svg",F=new Set(["Cars","Cars & Vehicles","Trucks","Buses","Buses & Coaches","Motorhomes","Motorcycles","Marine & Boating","RV & Camper Accessories","Vehicles"]),m=new Set(["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"]),j=["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"];function i(e,o){return e.specifications&&typeof e.specifications=="object"&&e.specifications[o]!=null?e.specifications[o]:e[o]}function k(e){return e.listing_type==="vehicle"||F.has(e.category)}function A(e){const o=new Set,t=[];for(const r of e){if(!r)continue;const c=r.property_id||r.id;!c||o.has(c)||(o.add(c),t.push(r))}return t}function S(e){return A(e).filter(o=>b&&b(o.property_id)?!1:o.property_id&&String(o.property_id).startsWith("W")?!0:o.is_active!==!1).sort((o,t)=>{const r=Number.isFinite(parseFloat(o.price))?parseFloat(o.price):0;return(Number.isFinite(parseFloat(t.price))?parseFloat(t.price):0)-r})}function D(e){const o=e.property_type||e.subcategory||e.category;if(o&&!l.has(o)){const t=String(o).toLowerCase();for(const r of j)if(t===r.toLowerCase()||t.includes(r.toLowerCase())){l.set(o,r);break}l.has(o)||l.set(o,String(o))}return o?l.get(o):"Homes"}const l=new Map;function q(){const o=(y()||[]).filter(r=>r.listing_type==="property"||r.category==="Real Estate"||r.category==="Houses & Real Estate"||m.has(r.property_type)||m.has(r.subcategory)),t=[...v,..._,...E].filter(r=>r&&(r.category==="Houses & Real Estate"||r.category==="Real Estate"||m.has(r.property_type||r.subcategory)));return S([...o,...t])}function X(){const o=(y()||[]).filter(r=>k(r)),t=[...M,...R,...v,..._].filter(r=>r&&k(r));return S([...o,...t])}function a(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function P(){if(document.getElementById("kco-hero-styles"))return;const e=document.createElement("style");e.id="kco-hero-styles",e.textContent=`
...
`,document.head.appendChild(e)}function J(e,o,t){T(e);const r=t,c=e.images&&e.images[0]||O,f=N(e.property_id||e.id),u=o==="vehicle"&&e.category==="Trucks"?V(e):B(e);let s="",p="";if(o==="house"){const $=I(e.country_code),g=[e.city,e.state].filter(Boolean).join(", ")||e.country||"";g&&(p=`<span class="kco-hero-loc"><i data-lucide="map-pin" class="w-3 h-3 shrink-0"></i>${a($+" "+g)}</span>`);const d=[];i(e,"bedrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bed-double" class="w-3.5 h-3.5"></i>${a(i(e,"bedrooms"))} Beds</span>`),i(e,"bathrooms")!=null&&d.push(`<span class="kco-hero-chip"><i data-lucide="bath" class="w-3.5 h-3.5"></i>${a(i(e,"bathrooms"))} Baths</span>`),e.land_size&&d.push(`<span class="kco-hero-chip"><i data-lucide="ruler" class="w-3.5 h-3.5"></i>${a(e.land_size)}</span>`),i(e,"building_size")&&d.push(`<span class="kco-hero-chip"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>${a(i(e,"building_size"))}</span>`),i(e,"year_built")&&d.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${a(i(e,"year_built"))}</span>`);const z=D(e);return s=d.join(""),p||(p=""),`
      <a href="/product/${f}" class="${r}">
        <div class="kco-hero-media">
          <img src="${a(c)}" alt="${a(e.title||"")}" loading="lazy">
          <span class="kco-hero-type"><i data-lucide="home" class="w-3 h-3"></i>${a(z)}</span>
          ${p}
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${u}</b><span>${e.listing_status==="rent"?"per month · for rent":"for sale"}</span></div>
      ...
      <a href="/product/${f}" class="${r}">
        <div class="kco-hero-media">
          <img src="${a(c)}" alt="${a(e.title||"")}" loading="lazy">
          <span class="kco-hero-type"><i data-lucide="car-front" class="w-3 h-3"></i>${a(G(e))}</span>
        </div>
        <div class="kco-hero-body">
          <div class="kco-hero-price"><b>${u}</b><span>· ${a(i(e,"condition")||"ready")}</span></div>
          <p class="kco-hero-title">${a(e.title||"")}</p>
          ${s?`<div class="kco-hero-chips">${s}</div>`:""}
          <span class="kco-hero-btn">View Vehicle <i data-lucide="arrow-right" class="w-4 h-4"></i></span>
        </div>
      </a>`}function N(e){return String(e??"").replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function G(e){const o=String(e.category||""),t={Cars:"Car","Cars & Vehicles":"Car",Trucks:"Truck",Buses:"Bus","Buses & Coaches":"Bus",Motorhomes:"Motorhome / RV",Motorcycles:"Motorcycle","Marine & Boating":"Boat / Marine"};return t[o]?t[o]:i(e,"body_type")||o||"Vehicle"}function h(){const e=document.getElementById(C);if(!e)return;P();const o=document.createDocumentFragment();e.replaceChildren(o),window.lucide&&lucide.createIcons()}let w=!1;function x(){w||(w=!0,document.getElementById(C)&&(H(),h(),L().then(()=>h()).catch(()=>{}),window.addEventListener("showroom-categories-ready",()=>h()),window.addEventListener("kco-db-refresh",()=>h())))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",x):x();export{X as a,D as b,J as c,q as g,P as h,G as v};
