...
import"./modulepreload-polyfill-B5Qt9EMX.js";import{l as te,f as H,_ as D,v as ae,d as le}from"./showroom-data-C8s3txE5.js";import{P as re,a as ie,g as oe,b as se}from"./motorhome-data-CupbOvk0.js";import{g as ne,a as de}from"./phone-data-Of7KtnOV.js";import{getCurrentUser as ce}from"./auth-CtTDfG3h.js";import{supabase as S}from"./supabase-client-DSkAnHxH.js";import{g as V,d as ue}from"./country-data-BXM8T1RS.js";import{c as pe,d as be,a as me,b as J,r as ge,e as fe}from"./payment-settings-BZM-gsG8.js";import{p as xe,c as L,a as T}from"./fx-B0aevBpC.js";/* empty css                                       */const ye=[...re,...ie];function ve(e){return ye.find(l=>l.property_id===e)||null}let Y=null,A=[],M="",O="";const R=[{id:"placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-600",bg:"bg-blue-50"},{id:"submitted",label:"Payment Submitted",icon:"upload",color:"text-cyan-600",bg:"bg-cyan-50"},{id:"verification",label:"Pending Verification",icon:"loader",color:"text-amber-600",bg:"bg-amber-50"},{id:"approved",label:"Approved",icon:"check-circle",color:"text-emerald-600",bg:"bg-emerald-50"},{id:"processing",label:"Processing",icon:"package",color:"text-blue-600",bg:"bg-blue-50"},{id:"shipped",label:"Shipped",icon:"truck",color:"text-indigo-600",bg:"bg-indigo-50"},{id:"delivered",label:"Delivered",icon:"package-check",color:"text-emerald-600",bg:"bg-emerald-50"}];function he(){return new URLSearchParams(window.location.search).get("id")}function we(){return localStorage.getItem("kco_country")||"US"}function ke(){const e=Date.now().toString(36).toUpperCase().slice(-6),l=Math.random().toString(36).toUpperCase().slice(2,6);return`W-${e}${l}`}function G(e,l){const t=()=>{const r=document.createElement("textarea");r.value=e,r.style.position="fixed",r.style.opacity="0",document.body.appendChild(r),r.select();try{document.execCommand("copy")}catch{}document.body.removeChild(r)};if(navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),l){const r=l.innerHTML;l.innerHTML='<i data-lucide="check" class="w-3.5 h-3.5 text-emerald-600"></i>',window.lucide&&lucide.createIcons(),setTimeout(()=>{l.innerHTML=r,window.lucide&&lucide.createIcons()},1500)}W("Copied Successfully.")}function W(e){let l=document.getElementById("payment-toast");l||(l=document.createElement("div"),l.id="payment-toast",l.className="fixed bottom-5 right-5 z-[100] transform translate-y-20 opacity-0 bg-gray-900 border border-blue-500/30 text-white px-5 py-3 rounded-xl shadow-xl text-xs flex items-center gap-2 font-medium transition-all duration-300",l.innerHTML='<i data-lucide="info" class="w-4 h-4 text-blue-600"></i><span id="payment-toast-msg">Action</span>',document.body.appendChild(l)),l.querySelector("#payment-toast-msg").textContent=e,l.classList.remove("translate-y-20","opacity-0"),clearTimeout(l._t),l._t=setTimeout(()=>l.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function X(e,l){const t=ge(A,e,l);return{...t,fallbackNotice:t.isFallback?fe(t.account,e,l,M):null}}function Ie(){const e=document.getElementById("particles");if(e)for(let l=0;l<14;l++){const t=document.createElement("div"),r=Math.random()*3+1;t.className="particle",t.style.width=r+"px",t.style.height=r+"px",t.style.left=Math.random()*100+"%",t.style.bottom="-10px",t.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",t.style.animationDuration=Math.random()*20+15+"s",t.style.animationDelay=Math.random()*20+"s",e.appendChild(t)}}Ie();function $e(e,l,t){const r=T(L(e.price,t),t);return`
    <div class="glass border border-blue-200 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="flex gap-4">
        <div class="w-24 h-24 rounded-xl overflow-hidden bg-gray-50 shrink-0 ring-1 ring-blue-500/10">
          ${(()=>{const i=ae(e);return i?`<video src="${i}" class="w-full h-full object-cover" muted playsinline preload="metadata"></video>`:'<div class="w-full h-full flex items-center justify-center bg-gray-50"><i data-lucide="video-off" class="w-6 h-6 text-gray-300"></i></div>'})()}
        </div>
...
`),v=r.amountLabel||"",$=r.orderNumber||"";return`
    <div class="glass border border-blue-200 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="bg-blue-600 text-white rounded-2xl p-4 mb-4 overflow-hidden relative">
        ...
        <div class="relative">
          ...
          <div class="text-2xl font-bold mt-0.5" id="bank-amount">${v}</div>
          <div class="mt-3 flex items-center justify-between gap-2 flex-wrap">
            <div>
              <div class="text-[10px] uppercase tracking-widest text-blue-100">Your order/reference number</div>
              <div class="text-sm font-bold font-mono mt-0.5" id="bank-ref">${$}</div>
            </div>
            <button onclick="copyToClipboard('${$}')" class="shrink-0 flex items-center gap-1.5 bg-white/20 hover:bg-white/30 rounded-lg px-3 py-1.5 text-xs font-bold transition">
  ...
  `}function Be(e,l,t){const r=J(A),i=t?V(t):null;return`
    <div class="glass border border-blue-200 rounded-2xl p-5 mb-5 slide-up">
      <div class="flex items-center gap-3 mb-4">
        ...
        <div class="flex-1">
          ...
          <p class="text-gray-500 text-xs">${i?i.flag+" "+l:l||"Select currency"} ${e?"→ "+e:"→ USD (default)"}</p>
        </div>
      </div>
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${r.map(o=>{const s=A.find(v=>v.currency===o);return`
            <button onclick="selectCurrency('${o}')" class="btn-press flex flex-col items-center gap-1 p-3 rounded-xl border transition relative overflow-hidden ${o===e?"bg-blue-50 border-blue-300 text-blue-600 pulse-glow":"bg-gray-50 border-blue-100 text-gray-600 hover:border-blue-200 hover:text-gray-900"}">
  ...
  `}function Z(e){const l=R.findIndex(t=>t.id===e);return`
    <div class="glass border border-blue-200 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="relative">
        ...
        <div class="absolute left-4 top-4 w-0.5 bg-blue-500 transition-all duration-500" style="height: ${l>=0?l/(R.length-1)*100:0}%; min-height: 0; max-height: calc(100% - 2rem)"></div>
        <div class="space-y-4">
          ${R.map((t,r)=>{const i=r<=l,o=r===l;return`
              <div class="flex items-center gap-3 relative">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-300 ${i?t.bg+" border border-blue-200":"bg-gray-50 border border-blue-100"} ${o?"pulse-glow":""}">
                  <i data-lucide="${t.icon}" class="w-4 h-4 ${i?t.color:"text-gray-600"} ${o?"animate-pulse":""}"></i>
                </div>
                <div class="flex-1">
                  <div class="text-sm font-medium ${i?"text-gray-900":"text-gray-600"}">${t.label}</div>
                </div>
                ${i&&!o?'<i data-lucide="check" class="w-4 h-4 text-emerald-600 shrink-0"></i>':""}
                ${o?'<span class="text-[10px] text-blue-600 font-bold uppercase shrink-0">Current</span>':""}
              </div>
            `}).join("")}
        </div>
      </div>
    </div>
  `}function Ee(e,l,t,r,i,o=""){const s=i?`
          ...
          <div class="relative">
            <div class="flex items-center justify-between gap-3 flex-wrap">
              <div>
                ...
                <div class="text-2xl font-bold mt-0.5" id="transfer-amount-display">${o}</div>
              </div>
  ...
  `}function Pe(e,l,t,r){const i=T(L(l.price,r),r);return`
  ...
  `}async function _e(){const e=document.getElementById("payment-content"),t=new URLSearchParams(window.location.search).get("guest")==="1",r=t?null:await ce();if(!r&&!t){window.location.href="/";return}const i=he();let o=await te(i)||H(i)||oe(i)||se(i)||ne(i)||de(i)||ve(i);if(!o){const[{generateListingById:I},{loadHiddenCatalogIds:N}]=await Promise.all([D(()=>import("./catalog-pfF1HJ-x.js"),__vite__mapDeps([0,1,2])),D(()=>import("./catalog-hidden-store-Ci-G2dxV.js"),__vite__mapDeps([1,2]))]);await N(),o=I(i)}if(o||(o=H(i)),!o){e.innerHTML=`
      ...
      </div>`,window.lucide&&lucide.createIcons();return}const s=o.listing_type==="property";let p=we();if(r&&!t){const{data:I}=await S.from("profiles").select("country_code").eq("user_id",r.id).maybeSingle();I?.country_code&&(p=I.country_code,localStorage.setItem("kco_country",p))}const v=V(p),$=v?v.name:p,b=ue(p),P=new URLSearchParams(window.location.search).get("order")||ke(),B=o.price;Y=await pe(),A=await be(),M=me(Y),await xe(),O=b||"";let m=O||"USD";J(A).includes(m)||(m="USD");const C=X(p,O||""),E=L(B,m);e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="currency-selector-container">${Be(m,$,p)}</div>

      <div id="bank-account-container">${C.isFallback?K(C.fallbackNotice):U(C.account,null,M,{amountLabel:T(E,m),orderNumber:P})}</div>

      <div id="upload-form-container">${Ee(P,o,E,m,t,T(E,m))}</div>
  ...
  `,window.lucide&&lucide.createIcons(),Se(o,B,P,r,t)}function Se(e,l,t,r,i){document.querySelectorAll(".btn-press").forEach(a=>{a.addEventListener("click",function(n){if(this.disabled)return;const d=this.getBoundingClientRect(),c=document.createElement("span");c.className="ripple";const u=Math.max(d.width,d.height);c.style.width=c.style.height=u+"px",c.style.left=n.clientX-d.left-u/2+"px",c.style.top=n.clientY-d.top-u/2+"px",this.appendChild(c),setTimeout(()=>c.remove(),600)})}),window.selectCurrency=a=>{const n=document.getElementById("bank-account-container"),d=X(countryCode,a);n.innerHTML=d.isFallback?K(d.fallbackNotice):U(d.account,null,M),document.querySelectorAll("#currency-selector-container button").forEach(x=>{x.getAttribute("onclick")?.includes(`'${a}'`)?x.className=x.className.replace("bg-gray-50 border-blue-100 text-gray-600 hover:border-blue-200 hover:text-gray-900","bg-blue-50 border-blue-300 text-blue-600 pulse-glow"):x.className=x.className.replace("bg-blue-50 border-blue-300 text-blue-600 pulse-glow","bg-gray-50 border-blue-100 text-gray-600 hover:border-blue-200 hover:text-gray-900")});const c=document.getElementById("form-currency-display");c&&(c.value=a);const u=document.getElementById("form-currency");u&&(u.value=a);const g=document.getElementById("form-amount-paid");g&&(g.value=L(e.price,a));const w=T(L(e.price,a),a),_=document.getElementById("bank-amount");_&&(_.textContent=w);const h=document.getElementById("transfer-amount-display");h&&(h.textContent=w),window.lucide&&lucide.createIcons()},window.copyAllDetails=a=>{const n=a.map(d=>`${d.label}: ${d.value}`).join(`
`);G(n)},window.copyToClipboard=G;const o=document.getElementById("file-drop-zone"),s=document.getElementById("form-receipt-file"),p=document.getElementById("file-prompt"),v=document.getElementById("file-info"),$=document.getElementById("file-name-display"),b=document.getElementById("file-size-display"),f=document.getElementById("file-error");window.removeReceiptFile=()=>{s.value="",p.classList.remove("hidden"),v.classList.add("hidden"),f.classList.add("hidden"),B.classList.add("hidden")};const P=document.getElementById("file-preview-img"),B=document.getElementById("file-preview-container"),m=a=>{if(f.classList.add("hidden"),!a)return;const n=["image/jpeg","image/jpg","image/png","image/webp","application/pdf"],d=a.name.split(".").pop()?.toLowerCase(),c=["jpg","jpeg","png","webp","pdf"].includes(d);if(!n.includes(a.type)||!c){f.textContent="Please upload a JPG, JPEG, PNG, WEBP, or PDF file.",f.classList.remove("hidden");return}if(a.size>20*1024*1024){f.textContent="File size must be 20 MB or less.",f.classList.remove("hidden");return}if($.textContent=a.name,b.textContent=(a.size/1024/1024).toFixed(2)+" MB",p.classList.add("hidden"),v.classList.remove("hidden"),a.type.startsWith("image/")){const u=new FileReader;u.onload=g=>{P.src=g.target.result,B.classList.remove("hidden")},u.readAsDataURL(a)}else B.classList.add("hidden");window.lucide&&lucide.createIcons()},C=document.getElementById("btn-open-gallery"),E=document.getElementById("btn-take-photo"),I=document.getElementById("btn-choose-pdf"),N=!!(window.Capacitor&&window.Capacitor.isNativePlatform&&window.Capacitor.isNativePlatform()),q=async a=>{if(a&&a.stopPropagation(),N){try{const{Camera:n,MediaTypeSelection:d}=await D(async()=>{const{Camera:h,MediaTypeSelection:x}=await import("@capacitor/camera");return{Camera:h,MediaTypeSelection:x}},[]),{results:c}=await n.chooseFromGallery({mediaType:d.Images,allowMultipleSelection:!1}),u=c&&c[0];if(!u||!u.webPath)return;const g=await fetch(u.webPath).then(h=>h.blob()),w=((u.metadata&&u.metadata.format||"jpg")+"").toLowerCase().replace(/^jpeg$/,"jpg"),_=new File([g],`receipt-${Date.now()}.${w}`,{type:g.type||"image/jpeg"});m(_)}catch(n){console.warn("Native gallery picker unavailable:",n),s.setAttribute("accept","image/*"),s.removeAttribute("capture"),s.click()}return}s.setAttribute("accept","image/*,.jpg,.jpeg,.png,.webp"),s.removeAttribute("capture"),s.click()};o.addEventListener("click",a=>{a.target.closest("#btn-open-gallery")||a.target.closest("#btn-take-photo")||a.target.closest("#btn-choose-pdf")||a.target.closest("#file-info")||q(a)}),C&&C.addEventListener("click",a=>q(a)),E&&E.addEventListener("click",a=>{a.stopPropagation(),N?D(()=>import("@capacitor/camera"),[]).then(async({Camera:n,CameraSource:d})=>{const c=await n.getPhoto({source:d.Camera,quality:90,allowEditing:!1});if(c&&c.webPath){const u=await fetch(c.webPath).then(g=>g.blob());m(new File([u],`receipt-${Date.now()}.jpg`,{type:u.type||"image/jpeg"}))}}).catch(()=>{s.setAttribute("capture","environment"),s.setAttribute("accept","image/*"),s.click()}):(s.setAttribute("capture","environment"),s.setAttribute("accept","image/*"),s.click())}),I&&I.addEventListener("click",a=>{a.stopPropagation(),s.setAttribute("accept",".pdf,application/pdf"),s.removeAttribute("capture"),s.click()}),s.addEventListener("change",a=>m(a.target.files[0])),o.addEventListener("dragover",a=>{a.preventDefault(),o.classList.add("border-blue-300","bg-blue-50")}),o.addEventListener("dragleave",()=>{o.classList.remove("border-blue-300","bg-blue-50")}),o.addEventListener("drop",a=>{if(a.preventDefault(),o.classList.remove("border-blue-300","bg-blue-50"),a.dataTransfer.files.length){const n=new DataTransfer;n.items.add(a.dataTransfer.files[0]),s.files=n.files,m(a.dataTransfer.files[0])}}),document.getElementById("receipt-form").addEventListener("submit",async a=>{a.preventDefault();const n=document.getElementById("submit-receipt-btn"),d=s.files[0];if(!d){f.textContent="Please upload your payment receipt.",f.classList.remove("hidden");return}n.disabled=!0,n.innerHTML='<i data-lucide="loader-2" class="w-5 h-5 animate-spin"></i> Submitting...',window.lucide&&lucide.createIcons();const c=document.getElementById("upload-progress"),u=document.getElementById("upload-progress-bar"),g=document.getElementById("upload-progress-text");c.classList.remove("hidden");try{const w=d.name.split(".").pop(),h=`${i?"guest":r.id}/${t}-${Date.now()}.${w}`,{error:x}=await S.storage.from("payment-receipts").upload(h,d,{onUploadProgress:k=>{const z=Math.round(k.loaded/k.total*100);u.style.width=z+"%",g.textContent=`Uploading receipt... ${z}%`}});if(x)throw new Error("Failed to upload receipt: "+x.message);u.style.width="100%",g.textContent="Saving payment record...";const y={order_number:t,listing_id:document.getElementById("form-listing-id").value,listing_title:document.getElementById("form-listing-title").value,amount:parseFloat(document.getElementById("form-amount-paid").value),currency:document.getElementById("form-currency").value,full_name:document.getElementById("form-full-name").value,email:document.getElementById("form-email").value,phone:document.getElementById("form-phone").value,payment_date:document.getElementById("form-payment-date").value,transaction_reference:document.getElementById("form-tx-ref").value,receipt_file_path:h,receipt_file_name:d.name,additional_notes:document.getElementById("form-notes").value||null,status:"pending_verification"};i?(y.is_guest=!0,y.user_id=null,y.guest_shipping_address=document.getElementById("form-shipping-address")?.value||null,y.guest_country=document.getElementById("form-guest-country")?.value||null,y.guest_state=document.getElementById("form-guest-state")?.value||null,y.guest_city=document.getElementById("form-guest-city")?.value||null,y.guest_postal_code=document.getElementById("form-guest-postal")?.value||null):y.user_id=r.id;const{data:F}=await S.from("payment_receipts").select("id").eq("order_number",t).limit(1);let j=null;if(F&&F.length){const{error:k}=await S.from("payment_receipts").update(y).eq("id",F[0].id);j=k||null}else{const{error:k}=await S.from("payment_receipts").insert(y);j=k||null}if(j)throw new Error("Failed to save payment: "+j.message);try{fetch("https://mzgrjwvwzgqgivwmlkno.supabase.co/functions/v1/send-order-notification",{method:"POST",headers:{Authorization:"Bearer sb_publishable_SqmJ1R-a-_0CYzuVbS1c7w_FSSIfEsX","Content-Type":"application/json"},body:JSON.stringify({order_number:t})}).catch(()=>{})}catch{}const Q=document.getElementById("payment-content"),ee=e;Q.innerHTML=Pe(t,ee,parseFloat(document.getElementById("form-amount-paid").value),document.getElementById("form-currency").value),window.lucide&&lucide.createIcons(),W("Payment receipt submitted successfully.")}catch(w){c.classList.add("hidden"),n.disabled=!1,n.innerHTML='<i data-lucide="send" class="w-5 h-5"></i> Submit Payment',window.lucide&&lucide.createIcons(),f.textContent=w.message||"Something went wrong. Please try again.",f.classList.remove("hidden")}})}_e();
