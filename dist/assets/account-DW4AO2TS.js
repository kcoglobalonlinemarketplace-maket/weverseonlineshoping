import{supabase as i}from"./supabase-client-D1iyov-x.js";import{C as N,g as S}from"./localization-CwZlVC1D.js";import{g as W,s as V}from"./auth-C_Eb0opg.js";import"./native-bridge-MbplFvoR.js";import"./live-stream-mode-B-NuA9nR.js";import"./customer-ai-widget-ktHDNJ0t.js";import"./brand-_M2LrXmp.js";import"./preload-helper-CLcXU_4U.js";async function G(){return!1}async function z(){return await G(),null}const b="/fallback.svg",X=[{group:"Overview",items:[{id:"home",label:"Dashboard Home",icon:"layout-dashboard"},{id:"profile",label:"My Profile",icon:"user"},{id:"edit-profile",label:"Edit Profile",icon:"user-cog"},{id:"change-password",label:"Change Password",icon:"key-round"}]},{group:"Orders & Shopping",items:[{id:"orders",label:"My Orders",icon:"shopping-bag"},{id:"special-orders",label:"Special Orders",icon:"package-plus"},{id:"tracking",label:"Order Tracking",icon:"truck"},{id:"history",label:"Order History",icon:"history"},{id:"cart",label:"Shopping Cart",icon:"shopping-cart"},{id:"wishlist",label:"My Wishlist",icon:"heart"}]},{group:"Account",items:[{id:"notifications",label:"Notifications",icon:"bell"},{id:"messages",label:"Messages",icon:"mail"},{id:"payment-methods",label:"Payment Methods",icon:"credit-card"},{id:"payments",label:"Payment History",icon:"receipt"},{id:"addresses",label:"Shipping Addresses",icon:"map-pin"},{id:"receipts",label:"Download Receipts",icon:"download"}]},{group:"Support & Settings",items:[{id:"email-prefs",label:"Email Preferences",icon:"settings"},{id:"support",label:"Customer Support",icon:"headphones"},{id:"help",label:"Help Center",icon:"help-circle"},{id:"privacy",label:"Privacy & Security",icon:"shield-check"}]}],p=[{id:"order_placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"payment_received",label:"Payment Received",icon:"credit-card",color:"text-cyan-400",bg:"bg-cyan-500/15"},{id:"pending_verification",label:"Pending Verification",icon:"shield-alert",color:"text-amber-400",bg:"bg-amber-500/15"},{id:"payment_approved",label:"Approved",icon:"check-circle",color:"text-emerald-400",bg:"bg-emerald-500/15"},{id:"order_processing",label:"Processing",icon:"package",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"order_shipped",label:"Shipped",icon:"truck",color:"text-indigo-400",bg:"bg-indigo-500/15"},{id:"out_for_delivery",label:"Out for Delivery",icon:"bike",color:"text-orange-400",bg:"bg-orange-500/15"},{id:"order_delivered",label:"Delivered",icon:"package-check",color:"text-emerald-400",bg:"bg-emerald-500/15"}],Q={approved:"payment_approved",submitted:"payment_received",placed:"order_placed"};function w(e){return Q[e]||e}function J(e){const t=p.findIndex(r=>r.id===w(e));return t>=0?t:0}function g(e){return e?new Date(e).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"—"}function E(e){return e?new Date(e).toLocaleString("en-US",{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}function j(e){return(e||"?").slice(0,2).toUpperCase()}function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function n(e){const t=document.getElementById("toast");document.getElementById("toast-msg").textContent=e,t.classList.remove("translate-y-20","opacity-0"),clearTimeout(t._t),t._t=setTimeout(()=>t.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function K(e){const t=()=>{const r=document.createElement("textarea");r.value=e,document.body.appendChild(r),r.select();try{document.execCommand("copy")}catch{}document.body.removeChild(r)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),n("Copied to clipboard.")}function $(e){const t=w(e),r=p.find(u=>u.id===t)||p[0],a={"text-blue-400":"bg-blue-500/10 text-blue-400 border-blue-500/20","text-cyan-400":"bg-cyan-500/10 text-cyan-400 border-cyan-500/20","text-amber-400":"bg-amber-500/10 text-amber-400 border-amber-500/20","text-emerald-400":"bg-emerald-500/10 text-emerald-400 border-emerald-500/20","text-indigo-400":"bg-indigo-500/10 text-indigo-400 border-indigo-500/20","text-orange-400":"bg-orange-500/10 text-orange-400 border-orange-500/20","text-red-400":"bg-red-500/10 text-red-400 border-red-500/20"},l=a[r.color]||a["text-blue-400"],d=e==="rejected"?"Rejected":r.label;return`<span class="inline-flex items-center gap-1 ${l} border text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide">${d}</span>`}function B(e){e.addEventListener("click",function(t){if(this.disabled)return;const r=this.getBoundingClientRect(),a=document.createElement("span");a.className="ripple";const l=Math.max(r.width,r.height);a.style.width=a.style.height=l+"px",a.style.left=t.clientX-r.left-l/2+"px",a.style.top=t.clientY-r.top-l/2+"px",this.appendChild(a),setTimeout(()=>a.remove(),600)})}let s={user:null,profile:null,orders:[],events:{},notifications:[],addresses:[],emailPrefs:null,messages:[],paymentMethods:[],activeSection:"home"};function Z(){const e=document.getElementById("particles");if(e)for(let t=0;t<12;t++){const r=document.createElement("div"),a=Math.random()*3+1;r.className="particle",r.style.width=r.style.height=a+"px",r.style.left=Math.random()*100+"%",r.style.bottom="-10px",r.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",r.style.animationDuration=Math.random()*20+15+"s",r.style.animationDelay=Math.random()*20+"s",e.appendChild(r)}}Z();async function ee(){if(document.getElementById("dashboard-root"),s.user=await W(),!s.user){window.location.href="/auth.html?redirect=/account.html";return}await H(),await te(),await C(),await D(),await q(),await I(),await R(),re(),U(),Be(),m("home"),z().then(e=>{e&&console.log("Push notifications enabled")})}async function H(){const{data:e}=await i.from("profiles").select("*").eq("user_id",s.user.id).maybeSingle();s.profile=e}async function te(){const{data:e}=await i.from("payment_receipts").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});if(s.orders=e||[],s.orders.length){const t=s.orders.map(l=>l.order_number),{data:r}=await i.from("order_events").select("*").in("order_number",t).order("created_at",{ascending:!0});s.events={},(r||[]).forEach(l=>{(s.events[l.order_number]||=[]).push(l)});const{data:a}=await i.from("notification_log").select("*").in("order_number",t).order("created_at",{ascending:!1});s.notifications=a||[]}}async function C(){const{data:e}=await i.from("shipping_addresses").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.addresses=e||[]}async function D(){const{data:e}=await i.from("email_preferences").select("*").eq("user_id",s.user.id).maybeSingle();s.emailPrefs=e}async function q(){const{data:e}=await i.from("support_messages").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.messages=e||[]}async function I(){const{data:e}=await i.from("customer_payment_methods").select("*").eq("user_id",s.user.id).order("is_default",{ascending:!1}).order("created_at",{ascending:!1});s.paymentMethods=e||[]}async function R(){const{data:e}=await i.from("wishlist").select("id, listing_id, showroom_listings(property_id, title, price, currency, images, listing_type, is_active, approval_status)").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.wishlist=e||[]}function re(){const e=X.map(t=>`
    <div class="mb-3">
      <p class="text-[10px] font-bold uppercase tracking-wider text-gray-600 px-3 mb-1.5">${t.group}</p>
      ${t.items.map(r=>`
        <button data-section="${r.id}" class="nav-item w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-400 hover:text-white hover:bg-blue-500/5 rounded-xl transition border border-transparent ${s.activeSection===r.id?"active":""}">
          <i data-lucide="${r.icon}" class="nav-icon w-4 h-4 shrink-0 text-gray-500"></i>
          <span class="truncate">${r.label}</span>
        </button>
      `).join("")}
    </div>
  `).join("");document.getElementById("nav-menu").innerHTML=e,document.getElementById("mobile-nav-menu").innerHTML=e,document.querySelectorAll("[data-section]").forEach(t=>{t.addEventListener("click",()=>{const r=t.dataset.section;h(r)})}),window.lucide&&lucide.createIcons()}function h(e){s.activeSection=e,document.querySelectorAll("[data-section]").forEach(t=>{t.classList.toggle("active",t.dataset.section===e)}),m(e),M(),window.scrollTo({top:0,behavior:"smooth"})}function U(){const e=s.profile?.display_name||s.profile?.first_name||"Customer",t=s.user.email,r=s.profile?.avatar_url?`<img src="${s.profile.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:j(t);["sidebar-avatar","mobile-avatar"].forEach(a=>{const l=document.getElementById(a);l&&(l.innerHTML=r)}),["sidebar-name","mobile-name"].forEach(a=>{const l=document.getElementById(a);l&&(l.textContent=e)}),["sidebar-email","mobile-email"].forEach(a=>{const l=document.getElementById(a);l&&(l.textContent=t)})}async function m(e){const t=document.getElementById("dashboard-root");let r="";switch(e){case"home":r=O();break;case"profile":r=ae();break;case"edit-profile":r=le();break;case"change-password":r=ie();break;case"orders":r=de();break;case"special-orders":r=await oe();break;case"tracking":r=ne();break;case"history":r=ce();break;case"cart":r=ue();break;case"wishlist":r=await _e();break;case"notifications":r=pe();break;case"messages":r=be();break;case"payments":r=me();break;case"payment-methods":r=xe();break;case"addresses":r=ge();break;case"receipts":r=ve();break;case"email-prefs":r=ye();break;case"support":r=we();break;case"help":r=$e();break;case"privacy":r=ke();break;default:r=O()}t.innerHTML=`<div class="section active fade-in">${r}</div>`,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(B),Se(e)}function c(e,t){return`
    <div class="mb-6">
      <h1 class="text-2xl sm:text-3xl font-black text-white tracking-tight">${e}</h1>
      ${t?`<p class="text-sm text-gray-500 mt-1">${t}</p>`:""}
    </div>
  `}function se(){const e=s.orders.length,t=s.orders.filter(l=>["order_placed","payment_received","pending_verification","order_processing"].includes(w(l.status))).length,r=s.orders.filter(l=>w(l.status)==="order_delivered").length,a=s.orders.filter(l=>l.status==="rejected").length;return{total:e,pending:t,completed:r,cancelled:a}}function k(e,t,r,a){return`
  ...
  `}function O(){const e=se(),t=s.profile?.display_name||s.profile?.first_name||"Customer",r=s.profile?.country_code?S(s.profile.country_code):null,a=s.orders.slice(0,4),l=s.notifications.slice(0,5);return`
          ...
          ${l.length===0?'<p class="text-xs text-gray-500 text-center py-4">No notifications yet.</p>':l.map(d=>{const u=p.find(v=>v.id===d.event_type)||p[0];return`
              <div class="flex items-start gap-2.5 p-2.5 hover:bg-blue-500/5 rounded-xl transition cursor-pointer" onclick="navigateTo('notifications')">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p class="text-[10px] text-gray-500">${E(d.created_at)}</p>
                </div>
              </div>
            `}).join("")}
        </div>
    ...
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mt-5 slide-up">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Email</p><p class="text-sm text-white truncate">${s.user.email}</p></div>
        ...
        <div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Member Since</p><p class="text-sm text-white">${g(s.user.created_at)}</p></div>
      </div>
    </div>
  `}function ae(){const e=s.profile||{},t=e.country_code?S(e.country_code):null,r=e.avatar_url?`<img src="${e.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:`<span class="text-2xl font-black">${j(s.user.email)}</span>`;return`
    ...
    <div class="glass border border-blue-500/20 rounded-2xl p-6 slide-up">
      <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6">
        ...
        <div class="flex-1 text-center sm:text-left">
          ...
          <p class="text-sm text-gray-400">${s.user.email}</p>
          <div class="flex flex-wrap justify-center sm:justify-start gap-2 mt-3">
            ...
            <span class="inline-flex items-center gap-1.5 bg-amber-500/10 border border-amber-500/20 rounded-full px-3 py-1 text-xs text-amber-400 font-medium"><i data-lucide="calendar" class="w-3 h-3"></i> ${g(s.user.created_at)}</span>
          </div>
  ...
  `}function f(e,t){return`<div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">${e}</p><p class="text-sm text-white">${t}</p></div>`}function le(){const e=s.profile||{},t=e.country_code?S(e.country_code):null;return`
      ...
      <form id="edit-profile-form" class="space-y-5">
        ...
        <div class="flex items-center gap-4">
          <div id="edit-avatar-preview" class="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white font-black text-xl overflow-hidden shrink-0 ring-2 ring-blue-500/20">
            ${e.avatar_url?`<img src="${e.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:j(s.user.email)}
          </div>
          <div>
  ...
  `}function de(){return`
    ...
    <div id="orders-list" class="space-y-4">
      ${s.orders.length===0?x("No Orders Yet","You haven't placed any orders yet.","shopping-bag","Start Shopping"):s.orders.map(e=>F(e,!1)).join("")}
    </div>
  `}function F(e,t){const r=e.listing_image||b,a=s.events[e.order_number]||[],l=t===e.order_number;return`
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto scrollbar-none">
              ${a.length===0?'<p class="text-xs text-gray-600">No events yet.</p>':a.map(d=>`
                <div class="flex items-start gap-2.5 p-2.5 bg-blue-950/30 border border-blue-500/10 rounded-xl">
                  ...
                  <div class="flex-1 min-w-0"><p class="text-xs text-gray-200 font-medium">${d.message}</p><p class="text-[10px] text-gray-500 mt-0.5">${E(d.created_at)}</p></div>
                </div>
              `).join("")}
            </div>
          </div>
  ...
  `}function _(e,t,r){return`<div class="bg-blue-950/40 border border-blue-500/10 rounded-xl p-3"><div class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">${e}</div><div class="text-sm ${r==="mono"?"font-mono text-blue-400":r==="amber"?"text-amber-400 font-bold":"text-white"} break-all">${t}</div></div>`}function Y(e){const t=J(e);return`
    <div class="relative pt-2">
      ...
      <div class="space-y-3">
        ${p.map((r,a)=>{const l=a<=t,d=a===t;return`<div class="flex items-center gap-3 relative"><div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-all ${l?r.bg+" border border-blue-500/30":"bg-blue-950/40 border border-blue-500/10"} ${d?"pulse-glow":""}"><i data-lucide="${r.icon}" class="w-4 h-4 ${l?r.color:"text-gray-600"} ${d?"animate-pulse":""}"></i></div><div class="flex-1 flex items-center justify-between"><span class="text-sm font-medium ${l?"text-white":"text-gray-600"}">${r.label}</span>${l&&!d?'<i data-lucide="check" class="w-4 h-4 text-emerald-400 shrink-0"></i>':""}${d?'<span class="text-[10px] text-blue-400 font-bold uppercase shrink-0">Current</span>':""}</div></div>`}).join("")}
      </div>
    </div>
  `}let P=null;window.toggleOrder=e=>{P=P===e?null:e;const t=document.getElementById("orders-list");t&&(t.innerHTML=s.orders.map(r=>F(r,P)).join(""),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(B))};async function oe(){try{const{data:e,error:t}=await i.from("product_requests").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});if(t)throw t;const r={pending_review:"bg-yellow-500/10 text-yellow-400 border-yellow-500/30",under_review:"bg-blue-500/10 text-blue-400 border-blue-500/30",approved:"bg-emerald-500/10 text-emerald-400 border-emerald-500/30",rejected:"bg-red-500/10 text-red-400 border-red-500/30",quoted:"bg-cyan-500/10 text-cyan-400 border-cyan-500/30",fulfilled:"bg-green-500/10 text-green-400 border-green-500/30",cancelled:"bg-gray-500/10 text-gray-400 border-gray-500/30"};if(!e||e.length===0)return`<div class="text-center py-16"><div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-800 mb-4"><i data-lucide="package-plus" class="w-8 h-8 text-gray-500"></i></div><h3 class="text-lg font-bold text-white mb-2">No Special Orders yet</h3><p class="text-sm text-gray-500 mb-5">When you can't find a product in our marketplace, you can request it as a Special Order and we'll source it for you.</p><a href="/" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-400 hover:to-orange-500 text-white font-bold rounded-xl text-sm transition shadow-lg shadow-orange-500/30"><i data-lucide="search" class="w-4 h-4"></i> Search Products</a></div>`;let a='<div class="mb-4"><p class="text-sm text-gray-400">Track your special order requests and their status updates.</p></div><div class="space-y-3">';return e.forEach(l=>{const d=r[l.status]||r.pending_review,u=l.status.replace(/_/g," "),v=l.target_price?`${l.currency} ${Number(l.target_price).toLocaleString()}`:"—",T=l.quoted_price?`${l.quoted_currency} ${Number(l.quoted_price).toLocaleString()}`:null;a+=`<div class="glass border border-gray-800 rounded-xl p-4">
      ...
      </div>`}),a+="</div>",a}catch(e){return`<div class="text-red-400 text-sm p-4">Error loading special orders: ${o(e.message)}</div>`}}function ne(){const e=s.orders.filter(t=>!["order_delivered","rejected"].includes(w(t.status)));return`
  ...
  `}function ue(){const e=JSON.parse(localStorage.getItem("kco_cart")||"[]");return`
    ...
    ${e.length===0?x("Cart is Empty","Your shopping cart is empty.","shopping-cart","Browse Marketplace"):`
      <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
        <div class="space-y-3">
          ${e.map(t=>{const r=window.SHOWROOM_LISTINGS?.find(a=>a.property_id===t);return r?`<div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
    ...
    `}
  `}window.removeFromCart=e=>{let t=JSON.parse(localStorage.getItem("kco_cart")||"[]");t=t.filter(r=>r!==e),localStorage.setItem("kco_cart",JSON.stringify(t)),m("cart"),n("Removed from cart.")};window.clearCart=()=>{localStorage.removeItem("kco_cart"),m("cart"),n("Cart cleared.")};function pe(){return`
    ${c("Notifications","All email notifications related to your orders.")}
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
      ${s.notifications.length===0?x("No Notifications","You haven't received any notifications yet.","bell-off",null):`
        <div class="space-y-2">
  ...
  `}function fe(e){const t=A.find(r=>r.type===e.method_type)||{icon:"credit-card",color:"text-blue-400"};return`
  ...
  `,document.body.appendChild(r),window.lucide&&lucide.createIcons()};window.savePaymentMethod=async function(e,t){e.preventDefault();const r={method_type:document.getElementById("pm-type").value,label:document.getElementById("pm-label").value.trim(),provider:document.getElementById("pm-provider").value.trim()||null,account_holder:document.getElementById("pm-holder").value.trim()||null,identifier:document.getElementById("pm-identifier").value.trim()||null,is_default:document.getElementById("pm-default").checked};e.target.closest(".fixed").remove();try{if(r.is_default&&await i.from("customer_payment_methods").update({is_default:!1}).eq("user_id",s.user.id),t){const{error:a}=await i.from("customer_payment_methods").update({...r,updated_at:new Date().toISOString()}).eq("id",t).eq("user_id",s.user.id);if(a)throw a}else{const{error:a}=await i.from("customer_payment_methods").insert({...r,user_id:s.user.id});if(a)throw a}await I(),h("payment-methods"),n("Payment method saved successfully.")}catch(a){n("Failed to save payment method: "+(a.message||"Unknown error"))}};window.editPaymentMethod=function(e){openPaymentMethodModal(e)};window.deletePaymentMethod=async function(e){if(confirm("Remove this payment method?"))try{const{error:t}=await i.from("customer_payment_methods").delete().eq("id",e).eq("user_id",s.user.id);if(t)throw t;await I(),h("payment-methods"),n("Payment method removed.")}catch(t){n("Failed to remove: "+(t.message||"Unknown error"))}};window.setDefaultPaymentMethod=async function(e){try{await i.from("customer_payment_methods").update({is_default:!1}).eq("user_id",s.user.id).neq("id",e);const{error:t}=await i.from("customer_payment_methods").update({is_default:!0,updated_at:new Date().toISOString()}).eq("id",e).eq("user_id",s.user.id);if(t)throw t;await I(),h("payment-methods"),n("Default payment method updated.")}catch(t){n("Failed to set default: "+(t.message||"Unknown error"))}};function ge(){return`
    ...
    <div id="addresses-list" class="space-y-3">
      ${s.addresses.length===0?x("No Addresses","You haven't saved any shipping addresses yet.","map-pin",null):s.addresses.map(e=>{const t=S(e.country_code);return`
          <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up">
  ...
  `}window.showAddressForm=e=>{const t=document.getElementById("address-form-container"),r=e||{};t.innerHTML=`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-4 slide-up">
      ...
      <form id="address-form" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label class="block text-xs font-bold uppercase text-gray-400 mb-1.5">Country</label><select id="addr-country" class="input-field w-full bg-[#0a1124]/80 border border-blue-500/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500">${N.map(a=>`<option value="${a.code}" ${r.country_code===a.code?"selected":""}>${a.flag} ${a.name}</option>`).join("")}</select></div>
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(B),document.getElementById("address-form").addEventListener("submit",he)};window.cancelAddressForm=()=>{document.getElementById("address-form-container").innerHTML=""};window.editAddress=e=>{const t=s.addresses.find(r=>r.id===e);showAddressForm(t),document.getElementById("address-form-container").scrollIntoView({behavior:"smooth"})};async function he(e){e.preventDefault();const t=document.getElementById("addr-id").value,r={user_id:s.user.id,label:document.getElementById("addr-label").value,full_name:document.getElementById("addr-name").value,address_line1:document.getElementById("addr-line1").value,address_line2:document.getElementById("addr-line2").value||null,city:document.getElementById("addr-city").value,state:document.getElementById("addr-state").value,postal_code:document.getElementById("addr-postal").value,country_code:document.getElementById("addr-country").value,phone:document.getElementById("addr-phone").value,is_default:document.getElementById("addr-default").checked};r.is_default&&await i.from("shipping_addresses").update({is_default:!1}).eq("user_id",s.user.id),t?await i.from("shipping_addresses").update(r).eq("id",t):await i.from("shipping_addresses").insert(r),await C(),m("addresses"),n("Address saved successfully.")}window.deleteAddress=async e=>{await i.from("shipping_addresses").delete().eq("id",e),await C(),m("addresses"),n("Address deleted.")};function ve(){const e=s.orders.filter(t=>t.receipt_file_path);return`
  ...
  `}window.saveEmailPrefs=async()=>{const e={user_id:s.user.id};document.querySelectorAll("[data-pref]").forEach(t=>{e[t.dataset.pref]=t.checked}),s.emailPrefs?await i.from("email_preferences").update(e).eq("user_id",s.user.id):await i.from("email_preferences").insert(e),await D(),n("Email preferences saved.")};function we(){return`
  ...
  `}async function _e(){const e=s.wishlist||[];return e.length?`
    <div class="space-y-4">
        ...
        ${e.map(t=>{const r=t.showroom_listings;if(!r)return"";const a=r.images&&r.images[0]||"/fallback.svg",l=typeof r.price=="number"?r.price:parseFloat(r.price||0);return`
            <div class="glass border border-blue-500/15 rounded-2xl overflow-hidden group">
              <div class="relative aspect-square overflow-hidden bg-blue-950/50">
                <a href="/details.html?id=${r.property_id}"><img src="${o(a)}" alt="${o(r.title)}" class="w-full h-full object-cover group-hover:scale-105 transition" loading="lazy" onerror="this.src='/fallback.svg'"></a>
              ...
              <div class="p-3">
                <a href="/details.html?id=${r.property_id}" class="text-sm text-white font-bold hover:text-blue-400 transition line-clamp-2">${o(r.title)}</a>
                <div class="flex items-center justify-between mt-2">
                  <span class="text-sm text-amber-400 font-bold">${r.currency||"USD"} ${l.toLocaleString()}</span>
                  <a href="/details.html?id=${r.property_id}" class="text-xs font-bold text-blue-400 hover:text-blue-300 transition">View</a>
                </div>
              </div>
            </div>`}).join("")}
      </div>
    </div>
  `:x("Your Wishlist","Save items you love to find them quickly later.","heart","Browse Products","window.location.href='/'")}window.removeFromWishlist=async e=>{try{await i.from("wishlist").delete().eq("id",e),n("Removed from wishlist."),await R(),m("wishlist")}catch(t){n("Error: "+t.message)}};function x(e,t,r,a,l){return`
  ...
  `}function Se(e){if(e==="edit-profile"&&(document.getElementById("avatar-file")?.addEventListener("change",Ee),document.getElementById("edit-profile-form")?.addEventListener("submit",Ie)),e==="change-password"&&document.getElementById("change-password-form")?.addEventListener("submit",Pe),e==="messages"){document.getElementById("msg-form")?.addEventListener("submit",Me);const t=document.getElementById("msg-thread");t&&(t.scrollTop=t.scrollHeight)}e==="support"&&document.getElementById("support-form")?.addEventListener("submit",je)}let y=null;async function Ee(e){const t=e.target.files[0];if(!t)return;if(t.size>5*1024*1024){n("Image must be 5 MB or less.");return}y=t;const r=document.getElementById("edit-avatar-preview");r.innerHTML=`<img src="${URL.createObjectURL(t)}" class="w-full h-full object-cover">`}async function Ie(e){e.preventDefault();const t=document.getElementById("ep-save-btn");t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Saving...',window.lucide&&lucide.createIcons();let r=s.profile?.avatar_url;if(y){const l=y.name.split(".").pop(),d=`${s.user.id}/avatar-${Date.now()}.${l}`,{error:u}=await i.storage.from("avatars").upload(d,y,{upsert:!0});if(!u){const{data:v}=i.storage.from("avatars").getPublicUrl(d);r=v.publicUrl}y=null}const a={user_id:s.user.id,first_name:document.getElementById("ep-first-name").value,last_name:document.getElementById("ep-last-name").value,display_name:document.getElementById("ep-display-name").value,phone_code:document.getElementById("ep-phone-code").value,phone_number:document.getElementById("ep-phone-number").value,country_code:document.getElementById("ep-country").value,bio:document.getElementById("ep-bio").value,avatar_url:r};s.profile?await i.from("profiles").update(a).eq("user_id",s.user.id):await i.from("profiles").insert(a),await H(),U(),h("profile"),n("Profile updated successfully.")}async function Pe(e){e.preventDefault();const t=document.getElementById("cp-error");t.classList.add("hidden"),document.getElementById("cp-current").value;const r=document.getElementById("cp-new").value,a=document.getElementById("cp-confirm").value;if(r!==a){t.textContent="New passwords do not match.",t.classList.remove("hidden");return}if(r.length<6){t.textContent="Password must be at least 6 characters.",t.classList.remove("hidden");return}const l=document.getElementById("cp-submit");l.disabled=!0,l.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Updating...',window.lucide&&lucide.createIcons();const{error:d}=await i.auth.updateUser({password:r});if(l.disabled=!1,l.innerHTML='<i data-lucide="key-round" class="w-4 h-4"></i> Update Password',window.lucide&&lucide.createIcons(),d){t.textContent=d.message,t.classList.remove("hidden");return}document.getElementById("change-password-form").reset(),n("Password updated successfully.")}async function Me(e){e.preventDefault();const t=document.getElementById("msg-subject").value.trim(),r=document.getElementById("msg-body").value.trim();if(!r)return;const a=document.getElementById("msg-send-btn");a.disabled=!0,a.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Sending...',window.lucide&&lucide.createIcons(),await i.from("support_messages").insert({user_id:s.user.id,subject:t||"Message",message:r,from_admin:!1,read:!1}),await q(),m("messages"),n("Message sent to support.")}async function je(e){e.preventDefault();const t=document.getElementById("support-subject").value.trim(),r=document.getElementById("support-order").value.trim(),a=document.getElementById("support-message").value.trim();if(!t||!a)return;const l=document.getElementById("support-submit");l.disabled=!0,l.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Sending...',window.lucide&&lucide.createIcons(),await i.from("support_messages").insert({user_id:s.user.id,order_number:r||null,subject:t,message:a,from_admin:!1,read:!1}),await q(),document.getElementById("support-form").reset(),l.disabled=!1,l.innerHTML='<i data-lucide="send" class="w-4 h-4"></i> Submit Request',window.lucide&&lucide.createIcons(),n("Support request submitted. We'll respond within 24 hours.")}function Be(){document.getElementById("btn-signout-desktop")?.addEventListener("click",L),document.getElementById("btn-signout-mobile")?.addEventListener("click",L),document.getElementById("btn-mobile-menu")?.addEventListener("click",Ce),document.getElementById("btn-mobile-close")?.addEventListener("click",M),document.getElementById("mobile-backdrop")?.addEventListener("click",M)}async function L(){await V(),window.location.href="/"}function Ce(){document.getElementById("mobile-drawer").classList.remove("hidden")}function M(){document.getElementById("mobile-drawer").classList.add("hidden")}window.navigateTo=h;window.copyToClipboard=K;window.contactSupport=e=>{const t=encodeURIComponent(`Order ${e} — Support Request`),r=encodeURIComponent(`Hello KCO Global Online Marketplace Support,
