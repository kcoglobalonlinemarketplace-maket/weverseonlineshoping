...
<div class="space-y-4">
      ...
      </div>`,window._productsData=n,window._productsCardLimit=60,rn(n),filterProducts(),updateBulkBar(),window.lucide&&lucide.createIcons()}catch(t){e&&(e.innerHTML=`<div class="p-6 text-red-400 text-sm">Error: ${c(t.message)}</div>`)}}function Te(e){const t=parseFloat(e);return Number.isFinite(t)?t:0}function St(e){return Array.isArray(e.tags)?e.tags.filter(Boolean):[]}function Qs(e){const t=Te(e.price),i=parseFloat(e.real_price);if(Number.isFinite(i)&&i>0&&i>t)return`${Math.round((1-t/i)*100)}% OFF`;const a=parseFloat(e.discount_percent??e.discount??0);return Number.isFinite(a)&&a>0?`${Math.round(a)}% OFF`:"No discount"}function Xs(e){const t=Te(e.price),i=parseFloat(e.real_price),a=`$${t.toLocaleString()}`;return Number.isFinite(i)&&i>0&&i>t?`<span class="block text-xs text-gray-400 price-strike line-through">$${i.toLocaleString()}</span><span class="text-emerald-300 font-black">$${t.toLocaleString()}</span>`:a}function ra(e){return e.is_archived||e.availability_status==="Archived"?"archived":e.is_active?"active":"inactive"}function Ia(e){return parseInt(e.views??e.view_count??0,10)||0}function Ta(e){return parseInt(e.sales??e.sales_count??0,10)||0}function la(e){return e.sku||e.property_id||"N/A"}function Za(e){const t=Array.isArray(e&&e.images)?e.images:[];return t.find(a=>typeof a=="string"&&!/\.(mp4|webm|mov|m4v|avi|ogg|ogv)$/i.test(a.split("?")[0]))||t[0]||"/fallback.svg"}function Zs(e){const t=Za(e),i=St(e),a=ra(e),n=window._productSelection?.has(e.property_id),s=ie(a==="archived"?"inactive":a==="active"?"active":"inactive"),o=ge(e.created_at),r=!!e.is_featured,l=e.is_active?`unpublishProduct('${e.property_id}')`:`publishProduct('${e.property_id}')`,d=e.is_active?"Unpublish":"Publish",u=e.is_active?"bg-amber-500/15 text-amber-200 hover:bg-amber-500/25":"bg-emerald-500/15 text-emerald-200 hover:bg-emerald-500/25";return`<article data-id="${e.property_id}" data-cat="${c(e.category||"")}" data-status="${a}" data-featured="${r?"featured":"standard"}" onclick="editProduct('${e.property_id}')" title="Tap anywhere to edit this product" class="prod-card glass-soft border ${n?"border-blue-400/60":"border-blue-500/15"} rounded-3xl p-5 flex flex-col gap-4 transition hover:border-blue-400/40 hover:shadow-lg hover:shadow-blue-500/10 cursor-pointer select-none active:scale-[.99]">
