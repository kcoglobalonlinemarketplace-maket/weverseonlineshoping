import{supabase as A}from"./supabase-client-7_ZWSEp6.js";import{b as W,c as L,e as _}from"./localization-C0xPa6if.js";const q="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),j=`${q}/functions/v1/ai-customer-assistant`,U="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",H=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let n={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null,attachments:[],recording:!1,mediaRecorder:null,mediaChunks:[],recordTimer:null,recordSeconds:0};function y(t){const e=document.createElement("div");return e.textContent=t,e.innerHTML}function N(t){let e=y(t);return e=e.replace(/`([^`]+)`/g,"<code>$1</code>"),e=e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),e=e.replace(/^\- (.+)$/gm,"<li>$1</li>"),e=e.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),e=e.replace(/(<li>.*<\/li>\n?)+/g,a=>`<ul>${a}</ul>`),e=e.replace(/\n\n/g,"</p><p>"),e=e.replace(/\n/g,"<br>"),e=`<p>${e}</p>`,e=e.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),e}function g(){return W()||localStorage.getItem("kco_language")||"en"}function C(){const t=window.location.pathname;return t.includes("checkout")?"checkout":t.includes("payment")?"payment":t.includes("auth")?"sign-in":t.includes("account")?"account":t.includes("details")?"product-details":t.includes("contact")?"contact":t.includes("help")?"help":t.includes("about")?"about":"home"}function V(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(e=>({title:e.title||e.name,price:e.price,quantity:e.quantity||1}))}catch{return[]}}function k(t){if(n.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const e=new SpeechSynthesisUtterance(t.replace(/[*#`]/g,"")),a=g(),o=L(a);a==="en"?e.lang=n.voiceAccent==="UK"?"en-GB":"en-US":e.lang=o.lang,e.rate=o.rate,e.pitch=o.pitch;const i=_(a==="en"?(n.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):a);i&&(e.voice=i),window.speechSynthesis.speak(e)}catch{}}function T(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function P(){if(document.getElementById("kco-ai-styles"))return;const t=document.createElement("style");t.id="kco-ai-styles",t.textContent=`
  ...
  `,document.head.appendChild(t)}const w='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" fill="#60a5fa"/><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" stroke="#3b82f6" stroke-width="0.5"/></svg>',F='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.4 20.4l17.45-7.48a1 1 0 0 0 0-1.84L3.4 3.6a1 1 0 0 0-1.39 1.21L4 12l-1.99 7.19a1 1 0 0 0 1.39 1.21z" fill="white"/></svg>',D='<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 inline-block align-middle" aria-label="Verified" role="img"><circle cx="12" cy="12" r="11" fill="#1877F2"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>';function J(){P();const t=document.createElement("button");t.id="kco-ai-fab",t.setAttribute("aria-label","Chat with Weverse support"),t.className="fixed bottom-5 right-5 z-[60] w-14 h-14 rounded-full flex items-center justify-center shadow-xl transition-transform hover:scale-105 active:scale-95",t.style.background="linear-gradient(135deg,#3b82f6 0%,#2563eb 100%)",t.style.border="1px solid rgba(255,255,255,0.18)",t.style.boxShadow="0 8px 24px rgba(59,130,246,0.4)",t.innerHTML=`
  ...
  `,t.onclick=()=>d(!0),document.body.appendChild(t);const e=document.createElement("div");e.id="kco-ai-panel",e.className="hidden-panel fixed z-[60] flex flex-col overflow-hidden w-full sm:w-[400px] h-[92dvh] sm:h-[640px] sm:max-h-[calc(100dvh-120px)] bottom-0 sm:bottom-[90px] right-0 sm:right-5 rounded-t-2xl sm:rounded-2xl bg-slate-900/97 border-t sm:border border-blue-500/25 shadow-2xl",e.style.backdropFilter="blur(20px)",e.innerHTML=`
    ...
    <div class="flex items-center justify-between px-3.5 py-3 border-b border-white/10 shrink-0" style="background:linear-gradient(135deg,#1e1e2e 0%,#1a1a2e 50%,#16213e 100%)">
      <div class="flex items-center gap-2.5 min-w-0">
        ...
        <div class="min-w-0">
          <p class="text-sm font-bold text-white leading-tight tracking-wide flex items-center gap-1 truncate">Weverse Support ${D}</p>
  ...
  `:`
    <div class="max-w-[88%] flex gap-2">
      ...
      <div class="bg-slate-800/80 border border-white/10 rounded-2xl rounded-tl-sm px-3.5 py-2.5">
        <div class="text-sm text-gray-200 leading-relaxed">${N(t.content)}</div>
      </div>
    </div>
  `,a.appendChild(i),window.lucide&&lucide.createIcons(),M(),o||k(t.content)}function $(){const t=g(),e={en:`Welcome to Weverse Online Shop!
