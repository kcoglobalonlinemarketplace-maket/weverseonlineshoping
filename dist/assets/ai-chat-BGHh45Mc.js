import{x as _,A}from"./supabase-client-CDm4AUL7.js";const C=(_||"https://wttnvwpoqmbxryivcerf.supabase.co").replace(/\/$/,""),L=`${C}/functions/v1/customer-ai-chat`,y="kco_ai_chat_msgs",M=["Where can I track my order?","How long does shipping take?","How do refunds work?","Talk to a human"];let s={open:!1,busy:!1,inited:!1};function v(){try{const e=JSON.parse(localStorage.getItem(y)||"[]");if(Array.isArray(e))return e.filter(t=>t&&t.role&&t.content).slice(-20)}catch{}return[]}function g(e){try{localStorage.setItem(y,JSON.stringify(e.slice(-20)))}catch{}}function O(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function T(e){return O(e).replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/^[-•]\s+/gm,'<span class="kco-ai-bullet">•</span> ').replace(/\n/g,"<br>")}function f(e,t=20){const i={message:'<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/>',send:'<path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/>',close:'<path d="M18 6 6 18"/><path d="m6 6 12 12"/>',headset:'<path d="M3 11h3a2 2 0 0 1 2 2v3a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-5Z"/><path d="M21 11h-3a2 2 0 0 0-2 2v3a2 2 0 0 0 2 2h1a2 2 0 0 0 2-2v-5Z"/><path d="M3 11v-1a9 9 0 0 1 18 0v1"/><path d="M21 16v2a4 4 0 0 1-4 4h-5"/>'};return`<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${t}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${i[e]||i.message}</svg>`}function N(){if(document.getElementById("kco-ai-chat-style"))return;const e=document.createElement("style");e.id="kco-ai-chat-style",e.textContent=`
  ...
  `,document.head.appendChild(e)}function H(){const e=document.createElement("button");e.id="kco-ai-btn",e.setAttribute("aria-label","Open chat"),e.innerHTML=f("message",24),e.addEventListener("click",()=>E());const t=document.createElement("div");t.id="kco-ai-panel",t.setAttribute("role","dialog"),t.setAttribute("aria-label","Weverse Online Shop assistant chat"),t.innerHTML=`
  ...
  `;const i=t.querySelector(".kco-ai-body"),a=t.querySelector(".kco-ai-chips"),c=t.querySelector(".kco-ai-input"),d=t.querySelector(".kco-ai-send"),u=t.querySelector(".kco-ai-close");M.forEach(o=>{const n=document.createElement("button");n.className="kco-ai-chip",n.type="button",n.textContent=o,n.addEventListener("click",()=>{k(o),a.remove()}),a.appendChild(n)});const l=()=>{const o=c.value.trim();!o||s.busy||(c.value="",k(o))};return d.addEventListener("click",l),c.addEventListener("keydown",o=>{o.key==="Enter"&&l()}),u.addEventListener("click",h),document.body.appendChild(e),document.body.appendChild(t),{panel:t,body:i,input:c,sendBtn:d}}let r=null;function b(e){const t=document.createElement("div");return t.className="kco-ai-msg kco-ai-assistant",t.innerHTML=T(e),t}function $(){const e=document.createElement("div");return e.className="kco-ai-typing",e.innerHTML="<span></span><span></span><span></span>",e}function p(e){e.scrollTop=e.scrollHeight}function z(e){const t=v();for(const i of t){const a=document.createElement("div");a.className=i.role==="user"?"kco-ai-msg kco-ai-user":"kco-ai-msg kco-ai-assistant",a.textContent=i.content,e.appendChild(a)}p(e)}async function k(e){if(!r||s.busy)return;const{body:t,input:i}=r,a=v(),c=document.createElement("div");c.className="kco-ai-msg kco-ai-user",c.textContent=e,t.appendChild(c),p(t),a.push({role:"user",content:e}),g(a),s.busy=!0;const d=$();t.appendChild(d),p(t);const u=a.slice(0,-1).slice(-10);try{const o=await(await fetch(L,{method:"POST",headers:{Authorization:`Bearer ${A}`,"Content-Type":"application/json"},body:JSON.stringify({message:e,history:u})})).json(),n=String(o?.response||"").trim();if(!n){const S=String(o?.error||"").trim();throw new Error(S||"Empty reply")}d.remove();const m=b(n);t.appendChild(m),p(t),a.push({role:"assistant",content:n}),g(a)}catch(l){d.remove();const o=String(l?.message||"").trim(),n=o?`⚠️ The assistant hit an error: ${o}`:"I'm sorry — I couldn't reach the assistant just now. Please try again in a moment, or email support@weverseonlineshop.com and we'll reply within 24 hours.",m=b(n);t.appendChild(m),p(t)}s.busy=!1,i&&i.focus()}function w(){r&&(r.panel.classList.add("kco-ai-open"),s.open=!0,r.input&&setTimeout(()=>r.input.focus(),250))}function h(){r&&(r.panel.classList.remove("kco-ai-open"),s.open=!1)}function E(){s.open?h():w()}function j(){if(window.__KCO_ADMIN_PAGE__)return!0;const e=window.location.pathname||"";return/\/admin(-[a-z0-9]+)?(\.html)?$/.test(e)}function x(){s.inited||j()||(s.inited=!0,N(),r=H(),z(r.body),window.__kcoAiOpen=w,window.__kcoAiClose=h,window.__kcoAiToggle=E)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",x):x();
