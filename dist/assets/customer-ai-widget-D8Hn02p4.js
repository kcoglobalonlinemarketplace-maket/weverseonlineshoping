import{supabase as v}from"./supabase-client-7_ZWSEp6.js";import{b as S,c as I,e as E}from"./localization-D8zG2wD-.js";const B="https://wttnvwpoqmbxryivcerf.supabase.co".replace(/\/$/,""),y=`${B}/functions/v1/ai-customer-assistant`,O="sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa",A=window.self!==window.top||/\/admin(\-ai)?\.html$/i.test(window.location.pathname);let t={open:!1,history:[],sending:!1,voiceEnabled:localStorage.getItem("kco_voice")==="1",voiceAccent:localStorage.getItem("kco_voice_accent")||"US",welcomed:!1,proactiveShown:{},sessionId:null};function d(e){const n=document.createElement("div");return n.textContent=e,n.innerHTML}function C(e){let n=d(e);return n=n.replace(/`([^`]+)`/g,"<code>$1</code>"),n=n.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>"),n=n.replace(/^\- (.+)$/gm,"<li>$1</li>"),n=n.replace(/^\d+\. (.+)$/gm,"<li>$1</li>"),n=n.replace(/(<li>.*<\/li>\n?)+/g,a=>`<ul>${a}</ul>`),n=n.replace(/\n\n/g,"</p><p>"),n=n.replace(/\n/g,"<br>"),n=`<p>${n}</p>`,n=n.replace(/<ul><br>/g,"<ul>").replace(/<br><\/ul>/g,"</ul>"),n}function r(){return S()||localStorage.getItem("kco_language")||"en"}function _(){const e=window.location.pathname;return e.includes("checkout")?"checkout":e.includes("payment")?"payment":e.includes("auth")?"sign-in":e.includes("account")?"account":e.includes("details")?"product-details":e.includes("contact")?"contact":e.includes("help")?"help":e.includes("about")?"about":"home"}function j(){try{return JSON.parse(localStorage.getItem("kco_cart")||"[]").map(n=>({title:n.title||n.name,price:n.price,quantity:n.quantity||1}))}catch{return[]}}function u(e){if(t.voiceEnabled&&"speechSynthesis"in window)try{window.speechSynthesis.cancel();const n=new SpeechSynthesisUtterance(e.replace(/[*#`]/g,"")),a=r(),o=I(a);a==="en"?n.lang=t.voiceAccent==="UK"?"en-GB":"en-US":n.lang=o.lang,n.rate=o.rate,n.pitch=o.pitch;const i=E(a==="en"?(t.voiceAccent==="UK"?"en-GB":"en-US").slice(0,2):a);i&&(n.voice=i),window.speechSynthesis.speak(n)}catch{}}function k(){"speechSynthesis"in window&&window.speechSynthesis.cancel()}function W(){if(document.getElementById("kco-ai-styles"))return;const e=document.createElement("style");e.id="kco-ai-styles",e.textContent=`
  ...
  `,document.head.appendChild(e)}const p='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" fill="#60a5fa"/><path d="M12 2C7.58 2 4 5.58 4 10v4a3 3 0 0 0 3 3h1v-6H6v-1a6 6 0 0 1 12 0v1h-2v6h1a3 3 0 0 0 3-3v-4c0-4.42-3.58-8-8-8z" stroke="#3b82f6" stroke-width="0.5"/></svg>',q='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.4 20.4l17.45-7.48a1 1 0 0 0 0-1.84L3.4 3.6a1 1 0 0 0-1.39 1.21L4 12l-1.99 7.19a1 1 0 0 0 1.39 1.21z" fill="white"/></svg>',b='<svg viewBox="0 0 24 24" class="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0 inline-block align-middle" aria-label="Verified" role="img"><circle cx="12" cy="12" r="11" fill="#1877F2"/><path d="M10.8 15.6 7.4 12.2l1.5-1.5 1.9 1.9 3.9-3.9 1.5 1.5-5.4 5.4z" fill="#fff"/></svg>';function $(){W();const e=document.createElement("button");e.id="kco-ai-fab",e.setAttribute("aria-label","Contact Us"),e.className="fixed bottom-5 right-5 z-[60] flex items-center gap-2.5 px-5 py-3 rounded-xl text-white text-sm font-semibold shadow-lg transition-all hover:scale-105",e.style.background="#1e293b",e.style.border="1px solid rgba(255,255,255,0.12)",e.innerHTML=`
  ...
  `,e.onclick=c,document.body.appendChild(e);const n=document.createElement("div");n.id="kco-ai-panel",n.className="hidden-panel fixed bottom-[88px] right-5 z-[60] w-[calc(100vw-2.5rem)] sm:w-[400px] h-[560px] max-h-[calc(100vh-120px)] bg-slate-900/95 backdrop-blur-xl border border-blue-500/20 rounded-2xl shadow-2xl flex flex-col overflow-hidden",n.innerHTML=`
    ...
    <div class="flex items-center justify-between px-4 py-3 border-b border-white/10" style="background:linear-gradient(135deg,#1e1e2e 0%,#1a1a2e 50%,#16213e 100%)">
      <div class="flex items-center gap-2.5">
        ...
        <div>
          <p class="text-sm font-bold text-white leading-tight tracking-wide flex items-center gap-1">Contact Us ${b}</p>
  ...
  `,document.body.appendChild(n),document.getElementById("kco-ai-close").onclick=()=>c(!1),document.getElementById("kco-ai-voice-toggle").onclick=z,document.getElementById("kco-ai-voice-accent").onclick=T,document.getElementById("kco-ai-send").onclick=l;const a=document.getElementById("kco-ai-input");a.addEventListener("keydown",o=>{o.key==="Enter"&&!o.shiftKey&&(o.preventDefault(),l())}),a.addEventListener("input",()=>{a.style.height="auto",a.style.height=Math.min(a.scrollHeight,96)+"px"}),document.querySelectorAll(".kco-ai-quick-btn").forEach(o=>{o.onclick=()=>{a.value=o.dataset.q,l()}}),x(),window.lucide&&lucide.createIcons()}function c(e){const n=document.getElementById("kco-ai-panel"),a=document.getElementById("kco-ai-fab");t.open=e!==void 0?e:n.classList.contains("hidden-panel"),t.open?(n.classList.remove("hidden-panel"),a.style.display="none",t.welcomed||(L(),t.welcomed=!0),setTimeout(()=>document.getElementById("kco-ai-input")?.focus(),300)):(n.classList.add("hidden-panel"),a.style.display="flex",k())}function z(){t.voiceEnabled=!t.voiceEnabled,localStorage.setItem("kco_voice",t.voiceEnabled?"1":"0"),t.voiceEnabled||k(),x(),t.voiceEnabled&&u("Voice assistance is now on.")}function T(){t.voiceAccent=t.voiceAccent==="US"?"UK":"US",localStorage.setItem("kco_voice_accent",t.voiceAccent);const e=document.getElementById("kco-ai-voice-accent");e&&(e.textContent=t.voiceAccent);const n=t.voiceAccent==="UK"?"Voice accent switched to British English.":"Voice accent switched to American English.";u(n)}function x(){const e=document.getElementById("kco-ai-voice-toggle");if(!e)return;const n=t.voiceEnabled?"volume-2":"volume-x";e.title=t.voiceEnabled?"Voice is ON — click to mute":"Voice is OFF — click to enable",e.innerHTML=`<i data-lucide="${n}" class="w-4 h-4"></i>`,window.lucide&&lucide.createIcons()}function s(e,n=!0){const a=document.getElementById("kco-ai-messages");if(!a)return;const o=e.role==="user",i=document.createElement("div");i.className=`flex ${o?"justify-end":"justify-start"} ${n?"kco-ai-msg-in":""}`,i.innerHTML=o?`
  ...
  `:`
    <div class="max-w-[88%] flex gap-2">
      ...
      <div class="bg-slate-800/80 border border-white/8 rounded-2xl rounded-tl-sm px-3.5 py-2.5">
        <div class="text-sm text-gray-200 leading-relaxed">${C(e.content)}</div>
      </div>
    </div>
  `,a.appendChild(i),window.lucide&&lucide.createIcons(),w(),o||u(e.content)}function L(){const e=r(),n={en:`Welcome to Weverse Online Shop!
...
Hur kan jag hjälpa dig idag?`,sv2:"Välkommen till Weverse Online Shop!",el:`Καλώς ήρθατε στο Weverse Online Shop!
...
Naweza kukusaidia vipi leo?`};s({role:"assistant",content:n[e]||n.en})}function H(){const e=document.getElementById("kco-ai-messages");if(!e)return;const n=document.createElement("div");n.id="kco-ai-typing",n.className="flex justify-start kco-ai-fade",n.innerHTML=`
    ...
    </div>`,e.appendChild(n),window.lucide&&lucide.createIcons(),w()}function w(){const e=document.getElementById("kco-ai-messages");e&&(e.scrollTop=e.scrollHeight)}async function U(){const{data:e}=await v.auth.getSession();return{Authorization:`Bearer ${e.session?.access_token||O}`,"Content-Type":"application/json"}}async function l(){const e=document.getElementById("kco-ai-input");if(!e)return;const n=e.value.trim();if(!n||t.sending)return;e.value="",e.style.height="auto",t.sending=!0,document.getElementById("kco-ai-send").disabled=!0;const a={role:"user",content:n};t.history.push(a),s(a),H();try{const o=await U(),g=await(await fetch(y,{method:"POST",headers:o,body:JSON.stringify({action:"chat",message:n,history:t.history.slice(-20,-1).map(h=>({role:h.role,content:h.content})),language:r(),page_context:_(),cart_items:j()})})).json();document.getElementById("kco-ai-typing")?.remove();const m={role:"assistant",content:g.response||g.error||"I'm sorry, I didn't catch that. Could you rephrase?"};t.history.push(m),s(m)}catch{document.getElementById("kco-ai-typing")?.remove();const i={role:"assistant",content:"I'm having trouble connecting right now. Please try again in a moment."};t.history.push(i),s(i)}finally{t.sending=!1,document.getElementById("kco-ai-send").disabled=!1,document.getElementById("kco-ai-input")?.focus()}}function M(e,n){if(t.proactiveShown[e]||(t.proactiveShown[e]=!0,t.open)||!document.getElementById("kco-ai-fab"))return;const o=document.createElement("div");o.id=`kco-ai-nudge-${e}`,o.className="fixed bottom-[88px] right-5 z-[59] max-w-[280px] bg-slate-800 border border-blue-500/30 rounded-xl shadow-2xl px-3.5 py-2.5 kco-ai-fade",o.innerHTML=`
