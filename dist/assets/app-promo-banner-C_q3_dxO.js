import{D as q,l as g,b}from"./promo-backgrounds-B0OV0Ekm.js";import{d as H,W as B,D as G,l as j,a as P,g as D,p as F,e as u,c as w,b as v}from"./promo-pool-CHhdJDET.js";import{D as x,l as k,g as M}from"./site-content-Bs92LwMP.js";const I="support@weverseonlineshop.com",U="weverse_brand_v1";function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function O(){try{const e=JSON.parse(localStorage.getItem(U)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function R(){const e=O(),t=e.brand_name||e.site_name||H;return`
        ...
        <div class="mt-12 max-w-4xl mx-auto rounded-3xl border border-white/10 bg-white/[.05] backdrop-blur-md p-6 sm:p-8">
          <div class="flex flex-col lg:flex-row items-center gap-6 justify-between text-center lg:text-left">
            ...
            <div class="flex flex-col sm:flex-row gap-3 shrink-0">
              <a href="mailto:${o(I)}" class="btn-press inline-flex items-center gap-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
          ...
          <div class="mt-6 pt-5 border-t border-white/10 grid grid-cols-1 sm:grid-cols-3 gap-3 text-center text-[11px] text-slate-400">
            <span class="inline-flex items-center justify-center gap-1.5"><i data-lucide="mail" class="w-3.5 h-3.5 text-cyan-300"></i> ${o(I)}</span>
  ...
  <svg viewBox="0 0 560 720" class="w-full h-full block" aria-hidden="true">
      ...
      <a href="/product/${encodeURIComponent(i.property_id||i.id)}"
        ...
        <div class="aspect-square bg-gray-100 overflow-hidden">
          <img src="${l}" alt="${s}" loading="lazy" decoding="async"
               class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${C}'">
        </div>
        <div class="p-2">
          <p class="text-[10px] text-gray-700 font-bold leading-tight line-clamp-2 min-h-[26px]">${s}</p>
          <div class="flex items-center justify-between mt-1">
            <span class="text-[11px] text-blue-600 font-black">${v(i)}</span>
    ...
    <section class="relative overflow-hidden bg-[#060c1c] text-white">
      ...
      </a>`,window.lucide)try{lucide.createIcons()}catch{}};n(),y=setInterval(()=>{i+=2,n()},4500)}async function A(){if(document.body&&document.body.dataset.homepage==="true")return;const e=ie();if(!e)return;let t={...G};try{t=await j()}catch{}if(t.app_banner_enabled===!1)return;let a=[];try{await P(),a=D()||[]}catch{}const r=a.length?a:[{property_id:"browse",title:"Browse the full Weverse Online Shop",price:0,currency:"USD",images:["/fallback.svg"]}];let i={...q};try{i=await g()}catch{}const n=s=>{const p=e.querySelector('[data-bg-slot="app_banner"]');p&&(p.innerHTML=b(s.app_banner_bg_image,s.app_banner_bg_video))};async function l(){let s={...x};try{s=await k()}catch{}if(e.innerHTML=oe(t,r,s),window.lucide)try{lucide.createIcons()}catch{}de(r),window.dispatchEvent(new CustomEvent("app-promo-banner-ready")),n(i)}await l(),n(i),window.addEventListener("site-content-updated",()=>{l().catch(()=>{})}),window.addEventListener("promo-backgrounds-updated",()=>{g().then(s=>{i=s,n(s)}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",A):A();
