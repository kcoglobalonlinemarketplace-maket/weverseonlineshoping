import{V as f,k as w,f as G,J as P,G as F,H as R,S as Z,g as B,d as M,h as D,N as c,j as k,K as C}from"./supabase-client-DvCmNkpI-BBtcpOs0.js";const h={trust_promo_bg_image:"",trust_promo_bg_video:"",app_banner_bg_image:"",app_banner_bg_video:"",reviews_bg_image:"",reviews_bg_video:""},W=Object.keys(h).join(","),L="kco_promo_backgrounds_v1",V=60*1e3;function U(){try{const e=JSON.parse(localStorage.getItem(L)||"{}");if(e.ts&&Date.now()-e.ts<V&&e.data&&typeof e.data=="object")return e.data}catch{}return null}function N(e){try{localStorage.setItem(L,JSON.stringify({ts:Date.now(),data:e}))}catch{}}async function g(){const e=U();if(e)return{...h,...e};try{const t=await M(),{data:a,error:r}=await t.from("site_settings").select(W).limit(1).maybeSingle(),s={...h,...r||!a?{}:a};return N(s),s}catch{return{...h}}}function ge(){try{localStorage.removeItem(L)}catch{}window.dispatchEvent(new CustomEvent("promo-backgrounds-updated"))}function S(e,t){const a=(e||"").trim(),r=(t||"").trim(),s=[];return r&&s.push(`<video class="absolute inset-0 w-full h-full object-cover" autoplay muted loop playsinline webkit-playsinline preload="metadata" poster="${j(a)}"><source src="${j(r)}" type="video/mp4"></video>`),a&&s.push(`<div class="absolute inset-0 bg-cover bg-center" style="background-image:url('${Y(a)}')"></div>`),(r||a)&&s.push('<div class="absolute inset-0 bg-gradient-to-t from-slate-950/95 via-slate-900/70 to-slate-900/45"></div>'),s.join("")}function j(e){return String(e).replace(/&/g,"&amp;").replace(/"/g,"&quot;").replace(/</g,"&lt;").replace(/>/g,"&gt;")}function Y(e){return String(e).replace(/\\/g,"\\\\").replace(/'/g,"\\'")}const y="support@weverseonlineshop.com",z="weverse_brand_v1";function o(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function O(){try{const e=JSON.parse(localStorage.getItem(z)||"{}"),t=e.data&&typeof e.data=="object"?e.data:e;if(t&&typeof t=="object"&&(t.brand_name||t.site_name||t.brand_logo))return t}catch{}return{}}function J(){const e=O(),t=e.brand_name||e.site_name||Z;return`
    ...
    </div>`}function X(){const e=E.slice(0,6),t=E.slice(6),a=r=>r.map(Q).join("");return`
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 pt-14 sm:pt-20 pb-8 sm:pb-10">
        ...
        <div class="text-center max-w-3xl mx-auto">
          <div class="mx-auto w-16 h-16 rounded-2xl bg-white/[.07] border border-white/10 flex items-center justify-center overflow-hidden backdrop-blur">
            ...
            <span style="display:none">${B("w-9 h-9")}</span>
          </div>
    ...
    </section>`}function re(e){e.addEventListener("click",t=>{const a=t.target.closest("[data-acc]");if(!a)return;const r=a.dataset.acc,s=e.querySelector(`[data-acc-body="${r}"]`),i=e.querySelector(`[data-acc-icon="${r}"]`);s&&(s.dataset.open==="1"?(s.style.maxHeight="0px",s.style.opacity="0",s.dataset.open="0",i.classList.remove("rotate-180"),a.setAttribute("aria-expanded","false")):(s.style.maxHeight=s.scrollHeight+"px",s.style.opacity="1",s.dataset.open="1",i.classList.add("rotate-180"),a.setAttribute("aria-expanded","true")))})}function T(e){const t=document.querySelectorAll("[data-bg-slot]");t.length&&t.forEach(a=>{const r=a.dataset.bgSlot;r==="trust_promo"?a.innerHTML=S(e.trust_promo_bg_image,e.trust_promo_bg_video):r==="reviews"&&(a.innerHTML=S(e.reviews_bg_image,e.reviews_bg_video))})}function ie(e){const t=e.querySelector("#fb-stars");t&&t.addEventListener("click",s=>{const i=s.target.closest(".fb-star");if(!i)return;const l=parseInt(i.dataset.star,10),n=e.querySelector("#fb-rating");n&&(n.value=String(l)),t.querySelectorAll(".fb-star").forEach((p,x)=>{const u=p.querySelector("i, svg");u&&(x<l?(u.classList.add("fill-amber-400","text-amber-400"),u.classList.remove("text-slate-500")):(u.classList.remove("fill-amber-400","text-amber-400"),u.classList.add("text-slate-500")))})});const a=e.querySelector("#fb-form");a&&a.addEventListener("submit",s=>{s.preventDefault(),le(a)});const r=e.querySelector("[data-feedback-backtop]");r&&r.addEventListener("click",()=>{const s=e.querySelector('[data-acc="trust-reviews-more"]');s&&s.click();const i=document.getElementById("customer-feedback");i&&i.scrollIntoView({behavior:"smooth",block:"start"})})}function v(e,t,a){const r=e.closest("#customer-feedback")?.querySelector("#fb-msg");r&&(r.textContent=t,r.classList.remove("hidden","text-emerald-300","text-amber-300"),a&&r.classList.add(a))}async function le(e){const t=(e.querySelector("#fb-text")?.value||"").trim();if(!t){v(e,"Please write your feedback first.","text-amber-300");return}const a=e.querySelector("[type=submit]"),r=a.innerHTML;if(a.disabled=!0,a.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline mr-2"></i>Sending…',window.lucide)try{lucide.createIcons()}catch{}try{const s=await M();let i=null;try{i=(await s.auth.getUser()).data?.user?.id||null}catch{}const{error:l}=await s.from("site_feedback").insert({user_id:i,name:e.querySelector("#fb-name")?.value.trim()||"Anonymous shopper",email:e.querySelector("#fb-email")?.value.trim()||"",rating:parseInt(e.querySelector("#fb-rating")?.value||"5",10),feedback:t,is_approved:!1});if(l)throw new Error(l.message);v(e,"✓ Thank you! Your feedback has been sent.","text-emerald-300"),e.reset();const n=e.closest("#customer-feedback")?.querySelector("#fb-stars");n&&n.querySelectorAll(".fb-star").forEach(p=>{const x=p.querySelector("i, svg");x&&(x.classList.remove("fill-amber-400","text-amber-400"),x.classList.add("text-slate-500"))})}catch{v(e,"Could not send your feedback right now. Please try again later.","text-amber-300")}if(a.disabled=!1,a.innerHTML=r,window.lucide)try{lucide.createIcons()}catch{}}async function ne(e){const t=e?.querySelector("#fb-more-list");if(t)try{const a=await M(),{data:r,error:s}=await a.from("site_feedback").select("name,rating,feedback,created_at").eq("is_approved",!0).order("created_at",{ascending:!1}).limit(30);if(s||!r||!r.length)return;const i=r.map(l=>_({name:l.name||"Verified shopper",text:l.feedback||"",rating:l.rating||5,verified:!0,country:"Verified customer",date:l.created_at?new Date(l.created_at).toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"}):""})).join("");t.innerHTML=i+t.innerHTML}catch{}}function oe(){if(document.getElementById("trust-info-style"))return;const e=document.createElement("style");e.id="trust-info-style",e.textContent=`
    .trust-acc-body{overflow:hidden;max-height:0;opacity:0;transition:max-height .38s cubic-bezier(.2,.8,.2,1),opacity .28s ease}
    .trust-acc-body[data-open="1"]{opacity:1}`,document.head.appendChild(e)}async function A(){if(document.body&&document.body.dataset.homepage==="true")return;const e=document.getElementById("trust-info-area");if(!e)return;oe();let t={...f};try{t=await w()}catch{}if(e.innerHTML=[J(),K(),X(),se(),q(t)].join(""),window.lucide)try{lucide.createIcons()}catch{}re(e),ie(e),ne(e);let a={...h};try{a=await g()}catch{}T(a),window.addEventListener("promo-backgrounds-updated",()=>{g().then(T).catch(()=>{})}),window.addEventListener("site-content-updated",()=>{w().then(r=>{const s=e.querySelector("#site-closing-section");if(s&&(s.outerHTML=q(r)),window.lucide)try{lucide.createIcons()}catch{}}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",A):A();const de=()=>document.getElementById("app-promo-banner"),$="/fallback.svg";function ce(){return`
  ...
  <svg viewBox="0 0 560 720" class="w-full h-full block" aria-hidden="true">
  ...
  </svg>`}function ue(e){const t=e[0],a=e[1],r=(s,i)=>{const l=c(k(s)),n=c((s.title||s.name||"").slice(0,34));return`
    ...
    </div>`}function he(e,t,a){const r=D(t,e,12),s=(e.app_play_store_url||"").trim(),i={...f,...a||{}},l=(i.app_banner_title||e.app_banner_headline||G.app_banner_headline).trim(),n=i.app_banner_description,p=i.app_banner_button_text,x=i.app_banner_secondary_text,u=s?`<a href="${c(s)}" target="_blank" rel="noopener" class="inline-flex items-center gap-2.5 bg-white text-blue-900 font-black text-sm px-6 py-3.5 rounded-2xl shadow-xl shadow-blue-900/30 hover:scale-[1.03] active:scale-[.98] transition">
    ...
    <section class="relative overflow-hidden bg-[#060c1c] text-white">
      ...
      <div class="relative max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 py-10 sm:py-14 lg:py-16">
        <div class="grid lg:grid-cols-[1.05fr_.95fr] gap-10 lg:gap-6 items-center">
          ...
          <div class="max-w-xl lg:pr-6">
            ...
            <div class="flex flex-wrap items-center gap-3.5 mt-8">
              ...
              <a href="/#showroom-directory" class="inline-flex items-center gap-2 text-sm font-bold text-cyan-200 hover:text-white transition">
                ${c(x)} <i data-lucide="arrow-right" class="w-4 h-4"></i>
              </a>
            </div>
          ...
          <div class="relative mx-auto w-full max-w-[300px] sm:max-w-[360px] lg:max-w-[440px]">
            <div class="absolute inset-0 woman-back" aria-hidden="true">${ce()}</div>
      ...
      <a href="/details.html?id=${encodeURIComponent(l.property_id||l.id)}" class="block bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition active:scale-[.98]">
        <div class="aspect-square bg-gray-100 overflow-hidden"><img src="${p}" alt="${u}" class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${$}'"></div>
      ...
      </a>`,window.lucide)try{lucide.createIcons()}catch{}};i(),b=setInterval(()=>{s+=2,i()},4500)}async function H(){if(document.body&&document.body.dataset.homepage==="true")return;const e=de();if(!e)return;let t={...G};try{t=await P()}catch{}if(t.app_banner_enabled===!1)return;let a=[];try{await F(),a=R()||[]}catch{}const r=a.length?a:[{property_id:"browse",title:"Browse the full Weverse Online Shop",price:0,currency:"USD",images:["/fallback.svg"]}];let s={...h};try{s=await g()}catch{}const i=n=>{const p=e.querySelector('[data-bg-slot="app_banner"]');p&&(p.innerHTML=S(n.app_banner_bg_image,n.app_banner_bg_video))};async function l(){let n={...f};try{n=await w()}catch{}if(e.innerHTML=he(t,r,n),window.lucide)try{lucide.createIcons()}catch{}me(r),window.dispatchEvent(new CustomEvent("app-promo-banner-ready")),i(s)}await l(),i(s),window.addEventListener("site-content-updated",()=>{l().catch(()=>{})}),window.addEventListener("promo-backgrounds-updated",()=>{g().then(n=>{s=n,i(n)}).catch(()=>{})})}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",H):H();export{S,g as a,ge as g};
