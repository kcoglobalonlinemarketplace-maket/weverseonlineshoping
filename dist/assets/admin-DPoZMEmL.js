...
`}const ns=/^(n\/?a|none|unknown|not (available|specified|found|visible|applicable)|null|undefined|-{1,}|no data)$/i;function Gt(e,t){const a={...t||{}},i=new Set(Array.isArray(a.estimated)?a.estimated.map(u=>String(u)):[]),n=new Set(Array.isArray(a.missing_fields)?a.missing_fields.map(u=>String(u)):[]),s=[],r=[],l=u=>{if(u==null)return"";Array.isArray(u)&&(u=u.filter(h=>h!=null&&String(h).trim()!=="").join(", "));let p=String(u).replace(/\s+/g," ").trim();return p=p.replace(/^(answer|value|result|extracted)\s*[:\-]\s*/i,""),p},o=u=>{const p=l(u).replace(/[^0-9.,\-]/g,"").replace(/,(?=\d{3}\b)/g,"").replace(",","."),h=parseFloat(p);return Number.isFinite(h)?h:NaN};for(const u of e||[]){if(wi.has(u.key))continue;const p={key:u.key,label:u.label,status:"empty-ok",value:null,note:""};if(u.type==="checkbox-group"){const m=Array.isArray(a[u.key])?a[u.key].map(l).filter(Boolean):[],y=u.options&&u.options.length?m.filter(x=>u.options.includes(x)):m;y.length?(a[u.key]=y,p.status="filled",p.value=y.join(", ")):(delete a[u.key],m.length&&(p.status="flagged",p.note="values not in the allowed badge list were dropped",r.push(`${u.label}: invalid selection ignored`))),s.push(p);continue}if(!(a[u.key]!=null&&l(a[u.key])!=="")){p.status=n.has(u.key)?"missing":"empty-ok",s.push(p);continue}if(ns.test(l(a[u.key]))){delete a[u.key],n.add(u.key),p.status="missing",p.note="document/AI said the value is unavailable",s.push(p);continue}if(u.type==="number"){const m=l(a[u.key]),y=o(a[u.key]);if(!Number.isFinite(y)){delete a[u.key],n.add(u.key),p.status="flagged",p.note=`"${m}" is not a valid number`,r.push(`${u.label}: not a valid number`),s.push(p);continue}if(/year/.test(u.key)&&(y<1800||y>new Date().getFullYear()+2)){delete a[u.key],n.add(u.key),p.status="flagged",p.note=`${y} is outside the plausible range`,r.push(`${u.label}: implausible value ${y}`),s.push(p);continue}a[u.key]=y,p.status="filled",p.value=String(y),i.has(u.key)&&(p.status="estimated",p.note="AI estimate â€” confirm"),s.push(p);continue}if(u.type==="select"&&u.options&&u.options.length){const m=xi({options:u.options.map(y=>({value:y}))},l(a[u.key]));if(m==null){p.status="flagged",p.note=`"${l(a[u.key])}" does not match any option â€” left empty`,r.push(`${u.label}: no matching option`),delete a[u.key],n.add(u.key),s.push(p);continue}a[u.key]=m,p.status="filled",p.value=m,m!==l(t?.[u.key])&&(p.note="matched to the closest option"),s.push(p);continue}let f=l(a[u.key]);u.type!=="textarea"&&u.type!=="text-long"&&f.length>120&&!["title"].includes(u.key)&&(p.status="flagged",p.note="unusually long â€” check it landed in the right field",r.push(`${u.label}: suspiciously long value`)),a[u.key]=f,p.status="filled",p.value=f.length>48?f.slice(0,48)+"â€¦":f,i.has(u.key)&&(p.status="estimated",p.note="AI estimate â€” confirm"),s.push(p)}if(e&&e.length){const u=new Set([...e.map(p=>p.key),"estimated","missing_fields","features","highlights","seo_keywords"]);Object.keys(a).forEach(p=>{u.has(p)||delete a[p]})}a.missing_fields=s.filter(u=>u.status==="missing").map(u=>u.key),a.estimated=s.filter(u=>u.status==="estimated").map(u=>u.key);const c={total:s.length,filled:s.filter(u=>u.status==="filled").length,estimated:s.filter(u=>u.status==="estimated").length,flagged:s.filter(u=>u.status==="flagged").length,missing:s.filter(u=>u.status==="missing").length};return{specs:a,checklist:s,flags:r,summary:c}}function ya(e,t){if(!e||!e.length)return"";const a={filled:'<span class="text-emerald-400 font-bold">âœ“</span>',estimated:'<span class="text-blue-300 font-bold">â‰ˆ</span>',flagged:'<span class="text-red-400 font-bold">!</span>',missing:'<span class="text-gray-500">â€”</span>',"empty-ok":'<span class="text-gray-700">Â·</span>'},i=e.filter(s=>s.status!=="empty-ok").map(s=>`<li class="flex items-start gap-2"><span class="shrink-0 w-4">${a[s.status]||""}</span><span><b>${d(s.label)}</b> <span class="text-gray-600">(${d(s.key)})</span>${s.value?` â€” <span class="text-gray-300">${d(String(s.value))}</span>`:""}${s.note?` <span class="text-gray-500">${d(s.note)}</span>`:""}</span></li>`).join(""),n=t.total-t.filled-t.estimated-t.flagged-t.missing;return`<details class="mt-2 rounded-xl border border-violet-500/20 bg-violet-500/5 p-3">
  ...
  <div class="scan-review-card rounded-xl border border-violet-500/30 bg-violet-500/10 p-3 space-y-2 fade-in" data-i="${t}">
    ...
    </div>`),scanReviewRender(),window.lucide&&lucide.createIcons(),!0)};async function Qe(e,t){const a=Tt(e,ye),i=Ve(e.category),n=e.listing_type==="property"||i&&i.listing_type==="property",s=n?"Real Estate":i.category||e.category||"Other",r=ki,l=r-S.length;((c,u)=>{const p=document.getElementById("scanner-scan-status");p&&(p.classList.remove("hidden","text-red-400","text-emerald-300","text-amber-300","text-blue-300","text-gray-400"),p.classList.add(u),p.innerHTML=c)})(`Processing ${l+1} of ${r}: ${d(e.detected_name||e.title||"product")}â€¦`,"text-blue-300");try{if(n){Y++,S.splice(t,1),S.length?Qe(S[0],0):window.returnToScanReviewAfterSave(-1);return}let c=e.property_id?Te[e.property_id]:null;c&&c.specifications&&typeof c.specifications=="object"&&(c={...c,...c.specifications}),showAddProductStep2(s,c?{...c,images:a}:{images:a}),await new Promise(h=>setTimeout(h,250)),await et(e,a,s);const p=document.getElementById("product-form")?.querySelector("[type=submit][name=action][value=publish]");p?(te=t,p.click()):(Y++,closeProductFormModal(),S.splice(t,1),S.length?Qe(S[0],0):window.returnToScanReviewAfterSave(-1))}catch{Y++,closeProductFormModal(),S.splice(t,1),S.length?Qe(S[0],0):window.returnToScanReviewAfterSave(-1)}}window.openGeneralAiScanner=async function(e=!1){ve=!!e;const t=await Ai();D(`
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-box">
        <div class="flex items-center justify-between mb-5">
      ...
      <div class="space-y-4 fade-in">
        ...
        <div class="glass-soft border border-blue-500/15 rounded-2xl p-5 space-y-4">
          ...
          <div class="space-y-3" id="reviews-list">
            ${a.length===0?Ie("star","No Reviews","Customer reviews will appear here."):a.map(l=>Ts(l)).join("")}
          </div>
        </div>
        ...
        <div class="glass-soft border border-emerald-500/15 rounded-2xl p-5 space-y-4">
          ...
          <div class="space-y-3" id="feedback-list">
            ${s.length===0?Ie("message-square","No Feedback Yet","Site feedback will appear here."):s.map(l=>Is(l)).join("")}
          </div>
        </div>
