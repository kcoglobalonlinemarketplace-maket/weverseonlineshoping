...
import{supabase as f}from"./supabase-client-7_ZWSEp6.js";import"./localization-BOJr-DSq.js";import{_ as W}from"./preload-helper-CLcXU_4U.js";import{c as $,S as F,l as O,f as U,a as B,b as D,P as G}from"./showroom-data-D-nP9j-R.js";import{g as Y,a as K,b as J,c as Q,P as X,f as Z,T as ee,M as te,C as ae}from"./phone-data-CPq8Rkux.js";import{g as w,f as I}from"./auth-D3ZJ7kZA.js";import"./live-stream-mode-CqE1vhB3.js";import"./customer-ai-widget-D685McTv.js";import"./brand-DYhAh01b.js";const h="/fallback.svg";function T(e){return typeof e=="number"&&!isNaN(e)?e.toFixed(1):"0.0"}function j(e){return Array.isArray(e)&&e.length>0?e:[h]}function re(){return new URLSearchParams(window.location.search).get("id")}function le(e){const a=document.getElementById("details-content"),i=Z(e),d=j(e.images).map((t,c)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${c===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${r(t)}">
      <img src="${r(t)}" alt="View ${c+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${h}'">
    </button>`).join(""),s=["Front View","Rear View","Left Side","Right Side","Interior Dashboard","Driver Seat","Cargo Area / Truck Bed","Engine","Wheels / Tires","Additional View"],o=[{icon:"building-2",label:"Brand",value:e.brand},{icon:"car",label:"Model",value:e.model},{icon:"calendar",label:"Model Year",value:e.model_year},{icon:"badge-check",label:"Condition",value:e.condition},{icon:"gauge",label:"Mileage",value:e.mileage},{icon:"cog",label:"Transmission",value:e.transmission},{icon:"fuel",label:"Fuel Type",value:e.fuel_type},{icon:"zap",label:"Engine",value:e.engine},{icon:"truck",label:"Drive Type",value:e.drive_type},{icon:"palette",label:"Colour",value:e.color},{icon:"package",label:"Payload Capacity",value:e.payload_capacity},{icon:"link",label:"Towing Capacity",value:e.towing_capacity},{icon:"barcode",label:"VIN",value:e.vin},{icon:"tag",label:"Stock Number",value:e.stock_number}].filter(t=>t.value!=null&&t.value!==""&&t.value!=="N/A"),b=e.features?.length?`
  ...
  `;const g=document.getElementById("hero-image"),x=document.getElementById("gallery-label");a.querySelectorAll(".gallery-thumb").forEach((t,c)=>{t.addEventListener("click",()=>{a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.remove("active","border-blue-500")),a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.add("border-gray-800")),t.classList.add("active","border-blue-500"),t.classList.remove("border-gray-800"),g.src=t.dataset.img,x.textContent=s[c]||`View ${c+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const t=window.location.href;try{if(navigator.share)await navigator.share({title:r(e.title),url:t});else{await navigator.clipboard.writeText(t);const c=document.getElementById("share-btn"),m=c.innerHTML;c.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{c.innerHTML=m,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),_(e),window.lucide&&lucide.createIcons()}function ie(e){const a=document.getElementById("details-content"),i=B(e),d=j(e.images).map((t,c)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${c===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${r(t)}">
      <img src="${r(t)}" alt="View ${c+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${h}'">
    </button>`).join(""),s=["Exterior Front","Exterior Side","Exterior Rear","Living Area","Kitchen","Bedroom","Bathroom","Additional View"],o=[{icon:"building-2",label:"Brand",value:e.brand},{icon:"car",label:"Model",value:e.model},{icon:"calendar",label:"Model Year",value:e.model_year},{icon:"badge-check",label:"Condition",value:e.condition},{icon:"gauge",label:"Mileage",value:e.mileage},{icon:"cog",label:"Transmission",value:e.transmission},{icon:"fuel",label:"Fuel Type",value:e.fuel_type},{icon:"zap",label:"Engine",value:e.engine},{icon:"bus",label:"Type",value:e.property_type},{icon:"truck",label:"Drive Type",value:e.drive_type},{icon:"palette",label:"Colour",value:e.color},{icon:"moon",label:"Sleeping Capacity",value:e.sleeping_capacity},{icon:"users",label:"Seating Capacity",value:e.seating_capacity},{icon:"shower-head",label:"Bathroom",value:e.bathroom},{icon:"utensils",label:"Kitchen",value:e.kitchen},{icon:"droplet",label:"Water Tank",value:e.water_tank},{icon:"barcode",label:"VIN",value:e.vin},{icon:"tag",label:"Stock Number",value:e.stock_number}].filter(t=>t.value!=null&&t.value!==""&&t.value!=="N/A"),b=e.features?.length?`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
      <h3 class="text-sm font-bold text-white uppercase tracking-wide mb-4">Features</h3>
      <div class="flex flex-wrap gap-2">
        ${e.features.map(t=>`<span class="text-xs bg-gray-800 text-gray-300 px-3 py-1.5 rounded-full border border-gray-700">${r(t)}</span>`).join("")}
  ...
  `;const g=document.getElementById("hero-image"),x=document.getElementById("gallery-label");a.querySelectorAll(".gallery-thumb").forEach((t,c)=>{t.addEventListener("click",()=>{a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.remove("active","border-blue-500")),a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.add("border-gray-800")),t.classList.add("active","border-blue-500"),t.classList.remove("border-gray-800"),g.src=t.dataset.img,x.textContent=s[c]||`View ${c+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const t=window.location.href;try{if(navigator.share)await navigator.share({title:r(e.title),url:t});else{await navigator.clipboard.writeText(t);const c=document.getElementById("share-btn"),m=c.innerHTML;c.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{c.innerHTML=m,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),_(e,te),window.lucide&&lucide.createIcons()}function oe(e){const a=document.getElementById("details-content"),i=B(e),d=j(e.images).map((t,c)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${c===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${r(t)}">
      <img src="${r(t)}" alt="View ${c+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${h}'">
  ...
  `;const g=document.getElementById("hero-image"),x=document.getElementById("gallery-label");a.querySelectorAll(".gallery-thumb").forEach((t,c)=>{t.addEventListener("click",()=>{a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.remove("active","border-blue-500")),a.querySelectorAll(".gallery-thumb").forEach(m=>m.classList.add("border-gray-800")),t.classList.add("active","border-blue-500"),t.classList.remove("border-gray-800"),g.src=t.dataset.img,x.textContent=s[c]||`View ${c+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const t=window.location.href;try{if(navigator.share)await navigator.share({title:r(e.title),url:t});else{await navigator.clipboard.writeText(t);const c=document.getElementById("share-btn"),m=c.innerHTML;c.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{c.innerHTML=m,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),_(e,ae),window.lucide&&lucide.createIcons()}function se(e){const i=e.listing_type==="property"?"Share Property":"Share";return`
  ...
  `}function A(e){const a=e.listing_type==="property",i=`/contact.html?listing=${encodeURIComponent(e.property_id||"")}`;return`
    <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5">
    ...
    </div>`}function de(e){const a=e.images&&e.images[0]||"/fallback.svg";return`<a href="/details.html?id=${encodeURIComponent(e.property_id)}" class="block bg-gray-900/60 border border-gray-800 rounded-xl overflow-hidden hover:border-blue-500/30 transition group">
      ...
      <div class="p-2"><p class="text-xs text-white font-bold truncate">${r(e.title)}</p><p class="text-xs text-blue-500 font-bold mt-1">${B(e)}</p></div>
    </a>`}function C(e,a){const i=document.getElementById(e);if(!i)return;const u=i.querySelector(".rel-grid");if(u){if(!a.length){i.classList.add("hidden");return}i.classList.remove("hidden"),u.innerHTML=a.slice(0,4).map(de).join("")}}function _(e,a){let i;a?i=a.filter(o=>o.property_id!==e.property_id):e.listing_type==="pet"?i=G.filter(o=>o.property_id!==e.property_id):i=ee.filter(o=>o.property_id!==e.property_id);const u=i.filter(o=>o.category===e.category),d=i.filter(o=>o.breed&&o.breed===e.breed||o.brand&&o.brand===e.brand),s=[...i].sort((o,b)=>(b.rating||0)-(o.rating||0));C("similar-section",u.length?u:s.slice(0,4)),C("related-section",d.length?d:s.slice(0,4)),C("recommended-section",s)}function N(e){const a=document.getElementById("details-content"),i=e.listing_type==="property",u=B(e),d=D(e.country_code),s=e.listing_type==="product"?"Product ID":i?"Property ID":"Listing ID",b=j(e.images).map((n,l)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${l===0?"active border-blue-500":"border-gray-800"} shrink-0" data-img="${r(n)}">
      ...
      <div class="bg-[#0f172a]/60 border border-gray-800 rounded-xl p-5 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ${[{icon:"globe",label:"Country",value:`${d} ${e.country}`},{icon:"map-pin",label:"State / Province",value:e.state},{icon:"building",label:"City",value:e.city},{icon:"navigation",label:"Town / Local Area",value:e.town}].filter(l=>l.value).map(l=>`
    ...
    <div class="fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-black text-white leading-tight">${r(e.title)}</h1>
          <p class="text-gray-500 text-sm mt-1">${s}: <span class="text-blue-500 font-mono font-bold">${r(e.property_id)}</span></p>
        </div>
        <div class="text-right shrink-0">
  ...
  `;const m=document.getElementById("hero-image");a.querySelectorAll(".gallery-thumb").forEach(n=>{n.addEventListener("click",()=>{a.querySelectorAll(".gallery-thumb").forEach(l=>l.classList.remove("active","border-blue-500")),a.querySelectorAll(".gallery-thumb").forEach(l=>l.classList.add("border-gray-800")),n.classList.add("active","border-blue-500"),n.classList.remove("border-gray-800"),m.src=n.dataset.img})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const n=window.location.href;try{if(navigator.share)await navigator.share({title:r(e.title),url:n});else{await navigator.clipboard.writeText(n);const l=document.getElementById("share-btn"),E=l.innerHTML;l.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{l.innerHTML=E,window.lucide&&lucide.createIcons()},2e3)}}catch{}});const R=document.getElementById("view-details-btn");R&&R.addEventListener("click",()=>{const n=document.getElementById("listing-details");n&&n.scrollIntoView({behavior:"smooth",block:"start"})});const S=document.getElementById("add-cart-btn");S&&S.addEventListener("click",()=>{let n=JSON.parse(localStorage.getItem("kco_cart")||"[]");n.includes(e.property_id)||(n.push(e.property_id),localStorage.setItem("kco_cart",JSON.stringify(n))),S.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Added to Cart',window.lucide&&lucide.createIcons(),setTimeout(()=>{S.innerHTML='<i data-lucide="shopping-cart" class="w-5 h-5"></i> Add to Cart',window.lucide&&lucide.createIcons()},2e3)}),ne(e),ue(e),z(e),be(e),window.lucide&&lucide.createIcons(),e.property_id,e.title,parseFloat(e.price),e.currency;const y=document.getElementById("listing-map");if(y&&window.L){const n=parseFloat(e.latitude)||null,l=parseFloat(e.longitude)||null,E=[e.town,e.city,e.state,e.country].filter(Boolean).join(", ");if(n&&l){const v=L.map(y).setView([n,l],13);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(v),L.marker([n,l]).addTo(v).bindPopup(e.title)}else E?fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q="+encodeURIComponent(E)).then(v=>v.json()).then(v=>{if(v&&v[0]){const P=parseFloat(v[0].lat),H=parseFloat(v[0].lon),q=L.map(y).setView([P,H],12);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(q),L.marker([P,H]).addTo(q).bindPopup(e.title)}else y.style.display="none"}).catch(()=>{y.style.display="none"}):y.style.display="none"}}let k=0,V=!1;function ce(){if(V)return;V=!0;const e=document.createElement("style");e.textContent="@keyframes kcoWishPop{0%{transform:scale(1)}35%{transform:scale(1.45)}60%{transform:scale(.86)}100%{transform:scale(1)}}",document.head.appendChild(e)}function M(e,a){if(!e)return;ce(),e.innerHTML=`<i data-lucide="heart" class="w-5 h-5 ${a?"fill-red-500 text-red-500":""}"></i>`,e.classList.toggle("bg-red-500/10",a),e.classList.toggle("border",a),e.classList.toggle("border-red-500/20",a);const i=e.querySelector("span");i&&(i.textContent=a?"Saved to Wishlist":"Add to Wishlist"),window.lucide&&lucide.createIcons();const u=e.querySelector("i");u&&(u.style.animation="none",u.offsetWidth,u.style.animation="kcoWishPop .5s cubic-bezier(.34,1.56,.64,1)",setTimeout(()=>{u&&(u.style.animation="")},550))}async function ne(e){const a=document.getElementById("wishlist-btn");if(!a)return;const i=await w();if(!i){a.addEventListener("click",()=>{I(window.location.pathname+window.location.search),window.location.href=`/auth.html?redirect=${encodeURIComponent(window.location.pathname+window.location.search)}`});return}const{data:u,error:d}=await f.from("wishlist").select("id").eq("user_id",i.id).eq("listing_id",e.id).maybeSingle();if(d){console.error("Wishlist check failed:",d.message);return}u&&M(a,!0),a.addEventListener("click",async()=>{const{data:s,error:o}=await f.from("wishlist").select("id").eq("user_id",i.id).eq("listing_id",e.id).maybeSingle();if(o){console.error("Wishlist toggle failed:",o.message);return}if(s){const{error:b}=await f.from("wishlist").delete().eq("id",s.id);if(b){console.error("Wishlist delete failed:",b.message);return}M(a,!1)}else{const{error:b}=await f.from("wishlist").insert({user_id:i.id,listing_id:e.id});if(b){console.error("Wishlist insert failed:",b.message);return}M(a,!0)}})}async function ue(e){const a=document.getElementById("review-form"),i=document.getElementById("review-login-msg"),u=await w();if(!u){a.classList.add("hidden"),i.classList.remove("hidden");return}document.querySelectorAll(".star-btn").forEach(d=>{d.addEventListener("click",()=>{k=parseInt(d.dataset.rating,10),document.querySelectorAll(".star-btn").forEach((s,o)=>{const b=s.querySelector("i");o<k?(b.classList.add("fill-blue-500","text-blue-500"),b.classList.remove("text-gray-600")):(b.classList.remove("fill-blue-500","text-blue-500"),b.classList.add("text-gray-600"))})})}),a.addEventListener("submit",async d=>{d.preventDefault();const s=document.getElementById("review-text").value.trim();if(!k){alert("Please select a rating.");return}if(!s){alert("Please write a review.");return}const{error:o}=await f.from("product_reviews").insert({listing_id:e.id,user_id:u.id,rating:k,review_text:s,is_approved:!1});if(o){alert("Error: "+o.message);return}document.getElementById("review-text").value="",k=0,document.querySelectorAll(".star-btn").forEach(b=>{const p=b.querySelector("i");p.classList.remove("fill-blue-500","text-blue-500"),p.classList.add("text-gray-600")}),alert("Review submitted! It will appear after admin approval."),z(e)})}async function z(e){const a=document.getElementById("reviews-list");if(!a||!e.id)return;const{data:i,error:u}=await f.from("product_reviews").select("*, profiles(full_name)").eq("listing_id",e.id).eq("is_approved",!0).order("created_at",{ascending:!1});if(u){a.innerHTML='<p class="text-gray-500 text-sm">Unable to load reviews.</p>';return}if(!i||i.length===0){a.innerHTML='<p class="text-gray-500 text-sm">No reviews yet. Be the first to review this product!</p>';return}a.innerHTML=i.map(d=>`
    <div class="border-b border-gray-800 pb-3 mb-3 last:border-0">
      <div class="flex items-center gap-2 mb-1">
        <div class="flex gap-0.5">${[1,2,3,4,5].map(s=>`<i data-lucide="star" class="w-3.5 h-3.5 ${s<=d.rating?"fill-amber-400 text-amber-400":"text-gray-600"}"></i>`).join("")}</div>
        <span class="text-xs text-gray-400 font-bold">${r(d.profiles?.full_name||"Anonymous")}</span>
        <span class="text-xs text-gray-600">${new Date(d.created_at).toLocaleDateString()}</span>
        ${d.is_verified_purchase?'<span class="text-[10px] font-bold text-green-400 bg-green-500/10 px-2 py-0.5 rounded-full">Verified Purchase</span>':""}
      </div>
      <p class="text-sm text-gray-300">${r(d.review_text||"")}</p>
      ${d.vendor_response?`<div class="mt-2 bg-gray-800/50 rounded-lg p-2 text-xs text-gray-400"><strong class="text-gray-300">Seller response:</strong> ${r(d.vendor_response)}</div>`:""}
    </div>`).join(""),window.lucide&&lucide.createIcons()}async function be(e){const a=document.getElementById("recommendations-section"),i=document.getElementById("recommendations-grid");if(!a||!i||!e.id)return;const{data:u,error:d}=await f.from("product_recommendations").select("recommended_listing_id, showroom_listings!product_recommendations_recommended_listing_id_fkey(property_id, title, price, currency, images, listing_type)").eq("listing_id",e.id).order("score",{ascending:!1}).limit(4);if(d){console.error("Recommendations load failed:",d.message),a.classList.add("hidden");return}let s=(u||[]).map(o=>o.showroom_listings).filter(Boolean);if(s.length<4){const{data:o}=await f.from("showroom_listings").select("property_id, title, price, currency, images, listing_type").eq("category",e.category).neq("id",e.id).eq("is_active",!0).order("rating",{ascending:!1}).limit(4-s.length);s=[...s,...o||[]]}if(s.length===0){a.classList.add("hidden");return}a.classList.remove("hidden"),i.innerHTML=s.map(o=>{const b=o.images&&o.images[0]||"/fallback.svg",p=typeof o.price=="number"?o.price:parseFloat(o.price||0),g=o.currency||"USD";return`<a href="/details.html?id=${o.property_id}" class="block bg-gray-900/60 border border-gray-800 rounded-xl overflow-hidden hover:border-blue-500/30 transition group">
    ...
    </a>`}).join("")}function r(e){return String(e||"").replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}async function me(){const e=re();if(!e){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}const a=Y(e);if(a){$(a),document.title=`${a.title} | Weverse Online Shop`,le(a);return}const i=K(e);if(i){$(i),document.title=`${i.title} | Weverse Online Shop`,ie(i);return}const u=J(e);if(u){$(u),document.title=`${u.title} | Weverse Online Shop`,oe(u);return}const d=Q(e);if(d){$(d),document.title=`${d.title} | Weverse Online Shop`,N(d);try{_(d,X)}catch{}return}let s=F.find(o=>o.property_id===e);if(!s){const[{generateListingById:o},{loadHiddenCatalogIds:b}]=await Promise.all([W(()=>import("./catalog-CESpPuFx.js"),__vite__mapDeps([0,1,2])),W(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await b(),s=o(e)}if(s||(await O(),s=U(e)),!s){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}if($(s),document.title=`${s.title} | Weverse Online Shop`,N(s),s.listing_type==="product")try{_(s,F.filter(o=>o.listing_type==="product"))}catch{}}me();
