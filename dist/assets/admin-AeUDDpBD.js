...
<div class="glass-soft border border-blue-500/15 rounded-2xl overflow-hidden">
    ...
    </div>`,window.lucide&&lucide.createIcons())}window.openSidebar=()=>{document.getElementById("sidebar").classList.add("open"),document.getElementById("sidebar-overlay").classList.remove("hidden")};window.closeSidebar=()=>{document.getElementById("sidebar").classList.remove("open"),document.getElementById("sidebar-overlay").classList.add("hidden")};document.getElementById("close-sidebar")?.addEventListener("click",closeSidebar);const he="kco_admin_remember",It="kco_login_attempts",ct=5,gi=15*60*1e3;function D(e){const t=document.getElementById("login-error"),a=document.getElementById("login-error-text");!t||!a||(a.textContent=e,t.classList.remove("hidden"),document.getElementById("login-success")?.classList.add("hidden"),window.lucide&&lucide.createIcons())}function yi(e){const t=document.getElementById("login-success"),a=document.getElementById("login-success-text");!t||!a||(a.textContent=e,t.classList.remove("hidden"),document.getElementById("login-error")?.classList.add("hidden"))}function Je(){document.getElementById("login-error")?.classList.add("hidden"),document.getElementById("login-success")?.classList.add("hidden")}function Be(e){return String(e||"").trim().toLowerCase()}function fi(){try{const e=JSON.parse(localStorage.getItem(he)||"{}");e?.email&&!Be(e.email)&&localStorage.removeItem(he)}catch{localStorage.removeItem(he)}}function hi(){try{const e=JSON.parse(localStorage.getItem(he)||"{}");return Be(e?.email)}catch{return""}}function At(){fi();const e=hi(),t=document.getElementById("login-email");t&&(t.value=e||t.value||na,t.removeAttribute("readonly"));const a=document.getElementById("reset-email");a&&(a.value=e||a.value||"",a.removeAttribute("readonly"))}function vi(){return`${window.location.origin}/admin.html`}function ge(e){const t=document.getElementById("login-header-title"),a=document.getElementById("login-header-icon");document.getElementById("login-form")?.classList.toggle("hidden",e!=="login"),document.getElementById("twofa-form")?.classList.toggle("hidden",e!=="2fa"),document.getElementById("forgot-form")?.classList.toggle("hidden",e!=="forgot"),Je(),e==="login"&&(t&&(t.textContent="Admin Access"),a&&a.setAttribute("data-lucide","shield-check")),e==="2fa"&&(t&&(t.textContent="Two-Factor Auth"),a&&a.setAttribute("data-lucide","smartphone")),e==="forgot"&&(t&&(t.textContent="Reset Password"),a&&a.setAttribute("data-lucide","mail")),window.lucide&&lucide.createIcons()}function F(e,t,a=""){const i=document.getElementById(e);i&&(i.disabled=t,t?i.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline-block mr-1"></i> Please wait…':a&&(i.innerHTML=a),window.lucide&&lucide.createIcons())}function ua(){try{return JSON.parse(localStorage.getItem(It)||'{"count":0}')}catch{return{count:0}}}function pa(){const e=ua();return e.count=(e.count||0)+1,e.count>=ct&&(e.lockedUntil=Date.now()+gi),localStorage.setItem(It,JSON.stringify(e)),e}function ma(){localStorage.removeItem(It)}function ba(){const e=ua();if(!e.lockedUntil)return null;const t=e.lockedUntil-Date.now();return t<=0?(ma(),null):Math.ceil(t/6e4)}async function ne(e,t,a={}){try{await m.from("admin_security_logs").insert({user_id:e,event_type:t,ip_address:await xi(),user_agent:navigator.userAgent.slice(0,200),...a})}catch{}}async function xi(){try{return(await(await fetch("https://api64.ipify.org?format=json",{signal:AbortSignal.timeout(3e3)})).json()).ip||"unknown"}catch{return"unknown"}}async function ga(e){if(!e)return!1;let t=!1,a=!1;try{const{data:i}=await m.rpc("is_current_user_admin");t=!0,a=!!i}catch{t=!1}return t?a:Be(e.email)===na}async function wi(){const e=window.location.hash;if(e.includes("type=recovery")||e.includes("access_token")){Oe(),Ei();return}const{data:{session:t}}=await m.auth.getSession();if(t?.user&&await ga(t.user)){const{data:{currentUser:i}}=await m.auth.getUser(),o=await m.auth.mfa.getAuthenticatorAssuranceLevel(),n=o.data?.currentLevel;if(o.data?.nextLevel==="aal2"&&n!=="aal2"){I.user=t.user,Oe(),ge("2fa"),Ct();return}I.user=t.user,Qe();return}_i()}function Oe(){const e=document.getElementById("login-screen");e&&(e.style.display="flex")}function _i(){Oe(),ge("login"),At(),ya(),fa(),Ct(),ki();const e=ba();e&&(D(`Too many failed attempts. Try again in ${e} minute${e>1?"s":""}.`),document.getElementById("login-btn").disabled=!0)}function ki(){document.getElementById("toggle-pw")?.addEventListener("click",()=>{const e=document.getElementById("login-password"),t=document.querySelector("#toggle-pw i");e&&(e.type=e.type==="password"?"text":"password",t&&t.setAttribute("data-lucide",e.type==="password"?"eye":"eye-off"),window.lucide&&lucide.createIcons())})}function ya(){const e=document.getElementById("login-form");!e||e._bound||(e._bound=!0,e.addEventListener("submit",Si),document.getElementById("forgot-pw-btn")?.addEventListener("click",()=>ge("forgot")))}async function Si(e){e.preventDefault();const t=ba();if(t){D(`Account locked. Try again in ${t} minute${t>1?"s":""}.`);return}const a=document.getElementById("login-email"),i=Be(a?.value);if(!i){D("Enter your admin email address."),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}const o=document.getElementById("login-password").value,n=document.getElementById("remember-me")?.checked;F("login-btn",!0),Je();const{data:r,error:l}=await m.auth.signInWithPassword({email:i,password:o});if(l||!r.user){const g=String(l?.message||"").toLowerCase();if(g.includes("missing supabase credentials")||g.includes("authentication service is unavailable")){D("Authentication is temporarily unavailable due to configuration. Please contact support."),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}if(g.includes("failed to fetch")||g.includes("network request failed")){D("Network error while signing in. Check your connection and try again."),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}if(g.includes("email not confirmed")){D("Your admin email is not confirmed yet. Open your verification email and confirm first."),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In');return}const y=pa(),b=ct-y.count,v=y.lockedUntil?`Account locked for 15 minutes after ${ct} failed attempts.`:`Invalid email or password. ${b>0?b+" attempt"+(b!==1?"s":"")+" remaining.":""}`;D(v),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),r?.user&&await ne(r.user.id,"login_failed",{metadata:{reason:"wrong_password"}});return}if(!await ga(r.user)){await m.auth.signOut(),D(`Access denied for ${r.user.email}. This account is signed in but does not have administrator privileges.`),F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),await ne(r.user.id,"login_denied",{metadata:{reason:"not_admin"}});return}if(n?localStorage.setItem(he,JSON.stringify({email:i,ts:Date.now()})):localStorage.removeItem(he),ma(),I.user=r.user,(await m.auth.mfa.getAuthenticatorAssuranceLevel()).data?.nextLevel==="aal2"){F("login-btn",!1,'<i data-lucide="log-in" class="w-4 h-4 inline mr-1"></i> Sign In'),ge("2fa"),Ct(),setTimeout(()=>document.getElementById("totp-code")?.focus(),100);return}await ne(r.user.id,"login_success"),F("login-btn",!1),Qe()}function Ct(){const e=document.getElementById("verify-2fa-btn");e&&!e._bound&&(e._bound=!0,e.addEventListener("click",qt));const t=document.getElementById("totp-code");t&&!t._bound&&(t._bound=!0,t.addEventListener("input",i=>{i.target.value=i.target.value.replace(/\D/g,"").slice(0,6),i.target.value.length===6&&qt()})),document.getElementById("cancel-2fa-btn")?.addEventListener("click",async()=>{await m.auth.signOut(),I.user=null,ge("login")}),document.getElementById("use-backup-btn")?.addEventListener("click",()=>{document.getElementById("backup-code-wrap")?.classList.toggle("hidden");const o=document.getElementById("backup-code");o&&o.focus()});const a=document.getElementById("verify-backup-btn");a&&!a._bound&&(a._bound=!0,a.addEventListener("click",$i))}async function qt(){const e=document.getElementById("totp-code")?.value?.trim();if(!e||e.length!==6){D("Enter the 6-digit code from your authenticator app.");return}F("verify-2fa-btn",!0),Je();try{const{data:t}=await m.auth.mfa.listFactors(),a=(t?.totp||[])[0];if(!a){D("No 2FA factor found. Please re-login."),F("verify-2fa-btn",!1,'<i data-lucide="shield-check" class="w-4 h-4 inline mr-1"></i> Verify & Sign In');return}const{data:i,error:o}=await m.auth.mfa.challenge({factorId:a.id});if(o)throw o;const{error:n}=await m.auth.mfa.verify({factorId:a.id,challengeId:i.id,code:e});if(n)throw n;await ne(I.user.id,"login_2fa_success"),F("verify-2fa-btn",!1),Qe()}catch(t){pa(),D(t.message?.includes("Invalid")?"Incorrect code. Check your authenticator and try again.":t.message),F("verify-2fa-btn",!1,'<i data-lucide="shield-check" class="w-4 h-4 inline mr-1"></i> Verify & Sign In'),document.getElementById("totp-code").value="",document.getElementById("totp-code").focus()}}async function $i(){const e=document.getElementById("backup-code")?.value?.trim().toUpperCase().replace(/\s/g,"");if(!e){D("Enter a backup recovery code.");return}F("verify-backup-btn",!0);try{const{data:t}=await m.from("admin_2fa").select("backup_codes").eq("user_id",I.user.id).maybeSingle();if(!t?.backup_codes?.length){D("No backup codes found."),F("verify-backup-btn",!1,"Use Backup Code");return}if(!t.backup_codes.find(o=>(o.code||o).toUpperCase().replace(/-/g,"")===e.replace(/-/g,"")&&!o.used)){D("Backup code not found or already used."),F("verify-backup-btn",!1,"Use Backup Code");return}const i=t.backup_codes.map(o=>(o.code||o).toUpperCase().replace(/-/g,"")===e.replace(/-/g,"")?{...typeof o=="object"?o:{code:o},used:!0}:o);await m.from("admin_2fa").update({backup_codes:i}).eq("user_id",I.user.id),await ne(I.user.id,"login_backup_code_used"),Qe()}catch(t){D(t.message),F("verify-backup-btn",!1,"Use Backup Code")}}function fa(){document.getElementById("back-to-login")?.addEventListener("click",()=>ge("login")),document.getElementById("send-reset-btn")?.addEventListener("click",Pi)}async function Pi(){const e=document.getElementById("reset-email"),t=Be(e?.value);if(!t){D("Enter your admin email address to receive a reset link.");return}F("send-reset-btn",!0),Je();const{error:a}=await m.auth.resetPasswordForEmail(t,{redirectTo:vi()});if(F("send-reset-btn",!1,'<i data-lucide="mail" class="w-4 h-4 inline mr-1"></i> Send Reset Link'),a){D(a.message);return}yi("Reset link sent! Check your inbox and open it from this device to continue.")}function Ei(){const e=document.getElementById("login-screen");if(!e)return;const t=e.querySelector(".login-card");t&&(t.innerHTML=`
