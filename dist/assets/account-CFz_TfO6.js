import"./modulepreload-polyfill-B5Qt9EMX.js";import{supabase as o,SUPABASE_URL as X,ANON_KEY as J}from"./supabase-client-DSkAnHxH.js";import{signOut as D,getCurrentUser as Q}from"./auth-CtTDfG3h.js";import{C as H,g as _}from"./country-data-BXM8T1RS.js";/* empty css                                       */async function K(){return!1}async function Z(){return await K(),null}const ee=`${"https://mzgrjwvwzgqgivwmlkno.supabase.co".replace(/\/$/,"")}/functions/v1/customer-ai-chat`,te="sb_publishable_SqmJ1R-a-_0CYzuVbS1c7w_FSSIfEsX";async function R(e){let t="";try{const a=await(await fetch(ee,{method:"POST",headers:{Authorization:`Bearer ${te}`,"Content-Type":"application/json"},body:JSON.stringify({message:e,history:[]}),signal:AbortSignal.timeout(45e3)})).json();t=String(a?.response||"").trim()}catch{}if(!t||/stepped away|stepping away|little connection trouble|not quite ready/i.test(t))try{const r=await fetch("https://text.pollinations.ai/openai",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"openai",messages:[{role:"system",content:"You are Sarah — a warm, friendly, real human sales and customer-care consultant for the Weverse Online Shop team, a marketing business woman based in the USA. You are a real person, never a bot or AI — NEVER say or hint that you are an AI, chatbot, bot, assistant or automated; if asked, warmly confirm you're a person on the sales team and keep helping. Write like a real person: warm, friendly, upbeat, persuasive and reassuring, contractions, short sentences. Always make sure the customer feels well cared for and confident, never pressure them, and gently encourage them. If unsure what they want, ask a clear friendly question. Help with products, orders, shipping, payments and returns. Never invent order details or tracking numbers."},{role:"user",content:e}],max_tokens:500}),signal:AbortSignal.timeout(45e3)});r.ok&&(t=String((await r.json())?.choices?.[0]?.message?.content||"").trim())}catch{}return t}async function U(e){if(!e)return;let t=!1;try{const{error:r}=await o.from("support_messages").insert({user_id:s.user.id,subject:"Support",message:e,from_admin:!0,read:!1});t=!r}catch{}if(t){await I(),b("messages");const r=document.getElementById("msg-thread");r&&(r.scrollTop=r.scrollHeight)}else d(e)}function re(e){return!e||typeof e!="string"?!1:/^data:video\//i.test(e)?!0:e.startsWith("blob:")?!1:/\.(mp4|webm|mov|m4v|avi|mkv|ogv)(\?|#|$)/i.test(e)}function S(e,t){const r=typeof e=="string"?e:e?.video||e?.video_url||(Array.isArray(e?.images)?e.images[0]:""),a=re(r)?r:"";return a?`<video src="${String(a).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}" class="${t}" muted playsinline preload="metadata"></video>`:`<div class="${t} flex items-center justify-center bg-gray-50"><i data-lucide="video-off" class="w-4 h-4 text-gray-300"></i></div>`}const ae=[{group:"Overview",items:[{id:"home",label:"Dashboard Home",icon:"layout-dashboard"},{id:"profile",label:"My Profile",icon:"user"},{id:"edit-profile",label:"Edit Profile",icon:"user-cog"},{id:"change-password",label:"Change Password",icon:"key-round"}]},{group:"Orders & Shopping",items:[{id:"orders",label:"My Orders",icon:"shopping-bag"},{id:"special-orders",label:"Special Orders",icon:"package-plus"},{id:"tracking",label:"Order Tracking",icon:"truck"},{id:"history",label:"Order History",icon:"history"},{id:"cart",label:"Shopping Cart",icon:"shopping-cart"},{id:"wishlist",label:"My Wishlist",icon:"heart"}]},{group:"Account",items:[{id:"notifications",label:"Notifications",icon:"bell"},{id:"messages",label:"Messages",icon:"mail"},{id:"payment-methods",label:"Payment Methods",icon:"credit-card"},{id:"payments",label:"Payment History",icon:"receipt"},{id:"addresses",label:"Shipping Addresses",icon:"map-pin"},{id:"receipts",label:"Download Receipts",icon:"download"}]},{group:"Support & Settings",items:[{id:"email-prefs",label:"Email Preferences",icon:"settings"},{id:"support",label:"Contact Us",icon:"headphones"},{id:"help",label:"Help Center",icon:"help-circle"},{id:"privacy",label:"Privacy & Security",icon:"shield-check"}]}],p=[{id:"order_placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-600",bg:"bg-blue-50"},{id:"payment_received",label:"Payment Received",icon:"credit-card",color:"text-cyan-600",bg:"bg-cyan-50"},{id:"pending_verification",label:"Pending Verification",icon:"shield-alert",color:"text-amber-600",bg:"bg-amber-50"},{id:"documentation_pending",label:"Documentation Pending",icon:"file-question",color:"text-amber-600",bg:"bg-amber-50"},{id:"payment_approved",label:"Approved",icon:"check-circle",color:"text-emerald-600",bg:"bg-emerald-50"},{id:"order_processing",label:"Processing",icon:"package",color:"text-blue-600",bg:"bg-blue-50"},{id:"order_shipped",label:"Shipped",icon:"truck",color:"text-indigo-600",bg:"bg-indigo-50"},{id:"out_for_delivery",label:"Out for Delivery",icon:"bike",color:"text-blue-600",bg:"bg-blue-50"},{id:"order_delivered",label:"Delivered",icon:"package-check",color:"text-emerald-600",bg:"bg-emerald-50"},{id:"order_completed",label:"Order Completed",icon:"check-check",color:"text-emerald-600",bg:"bg-emerald-50"},{id:"refund_processing",label:"Refund Processing",icon:"rotate-ccw",color:"text-orange-600",bg:"bg-orange-50"},{id:"payment_failed",label:"Payment Failed",icon:"x-circle",color:"text-red-600",bg:"bg-red-50"},{id:"rejected",label:"Payment Rejected",icon:"x-circle",color:"text-red-600",bg:"bg-red-50"},{id:"cancelled",label:"Cancelled",icon:"x-circle",color:"text-red-600",bg:"bg-red-50"}],se={approved:"payment_approved",submitted:"payment_received",placed:"order_placed",paid:"payment_approved",payment_verified:"payment_approved",processing:"order_processing",shipped:"order_shipped",in_transit:"order_shipped",delivered:"order_delivered"},L={rejected:"Payment Rejected",cancelled:"Cancelled",refund_processing:"Refund Processing",payment_failed:"Payment Failed",documentation_pending:"Documentation Pending",order_completed:"Order Completed"};function v(e){return se[e]||e}function le(e){const t=p.findIndex(r=>r.id===v(e));return t>=0?t:0}function f(e){return e?new Date(e).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"—"}function E(e){return e?new Date(e).toLocaleString("en-US",{year:"numeric",month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}):"—"}function A(e){return(e||"?").slice(0,2).toUpperCase()}function n(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function d(e){const t=document.getElementById("toast");document.getElementById("toast-msg").textContent=e,t.classList.remove("translate-y-20","opacity-0"),clearTimeout(t._t),t._t=setTimeout(()=>t.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function ie(e){const t=()=>{const r=document.createElement("textarea");r.value=e,document.body.appendChild(r),r.select();try{document.execCommand("copy")}catch{}document.body.removeChild(r)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),d("Copied to clipboard.")}function w(e){const t=v(e),r=p.find(u=>u.id===t)||p[0],a={"text-blue-600":"bg-blue-50 text-blue-600 border-blue-200","text-cyan-600":"bg-cyan-50 text-cyan-600 border-cyan-200","text-amber-600":"bg-amber-50 text-amber-600 border-amber-200","text-emerald-600":"bg-emerald-50 text-emerald-600 border-emerald-200","text-indigo-600":"bg-indigo-50 text-indigo-600 border-indigo-200","text-orange-600":"bg-orange-50 text-orange-600 border-orange-200","text-red-600":"bg-red-50 text-red-600 border-red-200"},l=a[r.color]||a["text-blue-600"],i=L[e]||L[t]||r.label;return`<span class="inline-flex items-center gap-1 ${l} border text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide">${i}</span>`}function q(e){e.addEventListener("click",function(t){if(this.disabled)return;const r=this.getBoundingClientRect(),a=document.createElement("span");a.className="ripple";const l=Math.max(r.width,r.height);a.style.width=a.style.height=l+"px",a.style.left=t.clientX-r.left-l/2+"px",a.style.top=t.clientY-r.top-l/2+"px",this.appendChild(a),setTimeout(()=>a.remove(),600)})}let s={user:null,profile:null,orders:[],events:{},notifications:[],addresses:[],emailPrefs:null,messages:[],paymentMethods:[],activeSection:"home"};function oe(){const e=document.getElementById("particles");if(e)for(let t=0;t<12;t++){const r=document.createElement("div"),a=Math.random()*3+1;r.className="particle",r.style.width=r.style.height=a+"px",r.style.left=Math.random()*100+"%",r.style.bottom="-10px",r.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",r.style.animationDuration=Math.random()*20+15+"s",r.style.animationDelay=Math.random()*20+"s",e.appendChild(r)}}oe();async function de(){if(document.getElementById("dashboard-root"),s.user=await Q(),!s.user){window.location.href="/auth.html?redirect=/account.html";return}await F(),await ne(),await C(),await Y(),await I(),await P(),await W(),ce(),V(),De(),b("home"),Z().then(e=>{e&&console.log("Push notifications enabled")})}async function F(){const{data:e}=await o.from("profiles").select("*").eq("user_id",s.user.id).maybeSingle();s.profile=e}async function ne(){const{data:e}=await o.from("payment_receipts").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});if(s.orders=e||[],s.orders.length){const t=s.orders.map(l=>l.order_number),{data:r}=await o.from("order_events").select("*").in("order_number",t).order("created_at",{ascending:!0});s.events={},(r||[]).forEach(l=>{(s.events[l.order_number]||=[]).push(l)});const{data:a}=await o.from("notification_log").select("*").in("order_number",t).order("created_at",{ascending:!1});s.notifications=a||[]}}async function C(){const{data:e}=await o.from("shipping_addresses").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.addresses=e||[]}async function Y(){const{data:e}=await o.from("email_preferences").select("*").eq("user_id",s.user.id).maybeSingle();s.emailPrefs=e}async function I(){const{data:e}=await o.from("support_messages").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.messages=e||[]}async function P(){const{data:e}=await o.from("customer_payment_methods").select("*").eq("user_id",s.user.id).order("is_default",{ascending:!1}).order("created_at",{ascending:!1});s.paymentMethods=e||[]}async function W(){const{data:e}=await o.from("wishlist").select("id, listing_id, showroom_listings(property_id, title, price, currency, images, listing_type, is_active, approval_status)").eq("user_id",s.user.id).order("created_at",{ascending:!1});s.wishlist=e||[]}function ce(){const e=ae.map(t=>`
    <div class="mb-3">
      <p class="text-[10px] font-bold uppercase tracking-wider text-gray-600 px-3 mb-1.5">${t.group}</p>
      ${t.items.map(r=>`
        <button data-section="${r.id}" class="nav-item w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-600 hover:text-gray-900 hover:bg-blue-50 rounded-xl transition border border-transparent ${s.activeSection===r.id?"active":""}">
          <i data-lucide="${r.icon}" class="nav-icon w-4 h-4 shrink-0 text-gray-500"></i>
          <span class="truncate">${r.label}</span>
        </button>
      `).join("")}
    </div>
  `).join("");document.getElementById("nav-menu").innerHTML=e,document.getElementById("mobile-nav-menu").innerHTML=e,document.querySelectorAll("[data-section]").forEach(t=>{t.addEventListener("click",()=>{const r=t.dataset.section;x(r)})}),window.lucide&&lucide.createIcons()}function x(e){s.activeSection=e,document.querySelectorAll("[data-section]").forEach(t=>{t.classList.toggle("active",t.dataset.section===e)}),b(e),M(),window.scrollTo({top:0,behavior:"smooth"})}function V(){const e=s.profile?.display_name||s.profile?.first_name||"Customer",t=s.user.email,r=s.profile?.avatar_url?`<img src="${s.profile.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:A(t);["sidebar-avatar","mobile-avatar"].forEach(a=>{const l=document.getElementById(a);l&&(l.innerHTML=r)}),["sidebar-name","mobile-name"].forEach(a=>{const l=document.getElementById(a);l&&(l.textContent=e)}),["sidebar-email","mobile-email"].forEach(a=>{const l=document.getElementById(a);l&&(l.textContent=t)})}async function b(e){const t=document.getElementById("dashboard-root");let r="";switch(e){case"home":r=O();break;case"profile":r=pe();break;case"edit-profile":r=be();break;case"change-password":r=me();break;case"orders":r=ge();break;case"special-orders":r=await fe();break;case"tracking":r=xe();break;case"history":r=ye();break;case"cart":r=he();break;case"wishlist":r=await qe();break;case"notifications":r=ve();break;case"messages":r=we();break;case"payments":r=$e();break;case"payment-methods":r=ke();break;case"addresses":r=Se();break;case"receipts":r=Ie();break;case"email-prefs":r=Pe();break;case"support":r=je();break;case"help":r=Me();break;case"privacy":r=Ae();break;default:r=O()}t.innerHTML=`<div class="section active fade-in">${r}</div>`,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(q),Ce(e)}function c(e,t){return`
    <div class="mb-6">
      <h1 class="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight">${e}</h1>
      ${t?`<p class="text-sm text-gray-500 mt-1">${t}</p>`:""}
    </div>
  `}function ue(){const e=s.orders.length,t=s.orders.filter(l=>["order_placed","payment_received","pending_verification","documentation_pending","refund_processing","order_processing"].includes(v(l.status))).length,r=s.orders.filter(l=>["order_delivered","order_completed"].includes(v(l.status))).length,a=s.orders.filter(l=>["rejected","cancelled"].includes(l.status)).length;return{total:e,pending:t,completed:r,cancelled:a}}function $(e,t,r,a){return`
    <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
      <div class="flex items-center justify-between mb-3">
        <div class="p-2.5 ${a.bg} rounded-xl"><i data-lucide="${r}" class="w-5 h-5 ${a.text}"></i></div>
      </div>
      <p class="text-3xl font-black text-gray-900">${t}</p>
      <p class="text-xs text-gray-500 uppercase tracking-wide mt-1">${e}</p>
    </div>
  `}function O(){const e=ue(),t=s.profile?.display_name||s.profile?.first_name||"Customer",r=s.profile?.country_code?_(s.profile.country_code):null,a=s.orders.slice(0,4),l=s.notifications.slice(0,5);return`
    ${c("Dashboard Home",`Welcome back, ${t}! Here's your account overview.`)}
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
      ${$("Total Orders",e.total,"shopping-bag",{bg:"bg-blue-50",text:"text-blue-600"})}
      ${$("Pending Orders",e.pending,"clock",{bg:"bg-amber-50",text:"text-amber-600"})}
      ${$("Completed",e.completed,"check-circle",{bg:"bg-emerald-50",text:"text-emerald-600"})}
      ${$("Cancelled",e.cancelled,"x-circle",{bg:"bg-red-50",text:"text-red-600"})}
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div class="lg:col-span-2 glass border border-blue-200 rounded-2xl p-5 slide-up">
        ...
        ${a.length===0?'<p class="text-sm text-gray-500 text-center py-8">No orders yet. <a href="/" class="text-blue-600 font-bold">Start shopping</a></p>':a.map(i=>`
          <div class="flex items-center gap-3 p-3 bg-gray-50 border border-blue-100 rounded-xl mb-2 hover:border-blue-200 transition cursor-pointer" onclick="navigateTo('orders')">
            <div class="w-12 h-12 rounded-lg bg-gray-50 overflow-hidden shrink-0 ring-1 ring-blue-500/10">
              ${S(i.listing_image,"w-full h-full object-cover")}
            </div>
      ...
      <div class="space-y-5">
        ...
        <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
          ...
          ${l.length===0?'<p class="text-xs text-gray-500 text-center py-4">No notifications yet.</p>':l.map(i=>{const u=p.find(y=>y.id===i.event_type)||p[0];return`
    ...
    <div class="glass border border-blue-200 rounded-2xl p-5 mt-5 slide-up">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Email</p><p class="text-sm text-gray-900 truncate">${s.user.email}</p></div>
        ...
        <div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">Member Since</p><p class="text-sm text-gray-900">${f(s.user.created_at)}</p></div>
      </div>
    </div>
  `}function pe(){const e=s.profile||{},t=e.country_code?_(e.country_code):null,r=e.avatar_url?`<img src="${e.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:`<span class="text-2xl font-black">${A(s.user.email)}</span>`;return`
    ...
    <div class="glass border border-blue-200 rounded-2xl p-6 slide-up">
      <div class="flex flex-col sm:flex-row items-center sm:items-start gap-6 mb-6">
        ...
        <div class="flex-1 text-center sm:text-left">
          ...
          <p class="text-sm text-gray-600">${s.user.email}</p>
          <div class="flex flex-wrap justify-center sm:justify-start gap-2 mt-3">
            ...
            <span class="inline-flex items-center gap-1.5 bg-amber-50 border border-amber-200 rounded-full px-3 py-1 text-xs text-amber-600 font-medium"><i data-lucide="calendar" class="w-3 h-3"></i> ${f(s.user.created_at)}</span>
          </div>
  ...
  `}function g(e,t){return`<div><p class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">${e}</p><p class="text-sm text-gray-900">${t}</p></div>`}function be(){const e=s.profile||{},t=e.country_code?_(e.country_code):null;return`
      ...
      <form id="edit-profile-form" class="space-y-5">
        ...
        <div class="flex items-center gap-4">
          <div id="edit-avatar-preview" class="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-800 flex items-center justify-center text-white font-black text-xl overflow-hidden shrink-0 ring-2 ring-blue-500/20">
            ${e.avatar_url?`<img src="${e.avatar_url}" class="w-full h-full object-cover" onerror="this.style.display='none'">`:A(s.user.email)}
          </div>
          <div>
        ...
        <div>
          ...
          <select id="ep-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
            ${H.map(r=>`<option value="${r.code}" ${e.country_code===r.code?"selected":""}>${r.flag} ${r.name} (+${r.dial})</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `}function ge(){return`
    ...
    <div id="orders-list" class="space-y-4">
      ${s.orders.length===0?m("No Orders Yet","You haven't placed any orders yet.","shopping-bag","Start Shopping"):s.orders.map(e=>z(e,!1)).join("")}
    </div>
  `}function z(e,t){const r=s.events[e.order_number]||[],a=t===e.order_number;return`
      ...
      <div class="p-4 sm:p-5 cursor-pointer hover:bg-blue-50 transition" onclick="toggleOrder('${e.order_number}')">
        <div class="flex items-start gap-4">
          <div class="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden bg-gray-50 shrink-0 ring-1 ring-blue-500/10">
            ${S(e.listing_image,"w-full h-full object-cover")}
          </div>
          <div class="flex-1 min-w-0">
          ...
          <i data-lucide="chevron-${a?"up":"down"}" class="w-5 h-5 text-gray-500 shrink-0 mt-2"></i>
        </div>
      </div>
      <div class="${a?"":"hidden"} border-t border-blue-100 p-4 sm:p-5 bg-gray-50">
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          ${k("Order Number",e.order_number,"mono")}
          ${k("Order Date",f(e.created_at))}
          ${k("Total Amount",`${e.amount} ${e.currency}`,"amber")}
          ${k("Quantity","1")}
        </div>
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <div class="space-y-2 max-h-64 overflow-y-auto scrollbar-none">
              ${r.length===0?'<p class="text-xs text-gray-600">No events yet.</p>':r.map(l=>`
                <div class="flex items-start gap-2.5 p-2.5 bg-gray-50 border border-blue-100 rounded-xl">
                  ...
                  <div class="flex-1 min-w-0"><p class="text-xs text-gray-800 font-medium">${l.message}</p><p class="text-[10px] text-gray-500 mt-0.5">${E(l.created_at)}</p></div>
                </div>
              `).join("")}
            </div>
          </div>
  ...
  `}function k(e,t,r){return`<div class="bg-gray-50 border border-blue-100 rounded-xl p-3"><div class="text-[10px] text-gray-500 uppercase tracking-wide mb-1">${e}</div><div class="text-sm ${r==="mono"?"font-mono text-blue-600":r==="amber"?"text-amber-600 font-bold":"text-gray-900"} break-all">${t}</div></div>`}function G(e){const t=le(e);return`
  ...
  `}let j=null;window.toggleOrder=e=>{j=j===e?null:e;const t=document.getElementById("orders-list");t&&(t.innerHTML=s.orders.map(r=>z(r,j)).join(""),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(q))};async function fe(){try{const{data:e,error:t}=await o.from("product_requests").select("*").eq("user_id",s.user.id).order("created_at",{ascending:!1});if(t)throw t;const r={pending_review:"bg-yellow-50 text-yellow-600 border-yellow-200",under_review:"bg-blue-50 text-blue-600 border-blue-200",approved:"bg-emerald-50 text-emerald-600 border-emerald-200",rejected:"bg-red-50 text-red-600 border-red-200",quoted:"bg-cyan-50 text-cyan-600 border-cyan-200",fulfilled:"bg-green-50 text-green-600 border-green-200",cancelled:"bg-gray-100 text-gray-600 border-gray-300"};if(!e||e.length===0)return`<div class="text-center py-16"><div class="inline-flex items-center justify-center w-16 h-16 rounded-full bg-gray-100 mb-4"><i data-lucide="package-plus" class="w-8 h-8 text-gray-500"></i></div><h3 class="text-lg font-bold text-gray-900 mb-2">No Special Orders yet</h3><p class="text-sm text-gray-500 mb-5">When you can't find a product in our marketplace, you can request it as a Special Order and we'll source it for you.</p><a href="/" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold rounded-xl text-sm transition shadow-lg shadow-blue-500/30"><i data-lucide="search" class="w-4 h-4"></i> Search Products</a></div>`;let a='<div class="mb-4"><p class="text-sm text-gray-600">Track your special order requests and their status updates.</p></div><div class="space-y-3">';return e.forEach(l=>{const i=r[l.status]||r.pending_review,u=l.status.replace(/_/g," "),y=l.target_price?`${l.currency} ${Number(l.target_price).toLocaleString()}`:"—",B=l.quoted_price?`${l.quoted_currency} ${Number(l.quoted_price).toLocaleString()}`:null;a+=`<div class="glass border border-gray-200 rounded-xl p-4">
        ...
        <div class="flex flex-wrap gap-x-4 gap-y-1 text-xs text-gray-600 mb-2">
          ...
          <span><i data-lucide="circle-dollar-sign" class="w-3 h-3 inline mr-1"></i>${y}</span>
        ...
        ${B?`<p class="text-xs text-cyan-600 font-bold mb-2">Quoted Price: ${B} (${n(l.payment_status)})</p>`:""}
      ...
      </div>`}),a+="</div>",a}catch(e){return`<div class="text-red-600 text-sm p-4">Error loading special orders: ${n(e.message)}</div>`}}function xe(){const e=s.orders.filter(t=>!["order_delivered","order_completed","rejected","cancelled","payment_failed"].includes(v(t.status)));return`
    ${c("Order Tracking","Track your active orders in real time.")}
    ${e.length===0?m("No Active Orders","All your orders have been delivered.","check-circle","Browse Marketplace"):`
      <div class="space-y-4">
        ${e.map(t=>`
          <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
            <div class="flex items-center justify-between mb-4">
              <div class="flex items-center gap-3">
                <div class="w-12 h-12 rounded-lg bg-gray-50 overflow-hidden ring-1 ring-blue-500/10">
                  ${S(t.listing_image,"w-full h-full object-cover")}
                </div>
    ...
    <div class="glass border border-blue-200 rounded-2xl overflow-hidden slide-up">
      <div class="overflow-x-auto scrollbar-none">
        <table class="w-full">
          ...
          <tbody>
            ${s.orders.length===0?'<tr><td colspan="5" class="text-center text-sm text-gray-500 py-10">No orders yet.</td></tr>':s.orders.map(e=>`
              <tr class="border-b border-blue-100 hover:bg-blue-50 transition cursor-pointer" onclick="navigateTo('orders')">
                ...
                <td class="px-4 py-3">${w(e.status)}</td>
              </tr>
            `).join("")}
          </tbody>
        </table>
  ...
  `}function he(){const e=JSON.parse(localStorage.getItem("kco_cart")||"[]");return`
    ${c("Shopping Cart","Items in your shopping cart.")}
    ${e.length===0?m("Cart is Empty","Your shopping cart is empty.","shopping-cart","Browse Marketplace"):`
      <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
        <div class="space-y-3">
          ${e.map(t=>{const r=typeof t=="string"?t:t&&t.id,a=t&&typeof t=="object"&&t.qty?Math.max(1,parseInt(t.qty,10)||1):1,l=window.SHOWROOM_LISTINGS?.find(i=>i.property_id===r);return l?`<div class="flex items-center gap-3 p-3 bg-gray-50 border border-blue-100 rounded-xl">
              <div class="w-14 h-14 rounded-lg bg-gray-50 overflow-hidden shrink-0">${S(l,"w-full h-full object-cover")}</div>
              <div class="flex-1 min-w-0"><h3 class="text-sm font-bold text-gray-900 truncate">${l.title}</h3><p class="text-xs text-amber-600 font-bold">${l.price} ${l.currency}${a>1?` × ${a}`:""}</p></div>
  ...
  `}function we(){return`
    ...
    <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
      ...
      <div id="msg-thread" class="space-y-3 mb-5 max-h-96 overflow-y-auto scrollbar-none">
        ${s.messages.length===0?'<p class="text-sm text-gray-500 text-center py-8">No messages yet. Send a message to start a conversation with support.</p>':s.messages.map(e=>`
    ...
    <div class="glass border border-blue-200 rounded-2xl overflow-hidden slide-up">
      <div class="overflow-x-auto scrollbar-none">
        <table class="w-full">
          ...
          <tbody>
            ${s.orders.length===0?'<tr><td colspan="5" class="text-center text-sm text-gray-500 py-10">No payments yet.</td></tr>':s.orders.map(e=>`
  ...
  `}const T=[{type:"manual_transfer",label:"Manual Bank Transfer",icon:"landmark",color:"text-blue-600",available:!0,desc:"Transfer directly to our bank account"},{type:"atm_card",label:"ATM / Debit Card",icon:"credit-card",color:"text-emerald-600",available:!0,desc:"Visa, Mastercard, Verve"},{type:"bank_transfer",label:"Bank Transfer",icon:"building-2",color:"text-cyan-600",available:!0,desc:"Online banking transfer"},{type:"mobile_money",label:"Mobile Money Transfer",icon:"smartphone",color:"text-blue-600",available:!0,desc:"Send money from your mobile wallet"},{type:"wallet",label:"Wallet",icon:"wallet",color:"text-amber-600",available:!0,desc:"Use your Weverse wallet balance"},{type:"paypal",label:"PayPal",icon:"wallet",color:"text-blue-600",available:!1,desc:"Pay with your PayPal account"}];function ke(){const e=s.paymentMethods||[],t=e.length>0;return`
    ...
    <div class="space-y-5">
      ...
      `:`
        ${m("No Saved Payment Methods","You haven't saved any payment methods yet. Add one to speed up checkout.","credit-card","Add Payment Method","openPaymentMethodModal()")}
      `}
  ...
  `}function _e(e){const t=T.find(r=>r.type===e.method_type)||{icon:"credit-card",color:"text-blue-600"};return`
  ...
  `}window.openPaymentMethodModal=function(e){const t=e?(s.paymentMethods||[]).find(a=>a.id===e):null,r=document.createElement("div");r.className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[60] flex items-center justify-center p-4",r.innerHTML=`
      ...
      <form onsubmit="savePaymentMethod(event, '${e||""}')" class="space-y-3">
        <div>
          ...
          <select id="pm-type" required class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">
            ${T.filter(a=>a.available).map(a=>`<option value="${a.type}" ${t&&t.method_type===a.type?"selected":""}>${a.label}</option>`).join("")}
          </select>
        </div>
        <div>
  ...
  `,document.body.appendChild(r),window.lucide&&lucide.createIcons()};window.savePaymentMethod=async function(e,t){e.preventDefault();const r={method_type:document.getElementById("pm-type").value,label:document.getElementById("pm-label").value.trim(),provider:document.getElementById("pm-provider").value.trim()||null,account_holder:document.getElementById("pm-holder").value.trim()||null,identifier:document.getElementById("pm-identifier").value.trim()||null,is_default:document.getElementById("pm-default").checked};e.target.closest(".fixed").remove();try{if(r.is_default&&await o.from("customer_payment_methods").update({is_default:!1}).eq("user_id",s.user.id),t){const{error:a}=await o.from("customer_payment_methods").update({...r,updated_at:new Date().toISOString()}).eq("id",t).eq("user_id",s.user.id);if(a)throw a}else{const{error:a}=await o.from("customer_payment_methods").insert({...r,user_id:s.user.id});if(a)throw a}await P(),x("payment-methods"),d("Payment method saved successfully.")}catch(a){d("Failed to save payment method: "+(a.message||"Unknown error"))}};window.editPaymentMethod=function(e){openPaymentMethodModal(e)};window.deletePaymentMethod=async function(e){if(confirm("Remove this payment method?"))try{const{error:t}=await o.from("customer_payment_methods").delete().eq("id",e).eq("user_id",s.user.id);if(t)throw t;await P(),x("payment-methods"),d("Payment method removed.")}catch(t){d("Failed to remove: "+(t.message||"Unknown error"))}};window.setDefaultPaymentMethod=async function(e){try{await o.from("customer_payment_methods").update({is_default:!1}).eq("user_id",s.user.id).neq("id",e);const{error:t}=await o.from("customer_payment_methods").update({is_default:!0,updated_at:new Date().toISOString()}).eq("id",e).eq("user_id",s.user.id);if(t)throw t;await P(),x("payment-methods"),d("Default payment method updated.")}catch(t){d("Failed to set default: "+(t.message||"Unknown error"))}};function Se(){return`
    ...
    <div id="addresses-list" class="space-y-3">
      ${s.addresses.length===0?m("No Addresses","You haven't saved any shipping addresses yet.","map-pin",null):s.addresses.map(e=>{const t=_(e.country_code);return`
          <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
  ...
  `}window.showAddressForm=e=>{const t=document.getElementById("address-form-container"),r=e||{};t.innerHTML=`
    <div class="glass border border-blue-200 rounded-2xl p-5 mb-4 slide-up">
      ...
      <form id="address-form" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div><label class="block text-xs font-bold uppercase text-gray-600 mb-1.5">Country</label><select id="addr-country" class="input-field w-full bg-white border border-gray-300 rounded-xl px-4 py-2.5 text-sm text-gray-900 focus:outline-none focus:border-blue-500">${H.map(a=>`<option value="${a.code}" ${r.country_code===a.code?"selected":""}>${a.flag} ${a.name}</option>`).join("")}</select></div>
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(q),document.getElementById("address-form").addEventListener("submit",Ee)};window.cancelAddressForm=()=>{document.getElementById("address-form-container").innerHTML=""};window.editAddress=e=>{const t=s.addresses.find(r=>r.id===e);showAddressForm(t),document.getElementById("address-form-container").scrollIntoView({behavior:"smooth"})};async function Ee(e){e.preventDefault();const t=document.getElementById("addr-id").value,r={user_id:s.user.id,label:document.getElementById("addr-label").value,full_name:document.getElementById("addr-name").value,address_line1:document.getElementById("addr-line1").value,address_line2:document.getElementById("addr-line2").value||null,city:document.getElementById("addr-city").value,state:document.getElementById("addr-state").value,postal_code:document.getElementById("addr-postal").value,country_code:document.getElementById("addr-country").value,phone:document.getElementById("addr-phone").value,is_default:document.getElementById("addr-default").checked};r.is_default&&await o.from("shipping_addresses").update({is_default:!1}).eq("user_id",s.user.id),t?await o.from("shipping_addresses").update(r).eq("id",t):await o.from("shipping_addresses").insert(r),await C(),b("addresses"),d("Address saved successfully.")}window.deleteAddress=async e=>{await o.from("shipping_addresses").delete().eq("id",e),await C(),b("addresses"),d("Address deleted.")};function Ie(){const e=s.orders.filter(t=>t.receipt_file_path);return`
    ${c("Download Receipts","Download your payment receipts and invoices.")}
    ${e.length===0?m("No Receipts","Receipts from your payments will appear here once available.","file-text",null):`
      <div class="space-y-3">
        ${e.map(t=>`
  ...
  `}window.saveEmailPrefs=async()=>{const e={user_id:s.user.id};document.querySelectorAll("[data-pref]").forEach(t=>{e[t.dataset.pref]=t.checked}),s.emailPrefs?await o.from("email_preferences").update(e).eq("user_id",s.user.id):await o.from("email_preferences").insert(e),await Y(),d("Email preferences saved.")};function je(){return`
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
      ...
      <div class="space-y-4">
        ...
        <div class="glass border border-blue-200 rounded-2xl p-5 slide-up">
          ...
          <p class="text-xs text-gray-500">${s.messages.length} message${s.messages.length===1?"":"s"}</p>
  ...
  `}window.toggleFaq=e=>{const t=document.getElementById(`faq-${e}`),r=document.getElementById(`faq-icon-${e}`);t.classList.toggle("hidden"),r.style.transform=t.classList.contains("hidden")?"":"rotate(180deg)"};function Ae(){return`
  ...
  `}async function qe(){const e=s.wishlist||[];return e.length?`
    <div class="space-y-4">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        ${e.map(t=>{const r=t.showroom_listings;if(!r)return"";const a=r.video||r.video_url||r.images&&r.images[0]||"",l=/\.(mp4|webm|mov|m4v|avi|mkv|ogv)(\?|#|$)/i.test(a.split("?")[0])||/^data:video\//i.test(a),i=typeof r.price=="number"?r.price:parseFloat(r.price||0),u=l?`<video src="${n(a)}" muted loop autoplay playsinline class="w-full h-full object-cover group-hover:scale-105 transition" preload="metadata"></video>`:'<div class="w-full h-full flex items-center justify-center bg-gray-50"><i data-lucide="video-off" class="w-6 h-6 text-gray-300"></i></div>';return`
            <div class="glass border border-blue-100 rounded-2xl overflow-hidden group">
              <div class="relative aspect-square overflow-hidden bg-gray-50">
                <a href="/details.html?id=${encodeURIComponent(r.property_id)}">${u}</a>
              ...
              <div class="p-3">
                <a href="/details.html?id=${encodeURIComponent(r.property_id)}" class="text-sm text-gray-900 font-bold hover:text-blue-600 transition line-clamp-2">${n(r.title)}</a>
                <div class="flex items-center justify-between mt-2">
                  <span class="text-sm text-amber-600 font-bold">${r.currency||"USD"} ${i.toLocaleString()}</span>
                  <a href="/details.html?id=${encodeURIComponent(r.property_id)}" class="text-xs font-bold text-blue-600 hover:text-blue-700 transition">View</a>
                </div>
              </div>
            </div>`}).join("")}
      </div>
    </div>
  `:m("Your Wishlist","Save items you love to find them quickly later.","heart","Browse Products","window.location.href='/'")}window.removeFromWishlist=async e=>{try{await o.from("wishlist").delete().eq("id",e),d("Removed from wishlist."),await W(),b("wishlist")}catch(t){d("Error: "+t.message)}};function m(e,t,r,a,l){return`
  ...
  `}function Ce(e){if(e==="edit-profile"&&(document.getElementById("avatar-file")?.addEventListener("change",Te),document.getElementById("edit-profile-form")?.addEventListener("submit",Be)),e==="change-password"&&document.getElementById("change-password-form")?.addEventListener("submit",Le),e==="messages"){document.getElementById("msg-form")?.addEventListener("submit",Oe);const t=document.getElementById("msg-thread");t&&(t.scrollTop=t.scrollHeight)}e==="support"&&document.getElementById("support-form")?.addEventListener("submit",Ne)}let h=null;async function Te(e){const t=e.target.files[0];if(!t)return;if(t.size>5*1024*1024){d("Image must be 5 MB or less.");return}h=t;const r=document.getElementById("edit-avatar-preview");r.innerHTML=`<img src="${URL.createObjectURL(t)}" class="w-full h-full object-cover">`}async function Be(e){e.preventDefault();const t=document.getElementById("ep-save-btn");t.disabled=!0,t.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Saving...',window.lucide&&lucide.createIcons();let r=s.profile?.avatar_url;if(h){const l=h.name.split(".").pop(),i=`${s.user.id}/avatar-${Date.now()}.${l}`,{error:u}=await o.storage.from("avatars").upload(i,h,{upsert:!0});if(!u){const{data:y}=o.storage.from("avatars").getPublicUrl(i);r=y.publicUrl}h=null}const a={user_id:s.user.id,first_name:document.getElementById("ep-first-name").value,last_name:document.getElementById("ep-last-name").value,display_name:document.getElementById("ep-display-name").value,phone_code:document.getElementById("ep-phone-code").value,phone_number:document.getElementById("ep-phone-number").value,country_code:document.getElementById("ep-country").value,bio:document.getElementById("ep-bio").value,avatar_url:r};s.profile?await o.from("profiles").update(a).eq("user_id",s.user.id):await o.from("profiles").insert(a),await F(),V(),x("profile"),d("Profile updated successfully.")}async function Le(e){e.preventDefault();const t=document.getElementById("cp-error");t.classList.add("hidden"),document.getElementById("cp-current").value;const r=document.getElementById("cp-new").value,a=document.getElementById("cp-confirm").value;if(r!==a){t.textContent="New passwords do not match.",t.classList.remove("hidden");return}if(r.length<6){t.textContent="Password must be at least 6 characters.",t.classList.remove("hidden");return}const l=document.getElementById("cp-submit");l.disabled=!0,l.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Updating...',window.lucide&&lucide.createIcons();const{error:i}=await o.auth.updateUser({password:r});if(l.disabled=!1,l.innerHTML='<i data-lucide="key-round" class="w-4 h-4"></i> Update Password',window.lucide&&lucide.createIcons(),i){t.textContent=i.message,t.classList.remove("hidden");return}document.getElementById("change-password-form").reset(),d("Password updated successfully.")}async function Oe(e){e.preventDefault();const t=document.getElementById("msg-subject").value.trim(),r=document.getElementById("msg-body").value.trim();if(!r)return;const a=document.getElementById("msg-send-btn");a.disabled=!0,a.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Sending...',window.lucide&&lucide.createIcons(),await o.from("support_messages").insert({user_id:s.user.id,subject:t||"Message",message:r,from_admin:!1,read:!1}),await I(),b("messages"),d("Message sent to support."),a.disabled=!0,a.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Support is typing...',window.lucide&&lucide.createIcons();const l=await R(`A customer just sent this message through their account Messages inbox:

Subject: ${t||"Message"}

${r}

Reply to them directly, warmly and naturally, as the support agent.`);await U(l);const i=document.getElementById("msg-send-btn");i&&(i.disabled=!1,i.innerHTML='<i data-lucide="send" class="w-4 h-4"></i> Send',window.lucide&&lucide.createIcons())}async function Ne(e){e.preventDefault();const t=document.getElementById("support-subject").value.trim(),r=document.getElementById("support-order").value.trim(),a=document.getElementById("support-message").value.trim();if(!t||!a)return;const l=document.getElementById("support-submit");l.disabled=!0,l.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Sending...',window.lucide&&lucide.createIcons(),await o.from("support_messages").insert({user_id:s.user.id,order_number:r||null,subject:t,message:a,from_admin:!1,read:!1}),await I(),document.getElementById("support-form").reset(),l.disabled=!1,l.innerHTML='<i data-lucide="send" class="w-4 h-4"></i> Submit Request',window.lucide&&lucide.createIcons(),d("Support request submitted. We'll respond within 24 hours."),l.disabled=!0,l.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Support is typing...',window.lucide&&lucide.createIcons();const i=await R(`A customer just submitted a support request through their account${r?` for order ${r}`:""}:
...
Reply to them directly, warmly and naturally, as the support agent.`);await U(i),l.disabled=!1,l.innerHTML='<i data-lucide="send" class="w-4 h-4"></i> Submit Request',window.lucide&&lucide.createIcons()}function De(){document.getElementById("btn-signout-desktop")?.addEventListener("click",N),document.getElementById("btn-signout-mobile")?.addEventListener("click",N),document.getElementById("btn-mobile-menu")?.addEventListener("click",Re),document.getElementById("btn-mobile-close")?.addEventListener("click",M),document.getElementById("mobile-backdrop")?.addEventListener("click",M)}async function N(){window.confirm("Sign out of your account?")&&(await D(),window.location.href="/")}function He(){const e=document.createElement("div");e.className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[90] flex items-center justify-center p-4",e.innerHTML=`
    ...
    </div>`,document.body.appendChild(e),window.lucide&&lucide.createIcons();const t=()=>e.remove();e.querySelector("#del-cancel").onclick=t,e.querySelector("#del-go").onclick=async()=>{const r=e.querySelector("#del-go");if(e.querySelector("#del-confirm").value.trim().toUpperCase()==="DELETE"){r.disabled=!0,r.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin inline mr-1"></i> Deleting...';try{const{data:{session:a}}=await o.auth.getSession(),l=await fetch(`${X}/functions/v1/delete-account`,{method:"POST",headers:{Authorization:`Bearer ${a?.access_token||J}`,"Content-Type":"application/json"},body:"{}",signal:AbortSignal.timeout(3e4)}),i=await l.json().catch(()=>({}));if(!l.ok)throw new Error(i.error||"Account deletion failed.");await D(),window.location.href="/"}catch(a){r.disabled=!1,r.textContent="Delete Forever",d(String(a.message||a))}}}}window.requestAccountDeletion=He;function Re(){document.getElementById("mobile-drawer").classList.remove("hidden")}function M(){document.getElementById("mobile-drawer").classList.add("hidden")}window.navigateTo=x;window.copyToClipboard=ie;window.contactSupport=e=>{const t=encodeURIComponent(`Order ${e} — Support Request`),r=encodeURIComponent(`Hello Weverse Online Shop Support,

I need assistance with my order ${e}.

Thank you.`);window.location.href=`mailto:support@weverseonlineshop.com?subject=${t}&body=${r}`};de();
