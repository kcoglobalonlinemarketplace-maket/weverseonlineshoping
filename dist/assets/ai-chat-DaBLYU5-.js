import{o as S,A}from"./supabase-client-CEXxUHzJ.js";const _=(S||"https://wttnvwpoqmbxryivcerf.supabase.co").replace(/\/$/,""),C=`${_}/functions/v1/customer-ai-chat`,x="kco_ai_chat_msgs",L=["Where can I track my order?","How long does shipping take?","How do refunds work?","Talk to a human"];let s={open:!1,busy:!1,inited:!1};function y(){try{const e=JSON.parse(localStorage.getItem(x)||"[]");if(Array.isArray(e))return e.filter(t=>t&&t.role&&t.content).slice(-20)}catch{}return[]}function g(e){try{localStorage.setItem(x,JSON.stringify(e.slice(-20)))}catch{}}function M(e){return String(e||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;")}function O(e){return M(e).replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>").replace(/^[-•]\s+/gm,'<span class="kco-ai-bullet">•</span> ').replace(/\n/g,"<br>")}function f(e,t=20){const a={message:'<path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/>',send:'<path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/>',close:'<path d="M18 6 6 18"/><path d="m6 6 12 12"/>',bot:'<rect width="18" height="10" x="3" y="11" rx="2"/><circle cx="12" cy="5" r="2"/><path d="M12 7v4"/><path d="M8 16h.01"/><path d="M16 16h.01"/>'};return`<svg xmlns="http://www.w3.org/2000/svg" width="${t}" height="${t}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${a[e]||a.message}</svg>`}function N(){if(document.getElementById("kco-ai-chat-style"))return;const e=document.createElement("style");e.id="kco-ai-chat-style",e.textContent=`
  ...
  `,document.head.appendChild(e)}function T(){const e=document.createElement("button");e.id="kco-ai-btn",e.setAttribute("aria-label","Open AI chat assistant"),e.innerHTML=f("message",24),e.addEventListener("click",()=>v());const t=document.createElement("div");t.id="kco-ai-panel",t.setAttribute("role","dialog"),t.setAttribute("aria-label","Weverse Online Shop assistant chat"),t.innerHTML=`
    <div class="kco-ai-head">
      <div class="kco-ai-avatar">${f("bot",20)}</div>
      <div>
  ...
  `;const a=t.querySelector(".kco-ai-body"),o=t.querySelector(".kco-ai-chips"),c=t.querySelector(".kco-ai-input"),d=t.querySelector(".kco-ai-send"),u=t.querySelector(".kco-ai-close");L.forEach(n=>{const r=document.createElement("button");r.className="kco-ai-chip",r.type="button",r.textContent=n,r.addEventListener("click",()=>{h(n),o.remove()}),o.appendChild(r)});const p=()=>{const n=c.value.trim();!n||s.busy||(c.value="",h(n))};return d.addEventListener("click",p),c.addEventListener("keydown",n=>{n.key==="Enter"&&p()}),u.addEventListener("click",m),document.body.appendChild(e),document.body.appendChild(t),{panel:t,body:a,input:c,sendBtn:d}}let i=null;function b(e){const t=document.createElement("div");return t.className="kco-ai-msg kco-ai-assistant",t.innerHTML=O(e),t}function z(){const e=document.createElement("div");return e.className="kco-ai-typing",e.innerHTML="<span></span><span></span><span></span>",e}function l(e){e.scrollTop=e.scrollHeight}function H(e){const t=y();for(const a of t){const o=document.createElement("div");o.className=a.role==="user"?"kco-ai-msg kco-ai-user":"kco-ai-msg kco-ai-assistant",o.textContent=a.content,e.appendChild(o)}l(e)}async function h(e){if(!i||s.busy)return;const{body:t,input:a}=i,o=y(),c=document.createElement("div");c.className="kco-ai-msg kco-ai-user",c.textContent=e,t.appendChild(c),l(t),o.push({role:"user",content:e}),g(o),s.busy=!0;const d=z();t.appendChild(d),l(t);const u=o.slice(0,-1).slice(-10);try{const n=await(await fetch(C,{method:"POST",headers:{Authorization:`Bearer ${A}`,"Content-Type":"application/json"},body:JSON.stringify({message:e,history:u})})).json(),r=String(n?.response||"").trim();if(!r)throw new Error(n?.error||"Empty reply");d.remove();const E=b(r);t.appendChild(E),l(t),o.push({role:"assistant",content:r}),g(o)}catch{d.remove();const n=b("I'm sorry — I couldn't reach the assistant just now. Please try again in a moment, or email support@weverseonlineshop.com and we'll reply within 24 hours.");t.appendChild(n),l(t)}s.busy=!1,a&&a.focus()}function w(){i&&(i.panel.classList.add("kco-ai-open"),s.open=!0,i.input&&setTimeout(()=>i.input.focus(),250))}function m(){i&&(i.panel.classList.remove("kco-ai-open"),s.open=!1)}function v(){s.open?m():w()}function $(){if(window.__KCO_ADMIN_PAGE__)return!0;const e=window.location.pathname||"";return/\/admin(-[a-z0-9]+)?(\.html)?$/.test(e)}function k(){s.inited||$()||(s.inited=!0,N(),i=T(),H(i.body),window.__kcoAiOpen=w,window.__kcoAiClose=m,window.__kcoAiToggle=v)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",k):k();
