import{supabase as h}from"./supabase-client-D1iyov-x.js";import{g as w}from"./auth-C_Eb0opg.js";import"./brand-_M2LrXmp.js";const y="https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/custom-domains";let m=null,c=[];function n(s){const t=document.getElementById("toast");document.getElementById("toast-msg").textContent=s,t.classList.remove("translate-y-20","opacity-0"),clearTimeout(t._t),t._t=setTimeout(()=>t.classList.add("translate-y-20","opacity-0"),3500),window.lucide&&lucide.createIcons()}function l(s){if(s==null)return"";const t=document.createElement("div");return t.textContent=String(s),t.innerHTML}function g(s){return s?new Date(s).toLocaleDateString("en-US",{year:"numeric",month:"short",day:"numeric"}):"—"}function f(s){s&&s.target!==s.currentTarget||(document.getElementById("modal-container").innerHTML="")}window.closeModal=f;function p(s){s.addEventListener("click",function(t){const a=this.getBoundingClientRect(),e=document.createElement("span");e.className="ripple";const i=Math.max(a.width,a.height);e.style.width=e.style.height=i+"px",e.style.left=t.clientX-a.left-i/2+"px",e.style.top=t.clientY-a.top-i/2+"px",this.appendChild(e),setTimeout(()=>e.remove(),600)})}async function o(s,t={}){const a=await fetch(y,{method:"POST",headers:{Authorization:`Bearer ${m}`,"Content-Type":"application/json"},body:JSON.stringify({action:s,...t})}),e=await a.json();if(!a.ok)throw new Error(e.error||"Request failed");return e}function v(s){const t={pending:{cls:"bg-amber-500/10 text-amber-400 border-amber-500/20",icon:"clock",label:"DNS Pending"},dns_found:{cls:"bg-blue-500/10 text-blue-400 border-blue-500/20",icon:"search",label:"DNS Found"},ssl_installing:{cls:"bg-indigo-500/10 text-indigo-400 border-indigo-500/20",icon:"loader",label:"SSL Installing"},connected:{cls:"bg-emerald-500/10 text-emerald-400 border-emerald-500/20",icon:"check-circle",label:"Connected"},live:{cls:"bg-emerald-500/10 text-emerald-400 border-emerald-500/20",icon:"globe",label:"Live"},failed:{cls:"bg-red-500/10 text-red-400 border-red-500/20",icon:"x-circle",label:"Failed"}},a=t[s]||t.pending;return`<span class="inline-flex items-center gap-1 ${a.cls} border text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wide"><i data-lucide="${a.icon}" class="w-3 h-3 ${s==="ssl_installing"?"animate-spin":""}"></i>${a.label}</span>`}function $(s){const t={none:{cls:"bg-gray-500/10 text-gray-400 border-gray-500/20",label:"No SSL"},pending:{cls:"bg-amber-500/10 text-amber-400 border-amber-500/20",label:"Pending"},active:{cls:"bg-emerald-500/10 text-emerald-400 border-emerald-500/20",label:"SSL Active"},failed:{cls:"bg-red-500/10 text-red-400 border-red-500/20",label:"SSL Failed"}},a=t[s]||t.none;return`<span class="inline-flex items-center gap-1 ${a.cls} border text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wide">${a.label}</span>`}function S(s){const t=[{id:"pending",label:"DNS Pending",icon:"clock"},{id:"dns_found",label:"DNS Found",icon:"search"},{id:"ssl_installing",label:"SSL Installing",icon:"loader"},{id:"connected",label:"Connected",icon:"check-circle"},{id:"live",label:"Live",icon:"globe"}],a=t.findIndex(e=>e.id===s);return`
      ...
      `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(p),document.getElementById("add-domain-form").addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("new-domain-name").value.trim(),e=document.getElementById("new-domain-registrar").value;if(!a)return;const i=t.target.querySelector('button[type="submit"]');i.disabled=!0,i.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Adding...',window.lucide&&lucide.createIcons();try{const r=await o("add",{domain:a,registrar:e});n(r.message),f(),await d(),showDnsRecords(r.domain.id)}catch(r){n("Error: "+r.message),i.disabled=!1,i.innerHTML='<i data-lucide="plus" class="w-4 h-4"></i> Add Domain',window.lucide&&lucide.createIcons()}})};window.verifyDomain=async s=>{n("Checking DNS records...");try{const t=await o("verify",{domainId:s});n(t.message),await d()}catch(t){n("Error: "+t.message)}};window.removeDomain=async s=>{const t=c.find(a=>a.id===s);if(t&&confirm(`Are you sure you want to remove "${t.domain}"? This action cannot be undone.`))try{const a=await o("remove",{domainId:s});n(a.message),await d()}catch(a){n("Error: "+a.message)}};window.setPrimary=async s=>{try{const t=await o("set_primary",{domainId:s});n(t.message),await d()}catch(t){n("Error: "+t.message)}};window.renewSsl=async s=>{n("Renewing SSL certificate...");try{const t=await o("renew_ssl",{domainId:s});n(t.message),await d()}catch(t){n("Error: "+t.message)}};window.toggleRedirect=async(s,t)=>{try{const a=await o("set_redirect",{domainId:s,redirect_to_primary:t,redirect_type:301});n(a.message)}catch(a){n("Error: "+a.message),await d()}};window.showDnsRecords=s=>{const t=document.getElementById(`dns-records-${s}`);t&&t.classList.toggle("hidden")};let x=!1;window.toggleDnsInstructions=async()=>{const s=document.getElementById("dns-instructions"),t=document.getElementById("dns-toggle-text");if(s.classList.contains("hidden")){if(s.classList.remove("hidden"),t.textContent="Hide Instructions",!x)try{const a=await o("all_registrars");s.innerHTML=a.registrars.map(e=>`
