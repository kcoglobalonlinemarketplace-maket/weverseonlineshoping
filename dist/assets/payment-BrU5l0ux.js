...
import{supabase as L}from"./supabase-client-7_ZWSEp6.js";import{g as q,d as K}from"./localization-B1uQqmO-.js";import{_ as O}from"./preload-helper-CLcXU_4U.js";import{f as R,l as Z,a as T,b as Q}from"./showroom-data-Dj0dPKYk.js";import{g as ee,a as te,b as le}from"./car-data-BznJ4eS3.js";import{g as re}from"./auth-D3ZJ7kZA.js";import{c as ie,a as ae,b as oe,d as H,r as se,e as ne}from"./payment-settings-BTsVTq0X.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-BV4Diyt0.js";import"./customer-ai-widget-DHsAzpRV.js";import"./brand-DYhAh01b.js";const z="/fallback.svg";let P=null,k=[],I="",S="";const de=[{name:"PayPal",icon:"wallet",color:"text-blue-400"},{name:"Stripe",icon:"credit-card",color:"text-violet-400"},{name:"Flutterwave",icon:"zap",color:"text-blue-400"},{name:"Paystack",icon:"layers",color:"text-cyan-400"},{name:"Apple Pay",icon:"smartphone",color:"text-gray-300"},{name:"Google Pay",icon:"smartphone",color:"text-green-400"},{name:"Visa",icon:"credit-card",color:"text-blue-500"},{name:"Mastercard",icon:"credit-card",color:"text-red-500"},{name:"American Express",icon:"credit-card",color:"text-blue-300"},{name:"Discover",icon:"credit-card",color:"text-blue-500"},{name:"Verve",icon:"credit-card",color:"text-green-500"},{name:"Bitcoin (BTC)",icon:"bitcoin",color:"text-yellow-500"},{name:"Ethereum (ETH)",icon:"bitcoin",color:"text-indigo-400"},{name:"USDT",icon:"bitcoin",color:"text-green-400"}],_=[{id:"placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"submitted",label:"Payment Submitted",icon:"upload",color:"text-cyan-400",bg:"bg-cyan-500/15"},{id:"verification",label:"Pending Verification",icon:"loader",color:"text-amber-400",bg:"bg-amber-500/15"},{id:"approved",label:"Approved",icon:"check-circle",color:"text-emerald-400",bg:"bg-emerald-500/15"},{id:"processing",label:"Processing",icon:"package",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"shipped",label:"Shipped",icon:"truck",color:"text-indigo-400",bg:"bg-indigo-500/15"},{id:"delivered",label:"Delivered",icon:"package-check",color:"text-emerald-400",bg:"bg-emerald-500/15"}];function ce(){return new URLSearchParams(window.location.search).get("id")}function ue(){return localStorage.getItem("kco_country")||"US"}function pe(){const e=Date.now().toString(36).toUpperCase().slice(-6),l=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${e}${l}`}function U(e,l){const t=()=>{const a=document.createElement("textarea");a.value=e,a.style.position="fixed",a.style.opacity="0",document.body.appendChild(a),a.select();try{document.execCommand("copy")}catch{}document.body.removeChild(a)};if(navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),l){const a=l.innerHTML;l.innerHTML='<i data-lucide="check" class="w-3.5 h-3.5 text-emerald-400"></i>',window.lucide&&lucide.createIcons(),setTimeout(()=>{l.innerHTML=a,window.lucide&&lucide.createIcons()},1500)}J("Copied Successfully.")}function J(e){let l=document.getElementById("payment-toast");l||(l=document.createElement("div"),l.id="payment-toast",l.className="fixed bottom-5 right-5 z-[100] transform translate-y-20 opacity-0 bg-gray-900 border border-blue-500/30 text-white px-5 py-3 rounded-xl shadow-xl text-xs flex items-center gap-2 font-medium transition-all duration-300",l.innerHTML='<i data-lucide="info" class="w-4 h-4 text-blue-400"></i><span id="payment-toast-msg">Action</span>',document.body.appendChild(l)),l.querySelector("#payment-toast-msg").textContent=e,l.classList.remove("translate-y-20","opacity-0"),clearTimeout(l._t),l._t=setTimeout(()=>l.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function V(e,l){const t=se(k,e,l);return{...t,fallbackNotice:t.isFallback?ne(t.account,e,l,I):null}}function be(){const e=document.getElementById("particles");if(e)for(let l=0;l<14;l++){const t=document.createElement("div"),a=Math.random()*3+1;t.className="particle",t.style.width=a+"px",t.style.height=a+"px",t.style.left=Math.random()*100+"%",t.style.bottom="-10px",t.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",t.style.animationDuration=Math.random()*20+15+"s",t.style.animationDelay=Math.random()*20+"s",e.appendChild(t)}}be();function me(e,l,t){const a=T(e);return`
        ...
        ${a.map(r=>`
          <div class="flex items-center justify-between gap-3 bg-blue-950/40 border border-blue-500/10 rounded-xl px-4 py-2.5">
            <div class="min-w-0 flex-1">
              ...
              <div class="text-gray-100 text-sm font-medium font-mono break-all">${r.value}</div>
            </div>
            <button onclick="copyToClipboard('${r.value.replace(/'/g,"\\'")}', this)" class="shrink-0 p-2 bg-blue-900/40 hover:bg-blue-500/20 border border-blue-500/20 hover:border-blue-500/40 rounded-lg transition group" title="Copy ${r.label}">
  ...
  `}function xe(e,l,t){const a=H(k),o=t?q(t):null;return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${a.map(r=>{const n=k.find(u=>u.currency===r);return`
            <button onclick="selectCurrency('${r}')" class="btn-press flex flex-col items-center gap-1 p-3 rounded-xl border transition relative overflow-hidden ${r===e?"bg-blue-500/15 border-blue-500/50 text-blue-400 pulse-glow":"bg-blue-950/40 border-blue-500/10 text-gray-400 hover:border-blue-500/30 hover:text-white"}">
  ...
  `}function Y(e){const l=_.findIndex(t=>t.id===e);return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="relative">
        ...
        <div class="space-y-4">
          ${_.map((t,a)=>{const o=a<=l,r=a===l;return`
              <div class="flex items-center gap-3 relative">
                <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0 z-10 transition-all duration-300 ${o?t.bg+" border border-blue-500/30":"bg-blue-950/40 border border-blue-500/10"} ${r?"pulse-glow":""}">
                ...
                ${o&&!r?'<i data-lucide="check" class="w-4 h-4 text-emerald-400 shrink-0"></i>':""}
                ${r?'<span class="text-[10px] text-blue-400 font-bold uppercase shrink-0">Current</span>':""}
              </div>
            `}).join("")}
        </div>
      </div>
    </div>
  `}function ve(e,l,t,a,o){const r=o?`
  ...
  `}function ye(e,l,t,a){const o=T(l);return`
  ...
  `}async function he(){const e=document.getElementById("payment-content"),t=new URLSearchParams(window.location.search).get("guest")==="1",a=t?null:await re();if(!a&&!t){window.location.href="/";return}const o=ce();let r=R(o)||ee(o)||te(o)||le(o);if(!r){const[{generateListingById:w},{loadHiddenCatalogIds:i}]=await Promise.all([O(()=>import("./catalog-CESpPuFx.js"),__vite__mapDeps([0,1,2])),O(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await i(),r=w(o)}if(r||(await Z(),r=R(o)),!r){e.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}T(r);const n=r.listing_type==="property",y=r.images?.[0]||z;let u=ue();if(a&&!t){const{data:w}=await L.from("profiles").select("country_code").eq("user_id",a.id).maybeSingle();w?.country_code&&(u=w.country_code,localStorage.setItem("kco_country",u))}const $=q(u),E=$?$.name:u,p=K(u),g=new URLSearchParams(window.location.search).get("order")||pe(),h=r.price;P=await ie(),k=ae(P),I=oe(P),S=p||"";let f=S||"USD";H(k).includes(f)||(f="USD");const v=V(u,S||"");e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="currency-selector-container">${xe(f,E,u)}</div>

      <div id="bank-account-container">${v.isFallback?G(v.fallbackNotice):A(v.account,null,I)}</div>

      <div id="upload-form-container">${ve(g,r,h,f,t)}</div>
