import{supabase as _}from"./supabase-client-7_ZWSEp6.js";import{g as U,d as W}from"./localization-DKSFUP4y.js";import{_ as X}from"./preload-helper-CLcXU_4U.js";import{f as O,l as Z,a as T,b as Q}from"./showroom-data-DQuiOO-D.js";import{g as ee}from"./auth-D3ZJ7kZA.js";import{c as te,a as le,b as ae,d as q,r as re,e as ie}from"./payment-settings-Dd9rLt-m.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-a98hhW6V.js";import"./customer-ai-widget-BU0Xtg7v.js";import"./brand-BE-xoT4U.js";const H="/fallback.svg";let P=null,k=[],C="",S="";const oe=[{name:"PayPal",icon:"wallet",color:"text-blue-400"},{name:"Stripe",icon:"credit-card",color:"text-violet-400"},{name:"Flutterwave",icon:"zap",color:"text-orange-400"},{name:"Paystack",icon:"layers",color:"text-cyan-400"},{name:"Apple Pay",icon:"smartphone",color:"text-gray-300"},{name:"Google Pay",icon:"smartphone",color:"text-green-400"},{name:"Visa",icon:"credit-card",color:"text-blue-500"},{name:"Mastercard",icon:"credit-card",color:"text-red-500"},{name:"American Express",icon:"credit-card",color:"text-blue-300"},{name:"Discover",icon:"credit-card",color:"text-orange-500"},{name:"Verve",icon:"credit-card",color:"text-green-500"},{name:"Bitcoin (BTC)",icon:"bitcoin",color:"text-yellow-500"},{name:"Ethereum (ETH)",icon:"bitcoin",color:"text-indigo-400"},{name:"USDT",icon:"bitcoin",color:"text-green-400"}],L=[{id:"placed",label:"Order Placed",icon:"shopping-bag",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"submitted",label:"Payment Submitted",icon:"upload",color:"text-cyan-400",bg:"bg-cyan-500/15"},{id:"verification",label:"Pending Verification",icon:"loader",color:"text-amber-400",bg:"bg-amber-500/15"},{id:"approved",label:"Approved",icon:"check-circle",color:"text-emerald-400",bg:"bg-emerald-500/15"},{id:"processing",label:"Processing",icon:"package",color:"text-blue-400",bg:"bg-blue-500/15"},{id:"shipped",label:"Shipped",icon:"truck",color:"text-indigo-400",bg:"bg-indigo-500/15"},{id:"delivered",label:"Delivered",icon:"package-check",color:"text-emerald-400",bg:"bg-emerald-500/15"}];function se(){return new URLSearchParams(window.location.search).get("id")}function ne(){return localStorage.getItem("kco_country")||"US"}function de(){const e=Date.now().toString(36).toUpperCase().slice(-6),l=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${e}${l}`}function R(e,l){const t=()=>{const i=document.createElement("textarea");i.value=e,i.style.position="fixed",i.style.opacity="0",document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};if(navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(e).catch(()=>t()):t(),l){const i=l.innerHTML;l.innerHTML='<i data-lucide="check" class="w-3.5 h-3.5 text-emerald-400"></i>',window.lucide&&lucide.createIcons(),setTimeout(()=>{l.innerHTML=i,window.lucide&&lucide.createIcons()},1500)}z("Copied Successfully.")}function z(e){let l=document.getElementById("payment-toast");l||(l=document.createElement("div"),l.id="payment-toast",l.className="fixed bottom-5 right-5 z-[100] transform translate-y-20 opacity-0 bg-gray-900 border border-blue-500/30 text-white px-5 py-3 rounded-xl shadow-xl text-xs flex items-center gap-2 font-medium transition-all duration-300",l.innerHTML='<i data-lucide="info" class="w-4 h-4 text-blue-400"></i><span id="payment-toast-msg">Action</span>',document.body.appendChild(l)),l.querySelector("#payment-toast-msg").textContent=e,l.classList.remove("translate-y-20","opacity-0"),clearTimeout(l._t),l._t=setTimeout(()=>l.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function G(e,l){const t=re(k,e,l);return{...t,fallbackNotice:t.isFallback?ie(t.account,e,l,C):null}}function ce(){const e=document.getElementById("particles");if(e)for(let l=0;l<14;l++){const t=document.createElement("div"),i=Math.random()*3+1;t.className="particle",t.style.width=i+"px",t.style.height=i+"px",t.style.left=Math.random()*100+"%",t.style.bottom="-10px",t.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",t.style.animationDuration=Math.random()*20+15+"s",t.style.animationDelay=Math.random()*20+"s",e.appendChild(t)}}ce();function ue(e,l,t){const i=T(e);return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="flex gap-4">
        <div class="w-24 h-24 rounded-xl overflow-hidden bg-gray-900 shrink-0 ring-1 ring-blue-500/10">
          <img src="${l}" alt="${e.title}" class="w-full h-full object-cover" onerror="this.onerror=null;this.src='${H}'">
        </div>
  ...
  `}function pe(e,l,t){const i=q(k),o=t?U(t):null;return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="grid grid-cols-3 sm:grid-cols-5 gap-2">
        ${i.map(a=>{const n=k.find(u=>u.currency===a);return`
  ...
  `}function V(e){const l=L.findIndex(t=>t.id===e);return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 mb-5 slide-up">
      ...
      <div class="relative">
        ...
        <div class="absolute left-4 top-4 w-0.5 bg-blue-500 transition-all duration-500" style="height: ${l>=0?l/(L.length-1)*100:0}%; min-height: 0; max-height: calc(100% - 2rem)"></div>
        <div class="space-y-4">
          ${L.map((t,i)=>{const o=i<=l,a=i===l;return`
  ...
  `}function xe(e,l,t,i,o){const a=o?`
      ...
      <form id="receipt-form" class="space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" id="form-currency-display" value="${i}" disabled class="w-full bg-[#0a1124]/80 border border-blue-500/10 rounded-xl px-4 py-2.5 text-sm text-white font-mono">
          </div>
          <div>
  ...
  `}function fe(e,l,t,i){const o=T(l);return`
    <div class="fade-in text-center py-8">
      ...
      <div class="glass border border-blue-500/20 rounded-2xl p-5 max-w-md mx-auto mb-5 text-left">
        ...
        <div class="flex justify-between text-sm mb-4"><span class="text-gray-500">Currency</span><span class="text-white font-bold">${i}</span></div>
  ...
  `}async function ge(){const e=document.getElementById("payment-content"),t=new URLSearchParams(window.location.search).get("guest")==="1",i=t?null:await ee();if(!i&&!t){window.location.href="/";return}const o=se();let a=O(o);if(!a){const{generateListingById:w}=await X(async()=>{const{generateListingById:r}=await import("./catalog-Bmn1vD5s.js");return{generateListingById:r}},[]);a=w(o)}if(a||(await Z(),a=O(o)),!a){e.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}T(a);const n=a.listing_type==="property",y=a.images?.[0]||H;let u=ne();if(i&&!t){const{data:w}=await _.from("profiles").select("country_code").eq("user_id",i.id).maybeSingle();w?.country_code&&(u=w.country_code,localStorage.setItem("kco_country",u))}const $=U(u),E=$?$.name:u,p=W(u),g=new URLSearchParams(window.location.search).get("order")||de(),h=a.price;P=await te(),k=le(P),C=ae(P),S=p||"";let f=S||"USD";q(k).includes(f)||(f="USD");const v=G(u,S||"");e.innerHTML=`
    <div class="fade-in">
      ...
      <div id="bank-account-container">${v.isFallback?J(v.fallbackNotice):A(v.account,null,C)}</div>
