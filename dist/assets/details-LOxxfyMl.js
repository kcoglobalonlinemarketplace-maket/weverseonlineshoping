...
import{_ as W}from"./supabase-lazy-BQT6Mc9B.js";import"./localization-5a8b3DdX.js";import{c as $,S as z,l as U,f as G,a as A,b as D,g as Y,P as K}from"./showroom-data-43n_Qj5I.js";import{getCatalogCategory as J,getCatalogSample as Q}from"./catalog-DMzG21Ab.js";import{g as X,a as Z,b as ee,c as te,f as ae,T as re,M as oe,C as le,P as ie}from"./phone-data-CExU1cqD.js";import{r as se}from"./showroom-cards-Cuje2sVH.js";import{getCurrentUser as w,setRedirectAfterAuth as I}from"./auth-C8sthrmE.js";import{supabase as h}from"./supabase-client-nvpjTmO6.js";import"./live-stream-mode-Cn8AjDtr.js";import"./customer-ai-widget-Dot6E6Qg.js";import"./brand-QZwsptsl.js";import"./catalog-hidden-store-A7rGgF0I.js";const v="/fallback.svg";function B(e){return typeof e=="number"&&!isNaN(e)?e.toFixed(1):"0.0"}function T(e){return Array.isArray(e)&&e.length>0?e:[v]}function ne(){return new URLSearchParams(window.location.search).get("id")}function ce(e){const t=document.getElementById("details-content"),a=ae(e),n=T(e.images).map((r,u)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${u===0?"active border-blue-500":"border-gray-200"} shrink-0" data-img="${s(r)}">
    ...
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
      ...
      <div class="flex flex-wrap gap-2">
        ${e.features.map(r=>`<span class="text-xs bg-gray-100 text-gray-700 px-3 py-1.5 rounded-full border border-gray-300">${s(r)}</span>`).join("")}
      </div>
    </div>`:"",c=`
    <div class="flex items-center gap-4 mb-6">
      <div class="flex items-center gap-1.5">
        <i data-lucide="star" class="w-5 h-5 fill-amber-400 text-amber-600"></i>
        <span class="text-lg font-bold text-gray-900">${B(e.rating)}</span>
    ...
    <div class="fade-in">
      ...
      <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-6">
        <div>
          <h1 class="text-2xl sm:text-3xl font-black text-gray-900 leading-tight">${s(e.title)}</h1>
          <p class="text-gray-500 text-sm mt-1">Stock #: <span class="text-blue-500 font-mono font-bold">${s(e.stock_number||"—")}</span> &middot; VIN: <span class="text-gray-600 font-mono">${s(e.vin||"—")}</span></p>
        </div>
        <div class="text-right shrink-0">
          <div class="text-3xl font-black text-blue-500">${a}</div>
          <span class="inline-block bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full mt-1">${s(e.condition||"Used")} &middot; For Sale</span>
        </div>
      </div>
      ...
      <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
        ...
        <p class="text-gray-600 text-sm leading-relaxed">${s(e.description||"")}</p>
      </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          ${d.map(r=>`
            <div class="flex flex-col gap-1">
              ...
              <div class="text-gray-800 font-medium text-sm">${s(r.value)}</div>
            </div>
          `).join("")}
        </div>
      </div>
  ...
  `;const b=document.getElementById("hero-image"),f=document.getElementById("gallery-label");t.querySelectorAll(".gallery-thumb").forEach((r,u)=>{r.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(g=>g.classList.remove("active","border-blue-500")),t.querySelectorAll(".gallery-thumb").forEach(g=>g.classList.add("border-gray-200")),r.classList.add("active","border-blue-500"),r.classList.remove("border-gray-200"),b.src=r.dataset.img,f.textContent=l[u]||`View ${u+1}`})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const r=window.location.href;try{if(navigator.share)await navigator.share({title:s(e.title),url:r});else{await navigator.clipboard.writeText(r);const u=document.getElementById("share-btn"),g=u.innerHTML;u.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{u.innerHTML=g,window.lucide&&lucide.createIcons()},2e3)}}catch{}}),_(e),window.lucide&&lucide.createIcons()}function de(e){const t=document.getElementById("details-content"),a=A(e),n=T(e.images).map((r,u)=>`<button class="gallery-thumb rounded-lg overflow-hidden border-2 ${u===0?"active border-blue-500":"border-gray-200"} shrink-0" data-img="${s(r)}">
      <img src="${s(r)}" alt="View ${u+1}" loading="lazy" class="w-20 h-16 object-cover" onerror="this.onerror=null;this.src='${v}'">
  ...
  `}function H(e){const t=e.listing_type==="property",a=`/contact.html?listing=${encodeURIComponent(e.property_id||"")}`;return`
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ${[{icon:"globe",label:"Country",value:`${n} ${e.country}`},{icon:"map-pin",label:"State / Province",value:e.state},{icon:"building",label:"City",value:e.city},{icon:"navigation",label:"Town / Local Area",value:e.town}].filter(i=>i.value).map(i=>`
            <div class="flex items-center gap-2.5 text-sm">
              <div class="p-2 bg-gray-100 rounded-lg"><i data-lucide="${i.icon}" class="w-4 h-4 text-blue-500"></i></div>
              <div><div class="text-gray-500 text-xs">${i.label}</div><div class="text-gray-800 font-medium">${i.value}</div></div>
            </div>
          `).join("")}
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          ${[{icon:"bed-double",label:"Bedrooms",value:e.bedrooms},{icon:"bath",label:"Bathrooms",value:e.bathrooms},{icon:"building",label:"Building Size",value:e.building_size},{icon:"ruler",label:"Land Size",value:e.land_size},{icon:"car",label:"Parking Spaces",value:e.parking_spaces},{icon:"home",label:"Property Type",value:e.property_type},{icon:"sofa",label:"Furnished",value:e.furnished},{icon:"calendar",label:"Year Built",value:e.year_built},{icon:"tag",label:"Status",value:e.listing_status==="rent"?"For Rent":"For Sale"}].filter(i=>i.value!=null&&i.value!=="").map(i=>`
            <div class="flex flex-col gap-1">
              ...
              <div class="text-gray-800 font-medium text-sm">${s(i.value)}</div>
            </div>
          `).join("")}
        </div>
      ...
      </div>`);const f=e.features?.length?`
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wide mb-4">Features & Amenities</h3>
      <div class="flex flex-wrap gap-2">
        ${e.features.map(m=>`<span class="text-xs bg-gray-100 text-gray-700 px-3 py-1.5 rounded-full border border-gray-300">${s(m)}</span>`).join("")}
      </div>
    </div>`:"",r=e.highlights?.length?`
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-5 mb-6">
      <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wide mb-4">Highlights</h3>
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ${e.highlights.map(m=>`<div class="flex items-start gap-2 text-sm text-gray-700"><i data-lucide="badge-check" class="w-4 h-4 text-blue-500 mt-0.5"></i><span>${s(m)}</span></div>`).join("")}
      </div>
    </div>`:"",u=`
    <div class="flex items-center gap-4 mb-6">
      <div class="flex items-center gap-1.5">
    ...
    <div class="fade-in">
      <div class="flex items-center gap-2 text-xs text-gray-500 mb-4">
        ...
        <i data-lucide="chevron-right" class="w-3 h-3"></i>
        <span class="text-gray-700 truncate">${s(e.title)}</span>
      </div>
  ...
  `;const g=document.getElementById("hero-image");t.querySelectorAll(".gallery-thumb").forEach(m=>{m.addEventListener("click",()=>{t.querySelectorAll(".gallery-thumb").forEach(i=>i.classList.remove("active","border-blue-500")),t.querySelectorAll(".gallery-thumb").forEach(i=>i.classList.add("border-gray-200")),m.classList.add("active","border-blue-500"),m.classList.remove("border-gray-200"),g.src=m.dataset.img})}),document.getElementById("buy-now-btn").addEventListener("click",async()=>{await w()?window.location.href=`/checkout.html?id=${e.property_id}`:(I(`/checkout.html?id=${e.property_id}`),window.location.href=`/auth.html?redirect=${encodeURIComponent("/checkout.html?id="+e.property_id)}`)}),document.getElementById("share-btn").addEventListener("click",async()=>{const m=window.location.href;try{if(navigator.share)await navigator.share({title:s(e.title),url:m});else{await navigator.clipboard.writeText(m);const i=document.getElementById("share-btn"),E=i.innerHTML;i.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Copied!',window.lucide&&lucide.createIcons(),setTimeout(()=>{i.innerHTML=E,window.lucide&&lucide.createIcons()},2e3)}}catch{}});const R=document.getElementById("view-details-btn");R&&R.addEventListener("click",()=>{const m=document.getElementById("listing-details");m&&m.scrollIntoView({behavior:"smooth",block:"start"})});const S=document.getElementById("add-cart-btn");S&&S.addEventListener("click",()=>{let m=JSON.parse(localStorage.getItem("kco_cart")||"[]");m.includes(e.property_id)||(m.push(e.property_id),localStorage.setItem("kco_cart",JSON.stringify(m))),S.innerHTML='<i data-lucide="check" class="w-5 h-5"></i> Added to Cart',window.lucide&&lucide.createIcons(),setTimeout(()=>{S.innerHTML='<i data-lucide="shopping-cart" class="w-5 h-5"></i> Add to Cart',window.lucide&&lucide.createIcons()},2e3)}),ye(e),ve(e),O(e),he(e),window.lucide&&lucide.createIcons(),e.property_id,e.title,parseFloat(e.price),e.currency;const x=document.getElementById("listing-map");if(x&&window.L){const m=parseFloat(e.latitude)||null,i=parseFloat(e.longitude)||null,E=[e.town,e.city,e.state,e.country].filter(Boolean).join(", ");if(m&&i){const y=L.map(x).setView([m,i],13);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(y),L.marker([m,i]).addTo(y).bindPopup(e.title)}else E?fetch("https://nominatim.openstreetmap.org/search?format=json&limit=1&q="+encodeURIComponent(E)).then(y=>y.json()).then(y=>{if(y&&y[0]){const P=parseFloat(y[0].lat),q=parseFloat(y[0].lon),F=L.map(x).setView([P,q],12);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors"}).addTo(F),L.marker([P,q]).addTo(F).bindPopup(e.title)}else x.style.display="none"}).catch(()=>{x.style.display="none"}):x.style.display="none"}}let k=0,V=!1;function ge(){if(V)return;V=!0;const e=document.createElement("style");e.textContent="@keyframes kcoWishPop{0%{transform:scale(1)}35%{transform:scale(1.45)}60%{transform:scale(.86)}100%{transform:scale(1)}}",document.head.appendChild(e)}function j(e,t){if(!e)return;ge(),e.innerHTML=`<i data-lucide="heart" class="w-5 h-5 ${t?"fill-red-500 text-red-500":""}"></i>`,e.classList.toggle("bg-red-50",t),e.classList.toggle("border",t),e.classList.toggle("border-red-200",t);const a=e.querySelector("span");a&&(a.textContent=t?"Saved to Wishlist":"Add to Wishlist"),window.lucide&&lucide.createIcons();const o=e.querySelector("i");o&&(o.style.animation="none",o.offsetWidth,o.style.animation="kcoWishPop .5s cubic-bezier(.34,1.56,.64,1)",setTimeout(()=>{o&&(o.style.animation="")},550))}async function ye(e){const t=document.getElementById("wishlist-btn");if(!t)return;const a=await w();if(!a){t.addEventListener("click",()=>{I(window.location.pathname+window.location.search),window.location.href=`/auth.html?redirect=${encodeURIComponent(window.location.pathname+window.location.search)}`});return}const{data:o,error:n}=await h.from("wishlist").select("id").eq("user_id",a.id).eq("listing_id",e.id).maybeSingle();if(n){console.error("Wishlist check failed:",n.message);return}o&&j(t,!0),t.addEventListener("click",async()=>{const{data:l,error:d}=await h.from("wishlist").select("id").eq("user_id",a.id).eq("listing_id",e.id).maybeSingle();if(d){console.error("Wishlist toggle failed:",d.message);return}if(l){const{error:p}=await h.from("wishlist").delete().eq("id",l.id);if(p){console.error("Wishlist delete failed:",p.message);return}j(t,!1)}else{const{error:p}=await h.from("wishlist").insert({user_id:a.id,listing_id:e.id});if(p){console.error("Wishlist insert failed:",p.message);return}j(t,!0)}})}async function ve(e){const t=document.getElementById("review-form"),a=document.getElementById("review-login-msg"),o=await w();if(!o){t.classList.add("hidden"),a.classList.remove("hidden");return}document.querySelectorAll(".star-btn").forEach(n=>{n.addEventListener("click",()=>{k=parseInt(n.dataset.rating,10),document.querySelectorAll(".star-btn").forEach((l,d)=>{const p=l.querySelector("i");d<k?(p.classList.add("fill-blue-500","text-blue-500"),p.classList.remove("text-gray-600")):(p.classList.remove("fill-blue-500","text-blue-500"),p.classList.add("text-gray-600"))})})}),t.addEventListener("submit",async n=>{n.preventDefault();const l=document.getElementById("review-text").value.trim();if(!k){alert("Please select a rating.");return}if(!l){alert("Please write a review.");return}const{error:d}=await h.from("product_reviews").insert({listing_id:e.id,user_id:o.id,rating:k,review_text:l,is_approved:!1});if(d){alert("Error: "+d.message);return}document.getElementById("review-text").value="",k=0,document.querySelectorAll(".star-btn").forEach(p=>{const c=p.querySelector("i");c.classList.remove("fill-blue-500","text-blue-500"),c.classList.add("text-gray-600")}),alert("Review submitted! It will appear after admin approval."),O(e)})}async function O(e){const t=document.getElementById("reviews-list");if(!t||!e.id)return;const{data:a,error:o}=await h.from("product_reviews").select("*, profiles(full_name)").eq("listing_id",e.id).eq("is_approved",!0).order("created_at",{ascending:!1});if(o){t.innerHTML='<p class="text-gray-500 text-sm">Unable to load reviews.</p>';return}if(!a||a.length===0){t.innerHTML='<p class="text-gray-500 text-sm">No reviews yet. Be the first to review this product!</p>';return}t.innerHTML=a.map(n=>`
    <div class="border-b border-gray-200 pb-3 mb-3 last:border-0">
      <div class="flex items-center gap-2 mb-1">
        <div class="flex gap-0.5">${[1,2,3,4,5].map(l=>`<i data-lucide="star" class="w-3.5 h-3.5 ${l<=n.rating?"fill-amber-400 text-amber-600":"text-gray-600"}"></i>`).join("")}</div>
        <span class="text-xs text-gray-600 font-bold">${s(n.profiles?.full_name||"Anonymous")}</span>
      ...
      <p class="text-sm text-gray-700">${s(n.review_text||"")}</p>
      ${n.vendor_response?`<div class="mt-2 bg-gray-100 rounded-lg p-2 text-xs text-gray-600"><strong class="text-gray-700">Seller response:</strong> ${s(n.vendor_response)}</div>`:""}
    </div>`).join(""),window.lucide&&lucide.createIcons()}async function he(e){const t=document.getElementById("recommendations-section"),a=document.getElementById("recommendations-grid");if(!t||!a||!e.id)return;const{data:o,error:n}=await h.from("product_recommendations").select("recommended_listing_id, showroom_listings!product_recommendations_recommended_listing_id_fkey(property_id, title, price, currency, images, listing_type)").eq("listing_id",e.id).order("score",{ascending:!1}).limit(4);if(n){console.error("Recommendations load failed:",n.message),t.classList.add("hidden");return}let l=(o||[]).map(d=>d.showroom_listings).filter(Boolean);if(l.length<4){const{data:d}=await h.from("showroom_listings").select("property_id, title, price, currency, images, listing_type").eq("category",e.category).neq("id",e.id).eq("is_active",!0).order("rating",{ascending:!1}).limit(4-l.length);l=[...l,...d||[]]}if(l.length===0){t.classList.add("hidden");return}t.classList.remove("hidden"),a.innerHTML=l.map(d=>{const p=d.images&&d.images[0]||"/fallback.svg",c=typeof d.price=="number"?d.price:parseFloat(d.price||0),b=d.currency||"USD";return`<a href="/details.html?id=${d.property_id}" class="block bg-gray-50 border border-gray-200 rounded-xl overflow-hidden hover:border-blue-200 transition group">
      <div class="aspect-square overflow-hidden bg-gray-100"><img src="${s(p)}" alt="" class="w-full h-full object-cover group-hover:scale-105 transition" loading="lazy" onerror="this.src='/fallback.svg'"></div>
      <div class="p-2"><p class="text-xs text-gray-900 font-bold truncate">${s(d.title)}</p><p class="text-xs text-blue-500 font-bold mt-1">${b} ${c.toLocaleString()}</p></div>
    </a>`}).join("")}function s(e){return String(e||"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}async function fe(){const e=ne();if(!e){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}const t=X(e);if(t){$(t),document.title=`${t.title} | Weverse Online Shop`,ce(t);return}const a=Z(e);if(a){$(a),document.title=`${a.title} | Weverse Online Shop`,de(a);return}const o=ee(e);if(o){$(o),document.title=`${o.title} | Weverse Online Shop`,ue(o);return}const n=te(e);if(n){$(n),document.title=`${n.title} | Weverse Online Shop`,N(n);try{_(n)}catch{}return}let l=z.find(d=>d.property_id===e);if(!l){const[{generateListingById:d},{loadHiddenCatalogIds:p}]=await Promise.all([W(()=>import("./catalog-DMzG21Ab.js"),__vite__mapDeps([0,1,2])),W(()=>import("./catalog-hidden-store-A7rGgF0I.js"),__vite__mapDeps([1,2]))]);await p(),l=d(e)}if(l||(await U(),l=G(e)),!l){document.getElementById("details-content").innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}$(l),document.title=`${l.title} | Weverse Online Shop`,N(l);try{_(l)}catch{}}fe();
