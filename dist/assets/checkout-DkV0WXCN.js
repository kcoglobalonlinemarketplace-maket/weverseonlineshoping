import{supabase as g}from"./supabase-client-7_ZWSEp6.js";import{d as T,g as M,C as j}from"./localization-DKSFUP4y.js";import{_ as w}from"./preload-helper-CLcXU_4U.js";import{g as N}from"./auth-D3ZJ7kZA.js";import{f as $,l as k,g as E,a as L}from"./showroom-data-DQuiOO-D.js";import{c as q,a as D,b as U,d as C,r as F,e as O}from"./payment-settings-Dd9rLt-m.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-a98hhW6V.js";import"./customer-ai-widget-BU0Xtg7v.js";import"./brand-BE-xoT4U.js";const f="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function G(){const t=Date.now().toString(36).toUpperCase().slice(-6),s=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${s}`}function m(t){const s=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,s.classList.remove("translate-y-20","opacity-0"),clearTimeout(s._t),s._t=setTimeout(()=>s.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function S(t){t.addEventListener("click",function(s){if(this.disabled)return;const i=this.getBoundingClientRect(),r=document.createElement("span");r.className="ripple";const a=Math.max(i.width,i.height);r.style.width=r.style.height=a+"px",r.style.left=s.clientX-i.left-a/2+"px",r.style.top=s.clientY-i.top-a/2+"px",this.appendChild(r),setTimeout(()=>r.remove(),600)})}function b(t,s){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${s}`}function R(t){const s=()=>{const i=document.createElement("textarea");i.value=t,document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>s()):s(),m("Copied to clipboard.")}function H(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,s=F(e.manualPaymentAccounts,e.countryCode,t);return{...s,fallbackNotice:s.isFallback?O(s.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function z(){const t=document.getElementById("particles");if(t)for(let s=0;s<12;s++){const i=document.createElement("div"),r=Math.random()*3+1;i.className="particle",i.style.width=i.style.height=r+"px",i.style.left=Math.random()*100+"%",i.style.bottom="-10px",i.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",i.style.animationDuration=Math.random()*20+15+"s",i.style.animationDelay=Math.random()*20+"s",t.appendChild(i)}}z();async function J(){const t=document.getElementById("checkout-root"),s=new URLSearchParams(window.location.search),i=s.get("status"),r=s.get("transaction_id"),a=s.get("tx_ref"),c=s.get("order_number");if(i==="verify"&&r){await se(r,a,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=s.get("guest")==="1",!e.isGuest&&(e.user=await N(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const n=s.get("id");if(n){if(e.listing=$(n),!e.listing){const{generateListingById:o}=await w(async()=>{const{generateListingById:l}=await import("./catalog-Bmn1vD5s.js");return{generateListingById:l}},[]);e.listing=o(n)}if(e.listing||(await k(),e.listing=$(n)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await k();const l=E(),{generateListingById:d}=await w(async()=>{const{generateListingById:p}=await import("./catalog-Bmn1vD5s.js");return{generateListingById:p}},[]);if(e.cartItems=o.map(p=>{const v=l.find(B=>B.property_id===p)||d(p);return v?{listing:v,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=I(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await g.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=T(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await q(),e.manualPaymentAccounts=D(e.paymentSettings),e.manualPaymentInstructions=U(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),C(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await g.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const l=e.addresses.find(p=>p.is_default);l&&(e.selectedAddressId=l.id);const{data:d}=await g.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=G(),u()}function h(){return e.cartItems.reduce((t,s)=>t+s.listing.price*s.quantity,0)}function X(){return 0}function Y(){return 0}function y(){return Math.round(h()*Y()*100)/100}function x(){return h()+X()+y()}function u(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
  ...
  `,window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(S)}function V(){const t=[{num:1,label:"Review Cart",icon:"shopping-cart"},{num:2,label:"Shipping & Billing",icon:"map-pin"},{num:3,label:"Payment",icon:"credit-card"}];return`
        ...
        ${e.cartItems.map((t,s)=>`
            <div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-400 mt-1">${L(t.listing)}</p>
              </div>
  ...
  `}function W(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-white uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(i=>{const r=M(i.country_code);return`
  ...
  `}function K(){const t=C(e.manualPaymentAccounts),s=H(),i=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",r=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    ${e.paymentMethod==="manual_bank_transfer"?Z(s.account,s.fallbackNotice,e.manualPaymentInstructions):""}
  ...
  `}function ee(){const t=h(),s=x();return`
    <div class="glass border border-blue-500/20 rounded-2xl p-5 slide-up lg:sticky lg:top-20">
      ...
      <div class="space-y-2 pt-4 border-t border-blue-500/10">
        ...
        ${y()>0?`<div class="flex justify-between text-sm"><span class="text-gray-500">Tax</span><span class="text-white font-bold">${b(y(),e.cartItems[0]?.listing?.currency||"USD")}</span></div>`:""}
  ...
  `,window.lucide&&lucide.createIcons();try{const c=await fetch("https://wttnvwpoqmbxryivcerf.supabase.co/functions/v1/flutterwave-payment?action=verify",{method:"POST",headers:{Authorization:"Bearer sb_publishable_X_6kXsJwApi7v7HwoC1xtA_igns4Rxa","Content-Type":"application/json"},body:JSON.stringify({transaction_id:t,tx_ref:s,order_number:i})}),n=await c.json();if(!c.ok||n.error||n.status==="failed"){r.innerHTML=_(n.message||n.error||"Payment verification failed."),window.lucide&&lucide.createIcons();return}r.innerHTML=ie(i),window.lucide&&lucide.createIcons(),document.querySelectorAll(".btn-press").forEach(S),localStorage.removeItem("kco_pending_order"),localStorage.removeItem("kco_cart")}catch(a){r.innerHTML=_(a.message),window.lucide&&lucide.createIcons()}}function ie(t){return`
