import{h as D,a as N,S as W,e as _,c as E,b as C,d as H}from"./showroom-data-Db4XRqRK.js";import{P as B,a as z,T as q,M as G,f as U}from"./motorhome-data-CupbOvk0.js";import{isCatalogListingHidden as x}from"./catalog-hidden-store-D5ZSQTSm.js";const A="kco-hero-rows",T="/fallback.svg",X=new Set(["Cars","Cars & Vehicles","Trucks","Buses","Buses & Coaches","Motorhomes","Motorcycles","Marine & Boating","RV & Camper Accessories","Vehicles"]),v=new Set(["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"]),K=["Single-Family Home","Apartment","Condo","Townhouse","Villa","Mansion","Beach House","Farm House","House","Homes","Duplex","Penthouse","Bungalow","Cottage","Chalet","Studio","Loft"];function c(e,o){return e.specifications&&typeof e.specifications=="object"&&e.specifications[o]!=null?e.specifications[o]:e[o]}function $(e){return e.listing_type==="vehicle"||X.has(e.category)}function w(e){return!e||typeof e!="string"?!1:/^data:video\//i.test(e)?!0:e.startsWith("blob:")?!1:/\.(mp4|webm|mov|m4v|avi|mkv|ogv)(\?|#|$)/i.test(e)}function V(e){if(!e)return null;for(const o of[e.video,e.video_url])if(w(o))return o;if(Array.isArray(e.images)){for(const o of e.images)if(w(o))return o}return null}function Y(e){if(Array.isArray(e.images)){for(const o of e.images)if(!w(o))return o}return T}function J(){const e=new Set,o=new Set,t=[];for(const r of R()){if(!r)continue;const a=r.property_id||r.id;if(!a||e.has(a))continue;const d=V(r);d&&(o.has(d)||(e.add(a),o.add(d),t.push(r)))}return t}function Q(e){E(e);const o=F(e.property_id||e.id),t=V(e),r=Y(e),a=C(e),d=M(e),n=H(e.country_code),s=[e.city,e.state].filter(Boolean).join(", ")||e.country||"",l=s?`<span class="kco-video-loc"><i data-lucide="map-pin" class="w-3 h-3 shrink-0"></i>${i(n?n+" "+s:s)}</span>`:"",p=e.listing_status==="rent",k=p?"kco-sale-rent":"kco-sale-buy",u=p?"For Rent":"For Sale",h=c(e,"bedrooms")!=null?`<span class="kco-hero-chip"><i data-lucide="bed-double" class="w-3.5 h-3.5"></i>${i(c(e,"bedrooms"))} Bed${Number(c(e,"bedrooms"))===1?"":"s"}</span>`:"",m=c(e,"bathrooms")!=null?`<span class="kco-hero-chip"><i data-lucide="bath" class="w-3.5 h-3.5"></i>${i(c(e,"bathrooms"))} Bath${Number(c(e,"bathrooms"))===1?"":"s"}</span>`:"",g=c(e,"building_size")?`<span class="kco-hero-chip"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>${i(c(e,"building_size"))}</span>`:"",y=e.land_size?`<span class="kco-hero-chip"><i data-lucide="ruler" class="w-3.5 h-3.5"></i>${i(e.land_size)}</span>`:"";return`
    ...
    </a>`}function Z(e){const o="IntersectionObserver"in window?new IntersectionObserver(t=>{for(const r of t){const a=r.target;if(!(!a||typeof a.play!="function"))if(r.isIntersecting)a.play().catch(()=>{});else try{a.pause()}catch{}}},{rootMargin:"120px"}):null;e.querySelectorAll(".kco-video-el").forEach(t=>{t.addEventListener("click",r=>{t.paused&&(t.play().catch(()=>{}),r.preventDefault(),r.stopPropagation())}),o&&o.observe(t)}),Object.defineProperty(e,"_kcoVideoIO",{value:o,configurable:!0}),"_kcoVideoCleanup"in e||Object.defineProperty(e,"_kcoVideoCleanup",{value:()=>o&&o.disconnect(),configurable:!0})}function ee(){const e=J();if(!e.length)return document.createDocumentFragment();const o=document.createElement("section");o.className="kco-hero-section kco-video-section",o.setAttribute("aria-label","Houses For Sale & Rent — Video Tours"),o.setAttribute("data-property-videos","true");const t=`
...
`,document.head.appendChild(e)}function te(e,o,t){E(e);const r=t||"kco-hero-card",a=e.images&&e.images[0]||T,d=F(e.property_id||e.id),n=o==="vehicle"&&e.category==="Trucks"?U(e):C(e);let s="",l="";if(o==="house"){const k=H(e.country_code),u=[e.city,e.state].filter(Boolean).join(", ")||e.country||"";u&&(l=`<span class="kco-hero-loc"><i data-lucide="map-pin" class="w-3 h-3 shrink-0"></i>${i(k+" "+u)}</span>`);const h=[];c(e,"bedrooms")!=null&&h.push(`<span class="kco-hero-chip"><i data-lucide="bed-double" class="w-3.5 h-3.5"></i>${i(c(e,"bedrooms"))} Beds</span>`),c(e,"bathrooms")!=null&&h.push(`<span class="kco-hero-chip"><i data-lucide="bath" class="w-3.5 h-3.5"></i>${i(c(e,"bathrooms"))} Baths</span>`),e.land_size&&h.push(`<span class="kco-hero-chip"><i data-lucide="ruler" class="w-3.5 h-3.5"></i>${i(e.land_size)}</span>`),c(e,"building_size")&&h.push(`<span class="kco-hero-chip"><i data-lucide="building-2" class="w-3.5 h-3.5"></i>${i(c(e,"building_size"))}</span>`),c(e,"year_built")&&h.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${i(c(e,"year_built"))}</span>`);const m=M(e);return s=h.join(""),l||(l=""),`
      ...
      </a>`}const p=[];return c(e,"model_year")&&p.push(`<span class="kco-hero-chip"><i data-lucide="calendar" class="w-3.5 h-3.5"></i>${i(c(e,"model_year"))}</span>`),c(e,"mileage")&&p.push(`<span class="kco-hero-chip"><i data-lucide="gauge" class="w-3.5 h-3.5"></i>${i(c(e,"mileage"))}</span>`),c(e,"fuel_type")&&p.push(`<span class="kco-hero-chip"><i data-lucide="fuel" class="w-3.5 h-3.5"></i>${i(c(e,"fuel_type"))}</span>`),c(e,"transmission")&&p.push(`<span class="kco-hero-chip"><i data-lucide="cog" class="w-3.5 h-3.5"></i>${i(c(e,"transmission"))}</span>`),c(e,"body_type")&&p.push(`<span class="kco-hero-chip"><i data-lucide="car-front" class="w-3.5 h-3.5"></i>${i(c(e,"body_type"))}</span>`),s=p.join(""),`
      <a href="/product/${d}" class="${r}">
        <div class="kco-hero-media">
          <img src="${i(a)}" alt="${i(e.title||"")}" loading="lazy">
          <span class="kco-hero-type"><i data-lucide="car-front" class="w-3 h-3"></i>${i(ae(e))}</span>
        </div>
        <div class="kco-hero-body">
    ...
    <div class="kco-hero-panel">
      <div class="kco-hero-head">
        <div class="kco-hero-headleft">
          ${e.icon?`<span class="kco-hero-ic">${ie(e.icon)}</span>`:""}
          <div class="min-w-0">
            <h3>${i(e.title)}</h3>
            <p>${i(e.subtitle)}</p>
          </div>
      ...
      ${t}
    </div>`;o.innerHTML=r;const a=o.querySelector(".kco-hero-hscroll");O(a);const d=o.querySelector(".kco-hero-arrow.left"),n=o.querySelector(".kco-hero-arrow.right"),s=()=>{const l=a.scrollWidth-a.clientWidth-2;d.disabled=a.scrollLeft<=2,n.disabled=a.scrollLeft>=l};return d.addEventListener("click",()=>a.scrollBy({left:-a.clientWidth*.8,behavior:"smooth"})),n.addEventListener("click",()=>a.scrollBy({left:a.clientWidth*.8,behavior:"smooth"})),a.addEventListener("scroll",s,{passive:!0}),s(),o}const j='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-6-6 6-6"/></svg>',P='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6"/></svg>';function b(){const e=document.getElementById(A);if(!e)return;re();const o=R(),t=document.createDocumentFragment();t.appendChild(ee()),t.appendChild(ce({kindCls:"kco-hero-house",title:"Houses For Sale & Rent",subtitle:"Browse villas, mansions, apartments, beach houses, commercial properties and land listed by their sellers.",icon:"home",cat:"real-estate",seeAll:!0,seeAllLabel:"See All Homes",listings:o,card:r=>te(r,"house")})),e.replaceChildren(t),window.lucide&&lucide.createIcons()}let S=!1;function L(){S||(S=!0,document.getElementById(A)&&(D(),b(),N().then(()=>b()).catch(()=>{}),window.addEventListener("showroom-categories-ready",()=>b()),window.addEventListener("kco-db-refresh",()=>b())))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",L):L();export{le as a,M as b,te as c,ee as d,R as g,re as h,ae as v};
