...
import{supabase as f}from"./supabase-client-7_ZWSEp6.js";import{d as L,g as q,C as D}from"./localization-DZro7UN4.js";import{_ as g}from"./preload-helper-CLcXU_4U.js";import{g as O}from"./auth-D3ZJ7kZA.js";import{f as k,l as $,g as U,a as F}from"./showroom-data-D-nP9j-R.js";import{g as C,a as I,b as S,c as A}from"./phone-data-CExU1cqD.js";import{c as G,a as R,b as H,d as B,r as z,e as V}from"./payment-settings-DapmxLOR.js";import"./native-bridge-BNMM1XVo.js";import"./live-stream-mode-fNPFq7rS.js";import"./customer-ai-widget-L9BHjKej.js";import"./brand-DYhAh01b.js";const h="/fallback.svg";let e={user:null,isGuest:!1,listing:null,quantity:1,selectedCurrency:"USD",countryCode:"US",cartItems:[],step:1,paymentMethod:"flutterwave",addresses:[],selectedAddressId:null,billingSame:!0,billingAddress:"",fullName:"",email:"",phone:"",shippingAddr1:"",shippingAddr2:"",shippingCity:"",shippingState:"",shippingPostal:"",shippingCountry:"US",orderNumber:"",processing:!1,paymentSettings:null,manualPaymentAccounts:[],manualPaymentInstructions:"",paymentGateway:"both",autoDetectedCurrency:""};function J(){const t=Date.now().toString(36).toUpperCase().slice(-6),s=Math.random().toString(36).toUpperCase().slice(2,6);return`KCO-${t}${s}`}function m(t){const s=document.getElementById("toast");document.getElementById("toast-msg").textContent=t,s.classList.remove("translate-y-20","opacity-0"),clearTimeout(s._t),s._t=setTimeout(()=>s.classList.add("translate-y-20","opacity-0"),3e3),window.lucide&&lucide.createIcons()}function T(t){t.addEventListener("click",function(s){if(this.disabled)return;const i=this.getBoundingClientRect(),r=document.createElement("span");r.className="ripple";const l=Math.max(i.width,i.height);r.style.width=r.style.height=l+"px",r.style.left=s.clientX-i.left-l/2+"px",r.style.top=s.clientY-i.top-l/2+"px",this.appendChild(r),setTimeout(()=>r.remove(),600)})}function b(t,s){return`${(parseFloat(t)||0).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})} ${s}`}function X(t){const s=()=>{const i=document.createElement("textarea");i.value=t,document.body.appendChild(i),i.select();try{document.execCommand("copy")}catch{}document.body.removeChild(i)};navigator.clipboard&&window.isSecureContext?navigator.clipboard.writeText(t).catch(()=>s()):s(),m("Copied to clipboard.")}function Y(){const t=e.selectedCurrency==="USD"&&!e.autoDetectedCurrency?"":e.selectedCurrency,s=z(e.manualPaymentAccounts,e.countryCode,t);return{...s,fallbackNotice:s.isFallback?V(s.account,e.countryCode,e.selectedCurrency,e.manualPaymentInstructions):null}}function Q(){const t=document.getElementById("particles");if(t)for(let s=0;s<12;s++){const i=document.createElement("div"),r=Math.random()*3+1;i.className="particle",i.style.width=i.style.height=r+"px",i.style.left=Math.random()*100+"%",i.style.bottom="-10px",i.style.background=Math.random()>.5?"rgba(59,130,246,.4)":"rgba(251,191,36,.3)",i.style.animationDuration=Math.random()*20+15+"s",i.style.animationDelay=Math.random()*20+"s",t.appendChild(i)}}Q();async function W(){const t=document.getElementById("checkout-root"),s=new URLSearchParams(window.location.search),i=s.get("status"),r=s.get("transaction_id"),l=s.get("tx_ref"),c=s.get("order_number");if(i==="verify"&&r){await ne(r,l,c||localStorage.getItem("kco_pending_order"));return}if(e.isGuest=s.get("guest")==="1",!e.isGuest&&(e.user=await O(),!e.user)){window.location.href="/auth.html?redirect=/checkout.html";return}const a=s.get("id");if(a){if(e.listing=k(a)||C(a)||I(a)||S(a)||A(a),!e.listing){const[{generateListingById:o},{loadHiddenCatalogIds:n}]=await Promise.all([g(()=>import("./catalog-CESpPuFx.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);await n(),e.listing=o(a)}if(e.listing||(await $(),e.listing=k(a)),!e.listing){t.innerHTML='<div class="text-center py-20 text-gray-500">Listing not found.</div>';return}e.cartItems=[{listing:e.listing,quantity:1}]}else{const o=JSON.parse(localStorage.getItem("kco_cart")||"[]");await $();const n=U(),[{generateListingById:d},{loadHiddenCatalogIds:v}]=await Promise.all([g(()=>import("./catalog-CESpPuFx.js"),__vite__mapDeps([0,1,2])),g(()=>import("./catalog-hidden-store-DbF0xVeS.js"),__vite__mapDeps([1,2]))]);if(await v(),e.cartItems=o.map(p=>{const _=n.find(N=>N.property_id===p)||C(p)||I(p)||S(p)||A(p)||d(p);return _?{listing:_,quantity:1}:null}).filter(Boolean),e.cartItems.length===0){t.innerHTML=M(),window.lucide&&lucide.createIcons();return}e.listing=e.cartItems[0].listing}if(e.countryCode=localStorage.getItem("kco_country")||"US",e.user&&!e.isGuest){const{data:o}=await f.from("profiles").select("country_code").eq("user_id",e.user.id).maybeSingle();o?.country_code&&(e.countryCode=o.country_code)}if(e.autoDetectedCurrency=L(e.countryCode)||"",e.selectedCurrency=e.autoDetectedCurrency||"USD",e.paymentSettings=await G(),e.manualPaymentAccounts=R(e.paymentSettings),e.manualPaymentInstructions=H(e.paymentSettings),e.paymentGateway=e.paymentSettings.payment_gateway||"both",e.paymentGateway==="manual"&&(e.paymentMethod="manual_bank_transfer"),e.paymentGateway==="flutterwave"&&(e.paymentMethod="flutterwave"),B(e.manualPaymentAccounts).includes(e.selectedCurrency)||(e.selectedCurrency="USD"),e.user&&!e.isGuest){const{data:o}=await f.from("shipping_addresses").select("*").eq("user_id",e.user.id).order("created_at",{ascending:!1});e.addresses=o||[];const n=e.addresses.find(v=>v.is_default);n&&(e.selectedAddressId=n.id);const{data:d}=await f.from("profiles").select("*").eq("user_id",e.user.id).maybeSingle();d&&(e.fullName=d.display_name||`${d.first_name||""} ${d.last_name||""}`.trim(),e.email=e.user.email,e.phone=d.phone_code&&d.phone_number?`+${d.phone_code} ${d.phone_number}`:"",e.shippingCountry=d.country_code||e.countryCode)}e.orderNumber=J(),u()}function x(){return e.cartItems.reduce((t,s)=>t+s.listing.price*s.quantity,0)}function K(){return 0}function Z(){return 0}function w(){return Math.round(x()*Z()*100)/100}function y(){return x()+K()+w()}function u(){const t=document.getElementById("checkout-root");t.innerHTML=`
    <div class="fade-in">
      <!-- Breadcrumb -->
        ...
        ${e.cartItems.map((t,s)=>`
            <div class="flex items-center gap-3 p-3 bg-blue-950/30 border border-blue-500/10 rounded-xl">
              ...
              <div class="flex-1 min-w-0">
                ...
                <p class="text-sm font-bold text-amber-400 mt-1">${F(t.listing)}</p>
              </div>
  ...
  `}function se(){const t=e.addresses.length>0&&!e.isGuest?`
    <div class="mb-5">
      <h4 class="text-xs font-bold text-white uppercase tracking-wide mb-3">Saved Addresses</h4>
      <div class="space-y-2">
        ${e.addresses.map(i=>{const r=q(i.country_code);return`
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <select id="ship-country" class="input-field w-full bg-[#0a1124]/80 border border-blue-500/20 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-blue-500">
              ${D.map(i=>`<option value="${i.code}" ${e.shippingCountry===i.code?"selected":""}>${i.flag} ${i.name}</option>`).join("")}
            </select>
          </div>
        </div>
      `:""}
  ...
  `}function ie(){const t=B(e.manualPaymentAccounts),s=Y(),i=e.paymentGateway==="both"||e.paymentGateway==="flutterwave",r=e.paymentGateway==="both"||e.paymentGateway==="manual";return`
    ...
    ${e.paymentMethod==="manual_bank_transfer"?re(s.account,s.fallbackNotice,e.manualPaymentInstructions):""}
  ...
  `}function ae(){const t=x(),s=y();return`
