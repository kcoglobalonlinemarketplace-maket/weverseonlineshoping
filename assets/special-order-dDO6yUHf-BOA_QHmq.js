import{d as y}from"./supabase-client-DvCmNkpI-BBtcpOs0.js";const h=["United States","United Kingdom","Canada","Australia","Germany","France","Spain","Italy","Netherlands","Belgium","Sweden","Norway","Denmark","Finland","Switzerland","Austria","Ireland","Portugal","Greece","Poland","Czech Republic","Hungary","Romania","Bulgaria","Croatia","Slovenia","Slovakia","Estonia","Latvia","Lithuania","Russia","Ukraine","Turkey","Israel","Saudi Arabia","UAE","Qatar","Kuwait","Bahrain","Oman","Egypt","South Africa","Nigeria","Kenya","Ghana","Ethiopia","Morocco","Algeria","Tunisia","Tanzania","Japan","South Korea","China","Hong Kong","Taiwan","Singapore","Malaysia","Indonesia","Thailand","Vietnam","Philippines","India","Pakistan","Bangladesh","Sri Lanka","Nepal","Cambodia","Mongolia","Kazakhstan","Uzbekistan","Brazil","Argentina","Mexico","Colombia","Chile","Peru","Ecuador","Venezuela","Uruguay","Paraguay","Bolivia","Costa Rica","Panama","Dominican Republic","Jamaica","Trinidad and Tobago","Guatemala","Honduras","El Salvador","Nicaragua","New Zealand","Fiji","Papua New Guinea","Iceland","Luxembourg","Malta","Cyprus","Andorra","Monaco","Liechtenstein"];let c="",n=null;async function x(){const e=await y(),{data:t}=await e.auth.getUser();return t?.user||null}function i(e){if(window._showToast){window._showToast(e);return}if(window.showToast){window.showToast(e);return}const t=document.createElement("div");t.className="fixed top-6 right-6 z-[100] bg-gray-900 border border-blue-500/30 text-white px-4 py-3 rounded-xl shadow-lg text-sm",t.textContent=e,document.body.appendChild(t),setTimeout(()=>t.remove(),3e3)}function s(e){return String(e??"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function p(e){return s(e)}function k(e){c=e||"",n=null,f()}function m(e,t,a,r,o){n={title:e,brand:t,category:a,price:r,currency:o},c=e||"",f()}async function f(){if(!await x()){sessionStorage.setItem("kco_auth_redirect",window.location.pathname),sessionStorage.setItem("kco_special_order_query",c),i("Please sign in to place a Special Order."),setTimeout(()=>{window.location.href="/auth.html"},800);return}v()}function v(e){let t=document.getElementById("special-order-overlay");t||(t=document.createElement("div"),t.id="special-order-overlay",t.className="fixed inset-0 z-[60] bg-black/85 backdrop-blur-md overflow-y-auto",document.body.appendChild(t));const a=n?.title||c||"",r=n?.brand||"",o=n?.category||"",u=n?.price||"",l=n?.currency||"USD",w=h.map(g=>`<option value="${g}">${g}</option>`).join("");t.innerHTML=`
      ...
      <div class="glass bg-white border border-gray-200 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div id="so-panel-1" class="p-5 space-y-4">
          <div>
            ...
            <input id="so-title" type="text" value="${p(a)}" placeholder="What are you looking for?"
          ...
          <div class="grid grid-cols-2 gap-3">
            <div>
              ...
              <input id="so-category" type="text" value="${p(o)}" placeholder="e.g. Electronics, Fashion"
                class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 transition">
            </div>
            <div>
              <label class="block text-xs font-bold text-gray-600 mb-1.5 uppercase tracking-wide">Preferred Brand</label>
              <input id="so-brand" type="text" value="${p(r)}" placeholder="e.g. Apple, Nike"
                class="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-sm text-gray-900 placeholder-gray-600 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 transition">
            </div>
          </div>
  ...
  `,t.style.display="block",document.body.style.overflow="hidden",window.lucide&&lucide.createIcons()}let d=1;function b(e){d=e;for(let a=1;a<=3;a++){const r=document.getElementById(`so-panel-${a}`);r&&r.classList.toggle("hidden",a!==e)}for(let a=1;a<=3;a++){const r=document.getElementById(`so-step-${a}`);if(!r)continue;const o=r.querySelector(".so-step-dot");a<e?(r.className="flex items-center gap-2 text-emerald-600 text-xs font-bold",o.className="so-step-dot w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center text-[11px] font-bold",o.innerHTML='<i data-lucide="check" class="w-3.5 h-3.5"></i>'):a===e?(r.className="flex items-center gap-2 text-blue-600 text-xs font-bold",o.className="so-step-dot w-6 h-6 rounded-full bg-blue-500 text-white flex items-center justify-center text-[11px] font-bold",o.textContent=String(a)):(r.className="flex items-center gap-2 text-gray-500 text-xs font-bold",o.className="so-step-dot w-6 h-6 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center text-[11px] font-bold",o.textContent=String(a))}window.lucide&&lucide.createIcons();const t=document.getElementById("special-order-overlay");t&&t.scrollTo({top:0,behavior:"smooth"})}function S(){if(d===1){if(!document.getElementById("so-title").value.trim()){i("Please enter a product name.");return}b(2)}else if(d===2){const e=document.getElementById("so-delivery-name").value.trim(),t=document.getElementById("so-delivery-address").value.trim(),a=document.getElementById("so-delivery-city").value.trim(),r=document.getElementById("so-delivery-phone").value.trim();if(!e){i("Please enter recipient name.");return}if(!t){i("Please enter delivery address.");return}if(!a){i("Please enter city.");return}if(!r){i("Please enter phone number.");return}B(),b(3)}}function I(){d>1&&b(d-1)}function B(){const e=a=>(document.getElementById(a)?.value||"").trim()||"—",t=document.getElementById("so-review-content");t&&(t.innerHTML=`
    <div class="bg-gray-50 rounded-xl p-4 space-y-2.5 border border-gray-200">
      <div class="flex justify-between items-start"><span class="text-xs text-gray-500 uppercase font-bold">Product</span><span class="text-sm text-gray-900 font-semibold text-right">${s(e("so-title"))}</span></div>
      <div class="flex justify-between items-start"><span class="text-xs text-gray-500 uppercase font-bold">Category</span><span class="text-sm text-gray-700 text-right">${s(e("so-category"))}</span></div>
      <div class="flex justify-between items-start"><span class="text-xs text-gray-500 uppercase font-bold">Brand</span><span class="text-sm text-gray-700 text-right">${s(e("so-brand"))}</span></div>
      <div class="flex justify-between items-start"><span class="text-xs text-gray-500 uppercase font-bold">Target Price</span><span class="text-sm text-blue-600 font-bold text-right">${s(e("so-currency"))} ${s(e("so-price"))}</span></div>
      <div class="flex justify-between items-start"><span class="text-xs text-gray-500 uppercase font-bold">Quantity</span><span class="text-sm text-gray-700 text-right">${s(e("so-qty"))}</span></div>
      ${e("so-desc")!=="—"?`<div class="pt-2 border-t border-gray-200"><span class="text-xs text-gray-500 uppercase font-bold block mb-1">Description</span><p class="text-sm text-gray-700">${s(e("so-desc"))}</p></div>`:""}
    </div>
    <div class="bg-gray-50 rounded-xl p-4 space-y-2.5 border border-gray-200">
      <p class="text-xs text-gray-500 uppercase font-bold mb-1">Delivery To</p>
      <p class="text-sm text-gray-900 font-semibold">${s(e("so-delivery-name"))}</p>
      <p class="text-sm text-gray-600">${s(e("so-delivery-address"))}</p>
      <p class="text-sm text-gray-600">${s(e("so-delivery-city"))}, ${s(e("so-delivery-state"))}</p>
      <p class="text-sm text-gray-600">${s(e("so-delivery-postal"))} ${s(e("so-delivery-country"))}</p>
      <p class="text-sm text-gray-600"><i data-lucide="phone" class="w-3.5 h-3.5 inline mr-1"></i>${s(e("so-delivery-phone"))}</p>
    </div>
  `,window.lucide&&lucide.createIcons())}async function E(){const e=document.getElementById("so-submit-btn");e&&(e.disabled=!0,e.innerHTML='<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Submitting...',window.lucide&&lucide.createIcons());const t={request_title:document.getElementById("so-title").value.trim(),request_description:document.getElementById("so-desc").value.trim()||null,category:document.getElementById("so-category").value.trim()||null,brand:document.getElementById("so-brand").value.trim()||null,target_price:parseFloat(document.getElementById("so-price").value)||null,currency:document.getElementById("so-currency").value,quantity:parseInt(document.getElementById("so-qty").value)||1,delivery_full_name:document.getElementById("so-delivery-name").value.trim(),delivery_address:document.getElementById("so-delivery-address").value.trim(),delivery_city:document.getElementById("so-delivery-city").value.trim(),delivery_state:document.getElementById("so-delivery-state").value.trim()||null,delivery_country:document.getElementById("so-delivery-country").value,delivery_postal_code:document.getElementById("so-delivery-postal").value.trim()||null,delivery_phone:document.getElementById("so-delivery-phone").value.trim()};try{const a=await y(),{data:r,error:o}=await a.from("product_requests").insert(t).select().single();if(o)throw o;r&&await a.from("product_request_status_updates").insert({request_id:r.id,status:"pending_review",message:"Your request has been received and is awaiting review."}),[1,2,3].forEach(u=>document.getElementById(`so-panel-${u}`)?.classList.add("hidden")),document.getElementById("so-panel-success")?.classList.remove("hidden"),window.lucide&&lucide.createIcons(),i("Special Order submitted successfully!")}catch(a){i("Failed to submit request: "+(a.message||"Unknown error")),e&&(e.disabled=!1,e.innerHTML='<i data-lucide="send" class="w-4 h-4"></i> Submit Request',window.lucide&&lucide.createIcons())}}function $(){const e=document.getElementById("special-order-overlay");e&&(e.style.display="none"),document.body.style.overflow=""}window.openSpecialOrderModal=k;window.openSpecialOrderFromSearch=m;window.closeSpecialOrderModal=$;window.specialOrderNext=S;window.specialOrderBack=I;window.submitSpecialOrder=E;window._openSpecialOrderFromSearch=m;(async()=>{const e=sessionStorage.getItem("kco_special_order_query");e&&(sessionStorage.removeItem("kco_special_order_query"),await x()&&(c=e,v()))})();export{m as openSpecialOrderFromSearch,k as openSpecialOrderModal};
